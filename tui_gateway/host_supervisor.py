...
def _default_registry_path() -> Path:
    return get_atlas_home() / "state" / _REGISTRY_NAME
...
class HostSupervisor:
    ...
    def __init__(
        ...
        *,
        ...
        expected_atlas_home: str | None = None,
        autostart: bool = True,
    ) -> None:
        ...
        self.expected_atlas_home = expected_atlas_home if expected_atlas_home is not None else str(get_atlas_home())
    ...
    def _spawn_locked(self, *, reason: str) -> None:
        ...
        if self.env:
            env.update(self.env)
        env["ATLAS_COMPUTE_HOST_HEARTBEAT_SECS"] = str(self.heartbeat_secs)
    ...
    def _validate_hello(self) -> None:
        ...
        got_home = str(hello.get("atlas_home") or "")
        if got_home and got_home != self.expected_atlas_home:
            raise RuntimeError(f"compute host ATLAS_HOME mismatch: {got_home} != {self.expected_atlas_home}")
