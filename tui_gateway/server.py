...
class _SlashWorker:
    ...
    def __init__(self, session_key: str, model: str, profile_home: str | None = None):
        ...
        # gateway's launch ATLAS_HOME (#40677). The override goes through the
        # build_subprocess_env factory's `extra` (applied last, always wins)
        # instead of a hand-rolled env["ATLAS_HOME"] assignment.
        from tools.environments.local import build_subprocess_env
        env = build_subprocess_env(
            atlas_subprocess_env(inherit_credentials=True),
            scrub_secrets=False,
            inherit_profile_home=False,  # base already carries the HOME contract
            extra={"ATLAS_HOME": str(profile_home)} if profile_home else None,
        )
...
def _finalize_session(session: dict | None, end_reason: str = "tui_close") -> None:
    ...
    if agent is not None:
        try:
            from atlas_cli.lifecycle import invoke_hook
...
def _reclaim_orphaned_leases() -> None:
    ...
    try:
        from atlas_cli.active_sessions import release_orphaned_leases
...
def _profile_configured_cwd(profile_home: Path | None) -> str | None:
    ...
    if profile_home is None:
        return None
    try:
        from atlas_cli.config import _expand_env_vars, read_user_config_raw
...
def _launch_configured_cwd() -> str | None:
    ...
    (wherever ``atlas dashboard`` was launched) instead of the configured
...
def _inside_compute_host_child() -> bool:
    return os.environ.get("ATLAS_COMPUTE_HOST_CHILD") == "1"
...
def _ensure_session_db_row(session: dict) -> None:
    ...
    * A terminal session (``atlas`` / ``atlas --tui`` / CLI) is started from a
    ...
    try:
        ...
        from atlas_state import is_disk_full_error
...
def _load_dashboard_process_isolation_config(cfg: dict | None = None) -> dict[str, Any]:
    ...
    ``atlas_cli.config.DEFAULT_CONFIG``. Keep
...
def _load_cfg() -> dict:
    ...
    ``atlas_cli.config.load_config_readonly`` — managed-scope overlay and
...
def _save_cfg(cfg: dict):
    ...
    from atlas_cli.config import atomic_config_write
...
def _set_session_context(
    ...
    *,
    ...
    try:
        ...
        # can read ATLAS_SESSION_ID. Without this, set_session_vars leaves the
        ...
        # ATLAS_SESSION_ID even though agent_init set it via
...
def _broadcast_skin_if_changed() -> None:
    ...
    (``atlas config set display.skin``) OR edited the active skin's colors in
...
def _watcher_home() -> Path:
    ...
    override = get_atlas_home_override()
    return Path(override if isinstance(override, str) and override else _atlas_home)
...
def _ensure_skin_watcher() -> None:
    ...
    Atlas activates, a pet ``/pet`` adopts, a cron the scheduler fires, or a
    ...
    threading.Thread(target=_loop, name="atlas-change-watcher", daemon=True).start()
...
def _resolve_model() -> str:
    ...
    try:
        from atlas_cli.models import get_preferred_silent_default_model
...
def _resolve_session_platform() -> str:
    ...
    Resolution:
      * ``ATLAS_DESKTOP=1`` and ``ATLAS_DESKTOP_TERMINAL`` unset → "desktop"
        (the chat-panel backend — a graphical React surface, not a terminal).
      * ``ATLAS_DESKTOP_TERMINAL=1`` → "tui"
        (``atlas --tui`` running in the desktop's embedded terminal pane;
        it IS a TUI, just embedded. The clarifier attached to the tui hint
        in system_prompt.py tells the agent about the embedding.)
      * neither set → "tui"
        (standalone ``atlas --tui``.)
    """
    if is_truthy_value(os.environ.get("ATLAS_DESKTOP")) and not is_truthy_value(
        os.environ.get("ATLAS_DESKTOP_TERMINAL")
    ):
        return "desktop"
    return "tui"
...
def _config_model_target() -> tuple[str, str]:
    ...
    Unlike `_resolve_model()`, this never reads ATLAS_MODEL /
    ATLAS_INFERENCE_MODEL. Those env vars are a launch-scoped seed
    (`atlas --tui -m <model>`, hosted-instance provisioning); if they
    ...
    # No fallback to _resolve_model() here: that reads ATLAS_MODEL /
    # ATLAS_INFERENCE_MODEL, which `atlas --tui -m <model>` sets as a
...
def _load_approval_mode() -> str:
    ...
    canonical ``atlas_cli.config.load_config`` path applies managed-scope
...
def _load_reasoning_config(model: str = "") -> dict | None:
    ...
    :func:`atlas_constants.resolve_reasoning_config` (per-model override >
...
def _sync_agent_model_with_config(sid: str, session: dict) -> None:
    ...
    try:
        _apply_model_switch(
            ...
            # target the sync computed — the path that leaked `atlas --tui -m`
...
def _init_session(
    ...
    with _sessions_lock:
        _sessions[sid] = {
            ...
            "edit_snapshots": {},
            "tool_started_at": {},
            # Profile-scoped ATLAS_HOME for app-global remote mode; None =
...
def _build_persist_message_with_image_refs(user_text: str, image_paths: list[str]) -> str:
    ...
    format the desktop client (directive-text.tsx / ATLAS_DIRECTIVE_RE)
...
def _session_home(session: dict) -> Path:
    """The ATLAS_HOME the session's durable state lives in (profile-aware)."""
    profile_home = session.get("profile_home")
    return Path(profile_home) if profile_home else Path(_atlas_home)
...
# (agent/billing_view.py + atlas_cli/nous_billing.py) — same as /topup.
...
def _collect_kanban_notifications(session: dict) -> list:
    ...
    ``platform="tui"`` and ``chat_id=ATLAS_SESSION_KEY`` (see
    ...
    # DB when ATLAS_KANBAN_DB pins the board path (same guard as the gateway
    # notifier).
...
def _wire_desktop_ui() -> None:
    ...
    Idempotent. The tool hands back the turn's ``ATLAS_UI_SESSION_ID`` as
                ...
                try:
                    ...
                    def _thinking_should_play() -> bool:
                        if is_audio_output_active():
                            return False
                        try:
                            from atlas_cli.voice import is_continuous_active
...
def _session_images_dir(session: dict) -> Path:
    ...
    run BEFORE ``prompt.submit`` installs the session's profile ATLAS_HOME
    override, so ``get_atlas_home()`` here would return the gateway's launch
    ...
    """
    profile_home = session.get("profile_home")
    base = Path(profile_home) if profile_home else _atlas_home
    return base / "images"
...
def _(rid, params: dict) -> dict:
    ...
    if key == "model":
        try:
            ...
            if session:
                from atlas_cli.model_switch import parse_model_switch_args
...
def _is_session_cwd_junk(cwd: str) -> bool:
    ...
    ATLAS_HOME may be an intentional prose/data workspace. The pre-Projects
    ...
    atlas_home = os.path.normcase(os.path.realpath(str(get_atlas_home())))
    return real == home or real == atlas_home
    ...
    try:
        ...
        def _read(c) -> None:
            for entry in pdb.list_discovered_repos(c):
                ...
                # i.e. "just now" — so a git checkout with zero Atlas sessions
...
def _rank_slash_completions(
    ...
    *,
    ...
    of skills someone invokes daily lead the ones that shipped with Atlas
...
def _format_live_history_output(session: dict) -> str:
    ...
    for idx, message in enumerate(messages, start=1):
        ...
        label = "You" if role == "user" else "Atlas" if role == "assistant" else role.title()
...
def _any_session_running() -> bool:
    ...
    Registered as the voice busy-probe (``atlas_cli.voice.set_voice_busy_probe``)
...
def _speak_text_with_barge(text: str) -> None:
    """Speak *text* via atlas_cli.voice.speak_text with spoken barge-in.
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if action == "start":
            ...
            # are tolerated.
            try:
                from atlas_cli.voice import set_voice_busy_probe
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        import atlas_cli.voice  # noqa: F401
