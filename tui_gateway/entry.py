...
# shared owner in atlas_cli.mcp_startup — the wait/in-flight/join helpers
...
# atlas_cli.mcp_startup.start_background_mcp_discovery can actually fire —
...
def wait_for_mcp_discovery(timeout: "float | None" = None) -> None:
    ...
    # → atlas_cli.mcp_startup); wait on it so the first agent build still
    ...
    # session profile's ATLAS_HOME first), so a launch profile with no
...
def ensure_mcp_discovery_started() -> None:
    ...
    binding the session profile's ATLAS_HOME override — the shared owner in
    ``atlas_cli.mcp_startup`` captures the caller's context-local override
...
def main():
    ...
    # Live-apply skins Atlas activates mid-conversation.
