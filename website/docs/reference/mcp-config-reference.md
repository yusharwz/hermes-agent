...
Values resolve from the active profile's secret scope (falling back to the process environment), so put the secret in `~/.atlas/.env`. An unset variable keeps its literal placeholder.
