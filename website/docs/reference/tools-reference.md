...
:::tip MCP Tools
In addition to built-in tools, Atlas can load tools dynamically from MCP servers. MCP tools appear with the prefix `mcp__<server>__` (e.g., `mcp__github__create_issue` for the `github` MCP server). See [MCP Integration](/user-guide/features/mcp) for configuration.
:::
...
| `kanban_attach` | Attach a file to a task by passing its bytes inline (base64). Stored as a real attachment under the task's attachments dir, capped at 25 MB. | `ATLAS_KANBAN_TASK` or `kanban` toolset |
| `kanban_attach_url` | Attach a file to a task by URL — Atlas downloads it server-side and stores it as a real attachment (capped at 25 MB). Only http/https URLs. | `ATLAS_KANBAN_TASK` or `kanban` toolset |
| `kanban_attachments` | List the files attached to a task: id, filename, content_type, size, uploader, and the absolute on-disk path. | `ATLAS_KANBAN_TASK` or `kanban` toolset |
...
| `close_terminal` | Close the read-only terminal tab for a background process in the Atlas desktop GUI. Does NOT kill the process — only drops the tab/view; use process(action='kill') to stop it. Desktop-app only. | — |
| `open_preview` | Open a web URL, localhost dev-server URL, or file path in the preview pane beside the chat in the Atlas desktop app. Desktop-app only. | — |
| `focus_pane` | Reveal and focus a pane in the Atlas desktop app (chat, files, terminal, review, sessions). Desktop-app only. | — |
...
Registered by the bundled `spotify` plugin. Requires an OAuth token — run `atlas auth spotify` once to authorize.
