...
of fields, and Atlas schedules it as a cron job — no cron syntax required.
...
- **CLI, TUI, and messengers** — type `/blueprint <name>` (e.g.
  `/blueprint morning-brief`) and Atlas asks you for what it needs, one
...
A blueprint is just a skill with a `metadata.atlas.blueprint` block in its
