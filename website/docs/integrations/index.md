...
- **[AI Providers](/integrations/providers)** — OpenRouter, Anthropic, OpenAI, Google, and any OpenAI-compatible endpoint. Atlas auto-detects capabilities like vision, streaming, and tool use per provider.
...
- **[Buzz](/integrations/buzz)** — Block's Nostr-based human+agent workspace. Three integration paths: Buzz Desktop spawns Atlas as a managed ACP runtime, the `buzz-acp` relay bridge hosts a Atlas identity server-side, or the native gateway platform joins Buzz channels with full Atlas memory/skills/approvals/cron. The overview page compares all three.
...
- **[Build a Plugin](/developer-guide/plugins)** — Step-by-step guide for creating Atlas plugins with tools, hooks, and CLI commands.
