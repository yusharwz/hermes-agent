...
To easily install the command-line and desktop applications, [download the Atlas Desktop installer](https://hermes-agent.nousresearch.com/) from our website and run it.
...
atlas config get     # Inspect individual config values
...
Atlas auto-detects whether it was installed via the git installer, Docker, or NixOS, and `atlas update` prints the matching update command for that path. There's no env var to set — the detection is based on the install layout (`~/.atlas/atlas-agent/` checkout, Docker image stamp, or Nix store path). `atlas doctor` also surfaces the detected method under its environment summary.
