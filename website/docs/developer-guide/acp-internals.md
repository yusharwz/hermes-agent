...
So ACP advertises and uses the currently configured Atlas provider/credentials. It also always advertises a terminal setup auth method (`atlas-setup`, args `--setup`) so first-run ACP clients can open Atlas' interactive model/provider configuration before starting a normal ACP session.
