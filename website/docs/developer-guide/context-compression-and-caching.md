...
compression:
  ...
  codex_app_server_auto: native  # native|atlas|off for Codex app-server thread compaction
...
| `codex_app_server_auto` | `native` | `native`, `atlas`, `off` | Thread-compaction mode for Codex app-server sessions (see below) |
...
notice is shown once per profile — a marker under `$ATLAS_HOME`
...
thread context, so Atlas' auxiliary summarizer cannot shrink it — rewriting the
...
- Automatic compaction is controlled by `compression.codex_app_server_auto`:
  the default `native` lets the app-server decide when to compact and Atlas
  records the resulting compaction events (compression counters, session
  events). Set `atlas` to let Atlas' compression threshold initiate
  app-server compaction, or `off` to disable Atlas-initiated automatic
  compaction entirely (codex may still compact natively).

Atlas' local transcript is never rewritten on this runtime — state.db records
the compaction boundary while the visible transcript stays intact. All other
routes (including Codex OAuth chat sessions) keep Atlas' summary compressor.
...
5. **Model identity is part of the cache key**: Provider-side caches are scoped
   ...
   Atlas can avoid; user-facing docs for `/model`, fallback providers, and
