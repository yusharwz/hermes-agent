...
| `requires_env` | Interactive credential prompt during `atlas plugins install` (see [Build a Atlas Plugin](/developer-guide/plugins#gate-on-environment-variables) for the rich format) |
...
If your provider wraps a third-party SDK (like DDGS does with the `ddgs` package), don't `import` it at module top level. Use `tools.lazy_deps.ensure(...)` inside `is_available()` or `search()` — Atlas will install the package on first use, gated by `security.allow_lazy_installs`. See [Build a Atlas Plugin → Lazy-install](/developer-guide/plugins#lazy-install-optional-python-dependencies) for the security model.
...
- [Build a Atlas Plugin](/developer-guide/plugins) — general tools/hooks/slash commands guide
