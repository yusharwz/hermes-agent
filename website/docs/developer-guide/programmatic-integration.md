...
atlas acp --check          # verify ACP dependencies and adapter imports
atlas acp --setup          # interactive provider/model setup for ACP terminal auth
```
...
the compatibility endpoint frontends expect, not the full Atlas provider/model
picker catalog.

If an external control plane needs Atlas' curated provider rows, per-model
...
- Dashboard backend REST: `GET /api/model/options` with `X-Atlas-Session-Token`
...
`model.options` when you are building a Atlas-aware model picker.
