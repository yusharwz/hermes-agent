...
| Variable | Meaning |
|----------|---------|
| `ATLAS_MAIN_CHECKOUT` | The deps checkout — where `node_modules` really lives, and whose `.venv/bin/python` runs the backend. |
| `ATLAS_GUI_DEPS_CHECKOUT` | Where the desktop deps (`apps/desktop/node_modules`) live. Defaults to `ATLAS_MAIN_CHECKOUT`; override only if you keep desktop deps elsewhere. |

Neither is read by Atlas itself — they're private to these helpers. The variables Atlas *does* read are covered in [Environment Variables](../reference/environment-variables.md).

## `htui` — TUI from the worktree

The Ink TUI has a dev path already: `atlas --tui --dev` runs the TypeScript sources via `tsx` instead of the prebuilt bundle. `htui` is a one-liner over it that also points the run at the current worktree's `ui-tui/`:

```bash
htui() {
  local root
  root="$(_atlas_root)" || { echo "htui: not in a Atlas checkout" >&2; return 1; }
  ( cd "$root" && PYTHONPATH="$root" \
      "$ATLAS_MAIN_CHECKOUT/.venv/bin/python" -m atlas_cli.main --tui --dev "$@" )
}
```

`--dev` compiles from source, so it links `ui-tui/node_modules` from `ATLAS_MAIN_CHECKOUT` when the root lockfile matches and installs locally otherwise (see [`_atlas_root` / linking helpers](#shared-helpers)).

:::warning `--dev` and `ATLAS_TUI_DIR` are mutually exclusive
`ATLAS_TUI_DIR` points Atlas at a *prebuilt* bundle (Nix, system packages), which has no source to hot-reload. If it's set in your shell, `atlas --tui --dev` exits with an error. Run `unset ATLAS_TUI_DIR` before `htui`.
:::
...
hgui() {
  ...
  root="$(_atlas_root)" || { echo "hgui: not in a Atlas checkout" >&2; return 1; }
  deps="${ATLAS_GUI_DEPS_CHECKOUT:-$ATLAS_MAIN_CHECKOUT}"
  ...
  if cmp -s "$root/package-lock.json" "$deps/package-lock.json"; then
    _atlas_link_deps "$desktop" "$deps/apps/desktop"
    _atlas_link_deps "$root" "$deps"
  else
  ...
  trap '_atlas_gui_cleanup "$root"' INT TERM EXIT

  ( cd "$desktop"
    export PATH="$root/node_modules/.bin:$PATH"
    ATLAS_DESKTOP_ATLAS_ROOT="$root" \
    ATLAS_DESKTOP_PYTHON="$ATLAS_MAIN_CHECKOUT/.venv/bin/python" \
    ATLAS_DESKTOP_IGNORE_EXISTING=1 \
    ATLAS_DESKTOP_CWD="$root" \
    npm run dev )
}
```
...
|----------|----------------|
| `ATLAS_DESKTOP_ATLAS_ROOT` | Runs the backend from **this worktree**, not the packaged/PATH `atlas`. |
| `ATLAS_DESKTOP_PYTHON` | Reuses the deps checkout's venv instead of re-resolving a Python. |
| `ATLAS_DESKTOP_IGNORE_EXISTING` | Ignores any `atlas` on `PATH` so it can't shadow the worktree. |
| `ATLAS_DESKTOP_CWD` | Opens the desktop chat rooted at the worktree. |
...
```bash
# The enclosing worktree, verified as a real Atlas checkout.
_atlas_root() {
  local root
  root="$(git rev-parse --show-toplevel 2>/dev/null)" || return 1
  [[ -f "$root/atlas_cli/main.py" && -d "$root/ui-tui" ]] && print -r "$root"
}
...
_atlas_gui_cleanup() {
  ...
  lsof -t -i:5174 >/dev/null 2>&1 && killport 5174
  pgrep -f 'atlas_cli\.main.*dashboard.*--port 0' 2>/dev/null | xargs -r kill -TERM 2>/dev/null
}
```
...
- [TUI](../user-guide/tui.md) — `atlas --tui --dev` and the `ATLAS_TUI_DIR` prebuild path
...
- [Environment Variables](../reference/environment-variables.md) — every `ATLAS_*` variable Atlas reads
