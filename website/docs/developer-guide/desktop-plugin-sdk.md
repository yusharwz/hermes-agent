...
title: "Desktop Plugin SDK (@atlas/plugin-sdk)"
description: "Extend the native Atlas Desktop app — panes, pages, sidebar nav, status bar, palette commands, keybinds, themes, and a scoped backend namespace, with one import and no build step."
---

# Desktop Plugin SDK

The native [Atlas Desktop](/user-guide/desktop) app is contribution-driven: every
...
A **desktop plugin** is a single ESM file that default-exports a `AtlasPlugin`.
It imports one module — `@atlas/plugin-sdk` — and gets everything: the app's
...
`$ATLAS_HOME/desktop-plugins/<id>/plugin.js` and the app loads it within seconds
and hot-reloads every save.

:::warning This is not the web-dashboard plugin SDK
"Plugin" means several unrelated things across Atlas. This page is the **native
desktop app** (`atlas desktop`) SDK — the `@atlas/plugin-sdk` module and
`$ATLAS_HOME/desktop-plugins/`. The **web dashboard** (`atlas dashboard`) has
its own, unrelated plugin system on `window.__ATLAS_PLUGIN_SDK__` with a
`manifest.json` — documented at
[Extending the Dashboard](/user-guide/features/extending-the-dashboard). Python
CLI/gateway plugins are documented at [Build a Atlas Plugin](/developer-guide/plugins).
...
|------|-------|-----|------------|
| **Disk** (recommended) | `$ATLAS_HOME/desktop-plugins/<id>/plugin.js` | users, agents | none — plain ESM, loaded uncompiled |
| **Bundled** | `apps/desktop/src/plugins/<id>/plugin.tsx` | in-tree, shipped with the app | the app's own Vite build |

Both take the same `AtlasPlugin` contract, appear in **Settings → Plugins**, and
...
Create `$ATLAS_HOME/desktop-plugins/hello/plugin.js` (that's `~/.atlas/...`
by default, or `~/.atlas/profiles/<name>/...` under a named profile). The folder
name must equal the plugin `id`.

```javascript
// ~/.atlas/desktop-plugins/hello/plugin.js
import { host, haptic, useValue } from '@atlas/plugin-sdk'
...
function HelloPane() {
  ...
  return jsxs('div', {
    ...
    children: [
      jsx('div', { className: 'font-medium', children: 'Hello, Atlas' }),
      jsx('div', {
...
The only importable specifiers are `@atlas/plugin-sdk`, `react`, and
`react/jsx-runtime` — everything else fails to resolve, on purpose.
:::

## The plugin contract

A plugin default-exports a `AtlasPlugin`:
...
interface PluginContext {
  ...
  /** Plugin-scoped JSON persistence (keys live under `atlas.plugin.<id>.`). */
...
```javascript
import { ROUTES_AREA, SIDEBAR_NAV_AREA } from '@atlas/plugin-sdk'
...
```javascript
import { STATUSBAR_AREAS, TITLEBAR_AREAS } from '@atlas/plugin-sdk'
...
```javascript
import { PALETTE_AREA, KEYBINDS_AREA } from '@atlas/plugin-sdk'
...
```javascript
import { THEMES_AREA } from '@atlas/plugin-sdk'
...
```javascript
import { Contribute, TITLEBAR_AREAS } from '@atlas/plugin-sdk'
...
```javascript
import { useQuery, useMutation, useQueryClient, atom, computed, useValue } from '@atlas/plugin-sdk'
...
```javascript
import { queryClient } from '@atlas/plugin-sdk'
...
`dashboard/` subfolder of a regular Atlas plugin and declare it in a
`manifest.json`:
...
atlas-agent codebase directly (`atlas_state`, `atlas_cli.config`, …). See
...
`config.yaml` (and not in `plugins.disabled`). Project plugins (`./.atlas/`)
...
(`atlas.plugin.<id>.*`) so plugins can't read or clobber each other:
...
export a `AtlasPlugin`). It's discovered by `discoverBundledPlugins()` at boot —
...
1. It goes through the app's Vite build, so you can write **real JSX** and import
   the SDK by its `@atlas/plugin-sdk` alias.
2. It's still lint-fenced to `@atlas/plugin-sdk` + `react` only — no `@/…` app
   internals.
...
- **Only three specifiers resolve:** `@atlas/plugin-sdk`, `react`,
...
| Plugin contract | `AtlasPlugin`, `PluginContext`, `PluginContribution`, `PluginStorage`, `PluginRestOptions`, `Contribution` |
...
### Agents: the `atlas-desktop-plugins` skill

When an agent writes a desktop plugin, it should load the bundled
**`atlas-desktop-plugins`** skill — it carries the same contract as this page in
...
`$ATLAS_HOME/desktop-plugins/<id>/plugin.js` and the folder name matches the
export `id`. Run ⌘K → **Reload desktop plugins**. Check the app for an error
toast naming the failure, and tail `atlas logs gui -f`.

**"unsupported import" on load.** A disk plugin may only import
`@atlas/plugin-sdk`, `react`, and `react/jsx-runtime`. Remove any other import.
...
`~/.atlas/plugins/<id>/dashboard/manifest.json` has `"api": "plugin_api.py"`,
that the plugin is in `plugins.enabled` in `config.yaml`, and restart the gateway
(backend routes mount at startup). Tail `~/.atlas/logs/errors.log` for
