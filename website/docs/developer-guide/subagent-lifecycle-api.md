...
Plugins can launch and supervise fresh Atlas child sessions without importing
...
active agent turn fails closed with `No active Atlas parent session`.
...
overrides, and per-launch timeouts are explicitly rejected until Atlas can
support them without weakening isolation. Use `allowed_toolsets` to narrow a
child; Atlas's existing unsafe-tool block remains enforced.
