...
- Building a custom/local tool without modifying Atlas core? Start with [Build a Atlas Plugin](../developer-guide/plugins/index.md)
...
You can also run a fully isolated Atlas instance (throwaway ATLAS_HOME, separate Electron
userData, distinct Electron app name to avoid the single-instance lock):

```bash
scripts/dev-sandbox.sh python -m atlas_cli.main
scripts/dev-sandbox.sh --persistent python -m atlas_cli.main desktop  # state survives restarts, but lives in the worktree :)
```
...
- **Profile-safe paths**: Never hardcode `~/.atlas` — use `get_atlas_home()` from `atlas_constants` for code paths and `display_atlas_home()` for user-facing messages. See [AGENTS.md](https://github.com/NousResearch/hermes-agent/blob/main/AGENTS.md#profiles-multi-instance-support) for full rules.
