---
sidebar_position: 7
title: "Use SOUL.md with Atlas"
description: "How to use SOUL.md to shape Atlas Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Atlas

`SOUL.md` is the **primary identity** for your Atlas instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Atlas to feel like the same assistant every time you talk to it — or if you want to replace the Atlas persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Atlas should be
- what Atlas should avoid stylistically
- how Atlas should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Atlas is and how Atlas speaks
...
Atlas now uses only the global SOUL file for the current instance:

```text
~/.atlas/SOUL.md
```

If you run Atlas with a custom home directory, it becomes:
...
Atlas automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Atlas does not overwrite it
- if the file exists but is empty, Atlas adds nothing from it to the prompt

## How Atlas uses it

When Atlas starts a session, it reads `SOUL.md` from `ATLAS_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Atlas feels.
...
Atlas already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Atlas should sound.

# Avoid
What Atlas should not do.

# Defaults
How Atlas should behave when ambiguity appears.
```
...
Then restart Atlas or start a new session.
...
### I edited SOUL.md but Atlas still sounds the same

Check:
- you edited `~/.atlas/SOUL.md` or `$ATLAS_HOME/SOUL.md`
...
### Atlas is ignoring parts of my SOUL.md
