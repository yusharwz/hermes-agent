...
- **`google-auth`** — installed automatically the first time you select Vertex (lazy install). Run `atlas setup` to repair a managed install if that fails.
...
Atlas lazy-installs it the first time you select the Vertex provider. If that fails, run `atlas setup` to repair the managed install.
