...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `atlas send`."
---

# Pipe Script Output to Messaging Platforms

`atlas send` is a small, scriptable CLI that pushes a message to any
messaging platform Atlas is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | atlas send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `atlas
...
atlas send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
atlas send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
atlas send --list

# Filter by platform
atlas send --list telegram
```
...
Any platform Atlas ships adapters for works as a target:
...
`atlas send` resolves the message body in this order:

1. **Positional argument** — `atlas send --to telegram "hi"`
2. **`--file PATH`** — `atlas send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | atlas send --to telegram`

When stdin is a TTY (no pipe), Atlas does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  atlas send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `atlas send` reuses your Atlas config, the same script works on
any host where Atlas is installed — no need to export bot tokens into
...
`atlas send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  atlas send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | atlas send \
...
```bash
./train.py --epochs 200 && \
  atlas send --to telegram "training done" || \
  atlas send --to telegram "training failed (exit $?)"
```
...
atlas send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(atlas send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `atlas send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `atlas send` calls
the platform's REST endpoint directly using credentials from
`~/.atlas/.env` and `~/.atlas/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `atlas gateway start` and retry.
...
# Machine-readable
atlas send --list --json
```

The listing is built from `~/.atlas/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`atlas gateway
...
| Approach | Multi-platform | Reuses Atlas creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `atlas send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`atlas send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `atlas send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `atlas send` shares with cron delivery.
