...
One command — `atlas setup --portal` — and you're ready to `atlas chat`. See [Nous Portal](/integrations/nous-portal).
:::
...
atlas chat --provider nous        # Use Nous Portal
...
- **Approvals still apply.** A dangerous command (`rm -rf`, writes to `~/.atlas/config.yaml`, etc.) goes through the same approval prompt the agent's `terminal` tool uses. `!` is a cost/latency shortcut, not a security bypass.
...
The first time you press Enter while Atlas is working, Atlas prints a one-line reminder explaining the `/busy` knob. It only fires once per install; `onboarding.seen.busy_input_prompt` in `config.yaml` records that it was shown. Delete that key to see the tip again.
:::
