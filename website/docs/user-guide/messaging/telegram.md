...
- Renaming the bot's `@username` in BotFather is picked up automatically — Atlas follows the new handle for mention routing without a gateway restart. Collectible (Fragment) usernames that don't end in `bot` are supported too.
