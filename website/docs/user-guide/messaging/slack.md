...
1. Generate the manifest. New Slack apps must use Agent view:
   ```bash
   atlas slack manifest --agent-view --write
   ```
   ...
   ```bash
   atlas slack manifest --agent-view \
...
can spend the entire turn in hidden reasoning and never produce visible assistant text; Atlas now
...
[`COMMAND_REGISTRY`](https://github.com/NousResearch/hermes-agent/blob/main/atlas_cli/commands.py)
...
New Slack apps use Slack's **Agent** messaging experience. Existing Atlas
...
to `message.im`, `app_home_opened`, and `app_context_changed`, so Atlas can
identify a Messages-tab DM and receive the user's active Slack context with a
turn. Atlas only supplies that context as a label; it does not read the viewed
channel's history.
...
The bang form also works behind a mention (`@Atlas !stop`) and with
...
fails, Atlas retries via a second ephemeral API path — a slash reply is never
posted publicly to the channel as a fallback. (Commands typed as regular
messages — `!cmd` in threads, `@Atlas /cmd` — reply as normal visible
messages instead.)
...
platforms:
  slack:
    ...
    extra:
      ...
      # that message itself @mentions Atlas, and "all" accepts every
      # other bot. Atlas always ignores its own bot user to prevent
      # self-echoes.
      allow_bots: "none"
...
| `platforms.slack.extra.allow_bots` | `"none"` | Controls messages from other Slack bots: `"none"` ignores them, `"mentions"` accepts a bot message only when **that message itself** @mentions Atlas, and `"all"` accepts all of them. Use `"mentions"` for the safest bot-to-bot collaboration mode. See [Accepting messages from other bots](#accepting-messages-from-other-bots-allow_bots). |
...
name in the thread. By default Atlas sets it to `is thinking...`; customize it
...
(`platforms.google_chat.typing_status_text`, default `"Atlas is thinking…"`) —
...
By default Atlas ignores every message authored by another Slack bot or app (including Workflow Builder posts). For multi-agent workspaces — several Atlas instances or peer bots collaborating in one channel — opt in with `allow_bots`:
...
- Atlas always ignores its own messages, in every mode, to prevent self-echo loops.

`mentions` is the recommended mode for bot-to-bot collaboration: each agent must explicitly summon the other per turn. Avoid `all` unless every peer bot's own reply policy is loop-safe — two bots that answer everything will answer each other forever. Detection covers labeled bot messages (`bot_id`, `subtype: bot_message`), app-originated events, and unlabeled bot *users* (probed via `users.info`), so peer Atlas agents are filtered consistently across workspaces.
...
manifest — regenerate with `atlas slack manifest`):
...
Delivery works even when the cron process isn't co-located with the gateway — Atlas falls back to a standalone Web API sender using `SLACK_BOT_TOKEN`. `MEDIA:` attachments in the cron output are uploaded as native Slack file shares to the same target.
