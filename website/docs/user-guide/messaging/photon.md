...
6. **Runs `npm install`** inside the plugin's sidecar directory. On
   ...
   `~/.atlas/photon/sidecar`; set `PHOTON_SIDECAR_DIR` to pin an
   explicit location.
...
- **Outbound attachments are supported.** Atlas sends images, voice
  ...
  media.
- **Native polls are supported.** Atlas sends poll content through
  spectrum-ts' `poll()` builder via the sidecar's `/send-poll` endpoint.
- **Message effects are supported.** Atlas sends text with native iMessage
...
- **Cron and standalone sends need the gateway running.** Out-of-process
  senders (cron jobs, `atlas send`, the dashboard) reuse the sidecar the
  gateway spawned — they read its port/token from
  `<atlas-home>/runtime/photon-sidecar.json`, written once the sidecar
...
- **Shared/free-tier lines can't initiate conversations with new
  ...
  brand-new recipient will be rejected by Photon even when Atlas is set
