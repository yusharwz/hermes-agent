...
atlas curator adopt <skill>    # hand an unmanaged skill to the curator
atlas curator adopt --all-unmanaged   # hand over every unmanaged skill
atlas curator list-unmanaged   # itemize skills with no provenance marker
...
`atlas curator status` reports an **unmanaged** count alongside the managed
one:
...
unmanaged (no provenance marker): 112 total
  ...
  never auto-staled or archived — `atlas curator adopt <name>` hands one over
```
...
```bash
atlas curator list-unmanaged                    # itemize them, with reasons
atlas curator adopt <name> [<name> ...]         # hand specific skills over
atlas curator adopt --all-unmanaged --dry-run   # preview the full list
atlas curator adopt --all-unmanaged             # hand over everything (prompts)
atlas curator adopt --all-unmanaged --yes       # skip the prompt
```
...
`.usage.json`; read it as policy. `atlas curator adopt` changes the policy, and
...
**wrote** it — Atlas edits user-authored skills on your behalf constantly. An
