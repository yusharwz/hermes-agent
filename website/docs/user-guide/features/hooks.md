...
Declare shell-script hooks in your `~/.atlas/config.yaml` and Atlas will run them as subprocesses whenever the corresponding plugin-hook event fires — in both CLI and gateway sessions. No Python plugin authoring required.
...
3. `hooks_auto_accept: true` in `~/.atlas/config.yaml`
