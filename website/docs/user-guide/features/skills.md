...
and `examples/`. Unreferenced repository files are not copied. Atlas scans the
...
Install `SKILL.md` directly from any HTTP(S) URL — useful when an author hosts a skill on their own site (no hub listing, no GitHub path to type). Atlas also fetches explicitly referenced files under `references/`, `templates/`, `scripts/`, `assets/`, and `examples/`, then scans and installs the complete bundle.

- Atlas source id: `url`
- Identifier: the URL itself (no prefix needed)
- Scope: `SKILL.md` plus exact referenced support files in the allowlisted directories. Atlas does not enumerate or copy unrelated files from the host.
...
If your skills don't live under `skills/` (common when you're adding a `skills/` subtree to an existing project), edit the tap entry in `~/.atlas/skills/.hub/taps.json`:
...
Taps are stored in `~/.atlas/skills/.hub/taps.json` (created on demand).
