...
Type `@` followed by a reference to inject content directly into your message. Atlas expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Atlas env: `$ATLAS_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$ATLAS_HOME/skills/.hub/`
