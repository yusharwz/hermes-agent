...
strings) are masked by Atlas' central secret redactor; the MoA filter adds
...
Atlas' normal reasoning controls: `none`, `minimal`, `low`, `medium`, `high`,
`xhigh`, `max`, and `ultra`.
...
Omit `reasoning_effort` to use the provider/Atlas default for that slot.
