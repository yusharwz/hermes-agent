...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Atlas's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Atlas instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
atlas memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `atlas` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Atlas syncs them back at session init.
...
The `atlas honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `atlas memory setup honcho` (or run `atlas memory setup` and pick it from the list); the `atlas honcho` subcommand then appears on the next invocation.

```bash
atlas memory setup honcho    # Configure Honcho directly (works before activation)
atlas honcho status          # Connection status, config, and key settings
atlas honcho setup           # Redirects to `atlas memory setup` (post-activation alias)
atlas honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
atlas honcho peer            # Show or update peer names + dialectic reasoning level
atlas honcho mode            # Show or set recall mode (hybrid/context/tools)
atlas honcho tokens          # Show or set token budget for context and dialectic
atlas honcho identity        # Seed or show the AI peer's Honcho identity
atlas honcho sync            # Sync Honcho config to all existing profiles
atlas honcho peers           # Show peer identities across all profiles
atlas honcho sessions        # List known Honcho session mappings
atlas honcho map             # Map current directory to a Honcho session name
atlas honcho enable          # Enable Honcho for the active profile
atlas honcho disable         # Disable Honcho for the active profile
atlas honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `atlas honcho`

If you previously used the standalone `atlas honcho setup`:
...
No re-login or re-setup needed. Run `atlas memory setup` and select "honcho" — the wizard detects your existing config.
