...
- **Snapshot size** — large pages may be truncated or LLM-summarized at 15,000 characters (matching `web_extract`); the complete snapshot is saved to `~/.atlas/cache/web/` and the output points at it for `read_file` paging
