...
enumerate every authenticated provider/model combination Atlas can route to,
and it does not do pricing or capability enrichment.

### GET /api/model/options

Atlas-aware clients can request the same curated provider/model inventory used
...
Normal opens are intentionally conservative for custom providers: Atlas probes
...
authenticated UI needs the richer Atlas-specific picker metadata.
...
Authenticated clients can override Atlas' default model selection per request
by sending:

- `model` — the target model id for this turn
- `provider` — the Atlas provider slug to resolve credentials/runtime for this turn
...
alias, Atlas rejects the request with `400` instead of silently remixing route
...
Requests that include an explicit `provider` — and the Atlas-native
...
- Requests to `/p/<profile>/v1/...` must present that profile's own
  `API_SERVER_KEY` (from `~/.atlas/profiles/<profile>/.env`). The default
...
The same settings can live in `~/.atlas/config.yaml` under a nested `gateway.api_server:` section:
...
- **Simple OpenAI clients still see an alias** — `/v1/models` advertises the
  stable Atlas alias (`atlas-agent` or the active profile name). Richer
