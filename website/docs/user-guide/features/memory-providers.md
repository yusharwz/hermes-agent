...
See the [config reference](https://github.com/NousResearch/hermes-agent/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/atlas).
...
`atlas memory setup` can reuse or copy connection values from
`~/.openviking/ovcli.conf`. Manual setup uses the active profile's `.env` file;
for the default profile that is `~/.atlas/.env`, and for named profiles use
`~/.atlas/profiles/<profile>/.env`.
...
```bash
atlas memory setup    # select "mem0" → "Self-hosted server"
# Or via flags:
atlas memory setup mem0 --mode selfhosted --host http://localhost:8888 --api-key your-admin-api-key
```

Or configure manually — either as env vars:

```bash
echo "MEM0_HOST=http://localhost:8888" >> ~/.atlas/.env
echo "MEM0_API_KEY=your-admin-api-key" >> ~/.atlas/.env
```
...
**Switching modes:** Re-run `atlas memory setup mem0 --mode <platform|selfhosted|oss>` or edit `mem0.json` directly.
...
| **Requires** | `pip install supermemory` + [cloud API key](http://app.supermemory.ai/integrations?connect=atlas), or a [self-hosted server](https://supermemory.ai/docs/self-hosting/overview) |
...
Before running `atlas memory setup`, set `base_url` in
`$ATLAS_HOME/supermemory.json`:
...
Then run `atlas memory setup` and enter the API key printed by the local
