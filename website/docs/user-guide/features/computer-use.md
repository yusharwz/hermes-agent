...
Atlas maps its existing approval UX onto cua-driver 0.10's immutable daemon
modes. There is no second permission toggle to keep in sync:

| Atlas session | cua-driver mode | Human intervention | `existing_profile` |
|---|---|---|---|
| Manual or smart approvals (default) | `standard` | Normal Atlas approvals; Cua stops at its protected boundary | Refuses unless a certified protected host is available; Atlas does not claim one today |
| `--yolo`, `/yolo`, or `approvals.mode: off` | private `unrestricted` daemon | One explicit Atlas risk acceptance; no runtime Cua prompts | Allowed within Cua's built-in, managed, and user policy ceilings |

The unrestricted daemon is private to that Atlas session. Turning `/yolo`
...
daemon's mode or grants another Atlas conversation the same authority.
...
Atlas intentionally keeps its skill (`skills/autonomous-ai-agents/computer-use/SKILL.md`)
...
- **Atlas spawns a `cua-driver mcp` stdio proxy.** In a normal session the
  proxy connects to (and may start) the standard machine daemon. In explicit
  Atlas YOLO, Atlas instead owns a private `cua-driver serve --embedded`
...
- **Atlas-side skill** — `skills/autonomous-ai-agents/computer-use/SKILL.md` — teaches the
