...
:::tip
Not using OpenAI Codex? `atlas setup --portal` configures a non-Codex backend with Claude/Gemini/etc. in one step. See [Nous Portal](/integrations/nous-portal).
:::
...
2. **Codex OAuth login.** The codex subprocess reads `~/.codex/auth.json`. Two ways to populate it:
   ...
   ```
   Atlas' own `atlas auth add openai-codex` writes to `~/.atlas/auth.json` — that's a separate session. **Run `codex login` separately** if you haven't.
...
- **Atlas auth and codex auth are separate sessions.** You need both `codex login` AND `atlas auth add openai-codex` for the cleanest UX (the runtime uses codex's session for the LLM call). This is a deliberate design choice in Atlas' `_import_codex_cli_tokens` — Atlas won't share OAuth state with codex CLI to avoid clobbering each other on token refresh.
...
If you find a bug, [open an issue](https://github.com/NousResearch/hermes-agent/issues) with the output of `atlas logs --since 5m`. Mention `codex-runtime` in the title so it's easy to triage.
