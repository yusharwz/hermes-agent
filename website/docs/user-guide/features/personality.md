...
If the file is empty, whitespace-only, or cannot be read, Atlas falls back to a built-in default identity ("You are Atlas Agent, an intelligent AI assistant created by Dritech..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
