...
See the [Plugins](/user-guide/features/plugins) page for the general plugin system, and [Build a Atlas Plugin](/developer-guide/plugins) to write your own.
...
- Post-meeting artifacts (transcript, status) saved under `~/.atlas/workspace/meetings/<meeting_id>/`

**Setup:**

```bash
atlas plugins enable google_meet
atlas meet setup   # preflight: playwright, chromium, auth file
atlas meet auth    # opens a browser to sign into Google and saves session state —
...
**Disabling:** `atlas plugins disable google_meet`. Any saved transcripts stay in `~/.atlas/workspace/meetings/` until you remove them.
...
Bundled plugins are written exactly like any other Atlas plugin — see [Build a Atlas Plugin](/developer-guide/plugins). The only differences are:
