...
| `timeout` | `300` | Seconds Atlas waits for an approval reply before timing out. |
...
### Mining Approval History (`atlas approvals suggest`)
...
```bash
atlas approvals suggest            # dry run — prints a numbered proposal
atlas approvals suggest --apply 1,3  # merge picks into command_allowlist
atlas approvals suggest --json     # machine-readable output
```

The command scans the session database (`~/.atlas/state.db`) for
...
Before `write_file` or `patch` touches disk, Atlas checks the target path against a denylist and an optional sandbox. Blocked writes return an error to the agent immediately — **there is no approval prompt** and no way to override from the chat UI. The model may still claim the edit succeeded; when `display.file_mutation_verifier` is on (default), trust the [file-mutation verifier footer](./configuration.md#file-mutation-verifier) over the assistant's closing summary.

### Protected paths (always blocked)

These categories are always denied, even when `ATLAS_WRITE_SAFE_ROOT` is unset:

| Category | Examples |
|----------|----------|
| OS credential stores | `~/.ssh/`, `~/.aws/`, `~/.kube/`, `/etc/sudoers`, `~/.netrc` |
| Atlas credential stores | `auth.json`, `.env`, `.anthropic_oauth.json`, `mcp-tokens/`, `pairing/` under ATLAS_HOME (active profile and global root) |
| Project secret files | `.env`, `.env.local`, `.env.production`, `.envrc` anywhere on disk |

Sensitive paths inside the safe root are still blocked — pointing `ATLAS_WRITE_SAFE_ROOT` at `$HOME` does not allow writing `~/.ssh/id_rsa`.

Safe-root violations return `Write denied: '…' is outside ATLAS_WRITE_SAFE_ROOT (…)`. Credential-path blocks use `Write denied: '…' is a protected system/credential file.`

### ATLAS_WRITE_SAFE_ROOT (optional sandbox)
...
- Set automatically in the [official Docker image](https://github.com/NousResearch/hermes-agent) (`ATLAS_WRITE_SAFE_ROOT=/opt/data`)
- Supports multiple roots separated by `:` on Unix or `;` on Windows
- **Do not add to `~/.atlas/.env` casually.** If you set it to a project directory, the agent cannot write to `~/.atlas/cron/jobs.json`, profile skills, or other Atlas state outside that prefix

To allow both a workspace and Atlas home:

```bash
export ATLAS_WRITE_SAFE_ROOT=/path/to/project:/home/you/.atlas
```

Unset the variable to restore unrestricted writes (subject to the protected-path denylist). Full reference: [ATLAS_WRITE_SAFE_ROOT](../reference/environment-variables.md#atlas_write_safe_root).

### Cron and other Atlas state

Do not ask the agent to `patch` `~/.atlas/cron/jobs.json` directly. Use the `cronjob` tool, [`atlas cron`](./features/cron.md), or `/cron` — they update the job store through the supported API. The same applies to other Atlas control files when write safety blocks direct edits.
...
:::tip Docker users: run pairing commands as the `atlas` user
The official Docker image runs the gateway as the unprivileged `atlas` user
...
```bash
docker exec -u atlas atlas-agent atlas pairing approve telegram ABC12DEF
```
