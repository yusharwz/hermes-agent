...
| | |
|---|---|
| Source | Optional — install with `atlas skills install official/creative/unreal-mcp` |
...
Companion skill for the `unreal-engine` entry in the Atlas MCP catalog. The
...
Two halves, in this order: the editor side must be up before Atlas connects.
...
### One-time, Atlas side
    ...
    atlas mcp install unreal-engine
...
`~/.atlas/config.yaml` under `mcp_servers.unreal-engine` to match.

Do NOT use `ModelContextProtocol.GenerateClientConfig` for Atlas — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. Atlas connects
...
2. Start the Atlas session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through Atlas
they appear as:
...
then happens at `atlas mcp install`/`configure` time instead. Tool-search
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — Atlas runs batched calls
...
- **Start order matters.** Editor + server up first, then the Atlas
...
- **Timeouts on long operations.** Atlas' per-call default is 120 s; asset
  imports, big level saves, and renders can exceed it. Raise
  `mcp_servers.unreal-engine.timeout` in `~/.atlas/config.yaml` for
