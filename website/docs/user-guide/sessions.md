...
```bash
atlas --resume 20250305_091523_a1b2c3 --no-restore-cwd
```
...
`atlas sessions list` accepts `--workspace <needle>` to show only sessions whose workspace key (git repo root, else cwd) matches — by path substring or exact directory basename:

```bash
atlas sessions list --workspace my-project
atlas sessions list --workspace ~/code/atlas-agent
```
...
`atlas sessions export` is one surface for every export format, selected with `--format`:
...
`--format trace` emits Claude Code JSONL — the transcript shape the Hugging Face Hub auto-detects for its [Agent Trace Viewer](https://huggingface.co/docs/hub/agent-traces). Write it locally, or add `--upload` to push it to your own private `atlas-traces` dataset (reads `HF_TOKEN`):
...
Pass `--format md` or `--format qmd` when you want a readable, file-based archive before hiding or deleting old sessions. Markdown/QMD exports write one file per session into a directory (default: `~/.atlas/session-exports`).
...
atlas sessions prune --older-than 12h

# Delete only a specific time window (e.g. a batch of test sessions
# created in the last 5 hours)
atlas sessions prune --newer-than 5h

# Explicit window with absolute timestamps
atlas sessions prune --after "2026-07-05 09:00" --before "2026-07-05 14:30"

# Only prune sessions from a specific platform (all ages — any filter
# disables the implicit 90-day default)
atlas sessions prune --source telegram
atlas sessions prune --source cron --older-than 60   # add a time flag to narrow

# More filters — all AND together
atlas sessions prune --newer-than 5h --title "smoke test"   # title substring
atlas sessions prune --older-than 30 --max-messages 3        # tiny sessions
atlas sessions prune --cwd ~/scratch --end-reason done       # by cwd / end reason
atlas sessions prune --model gpt-5 --older-than 1w           # by model (substring)
atlas sessions prune --provider openrouter --older-than 60   # by billing provider
atlas sessions prune --branch feature/old-experiment         # by git branch
atlas sessions prune --user 12345678 --chat-type group       # by messaging origin
atlas sessions prune --max-tokens 500 --older-than 7         # by token usage
atlas sessions prune --max-cost 0.01 --max-tool-calls 0      # cheap, tool-less runs

# Preview what would be deleted, without deleting anything
atlas sessions prune --newer-than 5h --dry-run
...
the implicit 90-day default, so `atlas sessions prune --source cron` or
`--model gpt-4o` matches all ages — add a time flag to narrow it. Only a
completely bare `atlas sessions prune` keeps the 90-day cutoff. Every
...
`atlas sessions archive` takes the same filters as `prune` but soft-hides
...
atlas sessions archive --newer-than 5h

# Archive by title substring, preview first
atlas sessions archive --title "dry run" --dry-run
atlas sessions archive --title "dry run" --yes
```

At least one filter is required — a bare `atlas sessions archive` refuses to
archive your entire history. Archived sessions are hidden from
`atlas sessions list` and `/resume` but remain in the database and can be
...
| Gateway routing index | `gateway_routing` table in `~/.atlas/state.db` | Maps session keys to active session IDs (origin metadata, expiry flags) |
| Legacy routing mirror | `~/.atlas/sessions/sessions.json` | Backward-compat mirror of the routing index, written when `gateway.write_sessions_json: true` (the default) |
...
`state.db`; `~/.atlas/sessions/sessions.json` is a **legacy mirror** of it,
