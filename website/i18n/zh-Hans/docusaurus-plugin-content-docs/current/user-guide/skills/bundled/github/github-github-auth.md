...
- 点击"New SSH key"
- 粘贴公钥内容
- 填写标题，如"atlas-agent-&lt;machine-name>"
...
uv run python3 "${ATLAS_HOME:-$HOME/.atlas}/skills/github/github-auth/scripts/git-credential-token.py"
```
...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  export GITHUB_TOKEN=$(uv run python3 "${ATLAS_HOME:-$HOME/.atlas}/skills/github/github-auth/scripts/git-credential-token.py")
  echo "AUTH_METHOD=curl"
else
