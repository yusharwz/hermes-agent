...
Registry and ABC for every inference provider Atlas knows about.
...
`$ATLAS_HOME/plugins/model-providers/<name>/` (per-user overrides). The
...
- `atlas_cli/auth.py` extends `PROVIDER_REGISTRY` with every api-key
  profile it sees (skipping `copilot`, `kimi-coding`, `kimi-coding-cn`,
  `zai`, `openrouter`, `custom` — those need bespoke token resolution).
- `atlas_cli/models.py` extends `CANONICAL_PROVIDERS` and calls
  `profile.fetch_models()` inside `provider_model_ids()`.
- `atlas_cli/doctor.py` adds a `/models` health check for each
  `auth_type="api_key"` profile.
- `atlas_cli/config.py` injects every `env_var` into
  `OPTIONAL_ENV_VARS` so the setup wizard knows about it.
- `atlas_cli/runtime_provider.py` reads `profile.api_mode` as a fallback
...
under `$ATLAS_HOME/plugins/model-providers/` for a private plugin).
