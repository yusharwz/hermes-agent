...
# the module header): when several Atlas processes log to the same file, an
    ...
    *root* directly, so emitting threads never block on file I/O or the
    ...
    with _queue_state_lock:
        if _log_queue is None:
            ...
            qh._atlas_queue = True  # type: ignore[attr-defined]
...
def drain_log_queue(timeout: float = 1.0) -> None:
    ...
    t = threading.Thread(target=_drain, name="atlas-log-drain", daemon=True)
    t.start()
    t.join(timeout)
...
def _reset_queued_handlers() -> None:
    ...
    with _queue_state_lock:
        ...
        for h in list(root.handlers):
            if getattr(h, "_atlas_queue", False):
...
def _read_logging_config():
    ...
    try:
        ...
        # reuses the parse atlas_cli.main's early bridge already did (one
        # config.yaml parse per process instead of 3-4). Fall back to a
        # direct parse when atlas_cli.config isn't importable (bare
        # atlas_logging consumers).
        try:
            from atlas_cli.config import read_raw_config as _rrc
            cfg = _rrc() or {}
        except Exception:
