...
# ``get_atlas_home() / "plugins"`` (user) and ``./.atlas/plugins/`` (project,
# gated behind ``ATLAS_ENABLE_PROJECT_PLUGINS``) — see
# ``atlas_cli/plugins.py:10-12``.  The guard only checked the bundled
...
# this script runs as a standalone subprocess — ``atlas_constants`` isn't
...
def main() -> int:
    ...
    # Add repo root to sys.path so we can import atlas_constants (this script
    ...
    # plugins in get_atlas_home()/plugins/ and project plugins in
    # ./.atlas/plugins/, where code like ori/hooks.py can spawn
    # subprocesses with inherited stdin — #67639).
    plugin_roots: list[Path] = [get_atlas_home() / "plugins"]
    if os.environ.get("ATLAS_ENABLE_PROJECT_PLUGINS"):
        plugin_roots.append(Path.cwd() / ".atlas" / "plugins")
