...
# (ATLAS_PYTHON is exported by the devShell hook and ships [dev] extras:
# pytest, pytest-asyncio, pytest-timeout, ruff, ty).
#
# A candidate must have pytest INSTALLED, not merely exist. The release venv
# at ~/.atlas/atlas-agent/venv has bin/activate but no pytest, so an
...
if [ -n "$VENV" ]; then
  PYTHON="$VENV_PYTHON"
elif [ -n "${ATLAS_PYTHON:-}" ] && [ -x "$ATLAS_PYTHON" ] \
    && "$ATLAS_PYTHON" -c 'import pytest' 2>/dev/null; then
  # Guard with an import check: ATLAS_PYTHON may point at the RELEASE
  # venv (no pytest) when inherited from a wrapped `atlas` binary rather
  # than the devShell hook.
  PYTHON="$ATLAS_PYTHON"
  echo "▶ no local venv — using Nix dev venv via ATLAS_PYTHON: $PYTHON"
else
  echo "error: no virtualenv with pytest found in $REPO_ROOT/.venv or $REPO_ROOT/venv," >&2
  echo "       and ATLAS_PYTHON is not a python with pytest (enter the Nix devShell or create a venv)" >&2
  if [ -n "$SKIPPED_VENVS" ]; then
...
exec env -i \
  ...
  ${ATLAS_E2E_BROWSER:+ATLAS_E2E_BROWSER="$ATLAS_E2E_BROWSER"} \
