...
print_banner() {
    ...
    echo "│             ⚕ Atlas Agent Installer                    │"
    echo "├─────────────────────────────────────────────────────────┤"
    echo "│  An open source AI agent by Dritech.              │"
    echo "└─────────────────────────────────────────────────────────┘"
    echo -e "${NC}"
}
...
setup_path() {
    ...
    if [ "$USE_VENV" = true ]; then
        ATLAS_BIN="$INSTALL_DIR/venv/bin/python"
        ATLAS_ENTRYPOINT="$INSTALL_DIR/atlas"
    else
    ...
    if [ ! -x "$ATLAS_BIN" ] || { [ "$USE_VENV" = true ] && [ ! -f "$ATLAS_ENTRYPOINT" ]; }; then
        log_warn "Atlas launcher prerequisites not found"
...
unset PYTHONPATH
unset PYTHONHOME
exec "$ATLAS_BIN" "$ATLAS_ENTRYPOINT" "\$@"
EOF
    else
        cat > "$command_link_dir/atlas" <<EOF
#!/usr/bin/env bash
...
EOF
    ...
    # Also expose `atlas-agent`. The `atlas-agent` console script declared in
    ...
    rm -f "$command_link_dir/atlas-agent"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/atlas-agent" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$ATLAS_BIN" "$INSTALL_DIR/run_agent.py" "\$@"
EOF
    else
        cat > "$command_link_dir/atlas-agent" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$ATLAS_BIN" run_agent.py "\$@"
EOF
    fi
    chmod +x "$command_link_dir/atlas-agent"
    log_success "Installed atlas-agent launcher → $command_link_display_dir/atlas-agent"

    # Also expose `atlas-acp`. ACP hosts (Zed, JetBrains, Buzz) resolve the
    # agent by command name on the login-shell PATH, and the `atlas-acp`
    # console script lives inside the venv, which is not on that PATH. Without
    # this launcher those hosts report Atlas as not installed. (#21454 applies
    # here too: clear the path first so `cat >` cannot follow an old symlink
    # into the venv and overwrite the console script.)
    rm -f "$command_link_dir/atlas-acp"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/atlas-acp" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$ATLAS_BIN" "$ATLAS_ENTRYPOINT" acp "\$@"
EOF
    else
        cat > "$command_link_dir/atlas-acp" <<EOF
#!/usr/bin/env bash
...
EOF
    ...
    chmod +x "$command_link_dir/atlas-acp"
    log_success "Installed atlas-acp launcher → $command_link_display_dir/atlas-acp"
...
write_bootstrap_marker() {
    # Writes $INSTALL_DIR/.atlas-bootstrap-complete, which tells the Atlas
    ...
    local marker_path="$INSTALL_DIR/.atlas-bootstrap-complete"
...
install_desktop() {
    ...
    # `atlas desktop`, so install/repair and self-update agree about the app's
    ...
    # importing Python cannot reverse it by loading ATLAS_HOME/.env. If the
    ...
    if [ "$OS" = "macos" ] && [ -z "${CSC_LINK:-}" ] && [ -z "${APPLE_SIGNING_IDENTITY:-}" ] && command -v codesign >/dev/null 2>&1; then
        ...
        if [ -x "$config_python" ]; then
            if ATLAS_HOME="$ATLAS_HOME" "$config_python" - "$desktop_dir" <<'PYEOF'
import sys
from pathlib import Path
from atlas_cli.main import _desktop_macos_relaunchable_fixup
