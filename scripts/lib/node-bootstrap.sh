...
#   2. ~/.atlas/node/ from a prior Atlas-managed install
#   3. fnm, proto, nvm (in that order) if the user already uses a version manager
#   4. Termux `pkg`, macOS Homebrew
#   5. pinned nodejs.org tarball into ~/.atlas/node/ (always works, zero shell rc edits)
#
# Usage:
#   source scripts/lib/node-bootstrap.sh
#   ensure_node   # returns 0 on success, non-zero on failure
#   if [ "$ATLAS_NODE_AVAILABLE" = true ]; then ...; fi
#
# Env inputs (set before sourcing to override defaults):
#   ATLAS_NODE_MIN_VERSION   (default: 20)   — accepted on PATH
#   ATLAS_NODE_TARGET_MAJOR  (default: 22)   — installed when we install
#   ATLAS_HOME               (default: $HOME/.atlas)
# ============================================================================

ATLAS_NODE_MIN_VERSION="${ATLAS_NODE_MIN_VERSION:-20}"
ATLAS_NODE_TARGET_MAJOR="${ATLAS_NODE_TARGET_MAJOR:-22}"
ATLAS_HOME="${ATLAS_HOME:-$HOME/.atlas}"
ATLAS_NODE_AVAILABLE=false
...
# Redirect a Atlas-managed Node's `npm install -g` to the command link dir
# (already on PATH) instead of the default $ATLAS_HOME/node/bin, which is off
...
_nb_configure_npm_prefix() {
    ...
    printf 'prefix=%s\n' "$(dirname "$_link_dir")" > "$ATLAS_HOME/node/etc/npmrc"
}
...
_nb_have_modern_node() {
    ...
    [ "$(_nb_node_major)" -ge "$ATLAS_NODE_MIN_VERSION" ]
}
...
_nb_try_fnm() {
    ...
    _nb_log "fnm detected — installing Node $ATLAS_NODE_TARGET_MAJOR..."
    eval "$(fnm env 2>/dev/null)" || true
    fnm install "$ATLAS_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    fnm use     "$ATLAS_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
...
_nb_try_proto() {
    ...
    _nb_log "proto detected — installing Node $ATLAS_NODE_TARGET_MAJOR..."
    proto install node "$ATLAS_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    _nb_have_modern_node || return 1
...
_nb_try_nvm() {
    ...
    _nb_log "nvm detected — installing Node $ATLAS_NODE_TARGET_MAJOR..."
    nvm install "$ATLAS_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    nvm use     "$ATLAS_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    _nb_have_modern_node || return 1
...
_nb_try_brew() {
    ...
    brew install "node@${ATLAS_NODE_TARGET_MAJOR}" >/dev/null 2>&1 \
        || brew install node >/dev/null 2>&1 \
        || return 1
    brew link --overwrite --force "node@${ATLAS_NODE_TARGET_MAJOR}" >/dev/null 2>&1 || true
    _nb_have_modern_node || return 1
...
_nb_install_bundled_node() {
    ...
    local index_url="https://nodejs.org/dist/latest-v${ATLAS_NODE_TARGET_MAJOR}.x/"
    local tarball
    tarball=$(curl -fsSL "$index_url" \
        | grep -oE "node-v${ATLAS_NODE_TARGET_MAJOR}\.[0-9]+\.[0-9]+-${node_os}-${node_arch}\.tar\.xz" \
        | head -1)
    if [ -z "$tarball" ]; then
        tarball=$(curl -fsSL "$index_url" \
            | grep -oE "node-v${ATLAS_NODE_TARGET_MAJOR}\.[0-9]+\.[0-9]+-${node_os}-${node_arch}\.tar\.gz" \
            | head -1)
    fi
    if [ -z "$tarball" ]; then
        _nb_warn "Could not resolve Node $ATLAS_NODE_TARGET_MAJOR binary for $node_os-$node_arch"
        return 1
    fi
    ...
    _nb_log "Extracting to $ATLAS_HOME/node/..."
    ...
    mv "$extracted" "$ATLAS_HOME/node"
    rm -rf "$tmp"

    local _link_dir
    _link_dir="$(_nb_get_link_dir)"
    mkdir -p "$_link_dir"
    ln -sf "$ATLAS_HOME/node/bin/node" "$_link_dir/node"
    ln -sf "$ATLAS_HOME/node/bin/npm"  "$_link_dir/npm"
    ln -sf "$ATLAS_HOME/node/bin/npx"  "$_link_dir/npx"
    ...
    _nb_have_modern_node || return 1
    _nb_ok "Node $(node --version) installed to $ATLAS_HOME/node/"
    return 0
}

# ---------------------------------------------------------------------------
# Heal a broken Atlas-managed Node tree (partial upgrade / missing lib/)
# ---------------------------------------------------------------------------
...
_nb_managed_tool_broken() {
    ...
    local probe
    for probe in \
        "$ATLAS_HOME/node/bin/$tool" \
        "$ATLAS_HOME/node/${tool}.exe" \
        "$ATLAS_HOME/node/$tool"; do
...
# absent. Used by atlas_constants.find_atlas_node_executable() and safe
# to call from install reruns.
heal_managed_node() {
    [ -d "$ATLAS_HOME/node" ] || return 1
    if ! _nb_managed_node_needs_heal; then
        return 0
    fi
    _nb_log "Atlas-managed Node is broken — redownloading to $ATLAS_HOME/node/..."
    _nb_install_bundled_node
}
...
ensure_node() {
    ...
    if _nb_have_modern_node; then
        ...
        ATLAS_NODE_AVAILABLE=true
        return 0
    fi

    if [ -x "$ATLAS_HOME/node/bin/node" ]; then
        export PATH="$ATLAS_HOME/node/bin:$PATH"
        if _nb_have_modern_node; then
            _nb_ok "Node $(node --version) found (Atlas-managed)"
            ATLAS_NODE_AVAILABLE=true
            return 0
        fi
    fi
    ...
    _nb_try_fnm   && { ATLAS_NODE_AVAILABLE=true; return 0; }
    _nb_try_proto && { ATLAS_NODE_AVAILABLE=true; return 0; }
    _nb_try_nvm   && { ATLAS_NODE_AVAILABLE=true; return 0; }

    # Platform package managers.
    _nb_try_termux_pkg && { ATLAS_NODE_AVAILABLE=true; return 0; }
    _nb_try_brew       && { ATLAS_NODE_AVAILABLE=true; return 0; }

    # Last resort: pinned nodejs.org tarball.
    _nb_install_bundled_node && { ATLAS_NODE_AVAILABLE=true; return 0; }

    _nb_warn "Node.js install failed — TUI and browser tools will be unavailable."
    _nb_warn "Install manually: https://nodejs.org/en/download/  (or: \`brew install node\`, \`fnm install $ATLAS_NODE_TARGET_MAJOR\`, etc.)"
    return 1
}
