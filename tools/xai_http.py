...
def has_xai_credentials() -> bool:
    ...
    3. ``credential_pool.xai-oauth`` has any entry with a non-empty
       ``access_token`` (covers multi-account ``atlas auth add xai-oauth``
...
def atlas_xai_default_headers() -> Dict[str, str]:
    ...
    so chat/completions and Responses traffic is attributed as Atlas Agent,
    matching the direct HTTP integrations (search, TTS, STT, image, video).
    """
    return {"User-Agent": atlas_xai_user_agent()}
    ...
    *,
    ...
    try:
        ...
        override_base_url = str(
            get_env_value("ATLAS_XAI_BASE_URL")
