...
LAZY_DEPS: dict[str, tuple[str, ...]] = {
    ...
    # HF Agent Trace Viewer upload (atlas trace upload / /upload-trace).
    #
    ...
    # presence, the `atlas update` lazy-refresh pass re-asserts THIS pin on
...
def install_specs(specs: list[str] | tuple[str, ...], *, timeout: int = 300) -> InstallSpecsResult:
    ...
    * **Durable-target on immutable images** — when the deployment seals the
      agent venv (``ATLAS_DISABLE_LAZY_INSTALLS=1``) and sets
      ``ATLAS_LAZY_INSTALL_TARGET``, installs are redirected to the writable
