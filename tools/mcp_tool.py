...
def _check_logging_callback_support() -> bool:
    ...
    server emits, so server-side diagnostics never reach Atlas' logs.
    """
    if not _MCP_AVAILABLE:
        return False
    try:
...
def _build_safe_env(user_env: Optional[dict]) -> dict:
    ...
    Atlas explicitly tagged during dotenv loading, plus any variables
    ...
    an exception: users configured that backend specifically so Atlas and
...
def _render_mcp_resource_block(block, server_name: str = "") -> str:
    ...
    - ``EmbeddedResource`` with blob contents → bytes are decoded (size-capped)
      and materialized into the Atlas document cache; returns a marker with
...
def _unwrap_exception_group(exc: BaseException) -> BaseException:
    ...
    Adapted from :func:`atlas_cli.mcp_config._unwrap_exception_group` with
...
class MCPServerTask:
    ...
    def _make_logging_callback(self):
        ...
        server into Atlas' logging (agent.log via atlas_logging), tagged
    ...
    async def _run_stdio(self, config: dict):
        ...
        if not _MCP_AVAILABLE:
            raise ImportError(
                ...
                "it is not installed. Run `atlas setup` to install MCP support, "
                "then retry."
            )
        ...
        # ungraceful exit of this Atlas process (kill -9, crash, force-quit)
...
# ---------------------------------------------------------------------------
# Advisory file lock that prevents N concurrent Atlas processes (e.g.
    ...
    try:
        ...
        if _MCP_DISCOVERY_LOCK_PATH is None:
            _MCP_DISCOVERY_LOCK_PATH = str(
                get_atlas_home() / ".mcp-discovery.lock"
            )
...
# Native MCP tool-name prefix. Atlas uses the ``mcp__<server>__<tool>``
