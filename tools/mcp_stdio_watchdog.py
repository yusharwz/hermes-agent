...
<url>``) is spawned as a direct child of the Atlas process. Atlas's own
...
Atlas process dies hard — ``kill -9``, an OS-level crash, a force-quit of
...
parent dies, so nothing reaps these until the next Atlas startup's opt-in
