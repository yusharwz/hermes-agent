...
def _skills_dir() -> Path:
    ...
    kanban workers) import this module once under the launch ATLAS_HOME and
    ...
    live profile-scoped ATLAS_HOME on every call.
    """
...
def _background_review_write_guard(
    ...
    try:
        ...
        # shapes; `atlas curator adopt <name>` is the supported way in.
        ...
        if not skill_usage._is_curator_managed_record(usage_rec):
            ...
            return {
                ...
                "error": (
                    ...
                    f"Run `atlas curator adopt {name}` to opt it in."
                ),
            }
    except Exception:
    ...
    try:
        ...
        if not ssc.sync_org_auto_propose():
            return (
                ...
                f"Run `atlas sync propose {name}` to share it back."
            )
        ...
        return (
            ...
            f"right now — run `atlas sync propose {name}` to retry."
        )
...
def _org_mirror_write_guard(name: str, skill_path: Path, action: str) -> Optional[Dict[str, Any]]:
    ...
    reaches the organisation when the user runs `atlas sync propose`, or
        ...
        if is_org_mirror_path(skill_path, _skills_dir()):
            return {
                ...
                "error": (
                    ...
                    "and can be proposed back with `atlas sync propose "
                    f"{name}`.)"
                ),
            }
    except Exception:
