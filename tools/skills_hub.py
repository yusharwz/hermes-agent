...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by Dritech) but not activated
...
ATLAS_INDEX_URL = "https://hermes-agent.nousresearch.com/docs/api/skills-index.json"
...
def _load_atlas_index() -> Optional[dict]:
    ...
    if atlas_index_cache_file.exists():
        try:
            ...
            if age < ATLAS_INDEX_TTL:
                return json.loads(atlas_index_cache_file.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError):
            pass
    ...
    for accept_encoding in ("gzip, deflate", "identity"):
        try:
            ...
            logger.debug(
                "Atlas index decode failed (Accept-Encoding=%s): %s",
                accept_encoding,
                e,
            )
            continue
    ...
    try:
        ...
        atlas_index_cache_file.write_text(json.dumps(data), encoding="utf-8")
    except OSError:
        pass
