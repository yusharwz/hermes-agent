...
def _get_token_dir(atlas_home: str | Path | None = None) -> Path:
    ...
    base = Path(atlas_home) if atlas_home is not None else Path(get_atlas_home())
...
def _cached_redirect_port(storage: "AtlasTokenStorage | None") -> int | None:
    ...
    redirect URI that was registered with it. If Atlas restarts and chooses a
...
def _cached_redirect_uri(storage: "AtlasTokenStorage | None") -> str | None:
...
class AtlasTokenStorage:
    ...
    def __init__(self, server_name: str, *, atlas_home: str | Path | None = None):
        self._server_name = _safe_filename(server_name)
        self._atlas_home = Path(atlas_home) if atlas_home is not None else None

    def _tokens_path(self) -> Path:
        return _get_token_dir(self._atlas_home) / f"{self._server_name}.json"

    def _client_info_path(self) -> Path:
        return _get_token_dir(self._atlas_home) / f"{self._server_name}.client.json"

    def _meta_path(self) -> Path:
        return _get_token_dir(self._atlas_home) / f"{self._server_name}.meta.json"
    ...
    def restore(self, snapshot: dict[str, bytes], *, only_if_absent: bool = False) -> None:
        ...
        if not snapshot:
            return
        token_dir = _get_token_dir(self._atlas_home)
...
def remove_oauth_tokens(
    ...
    *,
    ...
    storage = AtlasTokenStorage(server_name, atlas_home=atlas_home)
    storage.remove()
...
def _configure_callback_port(
    cfg: dict,
    storage: "AtlasTokenStorage | None" = None,
) -> int:
...
#   "Claude Code" → 200
#   "Codex"       → 200
#   "Atlas Agent" / "Atlas" / "Cursor" / "VS Code" / … → 403
...
def humanize_oauth_registration_error(
    ...
    *,
    ...
    Figma's remote MCP gates DCR on exact ``client_name``. Atlas auto-sets
    ``Claude Code`` (known-good); this message fires when the user overrode
    that with something Figma still rejects, or an older Atlas is running.
    """
    msg = str(exc)
    lowered = msg.lower()
