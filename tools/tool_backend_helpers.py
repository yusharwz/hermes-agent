...
def resolve_provider_secret(
    ...
    Resolution order (fixes #68003 — keys added via ``atlas auth add
    ...
    2. The environment / ``~/.atlas/.env``. Under a multiplexed gateway turn
       ...
       ``atlas_cli.config.get_env_value`` (os.environ, then ``.env``),
       matching the tools' historical behaviour exactly.
    3. The credential pool / auth store for ``provider_id`` (``atlas auth
    ...
    when omitted, ``atlas_cli.config.get_env_value`` is used directly.
    """
    ...
    try:
        ...
        # `atlas auth add <provider>` keys a registry provider by its plain
...
def resolve_openai_audio_api_key() -> str:
    ...
    the credential pool (``atlas auth add openai-api``) via
