...
def _blocked_toolsets_for_role(role: str) -> List[str]:
    ...
    mixed platform bundles such as ``atlas-cli`` because those also contain
...
def _build_child_agent(
    ...
    # Blocked tools also live inside mixed platform bundles (atlas-cli,
    # atlas-telegram, etc.) that _strip_blocked_tools must keep because they
    ...
    # child Atlas/Qwen subagent onto the parent's Claude Messages wire (or the
...
def _finalize_child_results(
    ...
    with _parent_finalization_lock(parent_agent):
        ...
        parent_session_id = getattr(parent_agent, "session_id", None)
        try:
            from atlas_cli.plugins import invoke_hook as invoke_hook
        except Exception:
            invoke_hook = None
...
def delegate_task(
    ...
    # ATLAS_SESSION_ID ContextVar and os.environ with the subagent's internal
    # id before the background-dispatch code below would read it. The
    # request-scoped chat_id binding (the raw X-Atlas-Session-Id on
    ...
    if background:
        ...
        if not _async_ok:
            ...
            # with that id in X-Atlas-Session-Id once the batch completes.
            ...
            # ATLAS_SESSION_ID here would return the subagent's internal id.
        ...
        if not _async_ok:
            ...
            if isinstance(_sync_result, dict):
                _sync_result["note"] = (
                    ...
                    "one-shot runner such as `atlas -z`, a cron job, a Kanban "
        ...
        try:
            ...
            _source = get_session_env("ATLAS_SESSION_SOURCE", "")
            _origin_ui_session_id = get_session_env("ATLAS_UI_SESSION_ID", "")
        ...
        if not _session_key:
            ...
            # during gateway/TUI turns and ATLAS_SESSION_KEY is not in the CLI
...
def _load_config() -> dict:
    """Load delegation config from the active Atlas config.

    Prefer the shared persistent loader because it follows the active
    ATLAS_HOME/profile. ``cli.CLI_CONFIG`` is a legacy fallback for entry
    ...
    ``ATLAS_IGNORE_USER_CONFIG=1`` (``atlas chat --ignore-user-config``) is
    ...
    """
    prefer_legacy = os.environ.get("ATLAS_IGNORE_USER_CONFIG") == "1"
