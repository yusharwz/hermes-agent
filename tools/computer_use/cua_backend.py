...
_CUA_DRIVER_CMD_ENV = "ATLAS_CUA_DRIVER_CMD"
...
def _cua_no_overlay() -> bool:
    """True when Atlas should pass ``--no-overlay`` to cua-driver.
...
def _wsl_windows_path_to_posix(path: str) -> str:
    """Translate a Windows absolute manifest command when Atlas runs in WSL.

    Windows cua-driver manifests can report ``C:\\Users\\...\\cua-driver.exe``
    even though the Atlas process uses POSIX subprocess spawning inside WSL.
...
class _EmbeddedCuaDaemon:
    ...
    machine-wide daemon would therefore let one Atlas session's YOLO choice
    ...
    def __init__(self, driver_cmd: str, permission_mode: str) -> None:
        ...
        if sys.platform == "win32":
            self.socket_path = rf"\\.\pipe\atlas-cua-{token}"
        else:
...
def _resolve_mcp_invocation(
    ...
    *,
    ...
    # A Windows-installed cua-driver can hand a WSL-hosted Atlas an absolute
...
def _candidate_cua_driver_commands(override: Optional[str] = None) -> List[str]:
    ...
    ``ATLAS_CUA_DRIVER_CMD`` is authoritative; only when neither is set do we
    ...
    Atlas Desktop/TUI session can otherwise filter out the `computer_use`
    tool even though `atlas computer-use doctor` succeeds from a login shell.
    """
...
def resolve_cua_driver_cmd(override: Optional[str] = None) -> Optional[str]:
    ...
    A supplied override (or ``ATLAS_CUA_DRIVER_CMD``) is never silently
    ...
    def _start_lifecycle_locked(self) -> None:
        ...
        if not self._ready_event.wait(timeout=30.0):
            ...
            raise RuntimeError(
                ...
                "Run `atlas computer-use doctor` and check "
                f"{display_atlas_home()}/logs/agent.log for the phase timings."
            )
...
class CuaDriverBackend(ComputerUseBackend):
    ...
    def focus_app(self, app: str, raise_window: bool = False) -> ActionResult:
        ...
        separately approved by the Atlas adapter, and uses cua-driver's
