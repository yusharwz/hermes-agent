...
DANGEROUS_PATTERNS = [
    ...
    # These are native Windows EXEs / cmdlets reachable from ANY Atlas
    ...
    (r'\bappdata[\\/](?:local|roaming)[\\/]atlas[^\n]*\.env\b', "access to Atlas secrets (Windows path)"),
    # ─────────────────────────────────────────────────────────────────────
    ...
    # a lifecycle command past the guard — same treatment as the `atlas ...
...
def _mask_quoted_newlines(command: str) -> str:
    ...
    start. That made any multi-line quoted argument (``atlas send`` message
...
def _is_verification_artifact_cleanup(command: str) -> bool:
    """Return whether *command* only removes one Atlas ad-hoc temp script."""
    try:
    ...
    if os.path.dirname(target) != temp_dir:
        return False
    return re.fullmatch(r"atlas-(?:verify|ad-hoc)-[A-Za-z0-9_.-]+", basename) is not None
...
def _release_permission_mode_dependents(session_key: str) -> None:
    """Drop resources whose immutable mode is derived from Atlas YOLO.
...
def is_approval_bypass_active_for_session(session_key: str) -> bool:
    """Return whether one exact session bypasses Atlas approval prompts.
    ...
    *,
    ...
    Args:
        ...
        fail_closed_when_no_human: When True, a non-interactive non-gateway
            ...
            ATLAS_INTERACTIVE unset) BLOCKS instead of auto-approving. The
    ...
    if not is_cli and not is_gateway:
        ...
        if env_var_enabled("ATLAS_CRON_SESSION"):
            ...
            logger.warning(
                ...
                "BLOCKED (fail-closed). Set ATLAS_INTERACTIVE or "
                "ATLAS_GATEWAY_SESSION to answer the prompt.",
        ...
        logger.warning(
            "%s (pattern: %s): %s — set ATLAS_INTERACTIVE or "
            "ATLAS_GATEWAY_SESSION to require approval.",
            autoapprove_log_prefix, pattern_key, description,
        )
        return {"approved": True, "message": None}
...
def request_tool_approval(
    ...
    *,
    ...
    (a bare script with no ``ATLAS_INTERACTIVE``) fails CLOSED — a plugin-
