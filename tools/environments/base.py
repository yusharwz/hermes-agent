...
# the FIRST session's ATLAS_SESSION_ID into the snapshot makes every LATER
...
# ATLAS_SESSION_ID leak via the shared snapshot). Stripping them from the
...
# set), not Atlas' per-turn session identity.
#
# Kept in sync with gateway.session_context._VAR_MAP: every bridged name starts
# with one of these prefixes (or is ATLAS_UI_SESSION_ID). Used by unit tests
# as the Python-side contract for the exclusion set; the dump path unsets by
# name/prefix instead of grepping declare lines (see below / issue #71296).
_SNAPSHOT_EXCLUDED_ENV_REGEX = (
    "^declare -x (ATLAS_SESSION_|ATLAS_UI_SESSION_ID|ATLAS_CRON_AUTO_DELIVER_)"
)
...
def _export_dump_excluding_session_vars(tmp_path: str) -> str:
    ...
    Matrix room/display name via ``ATLAS_SESSION_CHAT_NAME``) land in the
    ...
    return (
        "{ ( "
        "unset ${!ATLAS_SESSION_*} ${!ATLAS_CRON_AUTO_DELIVER_*} "
        "ATLAS_UI_SESSION_ID 2>/dev/null; "
        "export -p; "
        ") || true; } "
        f"> {tmp_path}"
    )
...
class BaseEnvironment(ABC):
    ...
    def _wrap_command(self, command: str, cwd: str) -> str:
        ...
        # Restrict Atlas metadata files without changing the user's command
