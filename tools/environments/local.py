...
def _resolve_local_initial_cwd(cwd: str) -> str:
    ...
    directory Atlas was already launched from (for example ``atlas-agent``
    while the process cwd is ``~/.atlas/atlas-agent``), passing it through
    unchanged makes the wrapper run ``cd atlas-agent`` *inside* the project
    ...
    # Common recovery for config values like ``atlas-agent`` when Atlas was
    # launched from that directory already.  ``os.path.abspath`` would point at
    # a nonexistent nested ``./atlas-agent``; use the current directory instead.
...
def atlas_subprocess_env(*, inherit_credentials: bool = False) -> dict[str, str]:
    ...
    * **Tier 2 (conditional):** the rest of ``_ATLAS_PROVIDER_ENV_BLOCKLIST``
    ...
    # still see the parent's ATLAS_HOME but lose the DB mutation guard.
...
def build_subprocess_env(
    ...
    *,
    ...
    function (or :func:`atlas_subprocess_env` for the model-driving-CLI
    surface) instead of copying ``os.environ`` directly, so profile-home
    propagation (``ATLAS_HOME`` / subprocess ``HOME`` contract) and the
    Atlas secret-scrub policy have a single owner.  History: ~11 separate
    ...
    * ``scrub_secrets=True`` (default) — delegate to
      ...
      list (provider blocklist + ``_is_atlas_internal_secret`` dynamic
      patterns + kanban/venv-marker/session-context guards) **and** of
      ``ATLAS_HOME`` / subprocess-HOME propagation.  On this path profile
    ...
    * ``inherit_profile_home`` — on the non-scrub path, when True, bridge the
      context-local Atlas home override into ``ATLAS_HOME`` and apply the
      subprocess HOME contract (``atlas_constants.apply_subprocess_home_env``).
    ...
    * ``extra`` — applied **last** on the non-scrub path so explicit caller
      overrides (e.g. a session-scoped ``ATLAS_HOME``) always win.  On the
      scrub path it is forwarded as ``_sanitize_subprocess_env``'s
      ``extra_env`` (same force-prefix / blocklist handling as today).
    """
    if scrub_secrets:
        # _sanitize_subprocess_env already performs ATLAS_HOME override
...
def _find_bash() -> str:
    ...
    # system Git (or a stale ATLAS_GIT_BASH_PATH pointing at one) must not
    ...
    # ATLAS_GIT_BASH_PATH pointing at a broken Git-for-Windows install
    # (``Directory \\drivers\\etc does not exist``) must not win over a
    # healthy portable Git under %LOCALAPPDATA%\\atlas\\git.
    for candidate in candidates:
        if _bash_starts(candidate):
            if candidate != custom and custom and os.path.isfile(custom):
                logger.warning(
                    "ATLAS_GIT_BASH_PATH=%s fails to start; using %s instead",
                    custom,
                    candidate,
                )
            return candidate
...
def _git_bash_aslr_help(bash: str, details: str = "") -> str:
    ...
    return (
        ...
        "Then restart Atlas. If the override is blocked or later re-applied, "
