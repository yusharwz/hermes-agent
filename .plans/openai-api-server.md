# OpenAI-Compatible API Server for Atlas Agent
...
SSE streaming. By exposing this endpoint, atlas-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  atlas-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.atlas/.env`
2. Run `atlas gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with atlas-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns atlas-agent as a model) |
...
Authorization: Bearer atlas-api-key-here
...
But atlas-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "atlas-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured atlas-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of atlas-agent's core prompt
...
Once implemented, atlas-agent works as a drop-in backend for:
