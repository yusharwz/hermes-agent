"""MCP picker — interactive `atlas mcp picker` (also the default `atlas mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``atlas mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `atlas plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from atlas_cli.cli_output import prompt_yes_no
from atlas_cli.curses_ui import curses_single_select
from atlas_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Atlas session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from atlas_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `atlas mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: atlas mcp install <name>    Picker: atlas mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Atlas is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Atlas — run `atlas update` "
...
def show_catalog() -> None:
    """`atlas mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`atlas mcp picker` (and default `atlas mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`atlas mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `atlas mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
