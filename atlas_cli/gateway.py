...
def _scan_gateway_pids(
    ...
    # Forward slashes on both sides of the ATLAS_HOME= match — see
...
def _prepare_profile_gateway_update_restart(profile: str, pid: int) -> str | None:
    """Choose who relaunches a profile gateway after ``atlas update``.

    A gateway started with ``--external-supervisor`` must exit back to that
    manager. Starting Atlas's detached watcher as well would escape the
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    # interpreter and captures the stable cwd + env overlay (ATLAS_HOME,
...
def _atlas_home_from_systemd_unit_file(system: bool = False) -> str | None:
    """Read ``ATLAS_HOME`` from the on-disk unit file (not ``systemctl show``).
...
def _sync_atlas_home_from_systemd_unit(system: bool) -> None:
    ...
    unit_home = (_atlas_home_from_systemd_unit_file(system=True) or "").strip()
    if not unit_home:
        unit_home = _read_systemd_unit_environment(system=True).get("ATLAS_HOME", "").strip()
    if not unit_home:
        return
...
def _atlas_home_for_target_user(target_home_dir: str) -> str:
    ...
    """
    current_atlas_raw = os.environ.get("ATLAS_HOME", "").strip()
    current_atlas = (
        Path(current_atlas_raw).expanduser()
        if current_atlas_raw
        else get_atlas_home()
    )
    ...
    # different ATLAS_HOME into the generated service unit.
    current_default = Path.home() / ".atlas"
...
def _systemd_watchdog_seconds(atlas_home: str | Path | None = None) -> int:
    ...
    if atlas_home is not None:
        ...
        override_token = set_atlas_home_override(atlas_home)
        reset_home_override = reset_atlas_home_override
    try:
...
def _systemd_watchdog_service_fields(
    atlas_home: str | Path | None = None,
) -> tuple[str, str]:
    """Return systemd service fields for the effective gateway config."""
    seconds = _systemd_watchdog_seconds(atlas_home)
    if seconds <= 0:
        return "simple", ""
...
[Service]
Type={systemd_type}
{systemd_watchdog_directives}ExecStart={python_path} -m atlas_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
WorkingDirectory={working_dir}
Environment="PATH={sane_path}"
Environment="VIRTUAL_ENV={venv_dir}"
Environment="ATLAS_HOME={atlas_home}"
...
def systemd_unit_is_current(system: bool = False) -> bool:
    ...
    # pinned ATLAS_HOME is adopted before any compare/regenerate" at a single
    # site, so a future callsite cannot regress it by forgetting to pre-sync.
    #
    # Under ``sudo atlas gateway … --system``, ATLAS_HOME is often stripped
    # and falls back to ``/root/.atlas``. Adopting the unit's pinned home
    # first makes TimeoutStopSec / WorkingDirectory / ATLAS_HOME comparisons
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # single ATLAS_HOME-sync chokepoint (adopts the unit's pinned home before
...
def systemd_install(
    ...
    # Existing system units already pin ATLAS_HOME; adopt it before any
    ...
    # ``sudo atlas gateway install --system --force`` would bake /root/.atlas
...
def systemd_start(system: bool = False):
    ...
    # ATLAS_HOME sync happens inside refresh_systemd_unit_if_needed's
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    # the env vars ``ATLAS_GATEWAY_MAX_STARTS`` /
    # ``ATLAS_GATEWAY_START_WINDOW_S`` override for escape-hatch use.
    ...
    try:
        ...
        try:
            _env_starts = os.getenv("ATLAS_GATEWAY_MAX_STARTS")
            if _env_starts is not None:
                _max_starts = int(_env_starts)
        except ValueError:
            pass
        try:
            _env_win = os.getenv("ATLAS_GATEWAY_START_WINDOW_S")
    ...
    def _hard_exit_after_gateway_teardown(code: int) -> None:
        # ``atlas gateway run`` enters through this CLI wrapper, not through
...
def _setup_standard_platform(platform: dict):
    ...
    from atlas_cli.setup_hidden_env import is_setup_hidden_env as _is_setup_hidden_env
