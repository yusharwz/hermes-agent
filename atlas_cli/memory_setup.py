...
def _provider_pip_dependencies(provider_name: str, declared: list) -> list:
    ...
    ``atlas memory setup`` — if the update-time refresh only reinstalled
    ...
    if provider_name == "hindsight":
        try:
            import json
            cfg_path = get_atlas_home() / "hindsight" / "config.json"
...
def _install_dependencies(provider_name: str, *, force: bool = False) -> None:
    ...
    ranges. This is how ``atlas update`` heals the active memory provider
    ...
    # data volume (ATLAS_LAZY_INSTALL_TARGET). install_specs handles the
...
def cmd_status(args) -> None:
    ...
    # canonical resolver (handles composite toolsets like atlas-cli).
