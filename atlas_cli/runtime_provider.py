...
from atlas_cli.providers import custom_provider_aliases, custom_provider_slug
...
def canonical_custom_identity(
    *,
    ...
    3. ``config_provider`` — the active ``config.model.provider`` (or its
       ``provider``/``ATLAS_INFERENCE_PROVIDER`` equivalent). When neither
...
def resolve_runtime_provider(
    *,
    ...
    from atlas_cli.config import is_provider_enabled, load_config
    ...
    if requested_provider in ("vertex", "google-vertex", "vertex-ai", "gcp-vertex", "vertexai"):
        ...
        if not token or not base_url:
            raise AuthError(
                ...
                "Run `atlas setup` to install Vertex support."
            )
        return {
            "provider": "vertex",
            "api_mode": "chat_completions",
