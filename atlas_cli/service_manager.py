...
class S6ServiceManager:
    ...
    def _render_log_run(profile: str) -> str:
        ...
        return (
            ...
            # Create the leaf and clear a stale s6-log lock as atlas when
            # this script starts as root. Never chown or unlink atlas-writable
            # volume paths from this restartable root-context script:
            # log/supervise/control is atlas-owned, so an unprivileged user
            # can race a pathname op through a symlink swap (CWE-59 /
            # CWE-367). Parent logs/gateways is seeded atlas-owned at stage2
            # boot (#45258; tests/docker/test_log_dir_seed.py).
            f'if [ "$(id -u)" = 0 ]; then\n'
            f'  s6-setuidgid atlas mkdir -p "$log_dir"\n'
            f'  s6-setuidgid atlas rm -f "$log_dir/lock"\n'
            f'else\n'
            f'  mkdir -p "$log_dir"\n'
            f'  rm -f "$log_dir/lock"\n'
            f'fi\n'
