"""Atlas update pipeline — extracted from ``atlas_cli/main.py``.
...
change is that references to helpers/constants that STAY in ``atlas_cli.main``
(and to moved-but-test-patched siblings) are routed through ``_m()`` — a lazy
``atlas_cli.main`` reference — so existing call sites and test monkeypatches
that target ``atlas_cli.main.<name>`` (``PROJECT_ROOT``, ``_is_windows``,
...
the test-patch surface still resolve on ``atlas_cli.main``.
...
Imports are one-way: ``atlas_cli.main`` imports this module, never the reverse
...
def _m():
    """Lazy ``atlas_cli.main`` reference.

    Lets callers keep patching ``atlas_cli.main.<helper>`` (the historical
    test surface) and have those patches reach this code path, and defers the
    import so ``atlas_cli.main`` -> ``atlas_cli.update_cmd`` stays one-way
    at import time.
    """
...
def _reload_updated_runtime_modules() -> None:
    ...
    ``atlas update`` keeps running in the pre-pull Python process. After a
...
# Critical files that Atlas must be able to import immediately after an
...
def _validate_critical_modules_import(root) -> tuple[bool, str | None, str | None]:
    ...
    ``_venv_core_imports_healthy``): ``atlas update`` can be driven by a
    ...
    """
    from atlas_constants import FIRST_PARTY_MODULE_ROOTS
    ...
    probe = (
        ...
        # The root set is injected from atlas_constants so this can't drift
        # from the hint the user is shown (they disagreed once already).
        "        missing = (getattr(exc, 'name', '') or '').split('.')[0]\n"
        "        if missing in %r or missing.startswith('atlas_'):\n"
...
def _print_fts_optimize_available_notice() -> None:
    """Advertise the opt-in v23 search-index optimization after `atlas update`.
    ...
    try:
        ...
        return
    db_path = get_atlas_home() / "state.db"
    ...
    if interrupted:
        ...
        print(
            "  A previous `atlas sessions optimize-storage` run was "
            "interrupted. Search still works; re-run the command to resume "
            "and finish reclaiming disk:"
        )
        print("    atlas sessions optimize-storage")
        return
    ...
    print("  Run when convenient:  atlas sessions optimize-storage")
    print(
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    ``atlas_cli.main`` re-export surface and guards the #49145 regression.
    """
...
def _discard_staged(staged) -> None:
    ...
    the tree. The user then follows the "re-run `atlas update`" advice with
...
def _commit_staged_replacements(staged) -> None:
    ...
    first-party modules (``run_agent.py``, ``cli.py``, ``atlas_constants.py``
...
def _update_via_zip(args):
    ...
    # would actually be done. What it downloads is upstream Atlas; copying
    ...
    try:
        ...
        print(
            "  Re-run `atlas update` to retry; if the agent won't start, "
    ...
    if not import_ok:
        ...
        print("  Re-run `atlas update` to complete it.")
        _m().sys.exit(1)
    ...
    try:
        from atlas_cli.backup import _quick_snapshot_root, verify_sqlite_integrity

        _state_path = get_atlas_home() / "state.db"
        if _state_path.exists():
            ...
            if not _state_ok.get("valid"):
                ...
                _snap_root = _quick_snapshot_root(get_atlas_home())
...
def _refresh_active_lazy_features(
    ...
    *,
    ...
    if install_cmd_prefix is None:
        print("  ⚠ Lazy refresh failed; rerun `atlas update` once resolved.")
        return False
    ...
    if status == "healthy":
        ...
        print("  Rerun `atlas update` once the upstream issue is resolved.")
        return True
...
def _refresh_active_memory_provider_dependencies() -> None:
    ...
    ``hindsight-all``), NOT in Atlas' editable-install extras or
    ...
    try:
        from atlas_cli.memory_setup import _install_dependencies
    except Exception as exc:
...
def _npm_manifest_paths() -> tuple[Path, ...]:
    ...
    lockfile is then unchanged but `atlas update` is exactly the step
...
def _npm_lockfile_changed(atlas_root: Path) -> bool:
    ...
    # landed must NOT skip the reinstall — otherwise every later `atlas
    ...
    try:
        ...
        cache_file = atlas_root / f".npm_lock_hash_{cache_key}"
...
def _record_npm_lockfile_hash(atlas_root: Path) -> None:
...
def _update_node_dependencies() -> list[str]:
    ...
    if not npm:
        ...
        if is_wsl() and path_npm and _m()._is_windows_npm_path(path_npm):
            ...
            print("    package manager), then re-run `atlas update`.")
            failed = ["repo root"]
            if any(
    ...
    # Atlas profile using this checkout. Keep one per-checkout cache under the
    # shared Atlas root rather than rerunning npm once per named profile.
    shared_atlas_root = get_default_atlas_root()
    if not _m()._npm_lockfile_changed(shared_atlas_root):
    ...
    def _partial_update_failure(*labels: str) -> list[str]:
        ...
        print("    deps). Fix npm and re-run `atlas update`.")
        return list(labels)
    ...
    nixos_env = with_atlas_node_path(_m()._nixos_build_env())
    ...
    if ws_result.returncode == 0:
        _record_npm_lockfile_hash(shared_atlas_root)
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    ...
    """
    from atlas_cli.config import detect_install_method, recommended_update_command_for_method
...
def _ensure_acp_launcher() -> None:
    """Self-heal: install a ``atlas-acp`` launcher next to the ``atlas`` one.

    Mirrors the launcher block in ``scripts/install.sh`` so existing installs
    gain the ACP command on ``atlas update`` without a reinstall.  ACP hosts
    (Zed, JetBrains, Buzz Desktop) spawn the agent by resolving the
    ``atlas-acp`` command name against the login-shell PATH; the console
    script of that name lives inside the install's venv, which is not on that
    PATH, so those hosts report Atlas as not installed even when it is.

    The shim simply delegates to the sibling ``atlas`` launcher with the
    ...
    ``atlas-acp.exe`` already resolves) and wherever a ``atlas-acp`` is
    already present next to the ``atlas`` command.  Unwritable directories
    ...
    for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin")):
        ...
        acp_cmd = bin_dir / "atlas-acp"
        try:
            if not (atlas_cmd.is_file() or atlas_cmd.is_symlink()):
                continue
...
def _run_pre_update_backup(args) -> Optional[str]:
    ...
    - ``full``  — the quick snapshot PLUS a full zip of ATLAS_HOME under
      ``backups/`` (restorable via ``atlas import``; the #48200 wrong-path
    ...
    try:
        ...
        # NOTE: this function later does `from atlas_constants import
        # get_atlas_home`, which makes the name function-local — the
        # module-level import is shadowed and unbound here. Alias explicitly.
        from atlas_cli.config import get_atlas_home as _get_home
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    if not venv_python.exists():
        ...
        managed_markers = (
            _m().PROJECT_ROOT / ".atlas-bootstrap-complete",
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    for proc in proc_iter:
        ...
        # the `-m atlas_cli.main gateway run` argv, so autostarted gateways
...
def _venv_launcher_ancestors(pids: list[int]) -> list[int]:
    ...
    # Never return ourselves or our own ancestry: a CLI ``atlas update``
...
def _leftover_pausable_gateway_pids(
    ...
    """
    from atlas_cli._scan_venv_blockers import _is_pausable_gateway
...
def _for_each_systemd_gateway_unit(
    ...
    *,
    ...
    """Process each ``atlas-gateway*.service`` from ``systemctl list-units``.
    ...
    for line in (list_units_stdout or "").strip().splitlines():
        ...
        if not unit.startswith("atlas-gateway"):
            continue
...
def _refresh_windows_gateway_launchers() -> None:
    ...
    ``atlas update`` never touched them, so installs created before the
...
def _normalize_managed_eol(git_cmd, repo_root):
    ...
    ``install.ps1`` forever — so ``atlas update``, which ships with the checkout
...
def _run_locked_native_update(args, gateway_mode: bool) -> bool:
    ...
    tree, ``atlas update`` did one of two things, both bad:

    - POSIX: printed "Not a git repository. Please reinstall:" followed by
      upstream's install URL, which reinstalls **Atlas over Atlas** — leash,
    ...
    this returns False, and ``atlas update`` keeps its git behaviour.
    ...
    from atlas_cli import ninegate_update
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    # Before anything mutates the tree, and before the Atlas banner prints:
    ...
    if _m()._is_windows() and not getattr(args, "force", False):
        ...
        if scripts_dir is not None:
            concurrent = _m()._detect_concurrent_atlas_instances(scripts_dir)
            if concurrent:
    ...
    try:
        ...
        if commit_count == 0:
            ...
            if runtime_repaired is not None and not _m()._is_windows():
                ...
                print(
                    "  Any running Atlas gateways, Desktop backends, or other "
        ...
        # marker survives and the next ``atlas`` launch finishes the install
        ...
        # symbols in atlas_constants or lazy_deps. The dependency install
        ...
        if lazy_ok:
            ...
            print(
                "  ⚠ Lazy-refresh recovery incomplete — run `atlas` again "
        ...
        if not import_ok:
            ...
            print("    Run `atlas update` again — if it persists, reinstall:")
        ...
        if (desktop_dir / "package.json").exists() and _m()._resolve_node_runtime_npm() and has_desktop_app:
            ...
            # `atlas desktop --build-only` subprocess re-imports the whole
            ...
            if _skip_desktop_build:
                ...
                # debuggable.
                #
                # Start the build subprocess with the Atlas-managed Node on PATH:
                # when `atlas update` runs inside the desktop updater chain
                # (Desktop → atlas-setup → atlas update), the shell PATH
                # customizations are lost, so a bare-PATH child would fail with
                # `node: not found` before cmd_gui can self-heal.
                from atlas_constants import with_atlas_node_path

                _build_env = with_atlas_node_path()
        ...
        try:
            ...
            if _state_path.exists():
                ...
                if _state_ok.get("valid"):
                    ...
                    if _pre_snap_id:
                        _snap_state = (
                            _quick_snapshot_root(get_atlas_home())
                            / _pre_snap_id
                            / "state.db"
                        )
                        if _snap_state.exists():
        ...
        # Self-heal the atlas-acp launcher for installs that predate it, so
        # ACP hosts (Zed, JetBrains, Buzz) can resolve Atlas on PATH without
        # a reinstall.  No-op on Windows and when already present.
        try:
            _ensure_acp_launcher()
        except Exception as e:
            logger.debug("atlas-acp launcher self-heal failed: %s", e)
        ...
        try:
            ...
            if (
                ...
                # driver) keeps the installed version — `atlas update`
                # must stay fast; `atlas computer-use install --upgrade`
