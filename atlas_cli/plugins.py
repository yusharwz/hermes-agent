...
class PluginContext:
    ...
    def register_secret_source(self, source) -> None:
        ...
        sources run during ``load_atlas_dotenv()`` startup — after
        ``~/.atlas/.env`` loads, before Atlas reads credentials — when
