"""Atlas skin/theme engine — the theme SDK for every surface.

A data-driven skin system that lets users (and Atlas itself) customize the
visual appearance across the CLI, the TUI, and the desktop GUI from a single
file. Skins are defined as YAML files in ~/.atlas/skins/ or as built-in presets.
...
``resolve_skin`` / ``skin.changed``). A skin dropped in ~/.atlas/skins/ therefore
...
.. code-block:: yaml
    ...
    colors:
      ...
      banner_border: "#1A94B8"            # Panel border color
      banner_title: "#2FBFDE"             # Panel title text color
      banner_accent: "#FFBF00"            # Section headers (Available Tools, etc.)
      banner_dim: "#14788F"               # Dim/muted text (separators, labels)
      banner_text: "#FFF8DC"              # Body text (tool names, skill names)
      ui_accent: "#FFBF00"               # General UI accent
      ui_label: "#29A6C2"                # UI labels (warm gold; teal clashed w/ default banner gold)
...
.. code-block:: python
    ...
    print(skin.colors["banner_title"])    # "#2FBFDE"
...
class SkinConfig:
    ...
    banner_hero: str = ""    # Rich-markup hero art (replaces ATLAS_EMBLEM)
...
_BUILTIN_SKINS: Dict[str, Dict[str, Any]] = {
    "default": {
        "name": "default",
        "description": "Classic Atlas — gold and kawaii",
        # Dark-authored. Values match the TUI's DARK_THEME so the classic CLI
        # and the TUI render the same Atlas gold.
        "colors": {
            ...
            "status_bar_strong": "#2FBFDE",
            "status_bar_dim": "#8A7A4A",
            "status_bar_good": "#8FBC8F",
            "status_bar_warn": "#2FBFDE",
        ...
        # #2FBFDE/#FFBF00 read as glare and WCAG-darkened mustard (#867000)
        # reads as mud — the sweet spot is the statusbar's goldenrod family
        # (#14788F/#29A6C2): hue kept, saturation tamed, mid luminance.
...
def get_prompt_toolkit_style_overrides() -> Dict[str, str]:
    ...
    input_rule = skin.get_color("input_rule", "#1A94B8")
    title = skin.get_color("banner_title", "#2FBFDE")
