"""Default configuration data for Atlas Agent.

Pure-data leaf module: DEFAULT_CONFIG and OPTIONAL_ENV_VARS, extracted
verbatim from atlas_cli/config.py. Must not import from atlas_cli.config.
"""
...
DEFAULT_CONFIG = {
    ...
    "toolsets": ["atlas-cli"],
    # SQLite journal mode used by every Atlas database opener. WAL is the
    ...
    "agent": {
        ...
        # ``ATLAS_LOCAL_STREAM_STALE_TIMEOUT`` overrides for escape-hatch use.
    ...
    "checkpoints": {
        ...
        # ``atlas checkpoints prune`` command (add ``--keep-orphans`` to
    ...
    # Single-query (``atlas -q/-z "..."``) variant of mcp_discovery_timeout.
    ...
    "compression": {
        ...
        "codex_app_server_auto": "native",  # Codex app-server (codex CLI runtime) thread
                                      ...
                                      # thread context, so Atlas' summarizer cannot
                                      # shrink it (#36801). native = codex decides when
                                      # to compact its own thread (default); atlas =
                                      # Atlas' compression threshold triggers
    ...
    "display": {
        ...
        # model — see atlas_cli/focus_view.py.
        "focus_view": False,
        ...
        # can set `path`/`paths` to ATLAS_HOME-relative YAML files/directories
    ...
    # "Hey Atlas" hands-free wake word. Always-on, on-device hotword
    ...
    "wake_word": {
        ...
        "phrase": "hey atlas",       # for "sherpa" this IS the detected phrase (any text works); for other engines it's a cosmetic label — detection is keyed by the model/keyword below
        ...
        "openwakeword": {
            # "hey_atlas" (the bundled, works-out-of-the-box default) OR a
    ...
    "cron": {
        ...
        # ATLAS_CRON_SESSION_DB_TIMEOUT env var. 0 = unlimited (skip the bound).
    ...
    "gateway": {
        ...
        # producing ~/.atlas/sessions/sessions.json entirely.
        ...
        # ``ATLAS_GATEWAY_MAX_STARTS`` / ``ATLAS_GATEWAY_START_WINDOW_S``
    ...
    "sessions": {
        ...
        # the default of ``atlas sessions prune``.
        "retention_days": 90,
        ...
        # user runs `atlas sessions optimize-storage`, because the rebuild is
        # disk-heavy and long on large DBs (see that command's disk preflight).
        #
        #   "advise" (default): `atlas update` prints a one-line notice with
        ...
        # ~/.atlas/lib/libfts5_cjk.so), 1-2 char CJK terms (일본, 项目, ...)
        ...
        # serve the cjk index. Bridged to ATLAS_CJK_FTS (internal carrier).
        "cjk_fts": True,
        ...
        # ATLAS_SEARCH_SLOW_MS (internal carrier).
        "search_slow_ms": 1000,
    },
    ...
    "updates": {
        ...
        #     DBs) into <ATLAS_HOME>/state-snapshots/ before the update.
        ...
        #     jobs) safety net.
        #   full — the quick snapshot PLUS a full ``atlas backup``-style zip
        #     of ATLAS_HOME into <ATLAS_HOME>/backups/, restorable with
        #     ``atlas import``. Can add minutes on large homes. This is the
    ...
    "secrets": {
        ...
        "bitwarden": {
            ...
            # material under ~/.atlas/cache/. If a later startup cannot reach
            # Bitwarden due to NETWORK/TIMEOUT, Atlas may use this encrypted
        ...
        "onepassword": {
            ...
            # for headless auth.  Sourced from ~/.atlas/.env (or the shell)
    ...
    "computer_use": {
        ...
        # cua-driver ≥ 0.6.x supports --no-overlay; Atlas also calls
    ...
    #
    # Configure with `atlas egress setup`.  Disabled by default — the rest of
    # Atlas works exactly as before with `enabled: false`.
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned iron-proxy binary into ~/.atlas/bin/ on
...
OPTIONAL_ENV_VARS = {
    ...
    "PORCUPINE_ACCESS_KEY": {
        "description": "Picovoice access key for the Porcupine 'Hey Atlas' wake word engine (optional; openWakeWord is the free default)",
    ...
    # ATLAS_TOOL_PROGRESS_MODE is deprecated — tool progress is configured via
    # display.tool_progress in config.yaml (off|new|all|verbose|log). The
    # gateway still falls back to ATLAS_TOOL_PROGRESS_MODE for backward
    ...
    # ATLAS_TOOL_PROGRESS is fully unsupported since the v12 config support
