...
def remove_wrapper_script():
    ...
    wrapper_paths = [
        ...
        Path.home() / ".local" / "bin" / "atlas-acp",
        Path.home() / ".local" / "bin" / "atlas-agent",
        Path("/usr/local/bin/atlas"),
        Path("/usr/local/bin/atlas-acp"),
        Path("/usr/local/bin/atlas-agent"),
    ]
...
def _print_uninstall_dry_run(*, project_root: Path, atlas_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • Atlas PATH entries from shell configs / Windows User PATH")
    print("  • Atlas wrapper scripts and Atlas-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if full_uninstall:
        print(f"  • Atlas config/data: {atlas_home}")
        if _is_default_atlas_home(atlas_home):
        ...
        print(f"  • Keep Atlas config/data: {atlas_home}")
    print()
