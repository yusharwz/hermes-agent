...
async def _lifespan(app: "FastAPI"):
    ...
    # Import atlas_cli.gateway eagerly *before* the lifespan yield so the
...
def _resolve_session_token() -> str:
    return os.environ.get("ATLAS_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "proxy.enabled": {
        "type": "boolean",
        "description": (
            "Docker-only egress credential firewall. Requires `atlas egress setup` "
            "and `atlas egress start`; Modal/SSH/Daytona are not wired yet."
        ),
        "category": "security",
    },
    ...
    "updates.refresh_cua_driver": {
        "type": "boolean",
        "description": (
            "Refresh an already-installed cua-driver during atlas update. "
...
from atlas_cli.web_models import (  # noqa: F401
    ConfigUpdate,
    EnvVarUpdate,
    EnvVarDelete,
    EnvVarReveal,
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    from atlas_cli.providers import resolve_custom_provider, resolve_user_provider
    ...
    # not be treated as a stray vendor prefix -- it isn't a known Atlas
...
def _count_status_active_sessions() -> int:
    ...
    while another Atlas process is writing to it.
    """
    from atlas_state import DEFAULT_DB_PATH, SessionDB
...
async def upload_chat_image(payload: ChatImageUpload, profile: Optional[str] = None):
    ...
    The dashboard /chat page runs Atlas inside an xterm.js PTY. Browser
...
from atlas_cli.web_routers import git as _git_routes  # noqa: E402

app.include_router(_git_routes.router)
from atlas_cli.web_routers.git import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    git_status_route,
    git_worktrees_route,
    git_branches_route,
...
def _collect_profile_gateway_topology() -> Dict[str, Any]:
    ...
    * ``gateway_mode`` — ``"multiplex"`` when the default gateway serves
    ...
    """
    try:
        from atlas_cli.profiles import _check_gateway_running, profiles_to_serve
...
async def get_status(profile: Optional[str] = None):
    ...
    try:
        ...
        # ATLAS_HOME (see issue #69143). Plain /api/status keeps the exact
        ...
        # SPA's StatusPage can show "OAuth gate ON via Dritech" or
        ...
        try:
            from atlas_cli.auth import get_nous_session_validity
        ...
        try:
            ...
            storage_check = await asyncio.get_running_loop().run_in_executor(
                None, functools.partial(_probe_state_db, get_atlas_home())
            )
        ...
        try:
            from atlas_state import SessionDB as _SDB
        ...
        # ``gateway_mode`` are low-sensitivity PRODUCT surface — Atlas Cloud
...
async def get_portal_status():
    ...
    auth: Dict[str, Any] = {}
    try:
        from atlas_cli.auth import get_nous_auth_status_local
...
def _spawn_atlas_action(subcommand: List[str], name: str) -> subprocess.Popen:
    ...
    # _ATLAS_GATEWAY=1. Inheriting it makes a spawned `atlas gateway restart`
    ...
    action_env = {**os.environ, "ATLAS_NONINTERACTIVE": "1"}
    action_env.pop("_ATLAS_GATEWAY", None)
...
from atlas_cli.web_routers import sessions as _sessions_routes  # noqa: E402

app.include_router(_sessions_routes.list_router)
from atlas_cli.web_routers.sessions import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    get_sessions,
)


from atlas_cli.web_routers import profiles as _profiles_routes  # noqa: E402

app.include_router(_profiles_routes.sessions_router)
from atlas_cli.web_routers.profiles import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
...
def _install_memory_provider_pip_dependencies(dependencies: List[str]) -> List[Dict[str, Any]]:
    ...
    # under /opt/atlas is sealed read-only, and installs must be redirected
    # to the writable durable target on the data volume
    # (ATLAS_LAZY_INSTALL_TARGET, e.g. /opt/data/lazy-packages) — the same
...
def _read_memory_provider_existing_values(name: str) -> Dict[str, Any]:
    ...
    for path in (
        atlas_home / f"{name}.json",
        atlas_home / name / "config.json",
    ):
...
def _save_memory_provider_native_config(name: str, provider: Any, values: Dict[str, Any]) -> None:
    if provider is not None and hasattr(provider, "save_config"):
        try:
            ...
            provider.save_config(values, str(get_atlas_home()))
            return
...
async def get_egress_status():
    ...
    from atlas_cli.proxy_cli import format_status_text
...
async def get_model_options(
    ...
    """
    try:
        from atlas_cli.inventory import build_model_options_payload, load_picker_context
...
def get_recommended_default_model(provider: str = ""):
    ...
    try:
        ...
        from atlas_cli.models import pick_silent_default_model
...
def set_moa_models(body: MoaConfigPayload, profile: Optional[str] = None):
    ...
    try:
        from atlas_cli.moa_config import normalize_moa_config, validate_moa_payload
...
async def ninegate_update_apply():
    ...
    """
    from agent import ninegate_leash as _leash
    from atlas_cli.ninegate_update import JOB, spawn_detached
...
async def ninegate_update_progress():
    ...
    """
    from atlas_cli.ninegate_update import read_state
...
async def set_env_var(body: EnvVarUpdate, profile: Optional[str] = None):
    try:
        with _profile_scope(body.profile or profile):
            ...
            from atlas_cli.credential_lifecycle import save_provider_env_credential
...
async def remove_env_var(body: EnvVarDelete, profile: Optional[str] = None):
    try:
        with _profile_scope(body.profile or profile):
            ...
            from atlas_cli.credential_lifecycle import remove_provider_env_credential
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    ...
    "teams": {
        "description": "Connect Atlas to Microsoft Teams chats via the Bot Framework.",
    ...
    "irc": {
        "description": "Relay messages between an IRC channel (or DMs) and Atlas.",
        "docs_url": "https://hermes-agent.nousresearch.com/docs/user-guide/messaging/irc",
    },
    "line": {
        "description": "Use Atlas from LINE via the LINE Messaging API webhook.",
        "docs_url": "https://hermes-agent.nousresearch.com/docs/user-guide/messaging/line",
    },
    "ntfy": {
        "description": "Chat with Atlas over ntfy push topics (ntfy.sh or self-hosted).",
        "docs_url": "https://hermes-agent.nousresearch.com/docs/user-guide/messaging/ntfy",
    },
    "photon": {
        "description": "Use Atlas through iMessage via Photon's managed Spectrum platform.",
    ...
    "simplex": {
        "description": "Talk to Atlas over SimpleX Chat via a local simplex-chat daemon.",
    ...
    "whatsapp_cloud": {
        ...
        "description": "Use Atlas via Meta's hosted WhatsApp Cloud API (no local bridge).",
        "docs_url": "https://hermes-agent.nousresearch.com/docs/user-guide/messaging/whatsapp-cloud",
    },
    "relay": {
        "name": "Relay (experimental)",
        "description": "Generic relay adapter fronted by the Atlas Relay connector.",
        "docs_url": "",
        "required_env": (),
    },
}
...
# atlas_cli/setup_hidden_env.py. Shared with the `atlas setup gateway`
# wizard so the surfaces ask for the same things.
from atlas_cli.setup_hidden_env import (  # noqa: E402
...
def _messaging_platform_payload(
    ...
    # ATLAS_HOME contextvar override (#56986 / #69143), so the profile's
...
def _whatsapp_session_path() -> Path:
    from atlas_constants import get_whatsapp_session_dir
...
def _run_whatsapp_pairing(pairing_id: str, session_path: Path, mode: str) -> None:
    ...
    try:
        from atlas_constants import WHATSAPP_LOGGED_OUT_MARKER
...
async def get_whatsapp_link(profile: Optional[str] = None):
    ...
    """
    with _config_profile_scope(profile):
        from atlas_constants import whatsapp_session_is_linked
...
async def get_messaging_platforms(profile: Optional[str] = None):
    ...
    # load_env() honors the ATLAS_HOME contextvar override; the gateway
...
def _multiplex_port_binding_conflict(
    ...
    *enabling* is blocked; disabling/clearing stays allowed so users can
    ...
    if not requested or requested.lower() == "current":
        ...
        # The dashboard's own profile. "custom" (an unrecognized ATLAS_HOME)
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    try:
        from agent.anthropic_adapter import (
            ...
            _get_atlas_oauth_file,
        )
    except ImportError:
        read_atlas_oauth_credentials = None  # type: ignore
        _get_atlas_oauth_file = None  # type: ignore
    ...
    if atlas_creds and atlas_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "atlas_pkce",
            "source_label": f"Atlas PKCE ({_get_atlas_oauth_file() if _get_atlas_oauth_file else None})",
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": "atlas auth add nous",
...
async def disconnect_oauth_provider(
    ...
    with _profile_scope(profile):
        ...
        if provider_id == "anthropic":
            cleared = False
            try:
                from agent.anthropic_adapter import _get_atlas_oauth_file
                oauth_file = _get_atlas_oauth_file()
...
def _xai_device_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            # Mirror `atlas auth add xai-oauth`: first credential may become
...
def _codex_device_code_start_error(resp: Any) -> str:
    ...
    if "device" in lower and ("authori" in lower or "enable" in lower):
        message = (
            ...
            "account may need device-code authorization enabled before Atlas "
...
def _maybe_auto_archive_for_profile(db, profile: Optional[str]) -> None:
    ...
    The Desktop backend is spawned as ``atlas serve`` — it runs neither the
...
def _prune_sessions(body: SessionPrune):
    """Delete ended sessions matching filters (mirrors `atlas sessions prune`)."""
    has_window = (
...
def _cron_default_profile() -> str:
    ...
    A desktop pool backend runs one process per profile (ATLAS_HOME already
    ...
    fallback would write a non-default profile's job into ``~/.atlas``.
    Resolve the process's own profile instead. ``custom`` (an unrecognized
    ATLAS_HOME outside the profiles tree) has no profile-dir equivalent, so
...
from atlas_cli.web_routers import cron as _cron_routes  # noqa: E402

app.include_router(_cron_routes.router)
from atlas_cli.web_routers.cron import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    list_cron_jobs,
    get_cron_job,
    list_cron_job_runs,
    create_cron_job,
...
def _fire_cron_job_for_profile(profile: str, job_id: str) -> bool:
    ...
    Scope both cron storage and the runtime Atlas home so the job's store,
...
from atlas_cli.web_routers import mcp as _mcp_routes  # noqa: E402

app.include_router(_mcp_routes.router)
from atlas_cli.web_routers.mcp import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    list_mcp_servers,
    add_mcp_server,
    replace_mcp_servers,
    remove_mcp_server,
...
def _mcp_oauth_callback_url(request: Request, server_name: str) -> str:
    ...
    from atlas_cli.dashboard_auth.prefix import prefix_from_request, resolve_public_url
...
def _mcp_oauth_transaction(flow) -> threading.Lock:
    key = (flow.atlas_home, flow.server_name)
...
def _run_dashboard_mcp_oauth(flow, cfg: dict) -> None:
    ...
    try:
        ...
        home_token = set_atlas_home_override(flow.atlas_home)
        secret_token = set_secret_scope(build_profile_secret_scope(Path(flow.atlas_home)))
        try:
            transaction = _mcp_oauth_transaction(flow)
            with transaction, force_interactive_oauth(), dashboard_oauth_flow(flow):
                manager = get_manager()
                storage = AtlasTokenStorage(flow.server_name)
                backup = storage.snapshot()
                previous_entry = None
                try:
                    previous_entry = manager.remove(
                        flow.server_name,
                        atlas_home=flow.atlas_home,
                    )
...
async def add_credential_pool_entry(body: CredentialPoolAdd):
    ...
    try:
        ...
        # (via DELETE below or `atlas auth remove`) can seed again.
        # Mirrors the `atlas auth add` behaviour in auth_commands.py.
...
async def remove_credential_pool_entry(provider: str, index: int):
    ...
    registry the CLI ``atlas auth remove`` uses: each source cleans up its
...
from atlas_cli.web_routers import skills as _skills_routes  # noqa: E402

app.include_router(_skills_routes.hub_router)
from atlas_cli.web_routers.skills import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    install_skill_hub,
    uninstall_skill_hub,
    update_skills_hub,
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    ...
    """
    from atlas_constants import set_atlas_home_override, reset_atlas_home_override
    from atlas_cli.mcp_config import _save_bearer_auth_token
...
from atlas_cli.web_routers import tools as _tools_routes  # noqa: E402

app.include_router(_tools_routes.router)
from atlas_cli.web_routers.tools import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    get_toolsets,
    toggle_toolset,
    get_toolset_config,
    get_toolset_models,
...
from atlas_cli.pty_session import PtySessionRegistry, RegistryFull, run_reaper  # noqa: E402
...
def _resolve_chat_argv(
    ...
    """
    from atlas_cli.main import PROJECT_ROOT, _apply_tui_python_env, _make_tui_argv
    ...
    # Atlas TUI child: build via the single spawn-env factory (profile-home
    # contract applied; secrets kept — the spawned agent needs provider creds).
    # An explicit profile scope below still overrides ATLAS_HOME afterwards.
    from tools.environments.local import build_subprocess_env
    ...
    # bronze border #1A94B8 snaps to palette 173 (#D7875F, salmon-red) and
...
def _resolve_client_ws_host() -> Optional[str]:
    ...
    1. Explicit ``ATLAS_DASHBOARD_WS_HOST`` env var — wins always. Operators
    ...
    """
    explicit = os.environ.get("ATLAS_DASHBOARD_WS_HOST", "").strip()
    if explicit:
        return explicit
...
async def console_ws(ws: WebSocket) -> None:
    ...
    try:
        ...
        engine = AtlasConsoleEngine(output_limit=_CONSOLE_OUTPUT_LIMIT)
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if raw_resume and env:
        registry_resume = env.get("ATLAS_TUI_RESUME") or raw_resume
...
def _render_active_theme_bootstrap_css() -> str:
    ...
    default Atlas Teal canvas before the SPA flips the configured user
    theme into place.
    ...
    try:
        ...
        for theme in _discover_user_themes():
            ...
            return (
                '<style id="atlas-theme-bootstrap">'
                ":root{"
...
def mount_spa(application: FastAPI):
    ...
    """
    # `atlas serve` is the headless backend: it must NEVER serve the browser
    ...
    _headless = os.environ.get("ATLAS_SERVE_HEADLESS") == "1"
    if _headless or not WEB_DIST.exists():
        _msg = (
            "Headless backend (atlas serve): web UI disabled — use "
            "`atlas dashboard` for the browser UI."
            if _headless
    ...
    def _serve_index(prefix: str = ""):
        ...
        # (``ATLAS_HOME/dashboard-themes/<name>.yaml``), inject a minimal
        # critical-CSS block so the first paint uses the target palette.
        # Without this the SPA paints the default Atlas Teal canvas, then
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Atlas Teal",         "description": "Classic dark teal — the canonical Atlas look"},
    {"name": "default-large", "label": "Atlas Teal (Large)", "description": "Atlas Teal with bigger fonts and roomier spacing"},
...
def _discover_user_themes() -> list:
    ...
    Uses the dashboard process launch home, not ``get_atlas_home()``, so a
    transient profile override from embedded chat does not hide themes that
    live under the server's own ``ATLAS_HOME``.
    """
    themes_dir = get_process_atlas_home() / "dashboard-themes"
...
def _discover_dashboard_plugins() -> list:
    ...
    # ATLAS_HOME override (e.g. embedded /chat under --open-profile).
    search_dirs = [
        (get_process_atlas_home() / "plugins", "user"),
...
def start_server(
    ...
    build and no SPA mount (mount_spa() honours ``ATLAS_SERVE_HEADLESS``), so
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            _fix_hint = (
                ...
                "  • OAuth: run `atlas dashboard register` (Nous Portal) or "
            ...
            try:
                ...
                from atlas_cli.plugins_cmd import _BASIC_AUTH_PLUGIN_KEYS
                ...
                if _has_creds and (set(_disabled) & _BASIC_AUTH_PLUGIN_KEYS):
                    _fix_hint = (
                        ...
                        "`atlas plugins enable basic`), then restart the "
                        "dashboard.\n\n"
                    ) + _fix_hint
            except Exception:
                pass
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            ready_token = "ATLAS_BACKEND_READY" if headless else "ATLAS_DASHBOARD_READY"
            print(f"{ready_token} port={actual_port}", flush=True)
            if headless:
                # No SPA, and the JSON-RPC/WS endpoints are auth-gated — don't
                # advertise a paste-and-connect URL, just announce the bind.
                print(f"  Atlas backend listening on {host}:{actual_port}")
            else:
