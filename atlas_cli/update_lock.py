"""Cross-process mutual exclusion for in-flight Atlas updates.

Three different surfaces can start an update of the same install tree:

* ``atlas update`` from a terminal,
* the dashboard's Update button (``POST /api/atlas/update`` →
  ``_spawn_atlas_action(["update"])``, detached),
* the desktop's Update button, which hands off to the Tauri
  ``atlas-setup --update`` and, on its failure screen, to install-mode
...
once — so a dashboard-spawned ``atlas update`` and an installer-driven
...
unchanged and remain byte-compatible with the Rust and Electron readers:
    ...
    <ATLAS_HOME>/.atlas-update-in-progress   body: "<pid>\\n<started_at_unix>"
...
then spawns ``atlas update`` as a child stage. Without a handoff the child
sees its own parent's live marker and refuses — the GUI update deadlocks
against itself on every attempt ("Atlas is still running", retry forever).
...
* A live holder that is a *process ancestor* of ours is likewise our own
  ...
  ``atlas-setup`` binary under ``~/.atlas`` is only refreshed by a full
  installer run (``copy_self_to_atlas_home`` deliberately no-ops during
  ...
  ancestry check those users get exit 2 ("Atlas is still running") on every
  GUI update forever, with no Atlas process actually running.
"""
...
MARKER_NAME = ".atlas-update-in-progress"

# Set by an orchestrating updater (the Tauri `atlas-setup --update` flow) to
# its own pid before spawning `atlas update` as a child stage. The parent
...
HANDOFF_PID_ENV = "ATLAS_UPDATE_HANDOFF_PID"
...
# to show "Atlas is still running" instead of a generic failure. Naming it
...
def update_marker_path() -> Path:
    ...
    Uses the *process* Atlas home (never the context-local profile override):
    the Rust updater resolves ``$ATLAS_HOME`` or the platform default, and the
    ...
    """
    from atlas_constants import get_process_atlas_home

    return get_process_atlas_home() / MARKER_NAME
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``atlas update`` as a (grand)child, so a
    ...
    parent chain. This heals the fleet of staged ``atlas-setup`` binaries
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another Atlas update is already running (PID {holder.pid}, "
...
class UpdateLock:
    ...
    def acquire(self) -> bool:
        ...
        updater spawning `atlas update` as a stage): we run under ITS claim
