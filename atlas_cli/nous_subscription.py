...
def _has_agent_browser() -> bool:
    ...
    from atlas_constants import agent_browser_runnable, with_atlas_node_path
    ...
    # Atlas-managed Node dirs (Windows installer / POSIX $ATLAS_HOME/node)
    ...
    managed_path = with_atlas_node_path().get("PATH", "")
    if managed_path:
...
# ---------------------------------------------------------------------------
# Inline Nous Portal login for the Tool Gateway picker (`atlas tools`)
# ---------------------------------------------------------------------------
...
def ensure_nous_portal_access(
    *,
    ...
    backend (e.g. "Firecrawl (Nous Portal)").  Unlike ``atlas model``'s Nous
    login, this:
