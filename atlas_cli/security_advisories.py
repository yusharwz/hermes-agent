"""
Security advisory checker for Atlas Agent.
...
- **Acknowledgeable.** Once the user has read and acted on an advisory they
  can dismiss it via ``atlas doctor --ack <id>``; the ack is persisted to
...
1. ``atlas doctor`` (and ``atlas doctor --ack <id>``)
...
run in environments where the rest of Atlas failed to import.
"""
...
ADVISORIES: tuple[Advisory, ...] = (
    Advisory(
        ...
        summary=(
            ...
            "imported mistralai 2.4.6 — including atlas when configured "
        ...
        remediation=(
            ...
            "Rotate API keys in ~/.atlas/.env (OpenRouter, Anthropic, OpenAI, "
            ...
            "After cleanup: atlas doctor --ack shai-hulud-2026-05  to dismiss "
            "this warning.",
        ),
...
def _installed_version(pkg_name: str) -> Optional[str]:
    ...
    except ImportError:  # py<3.8 — Atlas requires 3.10+ but defensive.
        return None
    try:
...
def short_banner_lines(hits: list[AdvisoryHit]) -> list[str]:
    ...
    lines = [
        ...
        "  Run 'atlas doctor' for remediation steps.",
    ]
    if len(hits) > 1:
...
# ``~/.atlas/cache/advisory_banner_seen`` (a single line per advisory ID:
...
def _banner_cache_path() -> Optional[Path]:
    try:
        ...
        cache_dir = Path(get_atlas_home()) / "cache"
...
def render_doctor_section(hits: list[AdvisoryHit]) -> tuple[bool, list[str]]:
    """Render the security-advisory section for ``atlas doctor``.
...
def gateway_log_message(hits: list[AdvisoryHit]) -> Optional[str]:
    ...
    return (f"{len(fresh)} security advisories active "
            ...
            f"Run `atlas doctor` on the gateway host for details.")
