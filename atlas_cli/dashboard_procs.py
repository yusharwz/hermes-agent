"""Dashboard process-hygiene helpers — extracted from ``atlas_cli/main.py``.
...
``_detect_concurrent_atlas_instances``) are lifted verbatim. References to
helpers that STAY in ``atlas_cli.main`` (``_find_stale_dashboard_pids``,
``_respawn_dashboard_processes``, ``_is_windows``, ...) are routed through a
lazy ``_m()`` main reference so existing test monkeypatches on
``atlas_cli.main.<name>`` keep reaching this code path, and imports stay
...
patches on ``atlas_cli.main`` resolve unchanged.
"""
...
def _scan_dashboard_processes(
    *,
    ...
    ``atlas-dashboard.service`` systemd unit.  Managed units are restarted
...
def _kill_stale_dashboard_processes(
    ...
    *,
    ...
    """Kill running ``atlas dashboard`` / ``atlas serve`` processes.
    ...
    When ``restart_managed`` is true (the ``atlas update`` path), a detected
    ``atlas-dashboard.service`` is restarted through systemd; any OTHER
    killed PID that was supervised by a systemd unit (custom unit names —
    e.g. a remote backend's ``atlas-serve.service``) has its owning unit
    ...
    # caller asked for restarts (the `atlas update` path) — `--stop` must
    ...
    #    remote backend (atlas serve) under Restart=on-failure never comes
...
def _detect_concurrent_atlas_instances(
    ...
    for shim in _m()._atlas_exe_shims(scripts_dir):
        try:
