"""
Atlas Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``atlas desktop`` / ``atlas gui``)
     Built inside the agent checkout under ``$ATLAS_HOME/atlas-agent/``:
       ...
       - ``$ATLAS_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Atlas.app`` or ``~/Applications/Atlas.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Atlas`` (NSIS per-user)
...
the app name ("Atlas"), separate from ``$ATLAS_HOME``:
  - macOS:   ``~/Library/Application Support/Atlas``
  - Windows: ``%APPDATA%\\Atlas``
  - Linux:   ``$XDG_CONFIG_HOME/Atlas`` (default ``~/.config/Atlas``)
...
``atlas uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(atlas_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return atlas_home / "atlas-agent"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "Atlas"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Atlas"
...
def source_built_gui_artifacts(atlas_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``atlas desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``atlas-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(atlas_home)
    ...
    return [
        ...
        atlas_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Atlas" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Atlas.app"),
        ...
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Atlas).
            local_base / "Programs" / "Atlas",
            # Older / alternate layout some builds used.
            local_base / "atlas-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Atlas")
    else:
        ...
        paths += [
            data_base / "applications" / "atlas.desktop",
            data_base / "applications" / "Atlas.desktop",
        ]
    return paths


def agent_is_installed(atlas_home: Path) -> bool:
    """Return True when a usable Python agent install exists under ATLAS_HOME.
    ...
    if (agent_root / "atlas_cli").is_dir():
        return True
...
def gui_is_installed(atlas_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(atlas_home):
        if p.exists():
            return True
...
def gui_install_summary(atlas_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = atlas_home if atlas_home is not None else get_atlas_home()
...
def uninstall_gui(atlas_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``atlas-agent/atlas_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$ATLAS_HOME``.
    ...
    if sys.platform.startswith("linux"):
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove atlas' or "
            "'sudo dnf remove atlas'). AppImage builds are a single file you "
