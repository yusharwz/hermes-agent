...
# xAI's public API is OpenAI-compatible for the endpoints Atlas commonly
# uses. The Responses endpoint is included because Atlas' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Atlas-managed OAuth credentials."""

    auth_hint = "atlas auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`atlas auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`atlas auth reset xai-oauth` or re-authenticate with "
                    "`atlas auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `atlas auth add xai-oauth --type oauth`."
            )
