...
def start_background_mcp_discovery(*, logger, thread_name: str) -> None:
    ...
    with _mcp_discovery_lock:
        ...
        # Capture the caller's context-local ATLAS_HOME override (profile
...
def _resolve_discovery_timeout(
    ...
    When ``single_query`` is True (``atlas -z "..."`` / ``-q``), the larger
...
def ensure_mcp_discovery_before_agent_build(
    *,
    ...
    Non-interactive first turns (``chat -q``, ``atlas -z``) can construct
