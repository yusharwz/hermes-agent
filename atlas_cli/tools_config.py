...
def _post_setup_no_window_flags(*, streams_to_console: bool = False) -> int:
    ...
    ``atlas tools post-setup <key>`` child. On Windows, every console child
    ...
    :func:`atlas_cli._subprocess_compat.windows_hide_flags`) suppresses it
...
# Config-only capabilities: they appear in `atlas tools` for provider/API-key
...
def install_cua_driver(
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
    ...
    falling through to the full upstream installer. ``atlas update`` sets
    ...
    a further ~600s wait on Windows). ``atlas computer-use install
    ...
    ``atlas update`` already prints a contextual line before its update
    ...
    if not _cua_install_target_writable():
        ...
        _print_info(
            "    Run `atlas computer-use install --upgrade` from an admin account to update it."
        )
        return bool(binary)
    ...
    # caller: `atlas update` (require_confirmed_update=True) keeps the
    ...
    # `atlas computer-use install --upgrade` falls through and re-runs the
    # installer as before.
    ...
    if binary:
        ...
        if _state is None and require_confirmed_update:
            ...
            _print_info(
                "    Force a refresh with: atlas computer-use install --upgrade"
            )
            return True
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                _print_info("    Change via stt.local.model in ~/.atlas/config.yaml")
            else:
...
def _exempt_explicit_platform_native(
    ...
    the composite they chose (e.g. ``atlas-discord``, which contains those
...
#:
#: Saving ``atlas tools`` (or one toggle in the desktop Toolsets UI) replaces
...
#: has touched the picker, while everyone still on ``[atlas-cli]`` inherits it
...
def _enable_recently_shipped_toolsets(
    ...
    Either way of saying no outlives this: unchecking in ``atlas tools``
    ...
    default_ts = plat_info["default_toolset"] if plat_info else f"atlas-{platform}"
    ...
    for ts_key in sorted(_RECENTLY_SHIPPED_TOOLSETS):
        ...
        # narrow composites (atlas-acp, atlas-webhook) stay narrow.
...
def _get_platform_tools(
    ...
    *,
    ...
    # ``atlas-discord``) is an opt-in to the platform's native default-off
...
def _agent_browser_installed() -> bool:
    ...
    from atlas_cli.nous_subscription import _local_browser_runnable

    # The install hook runs in a spawned ``atlas tools post-setup`` process,
...
def _cloud_agent_browser_installed() -> bool:
    ...
    from atlas_cli.nous_subscription import _has_agent_browser
...
# Kept in sync with the dashboard selects (atlas_cli/web_server.py
...
def _configure_vision_provider_model(config: dict, vision_cfg: dict) -> None:
    ...
    substrate — so this picker lists exactly what the ``atlas model`` aux-task
