...
on Japanese locales, etc.).  Atlas's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``atlas_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Atlas is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``ATLAS_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("ATLAS_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Atlas-managed Git install directories so
    ...
    # already-running shells don't see the change — which means atlas
    ...
    # install without Atlas-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Atlas's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Atlas's
    ``atlas config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Atlas (User env var in Windows Settings, or
    export in a PowerShell profile) and Atlas picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Atlas-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\atlas\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``atlas`` from right after install) retain their old PATH.

    Any subprocess Atlas spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Atlas-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "atlas", "git", "cmd"),
        os.path.join(local_appdata, "atlas", "git", "bin"),
        os.path.join(local_appdata, "atlas", "git", "usr", "bin"),
        # Atlas venv Scripts directory — host of the atlas.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes atlas, but harmless to include.
        os.path.join(local_appdata, "atlas", "atlas-agent", "venv", "Scripts"),
