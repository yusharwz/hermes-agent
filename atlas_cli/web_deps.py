...
Why this exists
---------------
``atlas_cli/web_server.py`` owns all dashboard runtime state: the ephemeral
...
``APIRouter`` modules under ``atlas_cli/web_routers/`` need those helpers, but
...
thin proxy that resolves ``atlas_cli.web_server.<name>`` *at call time*.  This
...
def _server():
    """Return the live ``atlas_cli.web_server`` module (imported on demand)."""
    mod = sys.modules.get("atlas_cli.web_server")
    if mod is None:  # pragma: no cover - routers are only mounted by web_server
        import atlas_cli.web_server as mod  # type: ignore[no-redef]
    return mod
