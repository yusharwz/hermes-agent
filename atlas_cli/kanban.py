...
def _check_dispatcher_presence(
    ...
    ``atlas_home`` scopes the probe to a named profile's directory. The
    dashboard plugin API passes it because the dashboard backend process can
    be running under a different ATLAS_HOME than the profile the request
    ...
    try:
        ...
        liveness = resolve_gateway_liveness(
            profile_dir=atlas_home, use_cache=False
        )
    except Exception:
...
def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    p_attach.add_argument("--author", default=None,
                          help="uploaded_by label (default: $ATLAS_PROFILE or 'user')")
...
def kanban_command(args: argparse.Namespace) -> int:
    ...
    # atlas_cli.kanban_db, because children can import DB mutators directly.
    ...
    with board_scope:
        ...
        # every `atlas kanban repair` into "could not initialize database"
...
def _is_delegated_child_cli_mutation(args: argparse.Namespace) -> bool:
    ...
    try:
        ...
        return bool(os.environ.get("ATLAS_DELEGATED_CHILD_CONTEXT"))
...
def _cmd_complete(args: argparse.Namespace) -> int:
    ...
    with kb.connect_closing() as conn:
        for tid in ids:
            ...
            # `atlas kanban complete <id>` from the terminal tool and
            ...
            if task and task.goal_mode:
                ...
                if judge_available:
                    ...
                    try:
                        ...
                        # atlas_cli/goals.py. Unpacking fewer raises
