...
- atlas config get      - Print a resolved configuration value
- atlas config set      - Set a specific value
- atlas config unset    - Remove a user configuration value
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    # this is the one writer every surface shares — CLI `atlas env`, the
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # ATLAS_TOOL_PROGRESS_MODE is deprecated (replaced by display.tool_progress
    ...
    # ATLAS_TOOL_PROGRESS variant is fully unsupported since the v12 config
...
from atlas_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
# from `nix run` / `nix profile install` (which don't set ATLAS_MANAGED).
...
_NIX_UPDATE_MSG = (
    "Update Atlas through the Nix source that installed it "
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    installs, ``/opt/atlas`` inside the published image. It is a property of
    the running interpreter, NOT of ``$ATLAS_HOME``, which is why a
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Atlas was installed: 'docker', 'nix', 'nixos', 'git', or 'unknown'.
    ...
    3. ATLAS_MANAGED env / .managed marker (NixOS managed mode)
    ...
    # detect Nix installs that don't set ATLAS_MANAGED (e.g. ``nix run``,
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method in {"nix", "nixos"}:
        return _NIX_UPDATE_MSG
    if method == "docker":
        return "docker pull nousresearch/atlas-agent:latest"
    return "atlas update"
...
from atlas_constants import get_atlas_home, get_process_atlas_home  # noqa: F811,E402
...
# ensure_atlas_home(). Only successful passes are recorded, so a raised
# managed-mode/missing-profile error keeps re-checking on later loads.
_ATLAS_HOME_ENSURED: set = set()
...
def ensure_atlas_home():
    ...
    After the first successful pass for a given ``ATLAS_HOME`` we only re-run
    ...
    ``get_atlas_home()`` and therefore re-run for the new path.
    """
    home = get_atlas_home()
    key = str(home)

    if key in _ATLAS_HOME_ENSURED and home.is_dir():
        return
...
from atlas_cli.config_defaults import DEFAULT_CONFIG, OPTIONAL_ENV_VARS  # noqa: F401
...
def _is_env_config_key(key: str) -> bool:
    """Return whether `atlas config set` routes this key to .env."""
    if "." in key:
        return False
    key_upper = key.upper()
    api_keys = [
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    _KNOWN_KEYS = {
        ...
        # unused here, but Atlas' own config writer has historically emitted it
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Atlas versions) may write
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "plugins",           # plugin enable/disable lists (atlas_cli/plugins_cmd.py)
    "smart_model_routing",   # written by the setup wizard (atlas_cli/setup.py)
    "platform_toolsets",     # written by the setup wizard (atlas_cli/setup.py)
    "known_plugin_toolsets", # written/read by atlas_cli/tools_config.py toolset-save flow
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    # scalars are bridged into os.environ (gateway/run.py, atlas send) so
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    from atlas_cli.config_migrations import (
    ...
    if floor_refused:
        ...
        sys.stderr.write(f"⚠ atlas config: {msg}\n")
        if not quiet:
            print(f"  ⚠ {msg}")
    else:
        # ── Versioned migration ladder (table-driven) ──
        # The per-version steps live in atlas_cli.config_migrations as a
    ...
    * ``${env:VAR}`` — Cursor-style SecretRef, same resolution after the
    ...
    if inner.startswith("env:"):
        ...
        if val is not None:
            return val
        logger.warning(
            "Config ref %r: %s is not set (check ~/.atlas/.env); "
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    ``load_atlas_dotenv()`` populated the process env, or an env var
...
def read_user_config_raw(config_path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    ``_atlas_home``, tui profile override, multi-profile probes).
    """
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        if cached is not None and cache_sig is not None and cached[:4] == cache_sig:
            ...
            # Without this, a load_config() that ran before load_atlas_dotenv()
...
#   nous         (OAuth — atlas auth) — Nous Portal
...
def custom_endpoint_key_env(identity: str) -> str:
    ...
    - The fixed ``ATLAS_CUSTOM_`` prefix keeps the result a valid POSIX name
    ...
    return f"ATLAS_CUSTOM_{slug}_API_KEY" if slug else "ATLAS_CUSTOM_API_KEY"
...
def warn_unpinned_cron_jobs_after_model_config_change(
    ...
    print(
        ...
        "changed model/provider. Inspect with `atlas cron list`, then pin the "
...
def _known_top_level_keys() -> set[str]:
    ...
    to decide whether a ``atlas config set`` invocation is targeting a
    known shape.
    """
...
def _suggest_closest_key(key: str, candidates: set[str], cutoff: float = 0.6) -> Optional[str]:
    ...
    similar enough to ``key``, else None.  Used by ``atlas config set``
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    Args:
        ...
        force: When True, skip the unknown-key warning — useful for scripted
            ...
            CLI exposes this via ``atlas config set --force``.
    """
    if is_managed():
    ...
    if config_path.exists():
        try:
            ...
            print(
                ...
                f"  (atlas config edit will open it in your editor.)",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    # ``atlas config set model.provider openai`` doesn't silently
    ...
    # (e.g. ``atlas config set model gpt-5.6-sol`` when model already
    ...
    if "." not in key:
        ...
        if isinstance(_existing, dict):
            if key == "model":
                ...
                print(
                    f"    atlas config set {key}.<sub-key> <value>",
                    file=sys.stderr,
                )
                print(
                ...
                print(
                    f"    atlas config set --force {key} {value!r}",
                    file=sys.stderr,
                )
                sys.exit(1)
    ...
    if key == "display.skin" and isinstance(value, str) and value:
        try:
            skin_file = get_atlas_home() / "skins" / f"{value}.yaml"
            if skin_file.exists():
                skin_file.touch()
        except Exception:
...
def unset_config_value(key: str):
    ...
    if _is_env_config_key(key):
        ...
        # model-cache rows too, so `atlas config unset <KEY>` fully removes
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key:
            print("Usage: atlas config get <key> [--json]")
            print()
            print("Examples:")
            print("  atlas config get model")
            print("  atlas config get terminal.backend")
            print("  atlas config get skills.config --json")
            sys.exit(1)
        ...
        if not key or value is None:
            print("Usage: atlas config set [--force] <key> <value>")
            print()
            print("Examples:")
        ...
        key = getattr(args, 'key', None)
        if not key:
            print("Usage: atlas config unset <key>")
            print()
            print("Examples:")
            print("  atlas config unset model")
            print("  atlas config unset terminal.backend")
            print("  atlas config unset OPENROUTER_API_KEY")
            sys.exit(1)
        ...
        print("  atlas config get <key>          Print a resolved config value")
        print("  atlas config set <key> <value>   Set a config value")
        print("  atlas config unset <key>        Remove a config value")
