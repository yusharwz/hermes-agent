...
#
# UPSTREAM MODEL VOCABULARY, not branding residue. The "hermes" below names a
# model family Nous publishes; it arrives here from the provider, over the
# wire, and is never affected by what this fork calls itself. The Phase 3.2
# rename turned it into "atlas" — a model nobody serves — and the warning
# stopped firing for precisely the models it exists to warn about.
#
# The rebrand's protected spans cannot see it: the text here is a regex, so the
# characters are `hermes[-_ ]?[34]`, and no rule that reasons about model names
# recognises a character class. test_nous_atlas_non_agentic.py is the guard
# that does — it caught this, and it is why the port procedure runs the suite.
#
...
async def resolve_display_context_length_async(
    ...
    ``atlas_cli.route_identity.should_clear_context_pin_async``, which
...
def list_authenticated_providers(
    ...
    *,
    ...
    # Compared against atlas_id / mdev_id (section 1), pid / atlas_slug
    ...
    for atlas_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        if (
            ...
            continue
        # Resolve the canonical provider profile name.  Skip atlas_ids
        # that are mere aliases resolving to a different canonical profile
        # (e.g. "kimi" and "moonshot" both → "kimi-coding").  Only process
        # entries whose atlas_id matches the canonical profile name so
        ...
        # same atlas_id).  Distinct canonical profiles that share a
        ...
        if slug.lower() in seen_slugs:
            continue
        if atlas_id.lower() in _excluded or mdev_id.lower() in _excluded:
            continue
        ...
        if pconfig and pconfig.auth_type != "api_key":
            continue
        # models.dev catalogs include providers Atlas may not route yet.
        ...
        from atlas_cli.auth import is_runtime_provider_routable
        if not is_runtime_provider_routable(atlas_id):
            continue
        ...
        if not has_creds:
            try:
                ...
                raw_pool_present = bool(
                    store and store.get("credential_pool", {}).get(atlas_id)
                )
                if raw_pool_present:
                    has_creds = _credential_pool_is_usable(
                        atlas_id, raw_pool_present=True
                    )
            except Exception:
                pass
        if not has_creds:
        ...
        if isinstance(user_providers, dict):
            configured = user_providers.get(atlas_id)
    ...
    for pid, overlay in ATLAS_OVERLAYS.items():
        ...
        if atlas_slug.lower() in seen_slugs:
            continue
        if pid.lower() in _excluded or atlas_slug.lower() in _excluded:
            continue
        ...
        if not has_creds:
            try:
                if _credential_pool_is_usable(atlas_slug):
                    has_creds = True
                elif for_picker:
    ...
    if user_providers and isinstance(user_providers, dict):
        ...
        for ep_name, ep_cfg in user_providers.items():
            ...
            # Atlas writes ``models:`` as a dict keyed by model id, but older
            ...
            if group_key not in ep_groups:
                ...
                # Atlas's own writer uses (mirrors section-4 grouping).
