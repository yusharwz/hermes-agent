"""``atlas sessions`` command — extracted from ``atlas_cli/main.py``.
...
Helpers that stay in ``atlas_cli.main`` (``get_atlas_home``,
...
``atlas_cli.main.<name>`` keep reaching this code path, and so imports stay
...
def _m():
    """Lazy ``atlas_cli.main`` reference (call-time, keeps patches working)."""
...
def cmd_sessions(args, sessions_parser=None):
    ...
    if action == "repair":
        ...
        if report.get("repaired"):
            ...
            print(f"    atlas sessions recover --source {source_hint} \\")
            print("        --inspect-only")
    ...
    if action == "recover":
        ...
        from atlas_cli.session_recovery import (
    ...
    if action == "list":
        from atlas_state import workspace_key as _ws_key
        ...
        from atlas_cli.session_filters import (
            build_prune_filters,
            describe_filters,
        )
        ...
        def _redact(data):
            if not args.redact or data is None:
                return data
            from atlas_cli.session_export_md import redact_session_data
        ...
        if args.format == "html":
            if not args.output or args.output == "-":
                ...
                return
            from atlas_cli.session_export_html import (
            ...
            try:
                if len(ids) == 1:
                    ...
                    out_dir = (
                        ...
                        else get_atlas_home() / "session-exports"
                    )
        ...
        # Markdown / QMD export
        from atlas_cli.session_export_md import (
        ...
        if args.output == "-":
            ...
            db.close()
            return
        output_dir = Path(args.output).expanduser() if args.output else get_atlas_home() / "session-exports"
        ...
        # `atlas sessions prune`: no time window and no filters at all
        ...
        if result.get("vacuumed") is False:
            print("  (VACUUM was skipped or failed — run "
                  "`atlas sessions optimize` later to reclaim freed space.)")
