...
# Allow-list for ``export_profile("default")``: when ATLAS_HOME equals the
...
# into the export. The set below identifies the *known Atlas profile
# artifacts* at the root of ATLAS_HOME; everything else is excluded.
...
# (#58394). Add new artifacts here when introduced in ``atlas_constants``.
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in
      ...
      ATLAS_HOME equals the cwd) is excluded. Blacklisting was tried
      first and proved unable to anticipate every non-Atlas file the
      user may have lying alongside ATLAS_HOME (#58394).
...
def _scrub_export_secrets(staged: Path) -> None:
    ...
    ``atlas sessions export --redact``. Runs on the *staged copy only* so the
    live profile is never rewritten. ``force=True`` ignores
    ``security.redact_secrets`` / ``ATLAS_REDACT_SECRETS`` — share archives
