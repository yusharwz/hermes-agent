"""``atlas sync`` subcommand parser — Skill Sync.

Cloned from ``atlas_cli/subcommands/cron.py`` — same injected-handler shape
...
Skill Sync covers two surfaces, both under this one command for launch:
  ...
  Personal — your own skills, across your own devices:
    atlas sync status                 show gate/opt-in/head state
    atlas sync pull                   pull and materialize opted-in skills
    atlas sync push                   push opted-in skills
    atlas sync now                    reconcile: pull then push
    atlas sync enable <skill>         opt a skill into sync
    atlas sync disable <skill>        opt a skill out of sync
    atlas sync device [--name]        show or set this device's label

  Organisation — skills shared with your team:
    atlas sync propose <skill>        share a skill with your organisation
...
def build_sync_parser(subparsers, *, cmd_sync: Callable) -> None:
    ...
    sync_parser = subparsers.add_parser(
        ...
        epilog=(
            "Examples:\n"
            "  atlas sync status            what is synced, and from where\n"
            "  atlas sync enable my-skill   include a skill in your sync\n"
            "  atlas sync now               pull, then push\n"
            "  atlas sync propose my-skill  share a skill with your team\n"
        ),
