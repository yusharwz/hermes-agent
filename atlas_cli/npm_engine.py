...
Scope of the repair is deliberately narrow. Atlas only upgrades an npm that
lives inside its **own** managed Node tree (``$ATLAS_HOME/node``), installing
...
from atlas_constants import get_atlas_home, with_atlas_node_path
...
def managed_npm_prefix(npm: str | os.PathLike[str] | None) -> Path | None:
    """Return the Atlas-managed Node root *npm* lives in, else ``None``.

    Symlinks are resolved first: an install links ``~/.local/bin/npm`` at
    ``$ATLAS_HOME/node/bin/npm``, which itself links into
...
def upgrade_managed_npm(
    ...
    *,
    ...
    ``prefix=~/.local`` into ``$ATLAS_HOME/node/etc/npmrc`` so that global
    ...
    if not quiet:
        print(
            f"→ Upgrading Atlas-managed npm to satisfy {npm_range}…",
            flush=True,
        )
    try:
        # A temp cwd keeps the checkout's .npmrc (engine-strict, min-release-age)
        # from applying to the upgrade itself.
        with tempfile.TemporaryDirectory(prefix="atlas-npm-upgrade-") as tmp:
...
def _print_manual_fix(npm: str, npm_range: str, actual: str | None) -> None:
    ...
    print(
        ...
        f"  Resolved npm: {npm}\n"
        "  Atlas only upgrades npm inside its own managed Node install, so this\n"
...
def maybe_repair_npm_engine(
    ...
    *,
    ...
    """Repair an ``EBADENGINE`` failure when Atlas owns the npm involved.
    ...
    *output* is the combined stdout/stderr of the npm command that just failed.
    ...
    not an engine failure, a Node (not npm) mismatch, an npm Atlas does not
