...
the late-binding seam in :mod:`atlas_cli.web_deps` so tests that
...
from atlas_cli.web_deps import late
...
def get_sessions(
    ...
    try:
        ...
        try:
            ...
            # `atlas serve` backend. No-op when disabled or run recently.
...
async def import_sessions_endpoint(request: Request):
    ...
    restores a whole Atlas backup archive, while this endpoint is scoped to
