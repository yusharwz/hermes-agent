...
from atlas_constants import find_tree_venv, venv_python_path
...
def _smoke_test(tree: Path) -> None:
    ...
    result = subprocess.run(
        [python, "-c", "import agent, atlas_cli; print('ok')"],
        cwd=str(tree),
        capture_output=True,
        text=True,
        timeout=120,
...
def _carry_over_venv(source_tree: Path, target_tree: Path) -> Optional[Path]:
    ...
    ExecStart no longer exists, and `atlas gateway` cannot start again.
...
def spawn_detached(gateway: str, key: str) -> None:
    ...
    with open(logs / "update.log", "ab") as log:
        subprocess.Popen(
            [_runner_python(), "-m", "atlas_cli.ninegate_update", "--apply"],
...
def main() -> int:
    ...
    if "--apply" not in sys.argv[1:]:
        print("penggunaan: python -m atlas_cli.ninegate_update --apply", file=sys.stderr)
