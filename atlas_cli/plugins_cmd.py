...
def ensure_basic_auth_plugin_enabled_in_config(cfg: dict) -> bool:
    ...
    ``atlas setup`` / ``atlas plugins disable basic`` can park the plugin
...
def _discover_entrypoint_plugins() -> list[tuple[str, str, str, str]]:
    """Return plugin entries advertised through ``atlas_agent.plugins``.

    Entry-point plugins are installed as Python packages, so they do not have a
    plugin directory under ``~/.atlas/plugins``. Include package metadata here
    so ``atlas plugins list`` can show and enable them.
    """
    from atlas_cli.plugins import ENTRY_POINTS_GROUP
...
def cmd_toggle() -> None:
    ...
    # plugins.disabled, but ``atlas plugins enable web/firecrawl`` cleared
...
def dashboard_update_user_plugin(name: str) -> dict[str, Any]:
    ...
    # Sibling of the CLI ``atlas plugins update`` path: drop bytecode
...
def _clear_plugin_bytecode(target: Path) -> int:
    ...
    fingerprint sweep in ``atlas_cli.main`` never covers them. After a
