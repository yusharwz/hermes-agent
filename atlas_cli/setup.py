...
def _print_setup_summary(config: dict, atlas_home):
    ...
    if _stt_feature is not None and _stt_feature.managed_by_nous:
        ...
        if fw_ok:
            ...
            tool_status.append(
                ("Speech-to-Text (Local Whisper — not installed)", False, "run 'atlas tools' → Speech-to-Text")
            )
...
def _install_neutts_deps() -> bool:
    ...
    from atlas_cli.tools_config import _pip_install
...
def setup_terminal_backend(config: dict):
    ...
    if selected_backend == "local":
        ...
        if prompt_yes_no("  Enable egress firewall for Docker sandboxes?", False):
            ...
            print_info(
                "Run `atlas egress setup` then `atlas egress start` to mint "
                "tokens and launch the proxy."
            )
        else:
            print_info(
                "Skipping egress firewall. You can enable it later with `atlas egress setup`."
            )
        ...
        print_info("Requires the optional SDK: pip install 'atlas-agent[vercel]'")
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                print_warning("Install failed — run manually: pip install 'atlas-agent[vercel]'")
                if result.stderr:
                    print_info(f"  Error: {result.stderr.strip().splitlines()[-1]}")
...
def setup_telemetry(config: dict):
    ...
    print_info("Packages stay under this Atlas profile and are not uploaded.")
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    print(color("│     ⚕ Atlas Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      ...
      atlas setup telemetry — just local shared metrics
    ...
    if is_existing:
        ...
        if setup_mode == 0:
            _run_first_time_quick_setup(config, atlas_home, is_existing)
            return
        if setup_mode == 2:
