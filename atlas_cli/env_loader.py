...
# load_atlas_dotenv can call _sanitize_env_file_if_needed multiple times
...
def get_secret_source_values(
    atlas_home: str | os.PathLike,
) -> dict[str, str]:
    """Return the external-secret value snapshot for ``atlas_home``."""
    home_key = str(Path(atlas_home).resolve())
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    ``atlas_cli.config._sanitize_env_lines`` normalizes line endings while
...
def load_atlas_dotenv(
    *,
    ...
    #   EnvironmentFile=-/path/to/.atlas/.op.env
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    to locate bootstrap tokens) but BEFORE the rest of Atlas reads
    ...
    owns the once-per-ATLAS_HOME guard, the post-apply ASCII
    ...
    if not cfg:
        ...
        # on its next load_atlas_dotenv() call instead of never.
        return
    ...
    # so the 3-5 import-time load_atlas_dotenv() calls per startup don't
    ...
    if report.applied_any:
        ...
        _sanitize_loaded_credentials()
        # Remember where each var came from so setup / `atlas model`
...
def _load_secrets_config(home_path: Path) -> dict:
    ...
    # first config.yaml read in a normal `atlas` startup, so populating the
    # shared cache here lets main.py's early bridge and atlas_logging reuse
    ...
    if home_path == _process_atlas_home():
        try:
...
def _process_atlas_home() -> Path:
    """The ATLAS_HOME the shared config cache is keyed to."""
    try:
