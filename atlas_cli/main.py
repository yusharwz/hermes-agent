...
from atlas_cli._subprocess_compat import suppress_platform_ver_console
...
# a prior ``atlas update`` left a recovery marker and a core package's import
# files were wiped (#57828 — failed lazy backend refresh), the module-level
# ``from atlas_cli.env_loader import ...`` / ``from atlas_cli.config import
...
# package directory, so if IT can't import, nothing else in atlas_cli can
# either. It is also the canonical home of the probe/repair tables reused by
# the full recovery path below.
from atlas_cli import _early_recovery as _early_recovery_mod
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    explicit ``--tui``/``ATLAS_TUI=1``, then a real-TTY gate (a
    ...
    cron jobs, pipes run ``atlas … chat -q`` with stdio on a pipe. This
...
from atlas_cli.sessions_cmd import cmd_sessions  # noqa: F401
...
from atlas_cli.subcommands.sync import build_sync_parser
...
from atlas_cli.subcommands.approvals import build_approvals_parser
...
from atlas_cli.subcommands.import_agent import build_import_agent_parser
from atlas_cli.subcommands.config import build_config_parser
from atlas_cli.subcommands.skin import build_skin_parser
...
from atlas_cli.subcommands.monitoring import build_monitoring_parser
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    ``atlas -c`` from repo A continues repo A's last session rather than the
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    ...
    # atlas_cli/tui_dist/entry.js but never ships ui-tui/ at all (that
    ...
    if (
        ...
        if result.returncode != 0:
            ...
            # here before doing any work; upgrade a Atlas-managed npm once and
            # retry rather than dumping EBADENGINE at the user.
            from atlas_cli.npm_engine import maybe_repair_npm_engine
...
def _apply_tui_python_env(env: dict) -> None:
    ...
    src_root = str(env.get("ATLAS_PYTHON_SRC_ROOT") or "").strip()
    if not src_root or not Path(src_root).is_dir():
        env["ATLAS_PYTHON_SRC_ROOT"] = str(PROJECT_ROOT)

    cwd = str(env.get("ATLAS_CWD") or "").strip()
    if not cwd or not Path(cwd).is_dir():
        env["ATLAS_CWD"] = _safe_tui_cwd(env)

    python = str(env.get("ATLAS_PYTHON") or "").strip()
    if os.path.dirname(python):
        python_path = Path(python)
        if not python_path.is_absolute():
            python_path = Path(env["ATLAS_CWD"]) / python_path
...
def _launch_tui(
    ...
    # TUI child is a atlas process: propagate the profile-home contract via
...
def _resolve_use_tui(args) -> bool:
    ...
    workers, cron jobs, and pipelines run ``atlas … chat -q`` with stdout
...
def cmd_whatsapp(args):
    ...
    if not _wa_lock_held:
        ...
        print("    atlas gateway stop")
        print("    atlas whatsapp")
        print()
...
def select_provider_and_model(args=None):
    ...
    # Honor ``model_catalog.excluded_providers`` so the CLI ``atlas model``
    ...
    # matching against atlas_id / alias / canonical slug.
    _cli_excluded = {
        str(p).strip().lower()
...
def _aux_select_for_task(task: str) -> None:
    ...
    up new ones through the normal ``atlas model`` flow, then route aux tasks
    to them here.
    """
    from atlas_cli.config import load_config
    from atlas_cli.inventory import build_aux_picker_rows, format_aux_picker_entries
...
def cmd_sync(args):
    ...
    if sub in {"enable", "disable"}:
        ...
        if not is_curation_eligible(skill):
            print(
                ...
                f"skills under ~/.atlas/skills/ can sync.",
                file=sys.stderr,
            )
            return 1
    ...
    if sub == "status":
        ...
        if status.get("org_available"):
            ...
            if modified:
                print(
                    ...
                    f"{', '.join(modified)}\n"
                    f"  Share them back with `atlas sync propose <skill>`. "
        ...
        if not status.get("logged_in"):
            ...
            print(
                "\nSync feature is off for this instance (set ATLAS_SYNC_ENABLED=1 "
            ...
            print(
                ...
                "ATLAS_SYNC_BASE_URL). Sync is inert.",
                file=sys.stderr,
            )
        return 0
    ...
    try:
        if sub == "pull":
            ...
            result = ssc.push_skills(identity=identity, message="atlas sync push")
        elif sub == "now":
            pull_res = ssc.pull_skills(identity=identity)
            push_res = ssc.push_skills(identity=identity, message="atlas sync now")
...
def cmd_approvals(args):
    """Dispatch `atlas approvals <subcmd>`."""
    from atlas_cli.approvals_suggest import approvals_command
...
def cmd_skin(args):
    ...
    from atlas_cli.skin_cmd import skin_command
...
# Update pipeline extracted to atlas_cli/update_cmd.py (main.py decomposition,
# mechanical move). Every moved name is re-exported here so the argparse wiring
# and existing test monkeypatches (atlas_cli.main._cmd_update_impl,
# atlas_cli.main._run_pre_update_backup, ...) keep resolving unchanged.
from atlas_cli.update_cmd import (  # noqa: F401
...
# validated against. Lives next to the checkout (NOT in ATLAS_HOME) because
...
def _sweep_stale_bytecode_if_checkout_changed() -> None:
    ...
    ``atlas update``, a manual ``git pull``, a ZIP update, a file-sync
    ...
    Update-time clears alone can never close this class: ``atlas update``
    ...
    ``atlas`` entry point compares the checkout fingerprint (cheap file
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    mtime comparison. ``git checkout`` / ``git pull`` / ``atlas update``
    ...
    ``atlas_cli/web_dist/`` (per vite.config.ts outDir), NOT ``web/dist/``,
...
def _web_ui_stamp_path() -> Path:
    """Return the path to the web UI build stamp file under $ATLAS_HOME."""
    from atlas_constants import get_atlas_home
    return get_atlas_home() / "web-ui-build-stamp.json"
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    # returns True only when it actually upgraded a Atlas-managed npm.
    from atlas_cli.npm_engine import maybe_repair_npm_engine
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    project_root = web_dir.parent.parent if web_dir.parent.name == "apps" else web_dir.parent
    dist_index = project_root / "atlas_cli" / "web_dist" / "index.html"
    try:
...
def _do_build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    from atlas_constants import with_atlas_node_path
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "win32" and len(existing) > 1:
        ...
        # Picking purely by mtime can then hand a wrong-architecture Atlas.exe
...
#
# The desktop self-update chain (Desktop → atlas-setup --update →
# `atlas update` → `atlas desktop --build-only` → relaunch) rebuilds
# Atlas.exe on the end user's machine and used to verify only that the file
...
# wrong-architecture Atlas.exe is caught BEFORE the updater replaces the
...
def _windows_native_machine_from_iswow64() -> Optional[str]:
    ...
    process arch) and rejected a correctly-built ARM64 ``Atlas.exe``.
...
def _windows_native_machine() -> str:
    ...
    emulation: the desktop update chain runs an x64 atlas-setup.exe (and thus
...
def _ensure_desktop_exe_launchable(
    ...
    print(f"✗ The built Atlas.exe failed its integrity check: {error}")
    ...
    if restored is not None:
        print("  ↩ Update aborted — restored the previous working Atlas.exe from backup.")
        print("    Your existing version was kept and still works. Run `atlas desktop`")
        print("    (or the in-app update) again to retry with a fresh Electron download.")
        return restored, True

    print("  ✗ No usable backup was found to restore.")
    print("    Run `atlas desktop --force-build` to rebuild, or re-run the Atlas")
...
def _desktop_macos_local_codesign(
    ...
    #    contains the outer Atlas.app component, so an absolute-parts check
...
def _desktop_macos_relaunchable_fixup(
    ...
    *,
    ...
    "Atlas is damaged and can't be opened" — and macOS TCC forgets every
...
def cmd_gui(args: argparse.Namespace):
    ...
    if getattr(args, "cwd", None):
        ...
        env["ATLAS_DESKTOP_CWD"] = os.getcwd()
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            # Put the Atlas-managed Node on PATH so npm's child scripts (which
            # shell out to bare `node`, e.g. electron-winstaller's
            # select-7z-arch.js) resolve it even when the parent PATH is
            # stripped — the desktop updater chain (Desktop → atlas-setup →
            # atlas update) loses shell PATH customizations. Wrapping the
            # NixOS build env keeps its PYTHON hint while restoring managed Node
            # ahead of a bare PATH (same idiom as the `atlas update` path).
            ...
            if not source_mode:
                ...
                # success on a Atlas.exe Windows cannot load (truncated PE from
...
# Dashboard process-hygiene helpers extracted to atlas_cli/dashboard_procs.py
# (main.py decomposition, mechanical move). Re-exported so callers and test
# monkeypatches on atlas_cli.main.<name> keep resolving unchanged.
from atlas_cli.dashboard_procs import (  # noqa: F401
...
_DASHBOARD_SYSTEMD_UNIT = "atlas-dashboard.service"
...
def _restart_managed_dashboard_service(
    ...
    # Probe the user manager first: Atlas installs Linux services in the
...
def _get_systemd_service_for_pid(pid: int) -> str | None:
    ...
    ``atlas-serve.service``).  Returns ``None`` when the PID is not
    ...
    try:
        ...
        for line in text.splitlines():
            line = line.strip()
            # Format: 0::/system.slice/atlas-serve.service
...
def _dashboard_cmdline_for_pid(pid: int) -> list[str] | None:
    ...
    is reconstructed, but atlas launch commands don't embed exotic args).
...
def _respawn_dashboard_processes(commands: list[list[str]]) -> list[list[str]]:
    """Best-effort respawn of manually-started dashboards after ``atlas update``.
    ...
    log_path = get_atlas_home() / "logs" / "dashboard-restart.log"
    try:
...
# Install-scoped breadcrumbs live next to the venv (not under $ATLAS_HOME)
...
def _recover_from_interrupted_install() -> None:
    """Finish update work left half-done by a prior ``atlas update``.
...
def _recover_core_update_marker_locked() -> None:
    ...
    # Windows: a normal ``atlas.exe`` launch always has the launcher as an
    ...
    self_locked = _windows_running_atlas_launcher_locked()
    if self_locked:
        install_prefix, install_env = _default_venv_install_target()
        print(
            "  → Running from atlas.exe; applying package-only first aid, "
    ...
    try:
        ...
        if self_locked:
            print(
                "  Atlas is still running from the launcher that needs "
                "replacing. Close other Atlas windows, restart from a "
...
def _windows_running_atlas_launcher_locked() -> bool:
    """True when a venv ``atlas*.exe`` shim is this process or an ancestor.
...
def _venv_scripts_dir() -> Path | None:
    ...
    if not venv_dir.is_dir():
        return None
    from atlas_constants import venv_bin_dir
...
def _run_package_only_install(
    ...
    *,
    ...
    rewrite ``atlas.exe``. The editable-install quarantine path would rename
...
def _repair_venv_via_import_probes(
    ...
    *,
    ...
    detected (#57828). Package-only reinstall — never rewrites ``atlas.exe``.
    ...
    print("  ⚠ Venv repair incomplete. Run manually, then `atlas update`:")
    print(
...
def _resolve_install_target_python(
    ...
    if env and "VIRTUAL_ENV" in env:
        from atlas_constants import venv_python_path
...
def cmd_update(args):
    ...
    from atlas_cli.update_lock import (
...
def cmd_profile(args):
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print("Switched to: default (~/.atlas)")
            else:
...
def _report_dashboard_status() -> int:
    ...
    print(f"{len(live)} atlas dashboard process(es) running:")
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    print("    [2] OAuth via Nous Portal (run `atlas dashboard register`)")
    print("    [3] Cancel")
    print()
    ...
    if choice == "2":
        ...
        print(
            ...
            "ATLAS_DASHBOARD_OAUTH_CLIENT_ID into ~/.atlas/.env for you.\n"
    ...
    try:
        from atlas_cli.config import load_config, save_config
        from atlas_cli.plugins_cmd import ensure_basic_auth_plugin_enabled_in_config
...
def _read_ssh_session_token_file(path: str) -> str:
    ...
    if sys.platform == "win32":
        from atlas_cli.windows_ssh_runtime import read_token
    ...
    token_root = _get_atlas_home() / "desktop-ssh"
    try:
...
def _is_electron_packaged_web_dist(path: str) -> bool:
    ...
    Packaged Desktop sets ``ATLAS_WEB_DIST`` to ``.../app.asar/dist`` or
    ``.../app.asar.unpacked/dist``. A standalone ``atlas dashboard`` that
...
def cmd_dashboard(args):
    ...
    if _token_file and not _headless_backend:
        raise SystemExit("--ssh-session-token-file is only valid with atlas serve")

    # ── Sanitize Desktop-inherited env that hijacks a standalone launch ─
    # Desktop Electron spawns its backend with ATLAS_DESKTOP=1 plus
    # ATLAS_WEB_DIST=<packaged app.asar[/unpacked]/dist> (and often
    # ATLAS_SERVE_HEADLESS=1 on the serve path). A shell that inherits
    # those vars then runs `atlas dashboard` would otherwise:
    #   - serve the desktop renderer → "Desktop IPC bridge is unavailable"
    #     (issue #52945), or
    #   - disable the SPA via inherited ATLAS_SERVE_HEADLESS.
    # Only strip Electron-packaged WEB_DIST contamination — caller-managed
    # ATLAS_WEB_DIST overrides (dev / custom builds) must still work.
    # The desktop-spawned backend itself (ATLAS_DESKTOP=1) keeps its dist.
    # Intentionally headless `serve` re-sets ATLAS_SERVE_HEADLESS below.
    if os.environ.get("ATLAS_DESKTOP") != "1":
        _inherited_web_dist = os.environ.get("ATLAS_WEB_DIST", "")
        if _is_electron_packaged_web_dist(_inherited_web_dist):
            os.environ.pop("ATLAS_WEB_DIST", None)
    if not _headless_backend:
        os.environ.pop("ATLAS_SERVE_HEADLESS", None)
    ...
    if (
        ...
        from tools.environments.local import build_subprocess_env
        # Exact env preservation: ATLAS_HOME is explicitly pinned to the
    ...
    if _headless_backend:
        ...
        os.environ["ATLAS_SERVE_HEADLESS"] = "1"
    elif "ATLAS_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        if not (_dist_root / "index.html").exists():
            ...
            # default dist location is recoverable: a custom ATLAS_WEB_DIST
            # points at a caller-managed directory the build cannot populate.
            _recoverable = "ATLAS_WEB_DIST" not in os.environ
            if _recoverable:
        ...
        # ATLAS_WEB_DIST is set without --skip-build: the build is skipped
        ...
        _dist_root = Path(os.environ["ATLAS_WEB_DIST"]).expanduser()
        if not (_dist_root / "index.html").exists():
            print(f"✗ ATLAS_WEB_DIST is set but no web dist found at: {_dist_root}")
            print("  Pre-build first:  npm install --workspace web && npm run build -w web")
            print("  Or unset ATLAS_WEB_DIST to build and use the default web UI dist.")
            sys.exit(1)
        # Write the expanded path back: web_server reads ATLAS_WEB_DIST raw
        # at import (no expanduser), so a validated "~/dist" would otherwise
        # pass here and still 404 there.
        os.environ["ATLAS_WEB_DIST"] = str(_dist_root)
        print(f"→ Using web dist from ATLAS_WEB_DIST: {_dist_root}")
...
def cmd_dashboard_register(args):
    ...
    from atlas_cli.dashboard_register import cmd_dashboard_register as _impl
...
def _resolve_deferred_platform_cli_command(command_name: str | None) -> None:
    ...
    registers a top-level ``atlas <name>`` command (e.g. Photon ->
    ...
    import it, so the CLI registration never happens and ``atlas photon``
...
def _prepare_agent_startup(args) -> None:
    ...
    # --yolo: chokepoint guarantee that ATLAS_YOLO_MODE is set before ANY
...
def cmd_monitoring(args):
    ...
    if action == "status":
        ...
        print(f"  OTel SDK:       {'installed' if otlp_exporter.is_available() else 'not installed'} "
              f"(optional extra: atlas-agent[otlp])")
...
def main():
    ...
    # If the checkout changed since the last launch (atlas update, manual
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.atlas/.env.  Supports Bitwarden "
    ...
    from atlas_cli import onepassword_secrets_cli as _op_secrets_cli
    ...
    # `atlas proxy` (defined elsewhere in this file) is a separate INBOUND
    ...
    from atlas_cli import proxy_cli as _proxy_cli
    ...
    def _dispatch_egress(args):  # noqa: ANN001
        ...
        # from the inbound OAuth ``atlas proxy`` subparser (dest='proxy_command').
    ...
    # =========================================================================
    # approvals command  (parser built in atlas_cli/subcommands/approvals.py)
    # =========================================================================
    ...
    # =========================================================================
    # import-agent command  (parser: atlas_cli/subcommands/import_agent.py)
    # =========================================================================
    def cmd_import_agent(args):
        from atlas_cli.agent_import import import_agent_command
    ...
    # =========================================================================
    # skin command  (parser built in atlas_cli/subcommands/skin.py)
    # =========================================================================
    ...
    sessions_export.add_argument(
        ...
        help=(
            ...
            "md/qmd: output directory (default: <atlas home>/session-exports)"
        ),
    )
    sessions_export.add_argument(
        "--format",
    ...
    # cmd_sessions lives in atlas_cli/sessions_cmd.py (main.py decomposition).
    ...
    # --yolo: set ATLAS_YOLO_MODE *before* plugin discovery.  The call to
    ...
    # ``atlas egress start`` refusing when credential_source=bitwarden
