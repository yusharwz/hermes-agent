...
def _assert_not_delegated_child_mutation() -> None:
    ...
    try:
        ...
        delegated = bool(os.environ.get("ATLAS_DELEGATED_CHILD_CONTEXT"))
    if delegated:
...
def _sqlite_connect(path: Path) -> sqlite3.Connection:
    ...
    advisory locks on the database (see ``atlas_cli.sqlite_safe_read``).
...
def _backup_corrupt_db(path: Path) -> Optional[Path]:
    ...
    # see atlas_cli.sqlite_safe_read), so it must only run once the board has
...
def repair_db(
    ...
    *,
    ...
    :class:`RepairResult` so ``atlas kanban repair`` can report and choose
    its own exit code.
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
    ...
    with _cross_process_init_lock(path):
        ...
        from atlas_state import preflight_db_writability
...
def _check_file_length_invariant(conn: sqlite3.Connection) -> None:
    ...
    """
    from atlas_cli.sqlite_safe_read import file_length_matches_header
...
def store_attachment_bytes(
    ...
    *,
    ...
    (``atlas kanban attach``) so name-sanitisation, the size cap, and the
...
def _default_spawn(
    ...
    *,
    ...
    # read on the board and in `atlas kanban log` — it is not a conversation
    ...
    # ("work kanban task t_…").
    env["ATLAS_SESSION_SOURCE"] = "kanban"
    ...
    _retag_legacy_worker_sessions(env["ATLAS_KANBAN_WORKSPACES_ROOT"])
    ...
    # A worker must NEVER boot the interactive TUI: an inherited ATLAS_TUI=1
    ...
    # older atlas builds on PATH that predate the flag's precedence.
    env.pop("ATLAS_TUI", None)
