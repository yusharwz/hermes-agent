"""Pydantic request/response models for the Atlas dashboard web server.

Extracted verbatim from ``atlas_cli/web_server.py`` (pure schema move).
``web_server`` re-exports every name here, so existing imports like
``from atlas_cli.web_server import ConfigUpdate`` keep working.
"""
...
class ModelAssignment(BaseModel):
    ...
    # ``atlas model`` custom flow collects.
    api_key: str = ""
