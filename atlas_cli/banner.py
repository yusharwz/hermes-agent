...
_ACCENT = "\033[1;38;2;47;191;222m"  # True-color #2FBFDE bold
_BOLD = "\033[1m"
_DIM = "\033[2m"
_RST = "\033[0m"
...
def cprint(text: str):
    ...
    try:
        ...
        # absent stdout (pythonw.exe, CI, `atlas ... > file`) raises
...
_OFFICIAL_REPO_CANONICAL = "github.com/nousresearch/atlas-agent"
...
def check_for_updates() -> Optional[int]:
    ...
    # ATLAS_REVISION (that's nix-only). Returning None makes both the Rich
    # banner (build_welcome_banner) and the Ink badge (branding.tsx, guarded
    # on `typeof === 'number' && > 0`) show nothing. The dashboard's REST
    # `/api/atlas/update/check` endpoint short-circuits docker the same way
    # (web_server.py); mirror that here so the banner/TUI surfaces agree.
    try:
        from atlas_cli.config import detect_install_method, get_project_root
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
                         ...
                         provider: str = None):
    """Build and print a welcome banner with emblem on left and info on right.
    ...
    dim = _skin_color("banner_dim", "#14788F")
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else ATLAS_EMBLEM
    except Exception:
        _bskin = None
        _hero = ATLAS_EMBLEM
    left_lines = ["", _hero, ""]
    if (provider or "").strip().lower() == "moa":
        ...
        left_lines.append(f"[{accent}]MoA: {preset_name}[/]{agg_str}{ctx_str} [dim {dim}]·[/] [dim {dim}]Dritech[/]")
    else:
        ...
        ctx_str = f" [dim {dim}]·[/] [dim {dim}]{_format_context_length(context_length)} context[/]" if context_length else ""
        left_lines.append(f"[{accent}]{model_short}[/]{ctx_str} [dim {dim}]·[/] [dim {dim}]Dritech[/]")
    ...
    title_color = _skin_color("banner_title", "#2FBFDE")
    border_color = _skin_color("banner_border", "#1A94B8")
