"""Shell completion script generation for atlas CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_atlas_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Atlas Agent bash completion
# Add to ~/.bashrc:
#   eval "$(atlas completion bash)"

_atlas_profiles() {{
    local profiles_dir="$HOME/.atlas/profiles"
...
_atlas_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_atlas_profiles)" -- "$cur"))
        return
    fi
...
_atlas_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.atlas/profiles" ]]; then
        profiles+=($HOME/.atlas/profiles/*(N/:t))
    fi
...
_atlas() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_atlas_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'atlas command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Atlas Agent fish completion",
        "# Add to your config:",
        "#   atlas completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __atlas_profiles",
        "    echo default",
        "    if test -d $HOME/.atlas/profiles",
        "        for d in $HOME/.atlas/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c atlas -f -s p -l profile"
        " -d 'Profile name' -xa '(__atlas_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__atlas_profiles)' -d 'Profile name'"
                )
