"""Bounded product contract for the first Atlas shared-metrics slice."""
...
SCHEMA_KEY = "atlas.metrics.schema_version"
SCHEMA_VERSION = "atlas.metrics.event.v1"
MODEL_CALL_SCOPE = "atlas.model_call"
TASK_SCOPE = "atlas.task_run"
SUBSCRIBER_NAME = "atlas.nemo_relay.shared_metrics"
PRIMARY_MODEL_CALL_ROLE = "primary"
MODEL_CALL_METRIC = "atlas.model_call.count"
TASK_STARTED_METRIC = "atlas.task_run.started"
TASK_FINISHED_METRIC = "atlas.task_run.finished"
...
# in Atlas's execution metadata because that flag has narrower routing/catalog
# semantics there.
...
# Atlas intentionally resolves these local runtimes through the generic custom
...
def task_terminal_state(kwargs: dict[str, Any]) -> tuple[str, str, str]:
    """Map Atlas terminal state to bounded task outcome dimensions."""
...
def provider_family(kwargs: dict[str, Any]) -> str:
    """Map a Atlas provider to a bounded product category."""
...
def _provider_metadata(provider: str) -> tuple[str, bool, bool]:
    ...
    try:
        ...
        from atlas_cli.providers import ATLAS_OVERLAYS, normalize_provider
...
def _known_provider_ids() -> frozenset[str]:
    """Cache Atlas's static provider catalog for the process lifetime."""
    try:
        from atlas_cli.provider_catalog import provider_catalog_by_slug
