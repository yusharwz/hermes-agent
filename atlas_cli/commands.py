...
class CommandDef:
    ...
    # Names a key in ``atlas_cli.slash_exec.EXECUTORS`` — a pure formatter
    # producing the canonical, surface-independent core text.  Surfaces
    # resolve it via ``atlas_cli.slash_exec.run_execute`` and apply only
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("wake", "Toggle the 'Hey Atlas' wake word listener", "Configuration",
               cli_only=True, args_hint="[on|off|status]",
               subcommands=("on", "off", "status")),
...
#   - topup: the billing/balance surface; reached via /atlas topup on Slack.
...
#   - egress: Docker-only proxy status; reachable as /atlas egress on Slack.
#   - init: repo-scan AGENTS.md bootstrap — a cwd-centric dev command that is
#     rare from Slack; reachable as /atlas init. Without this entry, adding
#     /init clamps /version off the native list and breaks Telegram parity.
#   - version: low-frequency info command; reachable as /atlas version on
...
#   - diff: git working-tree diff; reached via /atlas diff on Slack so it
...
#     /atlas update on Slack. Demoted to free the native slot /approvals now
...
_SLACK_VIA_ATLAS_ONLY = frozenset({"topup", "moa", "debug", "egress", "init", "version", "diff", "update"})
