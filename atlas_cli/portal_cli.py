"""``atlas portal`` — the human-readable entry point for Nous Portal.

Running ``atlas portal`` with no subcommand performs the one-shot Portal
...
``atlas auth add nous --type oauth`` (which still works), is identical to
``atlas setup --portal``, and runs the same Nous flow as the first-time quick
setup.
...
already in ``atlas auth`` or ``atlas tools``. It's the onboarding + discovery
...
def _cmd_status(args) -> int:
    ...
    if provider == "nous":
        ...
        print(f"  Model:   currently {provider} (switch with `atlas model`)")
...
def _cmd_tools(args) -> int:
    ...
    if not features.nous_auth_present:
        print(color("  Not logged into Nous Portal — sign in with `atlas portal`.", Colors.YELLOW))
        print()
...
def _cmd_login(args) -> int:
    ...
    This is the human-readable front door for `atlas auth add nous --type
    oauth`. It reuses the exact wiring behind `atlas setup --portal` (which in
    ...
    """
    from atlas_cli.setup import _run_portal_one_shot
...
def portal_command(args) -> int:
    ...
    if sub in {None, "", "login"}:
        # Default to the one-shot onboarding — `atlas portal` is the
        # human-readable alias for `atlas auth add nous --type oauth` /
        # `atlas setup --portal`.
    ...
    print("Run `atlas portal -h` for usage.", file=sys.stderr)
    return 1
...
def add_parser(subparsers) -> None:
    ...
    portal_parser = subparsers.add_parser(
        ...
        description=(
            "Run `atlas portal` with no subcommand to log in to Nous Portal "
            "and set it up — pick a model, set Nous as your provider, and offer "
            "the Tool Gateway (the human-readable alias for `atlas auth add "
            "nous --type oauth`, identical to `atlas setup --portal`). "
