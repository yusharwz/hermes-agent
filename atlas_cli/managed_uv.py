"""Atlas-managed uv and Python runtime repair.
...
The Python backing the install is different: it is shared by every Atlas
profile because the checkout's ``venv`` is shared.  Runtime repair therefore
uses an install-scoped store under ``<checkout>/.atlas-runtime/python``. A
...
from atlas_cli.sqlite_runtime import SQLiteRuntimeInfo, probe_sqlite_runtime

logger = logging.getLogger(__name__)

_PROJECT_ROOT = Path(__file__).resolve().parents[1]
_RUNTIME_DIR_NAME = ".atlas-runtime"
_VENV_NAME = "venv"
_ALT_VENV_NAME = ".venv"
...
def managed_python_env(
    ...
    *,
    ...
    """Return a sanitized environment for Atlas-private uv Python commands."""
    target = (
        Path(install_dir)
...
def _report_runtime_repair_failure(repair: RuntimeRepairResult) -> None:
    if repair.backup_venv is None:
        ...
        print(
            "    Sessions stay protected meanwhile: Atlas keeps databases "
            "out of WAL mode on this SQLite build. The next `atlas update` "
            "will retry."
        )
        return
...
def _uv_self_update_is_fresh(now: float | None = None) -> bool:
    ...
    uv releases roughly weekly while many users run ``atlas update`` daily;
    re-running a blocking network self-update on every invocation is waste
    and, offline, an unbounded hang risk. A stamp file under ATLAS_HOME
    caches the last successful self-update time.
    """
    try:
        from atlas_constants import get_atlas_home

        stamp = get_atlas_home() / "cache" / ".uv_self_update_stamp"
...
def _attempt_install_generation(
    ...
    *,
    ...
    try:
        ...
        logger.warning("uv resolved Python outside the Atlas generation: %s", python)
        _remove_tree(generation, boundary=python_root)
        return None
...
def _windows_runtime_holders() -> tuple[bool, str]:
    if platform.system() != "Windows":
        return False, ""
    main_module = sys.modules.get("atlas_cli.main")
    ...
    if holders:
        ...
        return True, f"other Atlas processes still hold the venv (PID {pids})"
    return False, ""
...
def _refresh_managed_uv_catalog(uv_bin: str) -> bool:
    ...
    unmanaged installs.  Only the Atlas-managed binary is ever refreshed;
...
def _default_live_venv(root: Path) -> Path:
    ...
    ``not-applicable`` on every ``atlas update`` and stayed on
    journal_mode=DELETE forever — even though the WAL fallback warning
    promises that ``atlas update`` repairs the runtime (issue class:
