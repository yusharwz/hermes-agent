...
from atlas_cli.urllib_security import open_credentialed_url
...
_PROVIDER_MODELS: dict[str, list[str]] = {
    ...
    # endpoint expects (see atlas_cli/model_setup_flows.py).
...
# In-repo fallback for the model Atlas silently lands on when the user never
...
def get_preferred_silent_default_model(provider: str = "openrouter") -> str:
    ...
    """
    try:
        from atlas_cli.model_catalog import get_default_model_from_cache
...
# ``atlas model``) uses the richer free/paid-tier-aware resolver — see
# ``get_recommended_default_model`` in atlas_cli/web_server.py and
    ...
    *,
    ...
    for preferred_id in preferred_ids:
        ...
        if preferred_id == silent_default:
            ...
            # picker shows which model Atlas lands on when none is selected.
            desc = "default"
        else:
...
def fetch_ai_gateway_models(
    ...
    *,
    ...
    from atlas_constants import AI_GATEWAY_BASE_URL
...
def fetch_ai_gateway_pricing(
    ...
    *,
    ...
    """Fetch Vercel AI Gateway /v1/models and return atlas-shaped pricing.

    Vercel uses ``input`` / ``output`` field names; atlas's picker expects
...
def _model_dedup_key(model_id: str) -> str:
    ...
    try:
        from atlas_cli.model_search import model_alias_canonical
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # ``atlas model`` shows.
            from atlas_cli.providers import is_official_openai_host
...
def _credential_fingerprint(provider: str) -> str:
    ...
    # env var, so it must change the fingerprint too or `atlas config set
...
def copilot_model_api_mode(
    ...
    *,
    ...
    # endpoint, not through Atlas' native Anthropic adapter. The live catalog may
...
_DEEPINFRA_MODELS_QUERY = "filter=true&sort_by=atlas"
