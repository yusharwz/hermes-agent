...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - invokes ``python -m atlas_cli.main [--profile X] gateway run``
    ...
    to that terminal like a normal foreground ``atlas gateway run``.
    ...
    prog_args = [python_exe_path, "-m", "atlas_cli.main"]
    if profile_arg:
...
def windowless_gateway_restart_spec(
    ...
    supplies the stable cwd + env overlay (ATLAS_HOME, VIRTUAL_ENV,
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``python.exe -m atlas_cli.main gateway run`` directly — NOT
