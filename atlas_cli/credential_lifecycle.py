"""Unified provider-credential lifecycle across every store Atlas reads.

A provider API key can live in up to THREE stores at once:

    1. ``~/.atlas/.env``                     — the canonical secret store
    2. ``~/.atlas/auth.json`` →
       ``credential_pool.<provider>[*]``      — env-seeded pool entries
       (``source == "env:<VAR>"``) persisted by the pool loader
    3. ``~/.atlas/config.yaml``              — inline mirrors written by the
...
def purge_env_credential_references(
    ...
    # Make the removal sticky the same way `atlas auth remove` does: a
...
def save_provider_env_credential(env_var: str, value: str) -> Dict[str, Any]:
    ...
    re-add through the UI behaves like ``atlas auth add``.
    """
    from atlas_cli.config import load_env, save_env_value
