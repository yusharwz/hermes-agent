...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    try:
        ...
        # ``provider: anthropic`` as a MoA advisor/aggregator has opted Atlas
    ...
    # the user initiated inside Atlas (manual add / device-code / PKCE), plus
    ...
    try:
        for entry in read_credential_pool(normalized):
            ...
            if source.startswith("env:"):
                ...
                continue
            if (
                source in {"device_code", "loopback_pkce", "atlas_pkce", "manual"}
...
def _nous_portal_env_override() -> Optional[str]:
    ...
    pointing Atlas at a non-production Nous Portal (e.g. a hosted agent
...
def _probe_codex_quota_restored(
    ...
    *,
    ...
    Atlas persists a Codex 429's ``reset_at`` locally and freezes the
...
def _quarantine_nous_oauth_state(
    ...
    *,
    ...
    # sizes/booleans. NEVER pass a raw token/agent_key into the log call — Atlas
...
def resolve_nous_access_token(
    *,
    ...
    with _provider_state_transaction("nous") as (
        ...
        if not state:
            raise AuthError(
                "Atlas is not logged into Nous Portal.",
                provider="nous",
                relogin_required=True,
            )
...
def resolve_nous_runtime_credentials(
    *,
    ...
    with _provider_state_transaction("nous") as (
        ...
        if not state:
            raise AuthError("Atlas is not logged into Nous Portal.",
...
def _prompt_model_selection(
    ...
    try:
        ...
        from atlas_cli.model_search import model_search_text
    ...
    from atlas_cli.curses_ui import format_radio_item_ansi
...
def _login_nous(args, pconfig: ProviderConfig) -> None:
    ...
    try:
        ...
        if not selected_model:
            ...
            print("  Run `atlas model` again to switch to Nous Portal.")
            return
