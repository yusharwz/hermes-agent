...
from atlas_cli.vercel_auth import describe_vercel_auth
...
def _sqlite_upgrade_hint(install_method: str | None = None) -> str:
    ...
    if method == "docker":
        ...
        action = f"run `{command}`, then recreate all Atlas containers"
    elif method in {"nix", "nixos"}:
        action = recommended_update_command_for_method(method)
    else:
        action = "run `atlas update`"
    return (
...
_DEPRECATED_ENV_VARS: tuple[tuple[str, str], ...] = (
    ...
    # ignored. ATLAS_TOOL_PROGRESS_MODE is still read by the gateway as a
    # back-compat fallback but remains deprecated.
    ("ATLAS_TOOL_PROGRESS", "display.tool_progress in config.yaml — ignored/unsupported since config floor v12"),
    ("ATLAS_TOOL_PROGRESS_MODE", "display.tool_progress in config.yaml"),
...
def check_certificates(should_fix: bool = False, issues: "list | None" = None) -> None:
    ...
    if not should_fix:
        ...
        if issues is not None:
            issues.append(
                "Repair the CA bundle: run `atlas doctor --fix`, or "
...
def run_doctor(args):
    ...
    try:
        import sqlite3
        from atlas_state import is_sqlite_wal_reset_vulnerable, sqlite_source_id
        ...
        if is_sqlite_wal_reset_vulnerable():
            # Warn-only: Atlas already refuses to enable WAL on fresh DBs.
    ...
    if env_path.exists():
        ...
        # matches atlas_cli.env_loader._load_dotenv_with_fallback.
        try:
    ...
    if config_path.exists():
        ...
        try:
            from atlas_cli.config import load_env, read_user_config_raw, remove_env_value
        ...
        try:
            from atlas_cli.config import load_env as _load_env_depr
            from atlas_cli.config import read_user_config_raw as _read_raw_depr
    ...
    if terminal_env == "vercel_sandbox":
        ...
        if importlib.util.find_spec("vercel") is not None:
            ...
            _fail_and_issue(
                ...
                "Install the Vercel optional dependency: pip install 'atlas-agent[vercel]'",
                issues,
            )
    ...
    if _safe_which("node"):
        ...
        # `atlas acp --setup-browser` installs agent-browser into the
        # Atlas-managed node prefix, which isn't necessarily on PATH. Mirror
        # dep_ensure._has_atlas_agent_browser() so doctor and dep_ensure agree
        ...
        _managed_ab = (
            _which_in(ATLAS_HOME / "node" / "bin")
            or _which_in(ATLAS_HOME / "node")
        )
        _legacy_ab = _which_in(ATLAS_HOME / "node_modules" / ".bin")
    ...
    try:
        from atlas_cli.config import read_user_config_raw as _read_raw_mem
