"""On-demand supply-chain audit for Atlas Agent installs.

Scans three surfaces a Atlas user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Atlas venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.atlas/plugins``
...
def _discover_plugins(atlas_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.atlas/plugins``.
...
def run_audit(
    *,
    ...
    home = atlas_home or Path(get_atlas_home())
...
def _count_components(
    *, skip_venv: bool, skip_plugins: bool, skip_mcp: bool, atlas_home: Path
) -> int:
    total = 0
    if not skip_venv:
        total += len(_discover_venv())
    if not skip_plugins:
        total += len(_discover_plugins(atlas_home))
    if not skip_mcp:
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `atlas security audit`."""
    ...
    total = _count_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, atlas_home=home
    )
    if total == 0:
