"""Offline, non-destructive recovery for a damaged Atlas session database.
...
def _copy_source_bundle(source: Path, snapshot_dir: Path) -> tuple[Path, list[str]]:
    ...
    locks -- the failure class ``atlas_cli.sqlite_safe_read`` exists to
    ...
    keeps this path consistent with ``atlas_state._backup_db_file``.
    """
    from atlas_cli.sqlite_safe_read import LiveConnectionError, offline_file_access
    ...
    try:
        ...
        if before != after:
            raise SessionRecoverySafetyError(
                ...
                "Stop every Atlas process using this profile and retry."
            )
