...
``atlas_cli.config.migrate_config``. Each step is a function
...
``atlas_cli.config`` imports :func:`run_migrations` lazily (inside
``migrate_config``), and every step function here resolves its helpers
(``read_raw_config``, ``_persist_migration``, ``get_env_value``, …) lazily
through the live ``atlas_cli.config`` module object at call time via
:func:`_cfg`. There is deliberately NO module-level import of
``atlas_cli.config`` here, so no circular import can form — and, just as
importantly, tests that monkeypatch helpers on ``atlas_cli.config`` (e.g.
``patch("atlas_cli.config.read_raw_config", ...)``) keep working, because
...
#: ATLAS_TOOL_PROGRESS*) forever is not worth it. Below-floor configs are
...
def support_floor_message() -> str:
    ...
    return (
        ...
        f"{display_atlas_home()}/config.yaml and run `atlas setup` to "
...
def _cfg():
    """Return the live ``atlas_cli.config`` module (lazy, cycle-free)."""
...
def _migrate_to_21(results: Dict[str, Any], quiet: bool) -> None:
    ...
    read_raw_config = _c.read_raw_config
    _persist_migration = _c._persist_migration
    get_atlas_home = _c.get_atlas_home
