#!/usr/bin/env python3
"""Canvas LMS API CLI for Atlas Agent.
...
def _check_config():
    ...
    if missing:
        atlas_env = os.path.join(
            os.environ.get("ATLAS_HOME", os.path.expanduser("~/.atlas")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {atlas_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Atlas Agent"
    )
