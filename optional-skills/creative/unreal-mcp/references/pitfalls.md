...
### 1. Start order: editor first, Atlas second

Atlas probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW Atlas session.
...
restart the editor, restart the Atlas session.
...
server fails to bind (Output Log) or Atlas' probe times out. Fix: change
Server Port Number in Editor Preferences > Model Context Protocol AND the
`url` in `~/.atlas/config.yaml` (`mcp_servers.unreal-engine`), then restart
...
### 5. GenerateClientConfig is not for Atlas

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. Atlas' connection
lives in `~/.atlas/config.yaml` via `atlas mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms Atlas — just don't
mistake it for the Atlas setup step.
...
explicitly warns against overlapping calls. Atlas executes same-turn tool
...
### 9. Timeouts: Atlas gives up before Unreal does

Atlas' default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If Atlas runs elsewhere (SSH backend,
