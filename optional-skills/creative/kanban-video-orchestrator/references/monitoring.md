...
atlas kanban watch --tenant <project-slug>

# Snapshot of the board
atlas kanban list --tenant <project-slug>
atlas kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
atlas kanban stats --tenant <project-slug>

# Visual dashboard (browser)
atlas dashboard

# Inspect a specific task (includes comments + events)
atlas kanban show <task-id>

# Follow a single task's event stream
atlas kanban tail <task-id>
```

Verify available subcommands with `atlas kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `atlas kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `atlas kanban block <id>` then `atlas kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `atlas kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `atlas kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `atlas profile list` (profile exists?), `atlas status` (gateway/dispatcher up?), and verify tenant. |
...
atlas kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
atlas kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(atlas kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
atlas kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
atlas kanban block <task-id>
atlas kanban archive <task-id>

# Diagnose what happened
atlas kanban show <task-id>      # task body, comments, recent events
atlas kanban tail <task-id>      # follow the live event stream
atlas kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `atlas kanban create` for the director
...
while true; do
    clear
    atlas kanban list --tenant <slug>
    echo "---"
    atlas kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `atlas kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `atlas profile list` and start
