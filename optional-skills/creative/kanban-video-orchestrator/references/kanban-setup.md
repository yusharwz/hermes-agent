...
the actual `setup.sh` that creates the project workspace, configures Atlas
...
2. **Create profiles** — `atlas profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.atlas/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `atlas kanban create` assigned to the director
...
```bash
atlas profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.atlas/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.atlas/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Atlas
...
Each profile gets a `SOUL.md` at `~/.atlas/profiles/<name>/SOUL.md` that
...
```bash
atlas kanban create "Direct production of <video title>" \
    --assignee director \
...
the Atlas `.env` (`${ATLAS_HOME:-$HOME/.atlas}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _atlas_env="${ATLAS_HOME:-$HOME/.atlas}/.env"
    if grep -q "^${var}=" "$_atlas_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_atlas_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_atlas_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY atlas ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY atlas OPENROUTER_API_KEY || exit 1
# ...
```
