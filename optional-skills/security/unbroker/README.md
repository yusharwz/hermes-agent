...
The design is **Atlas-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
atlas skills install official/security/unbroker
```

Then start a new Atlas session and drive it (below). The skill works zero-config; a few optional
...
- **PII handling.** Dossiers live under the Atlas home directory (`0600`, optionally
