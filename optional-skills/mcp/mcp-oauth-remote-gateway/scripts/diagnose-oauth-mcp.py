...
Usage:
  ...
  <server_name> matches the files in $ATLAS_HOME/mcp-tokens/<server>.json etc.
...
def _atlas_home():
    # Prefer Atlas' own resolver (profile-safe); fall back to env then ~/.atlas.
    try:
...
def _tokens_dir():
    return os.path.join(_atlas_home(), "mcp-tokens")
    ...
    status, hdrs, body = _post(
        ...
        data={
            ...
            "params": {"protocolVersion": "2025-06-18", "capabilities": {},
                       "clientInfo": {"name": "atlas-diag", "version": "1.0"}},
        },
        headers={
