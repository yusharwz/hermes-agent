...
Usage (via cron with --no-agent):
    ...
    atlas cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$ATLAS_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Atlas-Watcher/1.0"})
