...
$ATLAS_OSINT_CACHE/icij/ (default: ~/.cache/atlas-osint/icij/) and search
...
def _cache_dir() -> Path:
    base = os.environ.get("ATLAS_OSINT_CACHE")
    if base:
        return Path(base) / "icij"
    return Path.home() / ".cache" / "atlas-osint" / "icij"
...
def _download(dest: Path, force: bool = False) -> Path:
    ...
    req = urllib.request.Request(
        BULK_URL,
        headers={"User-Agent": "atlas-agent osint-investigation skill"},
    )
...
def main() -> int:
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help="Override cache directory (default: $ATLAS_OSINT_CACHE/icij or ~/.cache/atlas-osint/icij)",
    )
    p.add_argument(
        "--force-refresh",
        action="store_true",
