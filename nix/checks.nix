    ...
    in {
      ...
      checks = {
        ...
        bundled-skills = pkgs.runCommand "atlas-bundled-skills" { } ''
          ...
          SKILL_COUNT=$(find -L ${atlas-agent}/share/atlas-agent/skills -name "SKILL.md" | wc -l)
          ...
          test -d ${atlas-agent}/share/atlas-agent/optional-skills || \
            (echo "FAIL: optional-skills directory missing"; exit 1)
          OPT_COUNT=$(find -L ${atlas-agent}/share/atlas-agent/optional-skills -name "SKILL.md" | wc -l)
          test "$OPT_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files in optional-skills"; exit 1)
          grep -q "ATLAS_OPTIONAL_SKILLS" ${atlas-agent}/bin/atlas || \
            (echo "FAIL: ATLAS_OPTIONAL_SKILLS not in wrapper"; exit 1)
          echo "PASS: $OPT_COUNT optional skills found, ATLAS_OPTIONAL_SKILLS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "atlas-bundled-locales" { } ''
          ...
          LOC_COUNT=$(find -L ${atlas-agent}/share/atlas-agent/locales -name "*.yaml" | wc -l)
          ...
          # symlink + ATLAS_BUNDLED_LOCALES (not via wheel data-files).
        ...
        # ATLAS_OPTIONAL_MCPS (not via wheel data-files).
        bundled-mcps = pkgs.runCommand "atlas-bundled-mcps" { } ''
          set -e
          echo "=== Checking bundled optional-mcps ==="
          test -d ${atlas-agent}/share/atlas-agent/optional-mcps || (echo "FAIL: optional-mcps directory missing"; exit 1)
          echo "PASS: optional-mcps directory exists"

          MANIFEST_COUNT=$(find -L ${atlas-agent}/share/atlas-agent/optional-mcps -name "manifest.yaml" | wc -l)
          test "$MANIFEST_COUNT" -gt 0 || (echo "FAIL: no manifest.yaml files found"; exit 1)
          echo "PASS: $MANIFEST_COUNT catalog manifests found"

          grep -q "ATLAS_OPTIONAL_MCPS" ${atlas-agent}/bin/atlas || \
            (echo "FAIL: ATLAS_OPTIONAL_MCPS not in wrapper"; exit 1)
          echo "PASS: ATLAS_OPTIONAL_MCPS set in wrapper"

          export HOME=$(mktemp -d)
          CATALOG=$(cd "$HOME" && ${atlas-agent}/bin/atlas mcp catalog 2>/dev/null || true)
          echo "catalog output: $CATALOG"
          test -n "$CATALOG" || (echo "FAIL: atlas mcp catalog returned empty"; exit 1)
