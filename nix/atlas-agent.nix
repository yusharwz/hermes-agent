...
let
  ...
  mkAtlasVenv =
    ...
    callPackage ./python.nix {
      ...
      pythonSrc = atlasNpmLib.pythonSrc;
      dependency-groups = [ "all" ] ++ extraDependencyGroups;
    };

  atlasVenv = (mkAtlasVenv extraDependencyGroups).venv;
  ...
  # lib.nix) — the wrapper exposes them via ATLAS_OPTIONAL_SKILLS, the
  ...
  # exposed via ATLAS_OPTIONAL_MCPS.
...
stdenv.mkDerivation (finalAttrs: {
  ...
  installPhase = ''
    ...
    ln -s ${bundledSkills} $out/share/atlas-agent/skills
    ln -s ${bundledOptionalSkills} $out/share/atlas-agent/optional-skills
    ln -s ${bundledPlugins} $out/share/atlas-agent/plugins
    ln -s ${bundledLocales} $out/share/atlas-agent/locales
    ln -s ${bundledOptionalMcps} $out/share/atlas-agent/optional-mcps
    ln -s ${atlasWeb} $out/share/atlas-agent/web_dist
    ln -s ${atlasTui}/lib/atlas-tui $out/ui-tui
    ...
    ${lib.concatMapStringsSep "\n"
      (name: ''
        makeWrapper ${atlasVenv}/bin/${name} $out/bin/${name} \
          ...
          --set ATLAS_OPTIONAL_SKILLS $out/share/atlas-agent/optional-skills \
          --set ATLAS_BUNDLED_PLUGINS $out/share/atlas-agent/plugins \
          --set ATLAS_BUNDLED_LOCALES $out/share/atlas-agent/locales \
          --set ATLAS_OPTIONAL_MCPS $out/share/atlas-agent/optional-mcps \
  ...
  passthru =
    let
      devPython = (mkAtlasVenv (extraDependencyGroups ++ [ "dev" ])).editableVenv;
    in
    {
      inherit
        atlasTui
      ...
      devShellHook = ''
        export ATLAS_PYTHON=${devPython}/bin/python3
      '';
