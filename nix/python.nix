...
let
    ...
    (callPackage pyproject-nix.build.packages {
      ...
      (
        lib.composeManyExtensions [
          ...
          # Atlas derivation. This is deliberately a derivation environment
          # variable, not a devShell variable: ``nix develop -c uv build``
          # must remain blocked.
          (final: prev: {
            atlas-agent = prev.atlas-agent.overrideAttrs (_old: {
              ATLAS_NIX_BUILD = "1";
            });
          })
        ]
  ...
  # $ATLAS_PYTHON_SRC_ROOT at runtime.
  workspaceRoot = ./..;
  ...
  editableOverlay = editableWorkspace.mkEditablePyprojectOverlay {
    root = "$ATLAS_PYTHON_SRC_ROOT"; # resolved at shellHook time
  };
  ...
  editableSet = pythonSet.overrideScope (
    lib.composeManyExtensions [
      editableOverlay
      (final: prev: {
        atlas-agent = prev.atlas-agent.overrideAttrs (old: {
...
in
{
  venv = pythonSet.mkVirtualEnv "atlas-agent-env" {
    atlas-agent = dependency-groups;
  };
  editableVenv = editableSet.mkVirtualEnv "atlas-agent-editable-env" {
    atlas-agent = dependency-groups;
  };
}
