...
from atlas_cli.auth import AuthError, _read_codex_tokens, resolve_codex_runtime_credentials
...
def _resolve_codex_usage_credentials(
    ...
    state, then the credential pool.  Atlas's native OAuth setup now stores
    ...
    *,
    ...
    try:
        ...
        return CodexResetRedeemResult(
            status="unavailable",
            message="No Codex credentials available. Run `atlas auth` to sign in with your ChatGPT account.",
        )
        ...
        if code in (401, 403):
            return CodexResetRedeemResult(
                ...
                message=(
                    ...
                    "run `atlas auth` and sign in with your ChatGPT account."
                ),
            )
        return CodexResetRedeemResult(
            status="unavailable",
    ...
    if code == "reset":
        ...
        # persisted pool cooldowns so Atlas doesn't keep the credential
        # frozen behind the now-stale ``last_error_reset_at`` (issue #43747).
        try:
            from atlas_cli.auth import clear_codex_pool_quota_cooldowns
