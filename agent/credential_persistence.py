...
``auth.json``.  They intentionally have no dependency on ``atlas_cli.auth`` so
...
# Sources Atlas owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Atlas-owned OAuth/device-code state)
