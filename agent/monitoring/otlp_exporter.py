...
configured under ``monitoring.export.otlp``. Lets an operator stream Atlas
...
Notes:
  ...
  * ``opentelemetry-sdk`` + ``opentelemetry-exporter-otlp-proto-http`` are an
    optional extra (``pip install atlas-agent[otlp]``), imported lazily so the
...
def _span_attrs(ev: Dict[str, Any]) -> Dict[str, Any]:
    ...
    kind = ev.get("event")
    attrs: Dict[str, Any] = {"atlas.event": kind or "unknown"}
    keep_by_kind = {
...
def export_batch(provider, batch: List[Dict[str, Any]]) -> int:
    ...
    tracer = provider.get_tracer("atlas.monitoring")
    n = 0
    for ev in batch:
        try:
            name = f"atlas.{ev.get('event', 'event')}"
    ...
    *,
    ...
    try:
        ...
        logger.warning("monitoring.export.otlp.enabled but the OTel SDK could not "
                       "be installed/imported; install 'atlas-agent[otlp]'")
        return None
    from agent.monitoring.emitter import get_emitter
