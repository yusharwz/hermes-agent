...
def _record_codex_app_server_compaction(
    ...
    *,
    ...
    """Record a Codex-native context compaction boundary in Atlas state.

    The app-server owns the compacted thread context, so Atlas should not
    ...
    if compressor is not None:
        ...
        if callable(record_boundary):
            # Codex owns this summary. A prior Atlas deterministic-fallback
...
# ---------------------------------------------------------------------------
# Codex app-server → Atlas UI bridge (#33200)
#
# The codex_app_server runtime hands the entire turn to a subprocess and
# bypasses the normal Atlas tool loop. Without this bridge gateway
...
# Codex item types that map to a Atlas tool_call in the projector (and
...
# Internal MCP server that wraps Atlas' native tools for codex. When
# codex calls back through it, the inner dispatch runs in a SEPARATE
# atlas-tools-mcp-server subprocess that has no access to the parent
...
# the display event for those calls; we strip the mcp.atlas-tools.*
# namespacing and emit the bare tool name (web_search, browser_navigate,
# vision_analyze, ...) since the user thinks of these as Atlas tools,
# not as MCP calls.
_INTERNAL_MCP_SERVER = "atlas-tools"


def _codex_item_to_tool_name(item: dict) -> str:
    """Synthetic Atlas tool name for a codex item. Mirrors
...
def _codex_item_to_preview(item: dict) -> Any:
    ...
    None when no useful preview is available (Atlas' UI tolerates None)."""
    item_type = item.get("type") or ""
    if item_type == "commandExecution":
...
def make_codex_app_server_event_bridge(agent) -> Callable[[dict], None]:
    ...
    notifications into Atlas' gateway UI callbacks.
...
def run_codex_app_server_turn(
    ...
    *,
    ...
    if not hasattr(agent, "_codex_session") or agent._codex_session is None:
        ...
        # item/agentMessage/delta, ...) into Atlas' gateway UI callbacks
