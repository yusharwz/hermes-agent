...
# un-throttled log fires on *every* selection. On Windows several Atlas
...
# readiness handshake ("Timed out connecting to Atlas backend after
...
class CredentialPool:
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        # sync→POST→write-back sequence below must run atomically across Atlas
    ...
    def _single_use_refresh_lock_timeout(self) -> float:
        ...
        resolves.  Reads the provider's ``ATLAS_*_REFRESH_TIMEOUT_SECONDS``
        override.
        """
        env_var = (
            "ATLAS_CODEX_REFRESH_TIMEOUT_SECONDS"
            if self.provider == "openai-codex"
            else "ATLAS_XAI_REFRESH_TIMEOUT_SECONDS"
        )
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    def _get_env_prefer_dotenv(key: str) -> str:
        ...
        # load_atlas_dotenv() -> apply_onepassword_secrets()).  The raw
