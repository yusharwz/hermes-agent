...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Atlas with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.atlas/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Atlas
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``atlas tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``atlas setup`` for
