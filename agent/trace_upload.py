"""Upload a Atlas session transcript to Hugging Face as an agent trace.

Atlas stores sessions in its own SQLite store (``atlas_state.SessionDB``),
...
* **Zero LLM turn.** This is a deterministic export — it never spends a
  model call. The ``atlas trace upload`` subcommand calls
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through Atlas' secret redactor (``force=True``) unless the
...
DEFAULT_DATASET_NAME = "atlas-traces"
_ATLAS_VERSION = "atlas-agent"
...
# ---------------------------------------------------------------------------
# Conversion: Atlas OpenAI-format messages -> Claude Code JSONL
# ---------------------------------------------------------------------------
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    Non-strings pass through untouched. Uses Atlas' shared redactor with
...
def build_trace_jsonl(
    ...
    *,
    ...
    """Render Atlas conversation messages as Claude Code JSONL text.
...
_NO_TOKEN_MESSAGE = (
    ...
    "2. Add it to your environment as HF_TOKEN (e.g. in ~/.atlas/.env):\n"
    "     HF_TOKEN=hf_xxxxxxxxxxxxxxxxxxxx\n"
    "3. Run /upload-trace again (or `atlas trace upload`)."
)
...
def upload_session_trace(
    ...
    *,
    ...
    the user's private ``{user}/atlas-traces`` dataset. Returns a
