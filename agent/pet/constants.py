...
row taxonomy is inferred from the atlas shape so Atlas can render both legacy
...
# rows (1536x1872), while older Atlas/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Atlas' activity state names. They are not always identical to the
...
# Legacy Atlas/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Atlas activity names -> accepted row-name aliases in descending
