...
def _lock_api_is_absent_on_session_db(lock_db: Any) -> bool:
    ...
    imported ``atlas_state.SessionDB`` class (and its live instances) is old.
...
def _adopt_live_compression_child(
    ...
    try:
        ...
        os.environ["ATLAS_SESSION_ID"] = child_session_id
    try:
        ...
        if aux_context < threshold:
            ...
            if threshold_suggestion_viable:
                ...
                msg += (
                    ...
                    f"Atlas's small-context floor and output reservation "
...
def compress_context(
    ...
    *,
    ...
    # Atlas' summarizer would only rewrite a local mirror without shrinking
    # the actual thread (#36801). Route compaction to the app server's own
    # thread/compact mechanism. Behavior is controlled by
    # ``compression.codex_app_server_auto`` (native|atlas|off).
    # The memory-provider context handoff below is intentionally Atlas-only:
        ...
        if agent._session_db:
            ...
            try:
                ...
                if in_place:
                    ...
                    # whose thread lacks the ATLAS_HOME context.
                    try:
                    ...
                    agent._session_db.publish_compression_child(
                        ...
                        or os.environ.get("ATLAS_SESSION_SOURCE", "cli"),
                        model=agent.model,
...
def _compress_context_via_codex_app_server(
    ...
    *,
    ...
    Atlas' normal compressor rewrites the local OpenAI-style transcript.
    That does not shrink the actual Codex app-server thread context. For this
    runtime, ask Codex to compact its own thread and keep Atlas' transcript
    unchanged.
    """
    auto_mode = str(
        getattr(agent, "codex_app_server_auto_compaction", "native") or "native"
    ).lower()
    if auto_mode not in {"native", "atlas", "off"}:
        auto_mode = "native"
    if not force and auto_mode != "atlas":
        logger.info(
