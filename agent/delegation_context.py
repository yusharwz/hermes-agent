...
The parent Atlas process may itself be a Kanban dispatcher worker with
ATLAS_KANBAN_* variables in process env. delegate_task children run inside the
...
DELEGATED_CHILD_ENV_MARKER = "ATLAS_DELEGATED_CHILD_CONTEXT"
...
def delegated_child_subprocess_env(
    ...
    parent dispatcher ``ATLAS_KANBAN_*`` vars while losing the ContextVar in
