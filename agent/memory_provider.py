...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `atlas backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - atlas_home (str): The active ATLAS_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.atlas``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "atlas").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'atlas memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], atlas_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'atlas memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``atlas_home`` is the active ATLAS_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE ATLAS_HOME.

        ``atlas backup`` only walks ATLAS_HOME, so any provider state kept
        ...
        of the archive, and ``atlas import`` restores them to their original
