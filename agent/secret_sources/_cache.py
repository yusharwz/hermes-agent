...
cache problem must never block Atlas startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """Resolve the Atlas home used for cache paths.

    ``home_path`` is whatever ``load_atlas_dotenv()`` already resolved;
    falling back to ``$ATLAS_HOME`` / ``~/.atlas`` keeps direct callers
    (and tests that don't thread a home through) working.
    """
    if home_path is None:
        from atlas_constants import get_atlas_home

        home_path = get_atlas_home()
    return home_path
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend lives at ``<atlas_home>/cache/<basename>``::
    ...
    def write(
        ...
        try:
            ...
            # metadata isn't exposed if ATLAS_HOME is ever made traversable.
            try:
