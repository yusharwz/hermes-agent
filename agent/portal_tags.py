...
Every Atlas request that hits the Nous Portal — main agent loop, auxiliary
...
def conversation_tag(session_id: str) -> str:
    """Return the ``conversation=...`` tag for a Atlas session/conversation.

    Format: ``conversation=<session_id>``. ``session_id`` is the canonical
    Atlas conversation identifier (``AIAgent.session_id``) — the same value
    used for ``~/.atlas/sessions/`` storage, session logs, and lineage.
...
def nous_portal_tags(session_id: str | None = None) -> List[str]:
    ...
    appended so Portal usage can be attributed to a specific Atlas
    ...
    """
    tags = ["product=atlas-agent", atlas_client_tag()]
