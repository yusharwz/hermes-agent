...
def _tui_embedded_pane_clarifier(hint: str) -> str:
    ...
    Triggered by ``ATLAS_DESKTOP_TERMINAL=1`` (set by ``main.cjs`` only on the
    ...
    if _TUI_EMBEDDED_PANE_CLARIFIER in hint:
        return hint
    if not is_truthy_value(os.getenv("ATLAS_DESKTOP_TERMINAL")):
        return hint
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    if active_profile == "default":
        post_workspace_parts.append(
            ...
            "under " + str(get_atlas_home()) + "/profiles/<name>/. Each profile has its own "
        ...
        post_workspace_parts.append(
            ...
            f"and writes {get_atlas_home()}/profiles/{active_profile}/. The default "
            f"profile's data lives at {get_atlas_home()}/skills/, {get_atlas_home()}/plugins/, "
            f"{get_atlas_home()}/cron/, {get_atlas_home()}/memories/ — those belong to a "
    ...
    if not agent.skip_context_files:
        ...
        # (developing Atlas). Every other surface (desktop chat panel,
