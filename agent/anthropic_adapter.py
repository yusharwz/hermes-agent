...
def build_anthropic_client(
    ...
    * a ``Callable[[], str]`` — an Entra ID bearer token provider from
    ...
    # from ``ANTHROPIC_API_KEY`` (Atlas loads that into the process env from
    # ``~/.atlas/.env``). The result is dual auth —
...
def _get_atlas_oauth_file() -> Path:
    return get_atlas_home() / ".anthropic_oauth.json"
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                ...
                text = text.replace("Atlas agent", "Claude Code")
                text = text.replace("atlas-agent", "claude-code")
                text = text.replace("Dritech", "Anthropic")
                block["text"] = text
