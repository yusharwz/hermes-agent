...
///
/// The marker is also the cross-process update lock: `atlas update` claims
/// the same file (see `atlas_cli/update_lock.py`) so a dashboard-spawned
...
/// overwriting — the pre-fix clobber is what let a dashboard `atlas update`
...
/// UPDATE_MARKER_MAX_AGE_SECONDS in atlas_cli/update_lock.py — all three read
...
/// owner that is itself and aborts ("Another Atlas update is already
/// running"), then the desktop relaunches and retries forever. A foreign live
/// pid (e.g. a dashboard-spawned `atlas update`) still blocks.
fn live_marker_owner(path: &Path) -> Option<MarkerOwner> {
    ...
    // Desktop `writeUpdateMarker(atlasHome, child.pid)` races ahead of us;
...
async fn run_update(app: AppHandle) -> Result<()> {
    ...
    //
    // The same marker is the cross-process update lock (atlas_cli/
    // update_lock.py claims it too), so a live foreign owner means another
    // updater — most often a dashboard-spawned `atlas update` — is already
    ...
    let _update_marker = match UpdateMarkerGuard::acquire(
        ...
        Err(owner) => {
            ...
            let msg = format!(
                "Another Atlas update is already running (PID {}, started {} ago). \
...
fn update_child_env(install_root: &Path) -> Vec<(String, OsString)> {
    ...
    // `atlas update` is a Python CLI writing to a pipe here, so CPython
    ...
    // `atlas update` child claims that SAME lock (atlas_cli/update_lock.py).
    ...
    // refuses its parent's marker with exit 2 ("Atlas is still running")
    // and no number of retries can ever succeed. Keep the variable name in
    // sync with HANDOFF_PID_ENV in atlas_cli/update_lock.py.
    envs.push((
        "ATLAS_UPDATE_HANDOFF_PID".to_string(),
...
mod tests {
    ...
    fn update_child_env_forces_unbuffered_python() {
        let envs = update_child_env(Path::new("/x/atlas-agent"));
        assert!(
            envs.iter()
    ...
    fn update_child_env_names_our_pid_for_the_lock_handoff() {
        let envs = update_child_env(Path::new("/x/atlas-agent"));
        assert!(
            envs.iter().any(|(k, v)| k == "ATLAS_UPDATE_HANDOFF_PID"
                && v.to_str() == Some(std::process::id().to_string().as_str())),
            "the atlas update child claims the same marker we hold; without our pid \
    ...
    fn acquire_refuses_while_a_live_updater_owns_the_marker() {
        ...
        // dashboard `atlas update` and install-mode bootstrap mutate one tree
    ...
    fn acquire_adopts_a_marker_prewritten_with_our_own_pid() {
        // #74761: desktop writeUpdateMarker(atlasHome, child.pid) races ahead
