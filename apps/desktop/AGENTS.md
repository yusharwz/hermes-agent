...
How to build Atlas Desktop well. This is a judgment guide, not an inventory —
...
- The **backend** is authoritative for anything another Atlas surface can also
...
- A **runtime home change** (switching the underlying `ATLAS_HOME` profile) is
...
its shape. "Plugin" means several unrelated things across Atlas — do not assume
