...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'atlas:saveImageFromUrl'")
}
...
export function useImageDownload(src?: string) {
  ...
  const download = useCallback(async () => {
    ...
    try {
      if (window.atlasDesktop?.saveImageFromUrl) {
        if (await window.atlasDesktop.saveImageFromUrl(src)) {
