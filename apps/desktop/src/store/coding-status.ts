...
const EMPTY_WORKTREES: AtlasGitWorktree[] = []
...
export const $repoStatusByCwd = atom<Record<string, AtlasRepoStatus | null>>({})
export const $repoWorktreesByCwd = atom<Record<string, AtlasGitWorktree[]>>({})
...
export const $repoStatus: ReadableAtom<AtlasRepoStatus | null> = computed(
...
export const $repoWorktrees: ReadableAtom<AtlasGitWorktree[]> = computed(
...
const statusAtomByCwd = new Map<string, ReadableAtom<AtlasRepoStatus | null>>()
const worktreesAtomByCwd = new Map<string, ReadableAtom<AtlasGitWorktree[]>>()
const $noRepoStatus = atom<AtlasRepoStatus | null>(null)
const $noWorktrees = atom<AtlasGitWorktree[]>(EMPTY_WORKTREES)

/** Reactive status for one repo cwd (a tile's worktree). Stable per cwd. */
export function repoStatusForCwd(cwd?: null | string): ReadableAtom<AtlasRepoStatus | null> {
...
export function repoWorktreesForCwd(cwd?: null | string): ReadableAtom<AtlasGitWorktree[]> {
  const key = normalizeCwd(cwd)
...
function setRepoStatusEntry(target: string, status: AtlasRepoStatus | null): void {
...
function setRepoWorktreesEntry(target: string, worktrees: AtlasGitWorktree[]): void {
...
interface RepoStatusRefreshRequest {
  probe: (cwd: string) => Promise<AtlasRepoStatus | null>
  seq: number
  target: string
}
