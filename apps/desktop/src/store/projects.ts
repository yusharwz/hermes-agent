...
import type { AtlasGitBaseBranch, AtlasGitBranch } from '@/global'
import { getAtlasConfig, type AtlasGateway } from '@/atlas'
...
async function activeProjectsContext(): Promise<ActiveProjectsContext> {
  ...
  if (!gateway || gateway !== activeGateway() || profile !== ($activeGatewayProfile.get() || 'default')) {
    throw new Error('Active Atlas profile changed while connecting')
  }
...
async function refreshProjectTreeOn(gateway: AtlasGateway): Promise<void> {
...
const repoScanStates = new WeakMap<AtlasGateway, RepoScanState>()
const scanningGatewayGenerations = new WeakMap<AtlasGateway, number>()
...
export async function scanAndRecordRepos(force = false): Promise<void> {
  ...
  try {
    const policy = repoDiscoveryPolicyFromConfig(await getAtlasConfig(context.profile))
...
export async function listBaseBranches(repoPath: string): Promise<AtlasGitBaseBranch[]> {
  const git = desktopGit()
