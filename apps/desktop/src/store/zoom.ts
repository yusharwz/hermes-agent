...
export function setZoomPercent(percent: number): void {
  window.atlasDesktop?.zoom?.setPercent(percent)
}

if (typeof window !== 'undefined' && window.atlasDesktop?.zoom) {
  void window.atlasDesktop.zoom.get().then(({ percent }) => $zoomPercent.set(percent))
  window.atlasDesktop.zoom.onChanged(({ percent }) => $zoomPercent.set(percent))
}
