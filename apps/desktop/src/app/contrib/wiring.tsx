...
import { getSessionMessages, triggerCronJob } from '@/atlas'
...
export function ContribWiring({ children }: { children: ReactNode }) {
  ...
  const { refreshAtlasConfig, sttEnabled, voiceMaxRecordingSeconds } = useAtlasConfig({ activeSessionIdRef })
  ...
  useEffect(() => {
    ...
    window.addEventListener('atlas:open-keybinds', onOpenKeybinds)

    return () => window.removeEventListener('atlas:open-keybinds', onOpenKeybinds)
  }, [navigate])
  ...
  useEffect(() => {
    ...
    void refreshAtlasConfig(true)
    void refreshActiveProfile()
  }, [activeGatewayProfile, refreshCurrentModel, refreshAtlasConfig])
