...
beforeEach(() => {
  ...
  readDir.mockReset()
  ;(window as unknown as { atlasDesktop: { readDir: typeof readDir } }).atlasDesktop = { readDir }
})
...
function ok(entries: { name: string; path: string; isDirectory: boolean }[]): AtlasReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { atlasDesktop: unknown }).atlasDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: AtlasReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<AtlasReadDirResult>(resolve => {
  ...
  it('falls back to the sanitized workspace dir when the session cwd is gone', async () => {
    ...
    ;(window as unknown as { atlasDesktop: unknown }).atlasDesktop = { readDir, sanitizeWorkspaceCwd }
  ...
  it('returns no-bridge gracefully when window.atlasDesktop is missing', async () => {
    delete (window as unknown as { atlasDesktop?: unknown }).atlasDesktop
