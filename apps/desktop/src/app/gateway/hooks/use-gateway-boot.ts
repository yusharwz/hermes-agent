  ...
  useEffect(() => {
    ...
    const softSwitch = async () => {
      ...
      try {
        ...
        await Promise.all([
          seedDefaultCwd(),
          callbacksRef.current.refreshAtlasConfig().catch(() => undefined),
    ...
    const offBootProgress = desktop.onBootProgress(payload => {
      // Soft switch / post-boot startAtlas re-emits progress — ignore so the
    ...
    const gateway = adoptedFromHmr ? survivor!.gateway : new AtlasGateway()
    ...
    async function boot() {
      try {
        ...
        await Promise.all([
          seedDefaultCwd(),
          callbacksRef.current.refreshAtlasConfig(),
          ...
          // retry it), not brick boot behind the "Atlas couldn't start"
