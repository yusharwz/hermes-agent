...
function reveal(file: string) {
  void window.atlasDesktop?.revealPath?.(file)?.catch(() => undefined)
}

async function revealPluginsDir() {
  try {
    // Electron owns the local plugin root — deriving it from the backend's
    // atlas_home breaks against a remote backend (#66899).
    const dir = await window.atlasDesktop?.desktopPluginsRoot?.()
    ...
    const result = await window.atlasDesktop?.openDir?.(dir)
