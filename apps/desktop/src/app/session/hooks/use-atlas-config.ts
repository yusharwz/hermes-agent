...
export function useAtlasConfig({ activeSessionIdRef }: AtlasConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)
  const profileRefreshEpochRef = useRef(0)

  const refreshAtlasConfig = useCallback(
    async (force = false) => {
      if (force) {
      ...
      try {
        ...
        // rendering/applying Atlas' built-in medium over the user's config.
