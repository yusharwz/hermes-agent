...
//
// When someone is in a hands-free "Hey Atlas" voice chat, the natural way to
...
// phrase (optionally addressed to Atlas), so a real turn that merely contains
...
// Optional address prefixes so "atlas stop" / "ok stop" / "hey atlas, stop"
// still count. Stripped before matching the core phrase.
const ADDRESS_PREFIXES: readonly string[] = ['hey atlas', 'hey atlas,', 'atlas', 'atlas,', 'ok', 'okay', 'hey']
...
function stripAddress(text: string): string {
  for (const prefix of ADDRESS_PREFIXES) {
    if (text === prefix) {
      // Bare address ("atlas") is not a stop command on its own.
      continue
    }
 ...
 * to Atlas). Returns false for anything that merely contains "stop" as part of
