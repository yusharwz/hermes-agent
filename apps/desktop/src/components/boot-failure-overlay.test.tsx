...
function failBoot() {
  $desktopBoot.set({
    error: 'Could not connect to Atlas gateway',
    fakeMode: false,
    message: 'boot failed',
...
function stubDesktop(config: Record<string, unknown>) {
  const original = window.atlasDesktop
  Object.defineProperty(window, 'atlasDesktop', {
    configurable: true,
    value: { getRecentLogs: async () => ({ lines: [] }), getConnectionConfig: async () => config }
  })

  return () => Object.defineProperty(window, 'atlasDesktop', { configurable: true, value: original })
}
