...
describe('atlasDirectiveFormatter.parse', () => {
  ...
  it('parses session links with profile/id values', () => {
    const segments = atlasDirectiveFormatter.parse('see @session:work/20260101_abc123 next')
...
describe('inline skill references', () => {
  const skills = (text: string) =>
    [...atlasDirectiveFormatter.parse(text)]
  ...
  it('keeps the surrounding prose as text around the chip', () => {
    const segments = atlasDirectiveFormatter.parse('tidy this with /clean thanks')
  ...
  it('parses a skill chip alongside an @ reference', () => {
    const mentions = [...atlasDirectiveFormatter.parse('run /clean on @file:`src/a.ts`')].filter(
