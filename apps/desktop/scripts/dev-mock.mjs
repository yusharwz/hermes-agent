...
function createSandbox() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `atlas-dev-mock-${Date.now()}`))
  ...
  return { root, atlasHome, userDataDir, cleanup: () => fs.rmSync(root, { recursive: true, force: true }) }
}

function writeMockConfig(atlasHome, mockUrl) {
  fs.writeFileSync(
    path.join(atlasHome, 'config.yaml'),
...
`,
  ...
  fs.writeFileSync(path.join(atlasHome, '.env'), 'MOCK_API_KEY=e2e-mock-key\n', 'utf8')
}
...
async function main() {
  ...
  writeMockConfig(sandbox.atlasHome, mock.url)
  console.log(`  ATLAS_HOME: ${sandbox.atlasHome}`)
  ...
  const env = {
    ...
    ATLAS_DESKTOP_APP_NAME: `AtlasDevMock-${Date.now()}`,
  }
