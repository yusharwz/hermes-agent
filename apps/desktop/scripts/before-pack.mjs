 ...
 * Atlas.exe that Windows can't load (truncated PE from a corrupt cached
 * Electron zip, wrong arch), the updater's integrity gate in
 * `atlas desktop --build-only` (atlas_cli/main.py
 ...
 */
export function preserveRollbackBackup(appOutDir, productExeName = 'Atlas.exe') {
...
export default async function beforePack(context) {
  ...
  try {
    ...
    // rename fails.
    const productExe = `${(context && context.packager?.appInfo?.productFilename) || 'Atlas'}.exe`
