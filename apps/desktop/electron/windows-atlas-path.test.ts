// Unit tests for the pure Windows `atlas` resolution helpers extracted from
// main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
// unwrapWindowsVenvAtlasCommand(). These pin the two Windows resolution bugs
// that caused desktop reinstall loops:
//   1. buildPathExtCandidates() — PATHEXT extensions must be tried BEFORE the
//      empty extension, or an extensionless Git-Bash `atlas` shim shadows
//      the real atlas.cmd/atlas.exe.
//   2. chooseUpdaterArgs() — must gate on haveRealInstall (any real-install
//      signal), not just the atlas.exe console-script shim, or healthy
//      installs get forced into a destructive --repair.
//   3. resolveVenvAtlasCommand() — must probe the venv python via
//      canImportAtlasCli() before trusting it, or a broken venv gets
...
function makeDeps(overrides: Partial<Parameters<typeof resolveVenvAtlasCommand>[2]> = {}) {
  return {
    isWindows: true,
    isCommandScript: () => false,
    fileExists: () => true,
    directoryExists: () => false,
    canImportAtlasCli: () => true,
    getVenvPython: (venvRoot: string) => `${venvRoot}/Scripts/python.exe`,
    getVenvSitePackagesEntries: () => [],
    buildDesktopBackendEnv: () => ({ FAKE_ENV: '1' }),
    atlasHome: '/fake/atlas-home',
...
test('resolveVenvAtlasCommand: returns null off Windows', () => {
  const deps = makeDeps({ isWindows: false })

  assert.equal(resolveVenvAtlasCommand('/root/venv/Scripts/atlas.exe', [], deps), null)
})

test('resolveVenvAtlasCommand: returns null for a .cmd/.bat script command', () => {
  const deps = makeDeps({ isCommandScript: () => true })

  assert.equal(resolveVenvAtlasCommand('/root/venv/Scripts/atlas.cmd', [], deps), null)
})

test('resolveVenvAtlasCommand: returns null when the basename is not atlas/atlas.exe', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvAtlasCommand('/root/venv/Scripts/python.exe', [], deps), null)
})

test('resolveVenvAtlasCommand: returns null when the parent dir is not Scripts', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvAtlasCommand('/root/venv/bin/atlas.exe', [], deps), null)
})

test('resolveVenvAtlasCommand: returns null when the venv python does not exist on disk', () => {
  const deps = makeDeps({ fileExists: () => false })

  assert.equal(resolveVenvAtlasCommand('/root/venv/Scripts/atlas.exe', [], deps), null)
})

test('resolveVenvAtlasCommand: probes the venv python before trusting it (returns null on failed probe)', () => {
  let probed = false

  const deps = makeDeps({
    canImportAtlasCli: (python: string) => {
      probed = true
  ...
  const result = resolveVenvAtlasCommand('/root/venv/Scripts/atlas.exe', ['serve'], deps)
...
test('resolveVenvAtlasCommand: returns the resolved python backend descriptor when the probe passes', () => {
  const deps = makeDeps()
  const result = resolveVenvAtlasCommand('/root/venv/Scripts/atlas.exe', ['serve', '--port', '0'], deps)
  ...
  assert.deepEqual(result.args, ['-m', 'atlas_cli.main', 'serve', '--port', '0'])
...
test('resolveVenvAtlasCommand: is case-insensitive on atlas.exe and the Scripts dir name', () => {
  const deps = makeDeps()

  assert.ok(resolveVenvAtlasCommand('/root/venv/Scripts/ATLAS.EXE', [], deps))
  assert.ok(resolveVenvAtlasCommand('/root/venv/SCRIPTS/atlas.exe', [], deps))
})
