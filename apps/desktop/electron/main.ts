...
import { buildDesktopBackendEnv, normalizeAtlasHomeRoot } from './backend-env'
import { isReauthRequiredError, waitForAtlasReady } from './backend-health'
import {
  canImportAtlasCli,
  execProbeSync,
  PROBE_TIMEOUT_MS,
...
import {
  ...
  resolveVenvAtlasCommand
} from './windows-atlas-path'
import {
...
const IS_PACKAGED = app.isPackaged || Boolean(process.env.ATLAS_DESKTOP_IS_PACKAGED)
...
if (DEV_CDP.port) {
  ...
  console.log(
    `[atlas] renderer debugging on http://127.0.0.1:${DEV_CDP.port} — anything that can reach it ` +
      'can run code in the renderer. ATLAS_DESKTOP_CDP_PORT=off to disable.'
  )
} else {
  const why = describeDevCdpDecision(DEV_CDP)

  if (why) {
    console.warn(`[atlas] ${why}`)
  }
}
...
// never go through `atlas desktop`; it is version-scoped so an app update
...
if (IS_WINDOWS) {
  ...
  // expose Atlas sessions/config to every packaged app on the machine.
  if (shouldAttemptAclRepair(priorMarker)) {
    const exeDir = path.dirname(process.execPath)
    const acl = grantAllApplicationPackagesAcl(exeDir, { execFileSync })

    if (acl.ok) {
      console.log(`[atlas] granted ALL APPLICATION PACKAGES RX on ${exeDir} (#38216)`)
    } else if (acl.error && acl.error !== 'missing-target-or-exec') {
      console.warn(`[atlas] AppContainer ACL grant failed on ${exeDir}: ${acl.error}`)
    }
  }
  ...
  if (sandboxDecision.enable && sandboxDecision.reason !== 'already-enabled') {
    ...
    console.log(
      `[atlas] Windows sandbox fallback enabled (${sandboxDecision.reason}); launching with --no-sandbox (#38216)`
    )
  }
  ...
  app.on('child-process-gone', (_event, details) => {
    ...
    console.warn(
      `[atlas] Windows GPU sandbox crashed (exit=${details?.exitCode}); relaunching once with --no-sandbox (#38216)`
    )
    ...
    try {
      ...
      console.error(`[atlas] --no-sandbox relaunch failed: ${error?.message || error}`)
    }
  })
}
...
// always-on timers. A completely idle, minimized Atlas burned ~20% CPU
...
function loadInstallStamp() {
  ...
  for (const p of candidates) {
    try {
      ...
      console.warn(`[atlas] install-stamp.json found at ${p} , but parsing failed with ${e}`)
...
function resolveAtlasHome() {
  if (process.env.ATLAS_HOME) {
    return normalizeAtlasHomeRoot(process.env.ATLAS_HOME)
  }

  if (USER_DATA_OVERRIDE) {
    return path.join(path.resolve(USER_DATA_OVERRIDE), 'atlas-home')
  }
  ...
  if (IS_WINDOWS) {
    ...
    if (fromRegistry) {
      return normalizeAtlasHomeRoot(fromRegistry)
    }
  }
...
const BOOT_FAKE_ERROR = process.env.ATLAS_DESKTOP_BOOT_FAKE_ERROR || ''
...
const SKIP_QUIT_CONFIRM = process.env.ATLAS_DESKTOP_SKIP_QUIT_CONFIRM === '1'
...
const APP_NAME = process.env.ATLAS_DESKTOP_APP_NAME || 'Atlas'
...
// electron-builder derives from the build `appId` (id.co.dritech.atlas) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('id.co.dritech.atlas')
}
...
// startAtlas(); this pool only holds EXTRA profile
...
function promptFirstRunSetupChoice(backend) {
  broadcastBootstrapEvent({
    ...
    activeRoot: backend.activeRoot || ACTIVE_ATLAS_ROOT
  })
}
...
function updateGateDeps() {
  return {
    hasLiveMarker: () => Boolean(readLiveUpdateMarker(ATLAS_HOME)),
...
function unwrapWindowsVenvAtlasCommand(command, backendArgs) {
  return resolveVenvAtlasCommand(command, backendArgs, {
...
function backendSupportsServe(backend) {
  ...
  if (supported === null) {
    try {
      ...
      // `atlas --version` (~10.5s measured cold), and a false negative here
      ...
      execProbeSync(backend.command, [...prefix, 'serve', '--help'], {
        ...
        // (see resolveAtlasBackend step 4); execFileSync of a .cmd without
      ...
      try {
        const out = execFileSync(
          ...
          hiddenWindowsChildOptions({
            ...
            // Bare interpreter startup — much lighter than the atlas-import
...
// findGitBash — locate bash.exe on Windows. Resolves ATLAS_GIT_BASH_PATH
...
// Windows that binary owns ALL repo mutation — running `atlas update` +
...
function resolveUpdaterBinary() {
  return resolveStagedUpdaterBinary(ATLAS_HOME, { fileExists, isWindows: IS_WINDOWS })
}
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const pids = []
  const atlasProcess = backendConnectionState.getProcess()

  if (atlasProcess && Number.isInteger(atlasProcess.pid)) {
    pids.push(atlasProcess.pid)
  }
  ...
  while (Date.now() < deadlineMs) {
    ...
    const currentAtlasProcess = backendConnectionState.getProcess()

    if (currentAtlasProcess && Number.isInteger(currentAtlasProcess.pid)) {
      stragglers.push(currentAtlasProcess.pid)
    }
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      // macOS/Linux: never hand off, staged atlas-setup or not — the resolver
      ...
      // `atlas update` (backend) + `atlas desktop --build-only` (OS-aware GUI
    ...
    if (!updater) {
      ...
      try {
        ...
        if (head.code === 0 && current && current !== 'HEAD') {
          ...
          if (branch !== 'main') {
            command = `atlas update --branch ${branch}`
          }
        }
      } catch {
    ...
    preflightStateDb(ATLAS_HOME, rememberLog)
    ...
    // Atlas processes running from this venv.  The updater normally refuses
    ...
    if (Number.isInteger(child.pid)) {
      writeUpdateMarker(ATLAS_HOME, child.pid)
    }
...
// Python-level pre-update snapshot inside `atlas update`.
function preflightStateDb(atlasHome, rememberLog) {
  const stateDbPath = path.join(atlasHome, 'state.db')
  ...
  try {
    ...
    if (stat.size > 100) {
      ...
      const emergencyPath = path.join(atlasHome, `state.db.pre-update-emergency-${ts}.bak`)
      ...
      try {
        ...
        try {
          const homeDir = fs.readdirSync(atlasHome)
          ...
          for (const old of backups.slice(2)) {
            try {
              fs.unlinkSync(path.join(atlasHome, old))
            } catch {
              void 0
            }
          }
...
async function applyUpdatesPosixInApp(opts: any) {
  ...
  // PYTHONUNBUFFERED: `atlas update` writes to a pipe here, so CPython
...
function resolveAtlasBackend(backendArgs) {
  ...
  // 3. ACTIVE_ATLAS_ROOT — the canonical install at
  ...
  if (activeRuntime.shouldUseActiveRuntime && !bootstrapRepairRequested) {
    if (!activeRuntime.hasValidMarker) {
      rememberLog(
        `[bootstrap] Active Atlas runtime at ${ACTIVE_ATLAS_ROOT} is usable but the bootstrap marker is missing or stale; skipping first-run bootstrap.`
      )
    }
  ...
  if (process.env.ATLAS_DESKTOP_IGNORE_EXISTING !== '1') {
    ...
    if (atlasCommand) {
      ...
      // ATLAS_DESKTOP_ATLAS is an explicit deployment override (used by
      ...
      if (shouldTrustAtlasOverride(atlasOverride) || verifyAtlasCli(atlasCommand, { shell: shellForProbe })) {
        ...
        // unwrapWindowsVenvAtlasCommand here: the second call repeats the
...
async function ensureRuntime(backend) {
  ...
  if (backend.kind === 'bootstrap-needed') {
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Atlas install was cancelled.') as any
  ...
  if (!fileExists(venvPython)) {
    ...
    // normal flow because activeRuntimeState() requires isAtlasSourceRoot()
    // plus an importable atlas_cli before it hands back the active runtime.
...
// waitForAtlasReady how to read a 401 — rejected session vs gated route.
...
function sendOpenFolderRequested() {
  ...
  webContents.send('atlas:open-folder-requested')
}
...
// 'atlas:power-battery' push below.
ipcMain.handle('atlas:power-battery:get', () => onBatteryPower === true)
...
function broadcastBatteryState(next: boolean) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('atlas:power-battery', next)
    }
  }
}
...
// Dritech) instead of a static session token. The auth model is
...
// "not signed in", which on the initial boot path (startAtlas → the renderer's
// single-shot boot() with no retry) surfaced as the "Atlas couldn't start"
...
async function hasLiveOauthSession(baseUrl) {
  ...
  // for a signed-in user — the exact race that produced the transient "Atlas
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: silent ? 'Connecting to Atlas Cloud agent…' : 'Sign in to Atlas gateway',
...
// feature; the server half lives in atlas_cli/dashboard_auth/native_flow.py.
// ---------------------------------------------------------------------------
...
// --- Atlas Cloud discovery + silent per-agent sign-in (cloud-auto-discovery
// Phase 3) ---------------------------------------------------------------
//
...
// portal session cookie living in `persist:atlas-remote-oauth`:
...
// convention (atlas_cli/auth.py DEFAULT_NOUS_PORTAL_URL + the same env names)
// so a single override flips every Atlas surface to the same portal.
const DEFAULT_NOUS_PORTAL_URL = 'https://portal.nousresearch.com'

function resolvePortalBaseUrl() {
  const raw = process.env.ATLAS_PORTAL_BASE_URL || process.env.NOUS_PORTAL_BASE_URL || DEFAULT_NOUS_PORTAL_URL
...
// authenticates via PRIVY, not the Atlas gateway session cookies, so this
// checks for the `privy-token` cookie on the portal host (NOT
// hasLiveOauthSession, which looks for atlas_session_at/rt that the portal
...
function openPortalLoginWindow() {
  ...
  return new Promise((resolve, reject) => {
    if (!app.isReady()) {
      reject(new Error('Desktop is not ready to start a Atlas Cloud sign-in.'))
...
// Discover the hosted (Atlas Cloud) agents the signed-in user can see. Calls
...
async function discoverCloudAgents(org?: string) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error(
      'You are not signed in to Atlas Cloud. Open Settings → Gateway, choose Atlas Cloud, and sign in.'
    ) as any
  ...
  try {
    ...
    if (error && error.statusCode === 401) {
      const err = new Error('Your Atlas Cloud session has expired. Open Settings → Gateway and sign in again.') as any
      err.needsCloudLogin = true
      err.cause = error
      throw err
    }
...
async function cloudAgentSilentSignIn(dashboardUrl) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error('Your Atlas Cloud session has expired. Sign in to Atlas Cloud again.') as any
    err.needsCloudLogin = true
    throw err
  }
...
function sanitizeConnectionProfiles(raw: Record<string, any>) {
  ...
  for (const [name, entry] of Object.entries(raw)) {
    ...
    // Preserve the Atlas Cloud org tag on cloud-mode entries so Settings can
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  return {
    ...
    remoteUrl,
    // The persisted Atlas Cloud org (slug/id) for a cloud connection, or '' for
    ...
    sshRemoteAtlasPath: (ssh || savedSsh)?.remoteAtlasPath || '',
...
// `org` (optional) is the Atlas Cloud org slug/id the instance was discovered
...
function coerceDesktopConnectionConfig(input: any = {}, existing = readDesktopConnectionConfig(), options: any = {}) {
  ...
  // describe a discovered Atlas Cloud instance, NOT a user-owned remote gateway,
...
function buildSshBlock(input: any, existingBlock: any = {}) {
  ...
  const merged = normalizeSshConfig({
    ...
    remoteAtlasPath: input.sshRemoteAtlasPath ?? existingBlock.remoteAtlasPath
  })
...
async function buildRemoteConnection(
  ...
  if (authMode === 'oauth') {
    ...
    // refresh token (atlas #37247), and the gateway middleware transparently
    ...
    try {
      ...
      throw gatewayTicketFailure(
        ...
        'Could not reach the remote Atlas gateway while refreshing its WebSocket ticket. Try reconnecting.'
      )
    }
...
async function bootstrapSshConnectionInner(profile, sshConfig, reuseToken, source, fingerprint, lease) {
  ...
  try {
    ...
    result = await lifecycle({
      ...
      pickLocalPort,
      waitForAtlas: (baseUrl, token) => waitForAtlas(baseUrl, token, lease.signal, 'token'),
  ...
  sshRememberLog(
    `[ssh] connection ${result.reused ? 'REUSED' : 'spawned'} dashboard: ` +
      `${result.atlasVersion || 'atlas (version unknown)'} at ${result.atlasPath || '?'}`
  )
  ...
  return { ...connection, remoteAtlasVersion: result.atlasVersion || '' }
}
...
// env → global) so a startAtlas() failure can be classified as remote (never
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  if (authRequired) {
    ...
    // button can read "Sign in with Dritech" instead of a generic
...
async function testDesktopConnectionConfig(input: any = {}) {
  if (input.mode === 'ssh') {
    const sshConfig = normalizeSshConfig({
      ...
      remoteAtlasPath: input.sshRemoteAtlasPath
    })
    ...
    try {
      ...
      for (;;) {
        try {
          await ssh.open()
          const platform: any = await detectRemotePlatform(ssh, sshConfig.remoteAtlasPath || '')
          let atlasPath
          let atlasVersion
          let supported
          ...
          if (platform.os === 'Windows') {
            ...
            atlasPath = runtime.atlasPath
            const inspection = await helper(ssh, runtime, 'inspect', [runtime.atlasPath])
            atlasVersion = inspection.version
            supported = inspection.supported
          } else {
            atlasPath = await remoteLifecycle.locateAtlas(ssh, sshConfig.remoteAtlasPath || '')
            atlasVersion = await remoteLifecycle.probeAtlasVersion(ssh, atlasPath)
            supported = await remoteLifecycle.remoteSupportsSshOwnership(ssh, atlasPath)
          }
          ...
          if (!supported) {
            return {
              ...
              error: 'Update Atlas on the remote host before connecting with Desktop SSH.'
            }
          }
          ...
          return {
            ...
            remoteAtlasPath: atlasPath,
            remoteAtlasVersion: atlasVersion,
...
function resetAtlasConnection({ soft = false } = {}) {
  backendStartFailure = null
  remoteReauthFailure = null
  remoteLiveness.clear()
  const atlasProcess = backendConnectionState.invalidate()
...
async function teardownPrimaryBackendAndWait({ soft = false } = {}) {
  // Capture the reference before resetAtlasConnection() invalidates it.
  const atlasProcess = backendConnectionState.getProcess()
...
function sendConnectionApplied() {
  ...
  webContents.send('atlas:connection:applied')
}
...
async function ensureBackend(profile) {
  ...
  if (route.backend === 'primary') {
    const connection = await startAtlas()
...
async function spawnPoolBackend(profile, entry) {
  ...
  if (remote) {
    await waitForAtlas(remote.baseUrl, remote.token, undefined, remote.authMode)
  ...
  if (!wsProbe.ok) {
    throw new Error(
      `Atlas backend for profile "${profile}" is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
    )
  }
  ...
  const connectionPromise = (async () => {
    const connectRemote = async remote => {
      ...
      return {
        ...
        remoteAtlasVersion: remote.remoteAtlasVersion,
        token: remote.token,
        wsUrl: remote.wsUrl,
        logs: atlasLog.slice(-80),
        ...getWindowState()
    ...
    const setup = await runPrimaryBackendStartup({
      ...
      prepareLocalBackend: async () => {
        ...
        return resolveAtlasBackend(backendArgs)
      },
      resolveRemote: () => {
    ...
    const processOwner = backendConnectionState.attachProcess(connectionAttempt, atlasProcess)

    if (!processOwner) {
      stopBackendChild(atlasProcess)
      throw new Error('Atlas backend start was superseded by a newer connection attempt.')
    }
    ...
    atlasProcess.once('error', error => {
      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Atlas backend error: ${error.message}`)
        rejectBackendStart?.(new Error('Atlas backend start was superseded by a newer connection attempt.'))
    ...
    atlasProcess.once('exit', (code, signal) => {
      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Atlas backend exit (${signal || code})`)
    ...
    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      childAlive: () => atlasProcess.exitCode === null && !atlasProcess.killed,
      rememberLog
    })
    ...
    if (!wsProbe.ok) {
      throw new Error(
        `Local Atlas backend is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
      )
    }
...
function spawnQuickEntryWindow() {
  ...
  win.webContents.on('did-finish-load', () => {
    if (!win.isDestroyed() && quickEntryLastState) {
      win.webContents.send('atlas:quick-entry:state', quickEntryLastState)
    }
  })
...
function showQuickEntryWindow() {
  ...
  quickEntryWindow.webContents.send('atlas:quick-entry:shown')
}
  ...
  return remoteRevalidation.run(connectionPromise, async () => {
    const [result] = await Promise.all([
      revalidateRemoteConnection({
        ...
        resetConnection: resetAtlasConnection,
...
ipcMain.handle('atlas:gateway:ws-url', async (_event, profile) => {
...
ipcMain.handle('atlas:window:openInstance', async () => {
...
ipcMain.handle('atlas:zoom:get', event => {
...
ipcMain.on('atlas:zoom:set-percent', (event, percent) => {
  const window = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('atlas:bootstrap:repair', async () => {
  // Forceful repair: force the next startAtlas() through the full installer
  ...
  // "bypass active runtime" path inside startAtlas() and fall through
...
ipcMain.handle('atlas:bootstrap:continue-local', async () => {
...
ipcMain.handle('atlas:ssh-config:hosts', async () => ({ hosts: collectSshConfigHosts() }))
ipcMain.handle('atlas:ssh-config:resolve', async (_event, host) => {
...
ipcMain.handle('atlas:connection-config:oauth-login', async (_event, rawUrl) => {
  ...
  if (strategy === 'native') {
    try {
      ...
      // startAtlas() re-dials instead of replaying the stale rejection.
      remoteReauthFailure = null
...
ipcMain.handle('atlas:cloud:agent-sign-in', async (_event, dashboardUrl) => {
...
ipcMain.handle('atlas:ambient:claim', (_event, key) => !claimedAmbientCue(String(key ?? '')))
...
ipcMain.handle('atlas:data-url-read-max:get', () => ({
...
ipcMain.handle('atlas:data-url-read-max:set', (_event, maxMb) => {
...
ipcMain.handle('atlas:readFileDataUrlForAttach', async (_event, filePath) => {
  return readFileDataUrlForIpc(filePath, {
...
ipcMain.handle('atlas:selectPaths', async (_event, options: any = {}) => {
...
ipcMain.handle('atlas:readClipboard', () => clipboard.readText())
...
ipcMain.handle('atlas:watchDirectory', (_event, dir) => watchDirectory(String(dir || '')))
...
ipcMain.on('atlas:active-work', (event, payload) => {
...
ipcMain.on('atlas:keep-awake', (_event, on) => {
...
ipcMain.handle('atlas:quick-entry:settings:get', async () => {
...
ipcMain.handle('atlas:quick-entry:settings:set', async (_event, patch) => {
...
ipcMain.on('atlas:quick-entry:submit', (_event, payload) => {
  hideQuickEntryWindow()
  ...
  mainWindow.webContents.send('atlas:quick-entry:submit', {
...
ipcMain.on('atlas:quick-entry:state', (_event, payload) => {
  quickEntryLastState = payload ?? null

  if (quickEntryWindow && !quickEntryWindow.isDestroyed()) {
    quickEntryWindow.webContents.send('atlas:quick-entry:state', payload)
  }
})

ipcMain.on('atlas:quick-entry:dismiss', () => hideQuickEntryWindow())
 ...
 * swap use.
 */
ipcMain.handle('atlas:app:restart', async (_event, installerPath) => {
...
// per-session secondary windows spawned via `atlas:window:openSession`).
...
ipcMain.handle('atlas:find-in-page', (event, query, options) => {
...
ipcMain.handle('atlas:stop-find-in-page', event => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('atlas:fs:openDir', async (_event, dirPath) => {
...
// The LOCAL Desktop runtime-plugin root: `<ATLAS_HOME>/desktop-plugins`,
// resolved from the main-process ATLAS_HOME (see resolveAtlasHome) — NOT from
// the connected backend. A remote backend reports its own `atlas_home` over
...
ipcMain.handle('atlas:fs:desktopPluginsRoot', async () => {
  // Profile-aware: a named Desktop profile gets its own plugin root under
  // profiles/<name>/, matching the profile-scoped atlas_home the backend
  // reported before this resolver existed. 'default'/unset pins the global root.
  const profile = readActiveDesktopProfile()
  const base = profile && profile !== 'default' ? path.join(ATLAS_HOME, 'profiles', profile) : ATLAS_HOME
...
ipcMain.handle('atlas:git:baseBranchList', async (_event, repoPath) => listBaseBranches(repoPath, resolveGitBinary()))
...
ipcMain.handle('atlas:terminal:cwd', async (_event, id) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
app.on('before-quit', event => {
  ...
  // quitting Atlas never keeps another app's chord hostage.
