 ...
 * resolveAtlasBackend (main.ts). The resolver walks a ladder of
 ...
 *     issue #61764 death-loop) with ATLAS_PROBE_TIMEOUT_MS override
 ...
 * Honours ATLAS_PROBE_TIMEOUT_MS when it parses as a positive integer.
 */
function resolveProbeTimeoutMs(env: NodeJS.ProcessEnv = process.env): number {
  const raw = env.ATLAS_PROBE_TIMEOUT_MS
...
function execProbeSync(
  ...
  try {
    ...
    // One cold-cache / AV miss should not force atlas-setup --update (#61764).
    execFileSync(command, args, options)
  }
}
...
function atlasRuntimeImportProbe() {
  return 'import yaml; import dotenv; import atlas_cli.config'
}
 ...
 * @returns {boolean}
 */
function canImportAtlasCli(pythonPath: string, opts: { env?: Record<string, string> } = {}) {
  if (!pythonPath) {
    return false
  }

  try {
    execProbeSync(pythonPath, ['-c', atlasRuntimeImportProbe()], {
 ...
 * its immutable, matching Atlas package; it must never fall through to the
 * mutable install-script bootstrap path if a best-effort probe is slow.
 */
function shouldTrustAtlasOverride(atlasOverride?: string) {
  return typeof atlasOverride === 'string' && atlasOverride.trim().length > 0
}

function verifyAtlasCli(atlasCommand: string, opts?: { shell?: boolean }) {
  if (!atlasCommand) {
    return false
  }

  try {
    execProbeSync(atlasCommand, ['--version'], {
      stdio: 'ignore',
      timeout: PROBE_TIMEOUT_MS,
