// Profile-delete routing logic for the `atlas:api` IPC handler.
//
...
// pool backend -- spawning a fresh one would call ensure_atlas_home() and
...
/**
 * Parse a `atlas:api` request into the profile name a DELETE targets, or
...
/**
 * Route the next `atlas:api` request away from the primary/window backend
 ...
 * ensure_atlas_home() recreates the directory the delete just removed.
 */
