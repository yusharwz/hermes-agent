...
function readExistingPinnedCommit(activeRoot: string | null | undefined): string | null {
  ...
  try {
    const raw = fs.readFileSync(path.join(activeRoot, '.atlas-bootstrap-complete'), 'utf8')
...
async function resolveInstallScript({
  ...
  const cached = cachedScriptPath(atlasHome, installRef.cacheKey)
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, atlasHome }: any = {}) {
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, atlasHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, atlasHome, pinCommit = true }) {
...
async function fetchManifest({ scriptPath, installerKind, emit, atlasHome, activeRoot, installStamp, pinCommit }) {
  const isPosix = installerKind === 'posix'

  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, atlasHome, pinCommit })]
...
async function runStage({
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, atlasHome, pinCommit })
      ]
