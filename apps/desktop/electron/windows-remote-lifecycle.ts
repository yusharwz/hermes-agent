...
const READY_RE = /^ATLAS_(?:BACKEND|DASHBOARD)_READY port=(\d+)/gm
...
async function probeWindowsRemote(ssh, explicitAtlasPath = '') {
  const explicit = psLiteral(explicitAtlasPath)

  const script = [
    '$ErrorActionPreference="Stop"',
    `$explicit=${explicit}`,
    '$atlasHome=$env:ATLAS_HOME',
    'if(-not $atlasHome){$atlasHome=Join-Path $env:LOCALAPPDATA "atlas"}',
    '$candidates=@()',
    'if($explicit){$candidates+=$explicit}',
    '$cmd=Get-Command atlas.exe -ErrorAction SilentlyContinue',
    'if($cmd){$candidates+=$cmd.Source}',
    '$candidates+=(Join-Path $atlasHome "atlas-agent\\venv\\Scripts\\atlas.exe")',
    '$candidates+=(Join-Path $HOME "atlas-agent\\.venv\\Scripts\\atlas.exe")',
    '$atlas=$candidates|Where-Object{Test-Path -LiteralPath $_ -PathType Leaf}|Select-Object -First 1',
    'if(-not $atlas){throw "Atlas is not installed on the remote Windows host."}',
    'if($explicit -and $atlas -ne $explicit){throw "The configured Atlas path is not an executable file."}',
    '$python=Join-Path (Split-Path $atlas) "python.exe"',
    'if(-not (Test-Path -LiteralPath $python -PathType Leaf)){throw "The remote Atlas Python runtime was not found."}',
    '[ordered]@{os="Windows";arch=$env:PROCESSOR_ARCHITECTURE;atlasHome=$atlasHome;atlasPath=$atlas;python=$python}|ConvertTo-Json -Compress'
  ].join(';')
...
async function detectRemotePlatform(ssh, explicitAtlasPath = '') {
  try {
  ...
  try {
    return await probeWindowsRemote(ssh, explicitAtlasPath)
  } catch (cause: any) {
...
function helperCommand(runtime, operation, args = []) {
  const argv = [runtime.python, '-m', 'atlas_cli.windows_ssh_runtime', operation, ...args]
...
function validLock(lock, ownershipId) {
  ...
  return Boolean(
    ...
    typeof lock.atlasPath === 'string' &&
    typeof lock.atlasHome === 'string'
  )
}
...
async function connectWindowsRemote(deps) {
  ...
  assertCurrent(signal)
  const runtime = await probeWindowsRemote(ssh, remoteAtlasPath)
  const inspection = await helper(ssh, runtime, 'inspect', [runtime.atlasPath])

  if (!inspection.supported) {
    const error: any = new Error('Update Atlas on the remote Windows host before connecting with Desktop SSH.')
    error.kind = 'update-required'
    throw error
  }

  runtime.atlasPath = inspection.path
  const atlasVersion = inspection.version || ''
  rememberLog(`[ssh-lifecycle] remote platform Windows/${runtime.arch}`)
  rememberLog(`[ssh-lifecycle] located atlas at ${runtime.atlasPath}`)
  ...
  if (validLock(lock, ownershipId)) {
    ...
    const reusable =
      ...
      lock.atlasPath === runtime.atlasPath &&
      lock.atlasHome === runtime.atlasHome
  ...
  try {
    spawned = await helper(
      ...
      'spawn',
      [],
      JSON.stringify({ ownershipId, spawnNonce, profile, atlasPath: runtime.atlasPath })
    )
  } catch (error) {
  ...
  try {
    ...
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForAtlas(baseUrl, token)
    assertCurrent(signal)
...
function buildWindowsInteractiveCommand(remoteCwd = '') {
  ...
  script.push('$host.UI.RawUI.WindowTitle="Atlas SSH"', 'powershell.exe -NoLogo')
