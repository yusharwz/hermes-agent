 ...
 * remote-lifecycle.ts
 *
 * Pure, electron-free remote Atlas dashboard lifecycle over SSH for Desktop
 ...
 *
 *   - locate the Atlas install on the remote (login-shell probe),
 ...
 *     `ATLAS_DASHBOARD_READY port=<n>` readiness line,
 ...
 * the real SshConnection, fetch, adoptServedDashboardToken, and waitForAtlas in.
 *
 * The minted ATLAS_DASHBOARD_SESSION_TOKEN is the SPAWN credential. After
...
const READY_RE = /^ATLAS_(?:BACKEND|DASHBOARD)_READY port=(\d+)/m
const REMOTE_LOCK_DIR = '~/.atlas/desktop-ssh'
...
// Resolve the remote atlas executable. An EXPLICIT path is honored strictly
...
async function locateAtlas(ssh, remoteAtlasPath) {
  ...
  if (remoteAtlasPath) {
    if (await isExecutable(remoteAtlasPath)) {
      return resolveLauncher(remoteAtlasPath)
    }

    const err: any = new Error(
      `The Atlas path you set is not an executable on the remote host: "${remoteAtlasPath}". ` +
        'Check the path (it must be the full path to the `atlas` binary on the remote, e.g. ' +
        '~/atlas-agent/.venv/bin/atlas), or clear it to auto-detect.'
    )

    err.kind = 'atlas-not-found'
    throw err
  }
  ...
  try {
    const found = (await ssh.exec(`bash -lc ${shq('command -v atlas')}`)).trim()
  ...
  candidates.push('~/.local/bin/atlas')
  candidates.push('/usr/local/bin/atlas')
  candidates.push('~/.atlas/atlas-agent/venv/bin/atlas')
  ...
  const err: any = new Error(
    'Atlas is not installed on the remote host (could not find a `atlas` executable). ' +
      'Install it on the remote with:  curl -fsSL https://hermes-agent.nousresearch.com/install.sh | sh  ' +
      '— or set the Atlas path explicitly in the SSH connection settings.'
  )
...
// Probe the resolved binary's version string (first line of `<atlas> --version`,
// e.g. "Atlas Agent v0.18.2 ..."), or '' on failure. Surfaces WHICH atlas a
// connection uses, so a stale/unexpected install is visible.
async function probeAtlasVersion(ssh, atlasPath) {
  try {
    const out = (await ssh.exec(`${expandRemotePath(atlasPath)} --version 2>&1`)).trim()
...
async function probeRemotePlatform(ssh) {
  ...
  if (!SUPPORTED_REMOTE_OS.has(osName)) {
    const err: any = new Error(
      `Unsupported remote platform "${osName || 'unknown'}". Atlas Desktop SSH mode supports Linux, macOS, and Windows remote hosts.`
    )
...
// The ATLAS_HOME the remote dashboard will use (explicit env wins, else
// ~/.atlas). Recorded in the lockfile so a future reuse can tell it's the same
// state store; best-effort.
async function probeRemoteAtlasHome(ssh) {
  try {
    const out = (await ssh.exec('echo "${ATLAS_HOME:-$HOME/.atlas}"')).trim().split('\n').pop()

    return out || '~/.atlas'
  } catch (cause) {
    const error: any = new Error('Could not resolve the remote Atlas home.')
...
async function readLockfile(ssh, ownershipId) {
  ...
  for (const field of ['profile', 'atlasPath', 'atlasHome', 'logPath', 'startedAt']) {
...
async function pidIsOurDashboard(ssh, pid, spawnNonce, atlasPath = '') {
  if (!pid || !/^[0-9a-f]{16}$/.test(String(spawnNonce || '')) || !atlasPath) {
    return false
  }
  ...
  try {
    const script =
      ...
      `pid=${Number(pid)}\n` +
      `expected=os.path.expanduser(${shq(atlasPath)})\n` +
...
async function cleanupStale(ssh, ownershipId, lock, pidAlive = true) {
  if (pidAlive && lock && (await pidIsOurDashboard(ssh, lock.pid, lock.spawnNonce, lock.atlasPath))) {
    try {
      const result = (
        await ssh.exec(
...
function buildSpawnCommand(atlasPath, profile, opts: any = {}) {
  const atlas = expandRemotePath(atlasPath)
  ...
  const dashCmd = `env ATLAS_DESKTOP=1 ${atlas} ${profileArgs}${subCmd}`
...
async function remoteSupportsSshOwnership(ssh, atlasPath) {
  const atlas = expandRemotePath(atlasPath)

  const out = await ssh.exec(
    `help="$(${atlas} serve --help 2>&1)"; ` +
...
async function spawnRemoteDashboard(ssh, { atlasPath, profile, token, ownershipId }) {
  if (!(await remoteSupportsSshOwnership(ssh, atlasPath))) {
    const err: any = new Error(
      'The remote Atlas install does not support --ssh-session-token-file and --ssh-owner-nonce. ' +
        'Update Atlas on the remote host to continue using Desktop SSH mode.'
    )
  ...
  try {
    out = await ssh.exec(buildSpawnCommand(atlasPath, profile, { spawnNonce, tokenFilePath, logPath }))
  } catch (error) {
    try {
      await ssh.exec(`rm -f ${expandRemotePath(tokenFilePath)}`)
    } catch {
 ...
 * opened SshConnection, forward/pickLocalPort/waitForAtlas, a token-gated
...
async function connect(deps) {
  ...
  const atlasPath = await locateAtlas(ssh, remoteAtlasPath)
  log(`located atlas at ${atlasPath}`)
  const atlasVersion = await probeAtlasVersion(ssh, atlasPath)

  if (atlasVersion) {
    log(`remote atlas version: ${atlasVersion}`)
  }

  const reuseToken = deps.reuseToken || ''
  const atlasHome = await probeRemoteAtlasHome(ssh)
  const lock = await readLockfile(ssh, ownershipId)

  if (lock) {
    const pidAlive = await remotePidAlive(ssh, lock.pid)
    const owned = pidAlive && (await pidIsOurDashboard(ssh, lock.pid, lock.spawnNonce, lock.atlasPath))
    ...
    const reusable =
      ...
      lock.atlasPath === atlasPath &&
      lock.atlasHome === atlasHome
  ...
  try {
    ...
    assertNotAborted(signal)
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForAtlas(baseUrl, spawnToken)
    assertNotAborted(signal)
