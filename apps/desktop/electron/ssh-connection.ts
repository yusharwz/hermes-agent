...
const _REDACTIONS: Array<[RegExp, string]> = [
  [/(ATLAS_DASHBOARD_SESSION_TOKEN=)(\S+)/g, '$1<redacted>'],
  [/(X-Atlas-Session-Token["']?\s*[:=]\s*["']?)([^\s"'&]+)/gi, '$1<redacted>'],
...
// root under a short per-user base (`~/.atlas/desktop-ssh`) so even worst case
...
function defaultControlDir() {
  ...
  if (process.platform === 'win32') {
    return path.join(os.tmpdir(), 'atlas-desktop-ssh')
  }

  return path.join(os.homedir(), '.atlas', 'desktop-ssh')
}
