...
// `atlas serve` announces ATLAS_BACKEND_READY; the legacy `atlas dashboard`
// backend announces ATLAS_DASHBOARD_READY. Accept either so the desktop spawn
// works against both the headless backend and old/dashboard runtimes.
const _READY_RE = /^ATLAS_(?:BACKEND|DASHBOARD)_READY port=(\d+)/m
...
/**
 * Watch a child process's stdout for the `ATLAS_(BACKEND|DASHBOARD)_READY
