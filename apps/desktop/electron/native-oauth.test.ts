...
test('buildNativeAuthorizeUrl omits provider when not given and preserves prefix', () => {
  const url = buildNativeAuthorizeUrl('https://gw.example.com/atlas', {
    challenge: 'C',
  ...
  assert.equal(parsed.pathname, '/atlas/auth/native/authorize')
...
test('nativeTokenUrl / nativeRefreshUrl build the right endpoints', () => {
  ...
  assert.equal(nativeRefreshUrl('https://gw.example.com/atlas'), 'https://gw.example.com/atlas/auth/native/refresh')
})
