...
//   ATLAS_WIN_SSH_HOST   ssh alias/host of the Windows box
//   ATLAS_WIN_SSH_USER   remote user
//   ATLAS_WIN_SSH_ATLAS absolute path to the remote atlas.exe under test
const liveHost = process.env.ATLAS_WIN_SSH_HOST || ''
const liveUser = process.env.ATLAS_WIN_SSH_USER || ''
const configuredAtlas = process.env.ATLAS_WIN_SSH_ATLAS || ''
const ownershipId = '89abcdef0123456789abcdef01234567'

function fetchJson(url, token, path) {
  return fetch(`${url}${path}`, { headers: { 'X-Atlas-Session-Token': token } }).then(async response => {
    if (!response.ok) {
...
test.skipIf(!liveHost || !liveUser || !configuredAtlas)(
  ...
  async () => {
    ...
    try {
      ...
      const runtimeScript = `& '${configuredAtlas.replace('atlas.exe', 'python.exe')}' -m atlas_cli.windows_ssh_runtime read-lock '${ownershipId}'`
      ...
      if (lock) {
        const python = configuredAtlas.replace('atlas.exe', 'python.exe')
        const terminate = `& '${python}' -m atlas_cli.windows_ssh_runtime terminate '${lock.pid}' '${lock.creationTimeNs}' '${lock.atlasPath}' '${lock.spawnNonce}'`
        await ssh.exec(`powershell.exe -NoProfile -NonInteractive -Command "${terminate}"`)
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m atlas_cli.windows_ssh_runtime remove-lock '${ownershipId}'"`
        )
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m atlas_cli.windows_ssh_runtime remove-log '${ownershipId}' '${lock.spawnNonce}'"`
        )
      }
