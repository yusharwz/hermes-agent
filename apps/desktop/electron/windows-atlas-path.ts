/**
 * windows-atlas-path.ts
 *
 * Pure, dependency-injected pieces of Windows `atlas` resolution pulled out
 * of main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
 * unwrapWindowsVenvAtlasCommand(). Each of the three functions here pins one
 ...
 *      FIRST, so an extensionless Git-Bash `atlas` shim shadowed the real
 *      atlas.cmd/atlas.exe; the shim then failed the --version probe and
 ...
 *      venv\Scripts\atlas.exe (the console-script shim, written at the END
 ...
 *   3. resolveVenvAtlasCommand() — unwrapWindowsVenvAtlasCommand() returned
 ...
 * extensionless file (e.g. a Git-Bash shell-script shim named `atlas`) must
 * not shadow `atlas.cmd`/`atlas.exe`. The empty entry is kept LAST so
 ...
 * signals (venv python interpreter, venv atlas shim, bootstrap-complete
 * marker) — gating on just the atlas.exe console-script shim alone is the
...
export interface ResolveVenvAtlasCommandDeps {
  isWindows: boolean
  isCommandScript: (command: string) => boolean
  fileExists: (filePath: string) => boolean
  directoryExists: (filePath: string) => boolean
  canImportAtlasCli: (python: string, opts?: { env?: Record<string, string> }) => boolean
...
/**
 * If `command` is a Windows venv `atlas`/`atlas.exe` console-script shim
 * (i.e. `<venvRoot>/Scripts/atlas(.exe)`), resolve it to the underlying
 * venv python invoked as `python -m atlas_cli.main <backendArgs>` — but
 * ONLY after smoke-testing that interpreter with canImportAtlasCli(). A
 * venv whose update died mid-`pip install` still has python.exe + atlas.exe
 ...
 *
 * Returns null when `command` is not a venv atlas shim, the underlying
 * python doesn't exist, or the import probe fails. Otherwise returns the
 * resolved backend descriptor.
 */
export function resolveVenvAtlasCommand(
  command: string,
  backendArgs: string[],
  deps: ResolveVenvAtlasCommandDeps
): {
  label: string
  command: string
  args: string[]
  ...
  if (!/^atlas(?:\.exe)?$/i.test(basename(resolved))) {
    return null
  }
  ...
  if (
    ...
    rememberLog?.(
      `Ignoring venv Atlas at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
