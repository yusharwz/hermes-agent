...
test('existing checkout detection requires git metadata', () => {
  ...
  try {
    const activeRoot = path.join(home, 'atlas-agent')
...
test('fresh bootstrap args include the packaged commit pin', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp,
      activeRoot: '/tmp/atlas-agent',
      atlasHome: '/tmp/atlas'
    }),
    ['--dir', '/tmp/atlas-agent', '--atlas-home', '/tmp/atlas', '--branch', 'main', '--commit', installStamp.commit]
  )
})
...
test('existing-checkout bootstrap args keep branch but skip the packaged commit pin', () => {
  ...
  assert.deepEqual(
    ...
    ['--dir', '/tmp/atlas-agent', '--atlas-home', '/tmp/atlas', '--branch', 'main']
  )
})
...
test('fallback install stamps use an unpinned branch ref', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp: stamp,
      activeRoot: '/tmp/atlas',
      atlasHome: '/tmp/home'
    }),
    ['--dir', '/tmp/atlas', '--atlas-home', '/tmp/home', '--branch', 'main']
  )
})
