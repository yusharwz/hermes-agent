...
test('locateAtlas prefers the explicit profile path when executable', async () => {
  const ssh = fakeSsh([[/\[ -x .*\/opt\/atlas/, 'OK']])
  assert.equal(await locateAtlas(ssh, '/opt/atlas'), '/opt/atlas')
})

test('locateAtlas throws (no silent fallback) when an EXPLICIT path is not executable', async () => {
  // command -v WOULD find a different install, but an explicit path must not
  // silently fall back to it — that is the "connected to the wrong atlas" bug.
  const ssh = fakeSsh([
    [/command -v atlas/, '/home/u/.local/bin/atlas\n'],
    [/\[ -x .*\.local\/bin\/atlas/, 'OK']
  ])

  await assert.rejects(
    () => locateAtlas(ssh, '/bad/path/atlas'),
    (err: any) => {
      assert.equal(err.kind, 'atlas-not-found')
      assert.match(err.message, /\/bad\/path\/atlas/)
...
test('locateAtlas falls back to the login-shell command -v probe', async () => {
  const ssh = fakeSsh([
    [/command -v atlas/, '/home/u/.local/bin/atlas\n'],
    [/\[ -x .*\.local\/bin\/atlas/, 'OK']
  ])

  assert.equal(await locateAtlas(ssh, ''), '/home/u/.local/bin/atlas')
})

test('locateAtlas canonicalizes an installer wrapper to its executable target', async () => {
  const ssh = fakeSsh([
    [/command -v atlas/, '/home/u/.local/bin/atlas\n'],
    [/\[ -x .*\.local\/bin\/atlas/, 'OK'],
    [/python3 -c/, '/home/u/.atlas/atlas-agent/venv/bin/atlas\n']
  ])

  assert.equal(await locateAtlas(ssh, ''), '/home/u/.atlas/atlas-agent/venv/bin/atlas')
})

test('locateAtlas falls back to ~/.local/bin/atlas when the login-shell probe misses', async () => {
  ...
  assert.equal(await locateAtlas(ssh, ''), '~/.local/bin/atlas')
})

test('locateAtlas tries the conventional venv path last', async () => {
  const ssh = fakeSsh([[/\[ -x .*venv\/bin\/atlas/, 'OK']])
  assert.equal(await locateAtlas(ssh, ''), '~/.atlas/atlas-agent/venv/bin/atlas')
})

test('locateAtlas throws a atlas-not-found error with an install hint', async () => {
...
test('locateAtlas uses a login shell for the command -v probe', async () => {
  const ssh = fakeSsh([
    [/command -v atlas/, '/x/atlas'],
    [/\[ -x/, 'OK']
  ])

  await locateAtlas(ssh, '')
  assert.ok(
...
test('ownership paths are isolated by ownership ID and spawn nonce', () => {
  assert.equal(ownershipDirectory(OWNERSHIP_ID), `~/.atlas/desktop-ssh/${OWNERSHIP_ID}`)
  assert.equal(lockfilePath(OWNERSHIP_ID), `~/.atlas/desktop-ssh/${OWNERSHIP_ID}/backend.lock.json`)
  assert.equal(spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE), `~/.atlas/desktop-ssh/${OWNERSHIP_ID}/${SPAWN_NONCE}.log`)
})
...
test('metadata and process proof transport failures remain indeterminate', async () => {
  ...
  await assert.rejects(
    () => pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, failure]]), 5, SPAWN_NONCE, '/x/atlas'),
    (error: any) => error.kind === 'transient-transport-error'
  )
})

test('pidIsOurDashboard requires the exact serve ownership nonce', async () => {
  const ours = `/x/atlas serve --isolated --ssh-owner-nonce ${SPAWN_NONCE}`
  assert.equal(await pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, 'OWNED\n']]), 5, SPAWN_NONCE, '/x/atlas'), true)
  assert.equal(
...
test('buildSpawnCommand is headless serve, detached, token not in argv', () => {
  const cmd = buildSpawnCommand('/x/atlas', 'work', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  ...
  assert.ok(!cmd.includes('ATLAS_DASHBOARD_SESSION_TOKEN'), 'token env var must not appear')
})
...
test('spawnRemoteDashboard always spawns serve (legacy dashboard path removed)', async () => {
  ...
  await spawnRemoteDashboard(ssh, { atlasPath: '/x/atlas', profile: '', token: 'tk', ownershipId: OWNERSHIP_ID })
...
test('READY_RE accepts both serve and dashboard sentinels', () => {
  assert.equal(READY_RE.exec('ATLAS_BACKEND_READY port=4321')?.[1], '4321')
  assert.equal(READY_RE.exec('ATLAS_DASHBOARD_READY port=8765')?.[1], '8765')
})
...
test('spawnRemoteDashboard rejects when no pid is returned', async () => {
  ...
  await assert.rejects(
    () => spawnRemoteDashboard(ssh, { atlasPath: '/x/atlas', profile: '', token: 't', ownershipId: OWNERSHIP_ID }),
    (err: any) => {
...
test('scrapeReadyPort reads only the named spawn log', async () => {
  ...
  const ssh = fakeSsh([[/cat/, 'some noise\nATLAS_DASHBOARD_READY port=51234\n']])
...
test('connect() spawns fresh when there is no lockfile, adopts the served token', async () => {
  const ssh = fakeSsh([
    ...
    [/setsid/, '777\n'],
    [/kill -0 777/, 'ALIVE'],
    [/cat .*\.log/, 'ATLAS_DASHBOARD_READY port=51999\n']
  ])
...
test('connect() respawns when the lockfile atlasPath differs from the resolved path', async () => {
  const reuseToken = 'stored-token'
  const lock = ownedLock({ atlasPath: '/old/stale/atlas', tokenFingerprint: fingerprintToken(reuseToken) })
  ...
  const ssh = fakeSsh([
    ...
    [/kill -0/, 'ALIVE'],
    [/print\("OWNED"/, 'FOREIGN\n'],
    [/--version/, 'Atlas Agent v0.18.2\n'],
    [/grep -q ssh-session-token-file/, 'YES\n'],
    [/python3 -c/, ''],
    [/setsid/, '890\n'],
    [/cat .*\.log/, 'ATLAS_DASHBOARD_READY port=52050\n']
  ])

  const result = await connect(
    connectDeps(ssh, { reuseToken, remoteAtlasPath: '/new/atlas', adoptServedToken: async () => 'fresh' })
  )
...
test('connect() respawns when the lockfile protocolVersion is incompatible', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '901\n'],
    [/kill -0 901/, 'ALIVE'],
    [/cat .*\.log/, 'ATLAS_DASHBOARD_READY port=44100\n']
  ])
...
test('connect() fresh spawn writes atlasHome + protocolVersion into the lockfile', async () => {
  const writes: string[] = []
  ...
  const ssh = fakeSsh([
    ...
    [/\[ -x/, 'OK'],
    [/cat .*lock\.json/, ''], // no lockfile
    [/ATLAS_HOME/, '/home/alice/.atlas\n'],
    [/grep -q ssh-session-token-file/, 'YES\n'],
    [/python3 -c/, ''],
    [/printf '%s\\n'/, ''],
    [/setsid/, '700\n'],
    [/kill -0 700/, 'ALIVE'],
    [/cat .*\.log/, 'ATLAS_DASHBOARD_READY port=45500\n'],
    [
      /printf '%s' '/,
      c => {
        writes.push(c)
  ...
  assert.match(lockWrite, /"atlasHome":"\/home\/alice\/\.atlas"/)
})
...
test('connect() respawns when the lockfile pid is dead (killed dashboard)', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '888\n'],
    [/kill -0 888/, 'ALIVE'],
    [/cat .*\.log/, 'ATLAS_DASHBOARD_READY port=42000\n']
  ])
...
test('connect() respawns when the dashboard is wedged (alive pid, probe fails)', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '999\n'],
    [/kill -0 999/, 'ALIVE'],
    [/cat .*\.log/, 'ATLAS_DASHBOARD_READY port=43000\n']
  ])
...
test('validateRemotePath accepts absolute POSIX paths', () => {
  assert.doesNotThrow(() => validateRemotePath('/usr/bin/atlas'))
  assert.doesNotThrow(() => validateRemotePath('/home/user/.atlas/atlas-agent/venv/bin/atlas'))
})

test('validateRemotePath accepts ~/ prefix paths', () => {
  assert.doesNotThrow(() => validateRemotePath('~/bin/atlas'))
  assert.doesNotThrow(() => validateRemotePath('~/.atlas/logs/desktop-ssh.log'))
  assert.doesNotThrow(() => validateRemotePath('~'))
})

test('validateRemotePath accepts paths with spaces and quotes', () => {
  assert.doesNotThrow(() => validateRemotePath('/home/user/my project/atlas'))
...
test('validateRemotePath rejects relative paths', () => {
  assert.throws(() => validateRemotePath('atlas'), /absolute|relative/i)
  assert.throws(() => validateRemotePath('./bin/atlas'), /absolute|relative/i)
  assert.throws(() => validateRemotePath('../etc/passwd'), /absolute|relative/i)
})

test('validateRemotePath rejects NUL and newline', () => {
  assert.throws(() => validateRemotePath('/usr/bin/atlas\x00'), /unsafe/i)
  assert.throws(() => validateRemotePath('/usr/bin/atlas\n'), /unsafe/i)
  assert.throws(() => validateRemotePath('/usr/bin/atlas\r'), /unsafe/i)
})

test('validateRemotePath preserves shell metacharacters as path data', () => {
  for (const p of ['/usr/$(whoami)/atlas', '/usr/`id`/atlas', '/usr/a;b|c&d<e>f']) {
...
test('expandRemotePath expands ~/ to "$HOME"/', () => {
  const result = expandRemotePath('~/.atlas/logs/desktop-ssh.log')
...
test('expandRemotePath returns quoted absolute paths unchanged', () => {
  const result = expandRemotePath('/usr/local/bin/atlas')
  assert.ok(result.includes('/usr/local/bin/atlas'))
  assert.ok(!result.includes('eval'))
})

test('expandRemotePath preserves spaces as data', () => {
  const result = expandRemotePath('/home/user/my project/atlas')
...
test('buildSpawnCommand does not embed the token in the command string', () => {
  ...
  assert.ok(!cmd.includes('ATLAS_DASHBOARD_SESSION_TOKEN'), 'env var name must not appear')
})

test('buildSpawnCommand includes --ssh-session-token-file when tokenFilePath is provided', () => {
  const cmd = buildSpawnCommand('/x/atlas', 'work', {
    tokenFilePath: `~/.atlas/desktop-ssh/${OWNERSHIP_ID}/${SPAWN_NONCE}.token`,
    logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE),
    spawnNonce: SPAWN_NONCE
  })

  assert.match(cmd, /--ssh-session-token-file/)
  assert.match(cmd, /\.atlas\/desktop-ssh\//)
})

test('buildSpawnCommand always uses serve, never dashboard', () => {
  const cmd = buildSpawnCommand('/x/atlas', '', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  assert.match(cmd, /serve --isolated/)
  assert.doesNotMatch(cmd, /\bdashboard\b/)
  assert.doesNotMatch(cmd, /--skip-build/)
...
test('spawnRemoteDashboard removes a token file when upload reporting fails', async () => {
  ...
  await assert.rejects(
    () => spawnRemoteDashboard(ssh, { atlasPath: '/x/atlas', profile: '', token: 'tok', ownershipId: OWNERSHIP_ID }),
    /channel closed/
  )
...
test('spawnRemoteDashboard fails with update-required when remote lacks --ssh-session-token-file', async () => {
  ...
  await assert.rejects(
    () => spawnRemoteDashboard(ssh, { atlasPath: '/x/atlas', profile: '', token: 'tk', ownershipId: OWNERSHIP_ID }),
    (err: any) => {
...
test('readLockfile rejects a log path outside the exact ownership and spawn path', async () => {
  const lock = ownedLock({ logPath: '~/.atlas/desktop-ssh/other.log' })
...
test('cleanupStale never deletes a lock-supplied unexpected log path', async () => {
  ...
  await cleanupStale(ssh, OWNERSHIP_ID, ownedLock({ logPath: '~/.atlas/unrelated.log' }))
...
test('pidIsOurDashboard requires an exact nonce option value', async () => {
  const prefix = `/x/atlas serve --isolated --ssh-owner-nonce ${SPAWN_NONCE}ff`
  const suffix = `/x/atlas serve --isolated --ssh-owner-nonce xx${SPAWN_NONCE}`
  assert.equal(await pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, 'FOREIGN\n']]), 5, SPAWN_NONCE, '/x/atlas'), false)
  assert.equal(await pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, 'FOREIGN\n']]), 5, SPAWN_NONCE, '/x/atlas'), false)
})
...
test('remote SSH ownership capability requires both secure bootstrap flags', async () => {
  ...
  assert.equal(await remoteSupportsSshOwnership(supported, '/x/atlas'), true)
  ...
  assert.equal(await remoteSupportsSshOwnership(unsupported, '/x/atlas'), false)
})
