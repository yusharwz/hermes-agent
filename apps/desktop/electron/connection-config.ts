...
// The Nous portal (NAS) does NOT use Atlas gateway session cookies — it is a
...
// Atlas Cloud connection (cloud-auto-discovery Q3/Q6): it carries a
...
function normalizeSshConfig(entry) {
  ...
  const remoteAtlasPath = String(entry.remoteAtlasPath || '').trim()

  if (remoteAtlasPath) {
    out.remoteAtlasPath = remoteAtlasPath
  }
 ...
 *     `--profile`, so its `ATLAS_HOME` scopes it.
 *
 ...
 * the Atlas gateway session cookies, so cloud sign-in / discovery liveness
