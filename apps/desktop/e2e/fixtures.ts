/**
 * Shared E2E fixtures for the Atlas desktop Playwright suite.
 *
 * Two fixture modes:
 *
 ...
 *     (electron → atlas serve → provider → inference → renderer) is
 ...
 * `electron-builder --dir` step and matches `atlas desktop --source`. The
...
export function createSandbox(prefix: string): Sandbox {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `atlas-e2e-${prefix}-${Math.random()}`))
  const atlasHome = path.join(root, 'atlas-home')
...
export function writeMockProviderConfig(
  ...
  const configPath = path.join(atlasHome, 'config.yaml')
 ...
 */
export function writeEnvFile(atlasHome: string, apiKey = 'e2e-mock-key'): void {
  const envPath = path.join(atlasHome, '.env')
 ...
 */
function writeEmptyConfig(atlasHome: string): void {
  const configPath = path.join(atlasHome, 'config.yaml')
 ...
 *
 * Key env vars:
 *  - ATLAS_HOME → sandbox atlas-home (isolated config/sessions)
 *  - ATLAS_DESKTOP_USER_DATA_DIR → sandbox electron-user-data
 *  - ATLAS_DESKTOP_IGNORE_EXISTING=1 → don't pick up `atlas` from PATH
 *    (we want the dev checkout at REPO_ROOT)
 *  - ATLAS_DESKTOP_ATLAS_ROOT → REPO_ROOT (dev checkout resolution)
 *  - ATLAS_DESKTOP_APP_NAME → unique-ish per test (avoids single-instance lock)
...
export function buildAppEnv(sandbox: Sandbox, extra: Record<string, string> = {}): Record<string, string> {
  ...
  return {
    ...clean,
    ATLAS_HOME: sandbox.atlasHome,
    ATLAS_DESKTOP_USER_DATA_DIR: sandbox.userDataDir,
    ATLAS_DESKTOP_IGNORE_EXISTING: '1',
    ATLAS_DESKTOP_ATLAS_ROOT: REPO_ROOT,
    ATLAS_DESKTOP_APP_NAME: `AtlasE2E-${Date.now()}`,
    ...
    ATLAS_DESKTOP_SKIP_QUIT_CONFIRM: '1',
 ...
 *
 * @param sandbox  - isolated ATLAS_HOME + userData
 * @param env      - the process environment (already has ATLAS_HOME etc.)
...
export async function setupMockBackend(options: MockBackendOptions = {}): Promise<MockBackendFixture> {
  ...
  writeEnvFile(sandbox.atlasHome)
...
export async function setupNoProvider(): Promise<NoProviderFixture> {
  ...
  writeEmptyConfig(sandbox.atlasHome)
...
export interface DeadBackendOptions {
  /**
   * When true, inject a fake boot error via ATLAS_DESKTOP_BOOT_FAKE_ERROR
 ...
 * nothing listens on). By default the backend still boots (`atlas serve`
...
export async function setupDeadBackend(options: DeadBackendOptions = {}): Promise<DeadBackendFixture> {
  ...
  const configPath = path.join(sandbox.atlasHome, 'config.yaml')
  fs.writeFileSync(
    configPath,
...
`,
  ...
  const env = buildAppEnv(sandbox, options.fakeError ? { ATLAS_DESKTOP_BOOT_FAKE_ERROR: 'Failed to connect to Atlas backend: connection refused' } : {})
  const { app, page } = await launchDesktop(env)
...
function resolvePackagedBinaryPath(): string {
  if (process.platform === 'win32') {
    return path.join(RELEASE_ROOT, 'win-unpacked', 'Atlas.exe')
  }

  if (process.platform === 'darwin') {
    const arch = process.arch === 'arm64' ? 'arm64' : 'x64'

    return path.join(RELEASE_ROOT, `mac-${arch}`, 'Atlas.app', 'Contents', 'MacOS', 'Atlas')
  }

  return path.join(RELEASE_ROOT, 'linux-unpacked', 'atlas')
}
 ...
 * progress without spawning a real Atlas backend.
 *
 * Uses the same sandbox isolation (credential stripping, isolated
 * ATLAS_HOME + userData, unique app name) as the dev-mode fixtures.
 *
...
export async function setupPackagedApp(): Promise<PackagedAppFixture> {
  ...
  const env = buildAppEnv(sandbox, {
    ...
    ATLAS_DESKTOP_BOOT_FAKE: '1',
    ATLAS_DESKTOP_BOOT_FAKE_STEP_MS: '120',
  })

  // Clear dev-server + atlas-root overrides — the packaged binary
  // should use its own bundled renderer, not the dev checkout.
  delete (env as Record<string, string | undefined>).ATLAS_DESKTOP_DEV_SERVER
  delete (env as Record<string, string | undefined>).ATLAS_DESKTOP_ATLAS
  delete (env as Record<string, string | undefined>).ATLAS_DESKTOP_ATLAS_ROOT
