 ...
 * packaged binary) with a real `atlas serve` backend pointed at a mock
 * inference server. The full chain is exercised:
 *
 *   electron → atlas serve (python) → mock provider → renderer
 *
...
test.describe('dev-mode boot with mock backend', () => {
  test('window opens with Atlas title', async () => {
    const title = await fixture!.page.title()
    expect(title).toContain('Atlas')
  })
  ...
  test('backend boots and app becomes ready', async () => {
    ...
    // electron starts → atlas serve is spawned → WS connects → config
