...
async function setupSeededMockBackend(): Promise<MockBackendFixture> {
  ...
  writeMockProviderConfig(sandbox.atlasHome, mock.url)
  fs.appendFileSync(
    path.join(sandbox.atlasHome, 'config.yaml'),
    '\ncompression:\n  threshold_tokens: 1\n',
    'utf8',
  )
  writeEnvFile(sandbox.atlasHome)
  const builder = await RealSessionBuilder.start(sandbox.atlasHome)
  try {
...
test('live verify-on-stop continuations stay out of the transcript', async ({}, testInfo) => {
  ...
  writeMockProviderConfig(sandbox.atlasHome, mock.url)
  fs.appendFileSync(path.join(sandbox.atlasHome, 'config.yaml'), '\nagent:\n  verify_on_stop: true\n', 'utf8')
  writeEnvFile(sandbox.atlasHome)
  const { app, page } = await launchDesktop(buildAppEnv(sandbox))
