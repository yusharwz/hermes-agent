    ...
    try:
        ...
        if task and task.status == "ready" and task.assignee:
            try:
                ...
                # backend can run under a different ATLAS_HOME than the
...
from atlas_cli.kanban_db import (  # noqa: E402
...
def model_options():
    ...
    Thin wrapper around ``atlas_cli.inventory.build_models_payload`` — the
    same substrate the dashboard Models page and the TUI picker use, so the
    dropdown can never offer a model/provider pair the rest of Atlas
...
def list_kanban_projects():
    """List first-class Atlas projects for board scoping.
