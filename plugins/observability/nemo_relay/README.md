...
Optional Atlas observability plugin that configures exporters and maps
Atlas-specific observer hooks to NeMo Relay marks and ATIF state. Atlas core
...
CLI UX. Atlas core emits NeMo Relay lifecycle events for provider and tool
...
- Export the Relay scopes and LLM/tool lifecycles emitted by Atlas core.
- Add Atlas session, turn, approval, and subagent mark events.
...
wheel, Atlas installs its supported NeMo Relay runtime as a normal dependency:
...
behavior; it does not control Atlas shared client metrics.
...
Atlas core routes provider and tool execution through NeMo Relay managed APIs
...
Atlas uses the dynamic-plugin activation API available in NeMo Relay 0.6 and
later. Configure native or worker plugins with Atlas-owned
...
with this Atlas-owned section. The gateway combines those records with
separate lifecycle state for enablement, trust policy, and worker environments;
the Python binding does not yet expose that resolver. Atlas rejects
...
Atlas activates these plugins before registering its managed LLM and tool
...
with Atlas and a local Ollama model served through the OpenAI-compatible API.
...
- Atlas core managed provider calls map to `llm` scope start/end events.
- Atlas core managed tool calls map to `tool` scope start/end events.
...
Atlas core owns the LLM and tool boundaries and enters NeMo Relay managed
execution while a Atlas-managed Relay consumer is active. With no shared
metrics subscriber or explicitly configured Relay plugin, Atlas calls the
...
Atlas provider call
  -> nemo_relay.llm.execute(...)
    -> Atlas provider adapter callback(...)

Atlas tool call
  -> nemo_relay.tools.execute(...)
    -> Atlas authorization and dispatch callback(...)
```
