...
- OpenViking server running and reachable from Atlas
...
default profile that is `~/.atlas/.env`; for a named profile use
...
OpenViking's server config is separate from Atlas:
...
Atlas-side provider config is read from environment variables in the active
profile's `.env`:
