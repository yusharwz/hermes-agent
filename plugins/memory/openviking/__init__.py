...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        self._turn_count = 0
        atlas_home = str(kwargs.get("atlas_home") or "").strip()
        if not atlas_home:
            try:
                from atlas_constants import get_atlas_home
                atlas_home = str(get_atlas_home())
            except Exception:
                atlas_home = str(Path.home() / ".atlas")
    ...
    def _ensure_client(self) -> Optional["_VikingClient"]:
        ...
        ``~/.atlas/.env`` after startup never reach the live client and tools
        keep running against stale auth until the user restarts atlas (#21130).
    ...
    def _pending_session_dir(self) -> Optional[Path]:
        if not self._atlas_home:
            return None
        return Path(self._atlas_home) / _PENDING_SESSIONS_RELATIVE_DIR
    ...
    def _run_lock_dir(self) -> Optional[Path]:
        if not self._atlas_home:
            return None
        return Path(self._atlas_home) / _RUN_LOCKS_RELATIVE_DIR
