...
- Hosted: API key from [app.supermemory.ai/integrations?connect=atlas](http://app.supermemory.ai/integrations?connect=atlas)
...
Before running `atlas memory setup`, add the local endpoint to
...
Then run `atlas memory setup` and enter the local server's API key. Configuring
...
`https://api.supermemory.ai`. Atlas resolves it once and uses the same endpoint
