# Atlas Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: atlas-achievements plugin
...
### `GET /api/plugins/atlas-achievements/achievements`
...
### `POST /api/plugins/atlas-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/atlas-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/atlas-achievements/overview`
...
- Persist plugin data under:
  - `~/.atlas/plugins/atlas-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.atlas/plugins/atlas-achievements/scan_checkpoint.json`
...
- `~/.atlas/plugins/atlas-achievements/`
