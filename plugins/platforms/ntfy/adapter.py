"""ntfy platform adapter (Atlas plugin).
...
``plugins/platforms/ntfy/``. The Atlas plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "atlas-in"              # subscribe topic (incoming)
          publish_topic: "atlas-out"     # optional — defaults to topic
...
_ECHO_TAG = "atlas-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``atlas cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Atlas dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `atlas gateway status` without
