...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._liveness_interval_seconds = self._finite_positive_config_float(
            ...
            15.0,
            env_key="ATLAS_DISCORD_LIVENESS_INTERVAL_SECONDS",
        )
        self._liveness_failure_threshold = self._config_int(
            "websocket_liveness_failure_threshold",
            2,
            env_key="ATLAS_DISCORD_LIVENESS_FAILURE_THRESHOLD",
        )
        ...
        self._discord_recovery_store = DiscordRecoveryStore(get_atlas_home())
    ...
    async def _run_missed_message_backfill(self) -> None:
        ...
        that lack a substantive non-outage Atlas response. Emoji-only acks are
    ...
    async def _should_backfill_discord_message(self, message: Any) -> bool:
        """Return True when a recent Discord message still needs Atlas work."""
    ...
    def _is_down_notice_content(self, content: str) -> bool:
        """Recognize only explicit Atlas/gateway outage notices."""
        text = (content or "").lower()
        subject = r"(?:atlas|the agent|agent|the gateway|gateway|bmo)"
    ...
    def _is_pairing_approved_user(self, user_id: str) -> bool:
        """True when the Discord user has an explicit Atlas pairing grant."""
    ...
    def _derive_auto_thread_name(self, content: str) -> str:
        ...
        Atlas has an LLM-generated session title and can safely rename
    ...
    async def _auto_create_thread(self, message: 'DiscordMessage') -> Optional[Any]:
        ...
        for attempt in range(2):
            try:
                ...
                try:
                    setattr(thread, "_atlas_auto_thread_initial_name", thread_name)
                except Exception:
                    pass
                return thread
            except Exception as direct_error:
    ...
    async def rename_thread(
        ...
        *,
        ...
        if edit is None:
            return False
        try:
            await edit(name=cleaned, reason="Atlas semantic session title")
            logger.info(
    ...
    async def send_exec_approval(
        ...
        try:
            ...
            prompt_prefix = (
                ...
                "Do you want Atlas to run this command?\n\n"
                "**Requested command:**\n```bash\n"
            )
            if smart_denied:
    ...
    async def send_clarify(
        ...
        try:
            ...
            content = self._self_contained_prompt_content(
                "❓ **Atlas needs your input**", str(question or "").strip(),
                tail=clarify_tail,
            )
        ...
        *,
        ...
        source = self.build_source(
            ...
            auto_thread_initial_name=(
                getattr(auto_threaded_channel, "_atlas_auto_thread_initial_name", None)
...
def _apply_yaml_config(yaml_cfg: dict, discord_cfg: dict) -> dict | None:
    ...
    _websocket_liveness_keys = (
        (
            ...
            "ATLAS_DISCORD_LIVENESS_INTERVAL_SECONDS",
        ),
        (
            "websocket_liveness_failure_threshold",
            "liveness_failure_threshold",
            "ATLAS_DISCORD_LIVENESS_FAILURE_THRESHOLD",
        ),
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `atlas setup` to install Discord support.",
