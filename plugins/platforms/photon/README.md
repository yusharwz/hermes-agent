...
- **Inbound attachments and voice notes are downloaded.** The sidecar reads
  ...
  something arrived. If Spectrum emits a `richlink` content object, Atlas
  ...
  `.pluginPayloadAttachment` images immediately after the URL; Atlas coalesces
...
- **Native polls are supported.** Atlas posts poll content through
...
- **Cron/standalone sends require a running gateway.** Processes outside
  the gateway (cron subprocesses, `atlas send`) cannot spawn the sidecar;
  they authenticate to the gateway's live sidecar via the runtime record at
  `<atlas-home>/runtime/photon-sidecar.json` (written after the sidecar's
