...
#
# Out-of-process senders (cron subprocesses, `atlas send`, the dashboard)
...
def _runtime_record_path() -> Path:
    # get_atlas_home() honors profile overrides — never hardcode ~/.atlas.
    from atlas_constants import get_atlas_home

    return get_atlas_home() / "runtime" / _RUNTIME_RECORD_NAME
...
# this module (atlas status, test collection, plugin discovery).
...
# artifacts so Atlas sees the link once, not a follow-up "(attachment)" prompt.
...
def sidecar_deps_installed() -> bool:
    ...
    _start_sidecar(), and `atlas photon status` so all three agree on
...
def check_requirements() -> bool:
    ...
    if not sidecar_deps_installed():
        ...
        # user has no CLI to run `atlas photon setup`, so the connect path
        # must self-heal). Otherwise keep returning False so
        # `atlas setup` / status surface the missing-deps state.
        ...
        # (load_gateway_config, atlas status, GET /api/status polling) —
        ...
        if npm_error:
            logger.debug(
                ...
                "(last npm error: %s) — run: atlas photon setup",
                _sidecar_dir(),
                npm_error,
            )
        else:
            logger.debug(
                "photon: spectrum-ts not installed at %s — run: atlas photon setup",
                _sidecar_dir(),
            )
        return False
    return True
...
def _sidecar_deps_stale() -> bool:
    ...
    `atlas update` rewrites ``package-lock.json`` when the spectrum-ts pin is
...
def _reinstall_sidecar_deps() -> None:
    ...
    Mirrors ``atlas photon install-sidecar``: ``npm ci`` for an exact,
...
def _richlink_candidate(text: str) -> Optional[str]:
    ...
    markdown/text path so Atlas does not drop labels or rewrite intent.
    """
...
class PhotonAdapter(BasePlatformAdapter):
    ...
    async def _start_sidecar(self) -> None:
        if not sidecar_deps_installed():
            ...
            # `atlas photon setup`, so the connect path must be able to
            ...
            if not sidecar_deps_installed():
                raise RuntimeError(
                    ...
                    f"Run: cd {_sidecar_dir()} && npm ci   (or `atlas photon setup`)"
                )
        # A `atlas update` that bumps the spectrum-ts pin rewrites
            ...
            while time.time() < deadline:
                ...
                try:
                    ...
                    if resp.status_code == 200:
                        ...
                        # (cron, `atlas send`) can reach this sidecar
...
async def _standalone_send(
    ...
    *,
    ...
    if not token:
        ...
        # the gateway process env otherwise, so cron/`atlas send` would be
        ...
        if not token:
            return {
                "error": (
                    ...
                    "Start the Atlas gateway (which spawns the sidecar and "
                    "records its address under <atlas-home>/runtime/"
