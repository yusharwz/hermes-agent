...
under an immutable ``/opt/atlas`` (read-only for the atlas user), which
...
4. Source dir read-only and deps missing or stale → mirror the sidecar
   source files to ``$ATLAS_HOME/photon/sidecar`` (the durable data volume,
...
def resolve_sidecar_dir(source_dir: Optional[Path] = None) -> Path:
    ...
    mirror = get_atlas_home() / "photon" / "sidecar"
    try:
