...
# identify AtlasAgent traffic — matching other Atlas outbound surfaces
# that already set ``AtlasAgent/<version>`` for platform-partner attribution.
try:
    from atlas_cli import __version__ as _ATLAS_VERSION
except Exception:
    _ATLAS_VERSION = "unknown"
_ATLAS_SLACK_USER_AGENT_PREFIX = f"AtlasAgent/{_ATLAS_VERSION}"
...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if not raw_token:
            logger.error(
                ...
                "error; set SLACK_BOT_TOKEN via `atlas gateway setup` "
                "or in the active profile's ~/.atlas/.env file, then restart "
                "the gateway.",
            )
            self._set_fatal_error(
                "missing_slack_bot_token",
                "SLACK_BOT_TOKEN not configured. Use `atlas gateway setup` "
                "or add it to your active profile's ~/.atlas/.env file, "
        ...
        if not app_token:
            logger.error(
                ...
                "error; set SLACK_APP_TOKEN via `atlas gateway setup` "
                "or in the active profile's ~/.atlas/.env file, then restart "
                "the gateway.",
            )
            self._set_fatal_error(
                "missing_slack_app_token",
                "SLACK_APP_TOKEN not configured. Use `atlas gateway setup` "
                "or add it to your active profile's ~/.atlas/.env file, "
                "then restart the gateway.",
                retryable=False,
            )
        ...
        try:
            ...
            primary_client = AsyncWebClient(
                token=primary_token,
                user_agent_prefix=_ATLAS_SLACK_USER_AGENT_PREFIX,
            )
            ...
            # Atlas has no listener for (e.g. user_change,
            ...
            async def handle_unhandled_event(event, body, logger):
                logger.debug(
                    ...
                    "registered; subscribed events not handled by Atlas can "
                    "be removed from the Slack app manifest via "
                    "`atlas slack manifest`)",
                    (event or {}).get(
                        "type",
            ...
            self._app.action("atlas_feedback")(self._handle_feedback_action)
            ...
            self._app.action(
                _re.compile(r"^atlas_clarify_choice_\d+$")
            )(self._handle_clarify_action)
            self._app.action("atlas_clarify_other")(self._handle_clarify_action)
            ...
            if _allow_bots_cfg != "none":
                logger.info(
                    ...
                    "'atlas slack manifest' if unsure), and (b) the other bot's "
    ...
    async def _handle_app_context_changed(
        ...
        # Atlas owns. Do not write it into _channel_team: channel IDs can be
    ...
    async def _handle_slack_reaction(self, event: dict, removed: bool = False) -> None:
        ...
        if target_channel:
            ...
            synthetic["_atlas_reaction_source_channel"] = channel_id
            if target_thread:
                synthetic["thread_ts"] = target_thread
            else:
                synthetic.pop("thread_ts", None)
                synthetic["_atlas_no_thread_response"] = True
    ...
    async def _handle_slack_message(
        ...
        if is_dm:
            ...
            if not thread_ts and self._dm_top_level_threads_as_sessions():
                thread_ts = ts
        elif event.get("_atlas_no_thread_response"):
        ...
        force_process = bool(event.get("_atlas_force_process"))
        ...
        # the shape produced by peer Atlas agents in Socket Mode on some
    ...
    async def send_clarify(
        ...
        (unique ``atlas_clarify_choice_<idx>`` action_id, ``value`` packs
        ``clarify_id|idx``) plus a final "✏️ Other…" button
        (``atlas_clarify_other``).  A choice click resolves the clarify
        ...
        try:
            ...
            for idx, choice in enumerate(choices):
                ...
                elements.append({
                    ...
                    "action_id": f"atlas_clarify_choice_{idx}",
                    "value": f"{clarify_id}|{idx}",
                })
            elements.append({
                "type": "button",
                "text": {"type": "plain_text", "text": "✏️ Other…", "emoji": True},
                "action_id": "atlas_clarify_other",
    ...
    async def _handle_clarify_action(self, ack, body, action) -> None:
        ...
        if action_id == "atlas_clarify_other" or token == "other":
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        _tokens_file = get_atlas_home() / "slack_tokens.json"
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `atlas setup` to install Slack support.",
