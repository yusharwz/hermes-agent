...
Auto-tracks and cleans up ephemeral files created during Atlas Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$ATLAS_HOME` and `/tmp/atlas-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `ATLAS_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `ATLAS_HOME` or `/tmp/atlas-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$ATLAS_HOME/disk-cleanup/` is itself excluded
- `$ATLAS_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
