...
Let the atlas agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where atlas runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `atlas meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `atlas meet` CLI |
...
| `cli.py` | `atlas meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$ATLAS_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `atlas meet node {run,list,approve,remove,status,ping}` |
...
atlas meet install                                      # pip + Chromium
atlas meet setup                                        # preflight
atlas meet auth                                         # optional
atlas meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
atlas meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.atlas/.env
atlas meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
atlas meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
atlas meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.atlas/.env
atlas meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, atlas will **not** switch your system audio input automatically — the
...
atlas plugins enable google_meet
atlas meet node run --display-name my-mac --host 0.0.0.0 --port 18789
