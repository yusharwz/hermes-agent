...
Uses a throwaway ATLAS_HOME so it never touches ~/.atlas. No real names.
"""
...
def _fresh_home():
    """Point ATLAS_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_atlas_home() at import time)."""
...
# Import AFTER ATLAS_HOME is set.
...
# a different get_atlas_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"ATLAS_HOME (throwaway): {HOME}")
