...
def _reset_logging_state():
    ...
    atlas_logging._reset_queued_handlers()
...
class TestSetupLogging:
    ...
    def test_creates_agent_log_handler(self, atlas_home):
        ...
        agent_handlers = [
            h for h in atlas_logging.rotating_file_handlers()
...
class TestAddRotatingHandler:
    ...
    def test_no_session_filter_on_handler(self, tmp_path):
        ...
        handlers = [h for h in atlas_logging.rotating_file_handlers() if isinstance(h, RotatingFileHandler)]
...
class TestAsyncQueueLogging:
    ...
    def test_file_handlers_not_on_root(self, atlas_home):
        atlas_logging.setup_logging(atlas_home=atlas_home)
        root = logging.getLogger()
        ...
        queue_handlers = [
            h for h in root.handlers if getattr(h, "_atlas_queue", False)
        ]
        assert len(queue_handlers) == 1
        # The real file handlers are discoverable via the accessor.
        assert any(
            "agent.log" in getattr(h, "baseFilename", "")
            for h in atlas_logging.rotating_file_handlers()
        )
