...
def test_registered_observable_metric_names_cover_snapshot_metrics(monkeypatch):
    ...
    gateway_snapshot = type("S", (), {"metrics": [
        _M("atlas.gateway.up"), _M("atlas.gateway.active_agents"),
        _M("atlas.gateway.busy"), _M("atlas.gateway.drainable"),
        _M("atlas.gateway.restart_requested"),
        _M("atlas.platform.up"), _M("atlas.platform.degraded"),
    ]})()
    cron_snapshot = type("S", (), {"metrics": [
        _M("atlas.cron.scheduler.heartbeat_age_seconds"),
        _M("atlas.cron.scheduler.last_success_age_seconds"),
        _M("atlas.cron.scheduler.catch_up_occurrences"),
        _M("atlas.cron.jobs.enabled"), _M("atlas.cron.jobs.running"),
        _M("atlas.cron.jobs.overdue"),
    ]})()
...
def test_monitoring_docs_distinguish_relay_health_scope_and_terminal_flush():
    ...
    assert "Atlas Agent-owned Relay transport health" in text
