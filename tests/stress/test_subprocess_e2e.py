...
This validates the IPC + lifecycle story that mocks can't:
  ...
  - the child process resolves atlas_cli.kanban_db on its own
  ...
  - worker logs are captured to ATLAS_HOME/kanban/logs/<task>.log
    ...
    def _spawn(task, workspace):
        ...
        env = {
            **os.environ,
            ...
            "HOME": home,
            "PYTHONPATH": WT,
            "ATLAS_KANBAN_TASK": task.id,
            "ATLAS_KANBAN_WORKSPACE": workspace,
...
def main():
    home = tempfile.mkdtemp(prefix="atlas_e2e_")
    os.environ["ATLAS_HOME"] = home
    os.environ["HOME"] = home
    sys.path.insert(0, WT)
    from atlas_cli import kanban_db as kb

    # Point the `atlas` CLI child processes will run at the worktree
    # atlas_cli.main. We do this by putting a shim on PATH.
    shim_dir = os.path.join(home, "bin")
    os.makedirs(shim_dir, exist_ok=True)
    shim_path = os.path.join(shim_dir, "atlas")
    with open(shim_path, "w") as f:
        f.write(f"""#!/bin/sh
exec {PY} -m atlas_cli.main "$@"
""")
    ...
    print("=" * 60)
    # Scenario A workers wrote to /tmp/atlas_e2e_*/worker_*.log
    import glob
