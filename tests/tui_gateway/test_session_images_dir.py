...
before ``prompt.submit`` installs the session's profile ATLAS_HOME override, so
...
def test_launch_home_fallback_when_no_profile(tmp_path):
    ...
    launch_home = tmp_path / ".atlas"
    session = {}

    with patch("tui_gateway.server._atlas_home", launch_home):
...
def test_two_profiles_are_isolated(tmp_path):
    ...
    home_a = tmp_path / ".atlas" / "profiles" / "a"
    home_b = tmp_path / ".atlas" / "profiles" / "b"
