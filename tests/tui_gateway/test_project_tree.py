...
def test_broad_default_non_git_cwd_stays_unscoped():
    detached = _session("/home/test/.atlas")
    ...
    tree = pt.build_tree(
        ...
        hydrate=True,
        is_junk_cwd=lambda path: path in {"/home/test", "/home/test/.atlas"},
    )
...
def test_deleted_sibling_worktree_folds_into_parent_home_checkout():
    ...
    resolve = _resolver({"/www/atlas-agent": ("/www/atlas-agent", "/www/atlas-agent")})
    sessions = [
        _session("/www/atlas-agent", branch="main"),
        _session("/www/atlas-agent-session-links"),
    ]

    tree = pt.build_tree([], sessions, [], resolve, hydrate=True)
    project = tree["projects"][0]

    assert [p["id"] for p in tree["projects"]] == ["/www/atlas-agent"]
    assert _lane_ids(project) == ["/www/atlas-agent::branch::main"]
    main = project["repos"][0]["groups"][0]
    assert main["isMain"] and main["path"] == "/www/atlas-agent"
