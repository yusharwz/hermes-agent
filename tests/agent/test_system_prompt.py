...
def test_coding_prompt_preserves_legacy_workspace_order(monkeypatch):
    ...
    monkeypatch.setattr(system_prompt, "ATLAS_AGENT_HELP_GUIDANCE", "HELP")
    monkeypatch.setattr(system_prompt, "STEER_CHANNEL_NOTE", "STEER")
    monkeypatch.setattr(system_prompt, "get_atlas_home", lambda: Path("/atlas"))

    expected_profile = (
        "Active Atlas profile: default. Other profiles (if any) live "
        "under /atlas/profiles/<name>/. Each profile has its own skills/, "
    ...
    with (
        ...
        patch("atlas_time.now", return_value=datetime(2026, 1, 2)),
    ):
...
class TestTelegramRichMessagesHint:
    ...
    def test_base_hint_without_rich_messages(self, monkeypatch):
        ...
        with patch("atlas_cli.config.load_config_readonly") as mock_cfg:
