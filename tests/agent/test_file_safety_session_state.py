...
manipulation. Atlas keeps canonical conversation history in state.db and may
...
def fake_homes(tmp_path, monkeypatch):
    """Point ATLAS_HOME at a temp profile dir.

    Uses the real env-var resolution chain (get_atlas_home /
    get_default_atlas_root) instead of monkeypatching private helpers —
    ...
    ATLAS_HOME=<root>/profiles/<name> makes get_default_atlas_root()
