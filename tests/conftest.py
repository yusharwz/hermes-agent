...
# ── Sandbox ATLAS_HOME before ANY test module is imported ──────────────────
# `atlas_cli/main.py` calls `setup_logging()` at MODULE level, which resolves
# `get_atlas_home()` and attaches rotating file handlers to the ROOT logger.
...
# real `~/.atlas/logs/agent.log` and `errors.log`.
#
# The `_isolate_env` fixture below also sandboxes ATLAS_HOME, but fixtures run
...
# the REAL Atlas root — capture it BEFORE the sandbox rewires ATLAS_HOME,
# otherwise the deny-list would point at the throwaway tempdir and the guard
# would silently stop protecting the operator's actual ~/.atlas (#69385).
_PRE_SANDBOX_KANBAN_OVERRIDE = os.environ.get("ATLAS_KANBAN_HOME", "").strip()
_PRE_SANDBOX_ATLAS_HOME = os.environ.get("ATLAS_HOME", "")
if not os.environ.get("ATLAS_HOME"):
    _SESSION_ATLAS_HOME = tempfile.mkdtemp(prefix="atlas-test-home-")
    os.environ["ATLAS_HOME"] = _SESSION_ATLAS_HOME
    atexit.register(shutil.rmtree, _SESSION_ATLAS_HOME, True)

#: ATLAS_HOME as it stood when conftest was imported - i.e. before any test
...
ATLAS_HOME_AT_CONFTEST_IMPORT = os.environ.get("ATLAS_HOME", "")
...
_ATLAS_BEHAVIORAL_VARS = frozenset({
    ...
    # final response text to ``atlas_cli.voice.speak_text`` — real synthesis,
    ...
    # otherwise-unrelated config tests away from the default "atlas" host.
    "ATLAS_HONCHO_HOST",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ATLAS_HONCHO_HOST", "atlas")
    ...
    # 3b. atlas_state computes ``DEFAULT_DB_PATH = get_atlas_home() / "state.db"``
    #     at import time. When the module is first imported at collection (any
    #     test file with a top-level ``from atlas_state import ...``) that
    ...
    atlas_state_mod = sys.modules.get("atlas_state")
    if atlas_state_mod is not None and hasattr(atlas_state_mod, "DEFAULT_DB_PATH"):
        monkeypatch.setattr(
            atlas_state_mod, "DEFAULT_DB_PATH", fake_atlas_home / "state.db"
        )
...
# invocation), kanban writes silently pollute the real ~/.atlas. This autouse
...
# allow-list because test-level fixtures legitimately move ATLAS_HOME to
...
def _capture_real_kanban_root() -> Path:
    ...
    file (before the session ATLAS_HOME sandbox rewired the env), so the
    deny-list keeps pointing at the operator's actual root. Mirrors
    ``kanban_db.kanban_home()`` resolution order:
    1. ``ATLAS_KANBAN_HOME`` env var when set and non-empty
    2. the real (pre-sandbox) Atlas root otherwise
    """
    if _PRE_SANDBOX_KANBAN_OVERRIDE:
        return Path(_PRE_SANDBOX_KANBAN_OVERRIDE).expanduser().resolve()
    if _PRE_SANDBOX_ATLAS_HOME:
        # ATLAS_HOME was genuinely set before the sandbox — honor it via the
        # normal resolver (it may be a profile dir whose root matters).
        from atlas_constants import get_default_atlas_root
        return get_default_atlas_root().resolve()
    # No pre-existing ATLAS_HOME: the real root is the platform default,
    # NOT the sandbox tempdir now sitting in the env.
    return (Path.home() / ".atlas").resolve()
...
def _kanban_write_guard(_hermetic_environment, monkeypatch):
    ...
    ``~/.atlas`` captured at import time. Hermetic tests that legitimately
    move ATLAS_HOME to sibling tempdirs are unaffected.

    Only patches when ``atlas_cli.kanban_db`` is *already imported* — a
    ...
    """
    _kdb = sys.modules.get("atlas_cli.kanban_db")
    if _kdb is None:
        return

    # The sys.modules probe can observe the module MID-IMPORT: a fixture
    # boundary firing while another test's lazy `import atlas_cli.kanban_db`
...
def _reset_tui_gateway_server_state():
    ...
    # A leaked context-local Atlas home override redirects every later
    # ``get_atlas_home()`` call (active-session registry, config paths)
    # to a stale per-test tmpdir. Force the main-thread ContextVar back
    # to its default.
    try:
        from atlas_constants import get_atlas_home_override, set_atlas_home_override

        if get_atlas_home_override() is not None:
            set_atlas_home_override(None)
    except Exception:
        pass
...
def _wal_is_usable() -> bool:
    """True when Atlas will actually put a database into WAL mode here.

    Atlas refuses journal_mode=WAL on SQLite builds carrying the upstream
    ...
    running Atlas can link DIFFERENT SQLite versions: a repo ``.venv`` on
    3.50.4 (vulnerable → DELETE) alongside a Atlas managed runtime on 3.53.1
    (fixed → WAL). The same test then passes in one and fails in the other.

    IMPORTANT: this must NOT import ``atlas_state``. That module computes
    ``DEFAULT_DB_PATH`` from ``get_atlas_home()`` at import time, so importing
    it during collection — before the per-test ``_isolate_atlas_home`` fixture
    redirects ``ATLAS_HOME`` — permanently caches the DEVELOPER'S REAL
    ``~/.atlas/state.db`` for the whole session. Tests then read live
    production sessions instead of a tempdir. The version predicate is
    duplicated from ``atlas_state._is_sqlite_wal_reset_vulnerable`` (upstream
...
#      process environment: ``os.environ["ATLAS_VOICE_TTS"] = "1"``. The
...
#      ``atlas_cli.voice.speak_text(final_response)`` on a daemon thread.
...
# to mock it:
#
#  • ``atlas_cli.voice.speak_text`` — the synth+playback entry point both
#    gateway call sites late-import, so patching the module attribute catches
#    them wherever they import it from.
#  • ``atlas_cli.voice.play_audio_file`` — the module-level binding
#    ``speak_text`` actually plays through. Patching the binding inside
#    ``atlas_cli.voice`` (not ``tools.voice_mode``) keeps the real function
...
def pytest_configure(config):  # noqa: D401 — pytest hook
    ...
    config.addinivalue_line(
        ...
        "SQLite WAL mode; skipped on builds where Atlas falls back to "
...
def pytest_collection_modifyitems(config, items):  # noqa: D401 — pytest hook
    ...
    reason = (
        f"SQLite {sqlite3.sqlite_version} has the WAL-reset bug — Atlas uses "
...
def _audio_playback_guard(request, monkeypatch):
    ...
    depth behind ``_ATLAS_BEHAVIORAL_VARS``: the env blanking stops the flag
    ...
    try:
        import atlas_cli.voice as _voice
    except Exception:
