...
from atlas_cli.browser_connect import ChromeDebugLaunch
...
def test_profile_scoped_mcp_discovery_uses_target_home(monkeypatch, tmp_path):
    """MCP discovery must start under the selected profile's ATLAS_HOME."""
    ...
    monkeypatch.setenv("ATLAS_HOME", str(tmp_path / "default"))
    ...
    monkeypatch.setattr(
        ...
        lambda: seen.append(str(get_atlas_home())),
    )
...
def test_prompt_submit_typed_stop_phrase_ends_voice_chat(monkeypatch):
    ...
    monkeypatch.setenv("ATLAS_VOICE", "1")
    monkeypatch.setenv("ATLAS_VOICE_TTS", "1")
    ...
    assert os.environ["ATLAS_VOICE"] == "0"
    assert os.environ["ATLAS_VOICE_TTS"] == "0"
...
def test_wake_owner_is_sticky_and_routes_detection_to_first_transport(monkeypatch):
    ...
    try:
        ...
        assert emitted == [(
            "wake.detected",
            "first-session",
            {"phrase": "hey atlas", "profile": None, "start_new_session": True},
            first,
        )]
...
def test_wake_toggle_persists_enabled_flag_only_on_explicit_gesture(monkeypatch):
    ...
    config = {"enabled": False, "phrase": "hey atlas", "surface": "auto",
              "start_new_session": True}
    persisted = []
...
def test_voice_toggle_tts_branch_also_carries_record_key(monkeypatch):
    ...
    monkeypatch.setenv("ATLAS_VOICE", "1")
    # setenv (not delenv) — the handler writes ATLAS_VOICE_TTS directly, and
    ...
    monkeypatch.setenv("ATLAS_VOICE_TTS", "0")
...
def test_load_enabled_toolsets_rejects_disabled_mcp_env(monkeypatch, capsys):
    ...
    from atlas_cli.tools_config import _RECENTLY_SHIPPED_TOOLSETS
...
def test_config_sync_ignores_env_seed_without_config_model(monkeypatch):
    # `atlas --tui -m <model>` sets ATLAS_MODEL/ATLAS_INFERENCE_MODEL as a
    ...
    # permanently.
    monkeypatch.setenv("ATLAS_MODEL", "one-shot/model")
    monkeypatch.setenv("ATLAS_INFERENCE_MODEL", "one-shot/model")
    monkeypatch.setattr(
...
def test_config_model_target_never_reads_env(monkeypatch):
    monkeypatch.setenv("ATLAS_MODEL", "seed/model")
    monkeypatch.setenv("ATLAS_INFERENCE_MODEL", "seed/model")
...
def test_apply_model_switch_persist_override_false_never_persists(monkeypatch):
    ...
    monkeypatch.setattr(
        "atlas_cli.model_switch.switch_model", lambda **kw: result
    )
    monkeypatch.setattr(
        "atlas_cli.model_switch.resolve_persist_behavior",
...
def test_make_agent_passes_configured_fallback_chain(monkeypatch):
    ...
    monkeypatch.delenv("ATLAS_DESKTOP_TERMINAL", raising=False)
    monkeypatch.setattr(
        server,
        "_load_cfg",
        lambda: {
...
def test_ensure_session_db_row_records_a_terminal_workspace(monkeypatch, tmp_path):
    ...
    The user cd'd there before running atlas. Leaving it null stranded the row
...
def test_ensure_session_db_row_stamps_profile_name(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("atlas_state.SessionDB", _ProfileDB)
    monkeypatch.setattr(server, "_resolve_model", lambda: "test-model")
...
def test_config_get_approval_mode_uses_smart_default_when_key_is_missing(
    ...
    monkeypatch.setattr(server, "_atlas_home", tmp_path)
    # Point the canonical resolver (load_config → env ATLAS_HOME) at the
    # temp home too, so the smart default is asserted against THIS config
    # rather than whatever the developer's real ~/.atlas happens to hold.
...
def test_config_get_approval_mode_fails_safe_to_manual_for_invalid_explicit_value(
    ...
    # tools.approval, which reads via atlas_cli.config.load_config —
    # that path resolves ATLAS_HOME from the environment, not
    # server._atlas_home.
    monkeypatch.setenv("ATLAS_HOME", str(tmp_path))
    (tmp_path / "config.yaml").write_text(
...
def test_config_get_approval_mode_normalizes_yaml_off(tmp_path, monkeypatch):
    ...
    # load_config, which resolves ATLAS_HOME from the environment.
    monkeypatch.setenv("ATLAS_HOME", str(tmp_path))
    (tmp_path / "config.yaml").write_text(
...
def test_config_set_approval_mode_persists_three_way_value_and_emits_live_status(
    ...
    monkeypatch.setattr(server, "_atlas_home", tmp_path)
    # config.set writes via server._atlas_home, but the post-write
    # session.info emit resolves the effective mode through the canonical
    # tools.approval resolver (load_config → env ATLAS_HOME).
    monkeypatch.setenv("ATLAS_HOME", str(tmp_path))
    emitted = []
    monkeypatch.setattr(server, "_emit", lambda *args: emitted.append(args))
...
def test_config_set_model_once_keeps_env_and_records_restore(monkeypatch):
    ...
    server._sessions["sid"] = session
    monkeypatch.setenv("ATLAS_INFERENCE_PROVIDER", "openrouter")
    monkeypatch.setenv("ATLAS_MODEL", "old/model")
    monkeypatch.setattr(
    ...
    try:
        ...
        assert os.environ["ATLAS_INFERENCE_PROVIDER"] == "openrouter"
        assert os.environ["ATLAS_MODEL"] == "old/model"
    finally:
        server._sessions.clear()
...
def test_config_set_model_session_switch_clears_pending_once_restore(monkeypatch):
    ...
    server._sessions["sid"] = session
    monkeypatch.setattr("atlas_cli.model_switch.switch_model", lambda **_kwargs: result)
    monkeypatch.setattr(server, "_restart_slash_worker", lambda *args, **kwargs: None)
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
...
def test_session_list_honors_params_profile_opens_profile_db(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(server, "_get_db", lambda: LaunchDB())
    monkeypatch.setattr("atlas_state.SessionDB", ProfileDB)
...
def test_session_most_recent_honors_params_profile(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(server, "_profile_home", lambda p: profile_home if p == "mlperf" else None)
    monkeypatch.setattr(server, "_get_db", lambda: LaunchDB())
    monkeypatch.setattr("atlas_state.SessionDB", ProfileDB2)
...
def test_model_options_hides_unconfigured_providers_by_default(monkeypatch):
    from atlas_cli.inventory import ConfigContext

    calls = []

    monkeypatch.setattr(server, "_resolve_model", lambda: "")
    monkeypatch.setattr(
        "atlas_cli.inventory.load_picker_context",
    ...
    monkeypatch.setattr(
        "atlas_cli.inventory.build_models_payload",
...
def test_model_options_preserves_canonical_custom_row_after_agent_init(monkeypatch):
    ...
    monkeypatch.setattr(
        "atlas_cli.runtime_provider.canonical_custom_identity",
        canonical,
    )
    monkeypatch.setattr(
        "atlas_cli.model_switch.list_authenticated_providers",
    ...
    monkeypatch.setattr("atlas_cli.inventory._apply_pricing", lambda *_args, **_kwargs: None)
    monkeypatch.setattr("atlas_cli.inventory._apply_capabilities", lambda *_args, **_kwargs: None)
...
def test_model_save_key_uses_credential_lifecycle_and_picker_context(monkeypatch):
    ...
    monkeypatch.setattr(
        "atlas_cli.auth.PROVIDER_REGISTRY",
        {
    ...
    monkeypatch.setattr(
        "atlas_cli.credential_lifecycle.save_provider_env_credential",
        save_credential,
    )
...
def test_browser_manage_connect_default_local_reports_launch_hint(monkeypatch):
    ...
    with patch.dict(sys.modules, {"tools.browser_tool": fake}):
        ...
        with (
            patch(
                "atlas_cli.browser_connect.launch_chrome_debug",
                return_value=ChromeDebugLaunch(),
            ),
            patch("atlas_cli.browser_connect.local_port_in_use", return_value=False),
            patch("atlas_cli.browser_connect.manual_chrome_debug_command", return_value=None),
            patch(
...
def test_browser_manage_connect_squatted_port_launches_on_alternate(monkeypatch):
    ...
    with patch.dict(sys.modules, {"tools.browser_tool": fake}):
        with (
            patch("atlas_cli.browser_connect.launch_chrome_debug", side_effect=_launch),
            patch("atlas_cli.browser_connect.local_port_in_use", return_value=True),
            patch("atlas_cli.browser_connect.find_free_debug_port", return_value=9223),
        ):
            resp = server.handle_request(
                {"id": "1", "method": "browser.manage", "params": {"action": "connect"}}
            )
...
def test_persist_model_switch_preserves_sibling_model_keys(tmp_path, monkeypatch):
    ...
    # save_config_value() resolves the config path from get_atlas_home() (live
    # env var), always targeting ATLAS_HOME/config.yaml — point it at tmp_path.
    monkeypatch.setenv("ATLAS_HOME", str(tmp_path))
...
def test_tts_stream_begin_and_stop_lifecycle(monkeypatch):
    ...
    monkeypatch.setenv("ATLAS_VOICE_TTS", "1")
    monkeypatch.setenv("ATLAS_VOICE", "0")  # no barge-in monitor (no mic)
    playback_stops: list = []
...
def test_full_duplex_stop_phrase_mid_generation_ends_voice_chat(monkeypatch, tmp_path):
    ...
    assert os.environ.get("ATLAS_VOICE") == "0"  # voice chat ended
...
def test_build_persist_message_quotes_paths_containing_spaces(tmp_path):
    ...
    img_dir = tmp_path / "Application Support" / "Atlas" / "composer-images"
