...
def test_every_lazy_deps_exact_pin_matches_uv_lock():
    ...
    install ships the locked version, and the ``atlas update`` lazy-refresh
    ...
    assert not drift, (
        ...
        "every package the core lock also ships — otherwise `atlas update` "
...
def test_huggingface_hub_lazy_pin_matches_uv_lock():
    ...
    a feature from mere package presence, the `atlas update` lazy-refresh
    ...
    assert lazy_pin == locked, (
        ...
        "huggingface-hub`), or `atlas update` will churn/downgrade the "
