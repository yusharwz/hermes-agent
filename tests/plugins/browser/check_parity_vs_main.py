...
Run from the PR worktree:
    ...
    cd ~/.atlas/atlas-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.atlas/atlas-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.atlas/atlas-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a atlas-agent checkout"
)
assert (PR_DIR / "tools" / "browser_tool.py").exists(), (
    f"PR_DIR={PR_DIR} doesn't look like a atlas-agent checkout"
)
...
# Isolated ATLAS_HOME for the config write.
