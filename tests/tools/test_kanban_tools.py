...
def test_unblock_with_pending_parents_returns_todo(monkeypatch, tmp_path):
    ...
    home.mkdir()
    monkeypatch.setenv("ATLAS_HOME", str(home))
    monkeypatch.setenv("ATLAS_PROFILE", "orchestrator")
    from pathlib import Path as _Path
    monkeypatch.setattr(_Path, "home", lambda: tmp_path)
...
def allow_private_urls(monkeypatch):
    ...
    Mirrors a user setting ATLAS_ALLOW_PRIVATE_URLS on a private network.
...
def default_url_guard(monkeypatch):
    ...
    Clears ATLAS_ALLOW_PRIVATE_URLS and resets url_safety's process-lifetime
