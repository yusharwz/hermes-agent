...
# boundary, blocking `atlas send` message bodies, multi-line
# `git commit -m` messages, and heredoc text that merely MENTION
# shutdown/reboot commands.
_QUOTED_NEWLINE_DATA_ALLOW = [
    # atlas send with a multi-line message body (the reported symptom)
    'atlas send -t telegram -s "spark1" "console output:\nsudo reboot\ndone"',
    'atlas send -t telegram "line1\nshutdown -h now\nline3"',
...
_QUOTED_NEWLINE_THREATS_BLOCK = [
    ...
    'atlas send -t telegram "$(sudo reboot)"',
    'echo "`shutdown -h now`"',
...
def test_quoted_newline_data_not_blocked_by_full_guard_chain(clean_session):
    ...
    command = 'atlas send -t telegram "status:\nsudo reboot happened at 3am"'
