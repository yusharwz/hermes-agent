...
class TestFirecrawlEnvResolution:
    """Verify Firecrawl reads env values from atlas_cli.config.get_env_value,
    not just os.getenv.  This catches the regression reported in #40190 where
    values stored in ~/.atlas/.env were invisible to the provider."""
...
class TestSiblingProvidersEnvResolution:
    ...
    ~/.atlas/.env), not bare os.getenv. Parametrized over the four
