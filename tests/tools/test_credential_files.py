...
class TestCacheDirectoryMounts:
    ...
    def test_images_upload_dir_is_mounted(self, tmp_path, monkeypatch):
        ...
        Desktop / clipboard / PDF uploads land in ``ATLAS_HOME/images``, not
        ...
        """
        atlas_home = tmp_path / ".atlas"
        (atlas_home / "images").mkdir(parents=True)
        monkeypatch.setenv("ATLAS_HOME", str(atlas_home))

        mounts = get_cache_directory_mounts()
        by_container = {m["container_path"]: m["host_path"] for m in mounts}
        assert "/root/.atlas/images" in by_container
        assert by_container["/root/.atlas/images"] == str(atlas_home / "images")
    ...
    def test_images_upload_file_maps_into_container(self, tmp_path, monkeypatch):
        ...
        atlas_home = tmp_path / ".atlas"
        (atlas_home / "images").mkdir(parents=True)
        upload = atlas_home / "images" / "upload_20260722_181019_1.png"
        upload.write_bytes(bytes.fromhex("89504e470d0a1a0a"))
        monkeypatch.setenv("ATLAS_HOME", str(atlas_home))

        assert (
            map_cache_path_to_container(str(upload))
            == "/root/.atlas/images/upload_20260722_181019_1.png"
        )
...
class TestMasterCredentialStoresAreNeverMountable:
    """Containment is not enough — ATLAS_HOME *is* where the keys live.
    ...
    ``../../.ssh/id_rsa`` from escaping ATLAS_HOME, but every master
    ...
    def test_master_credential_store_is_refused(self, tmp_path, rel_path):
        ...
        with patch.dict(os.environ, {"ATLAS_HOME": str(home)}):
    ...
    def test_refused_entry_does_not_block_the_rest_of_the_batch(self, tmp_path):
        ...
        paths = [m["container_path"] for m in mounts]
        assert "/root/.atlas/google_token.json" in paths
        assert "/root/.atlas/.env" not in paths
