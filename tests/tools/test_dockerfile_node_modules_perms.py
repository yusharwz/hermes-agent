...
Older images made /opt/atlas/ui-tui and /opt/atlas/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV ATLAS_TUI_DIR=/opt/atlas/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R atlas:atlas /opt/atlas/ui-tui" not in text
    assert "chown -R atlas:atlas /opt/atlas/node_modules" not in text
