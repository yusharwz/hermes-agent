...
def test_output_path_rejects_atlas_oauth_store(tmp_path, monkeypatch):
    """TTS output_path must not bypass the shared protected-file write guard."""
    import agent.file_safety as file_safety

    atlas_home = tmp_path / "atlas-home"
    atlas_home.mkdir()
    monkeypatch.setattr(file_safety, "_atlas_home_path", lambda: atlas_home)
    monkeypatch.setattr(file_safety, "_atlas_root_path", lambda: atlas_home)

    target = atlas_home / ".anthropic_oauth.json"
    result = json.loads(text_to_speech_tool(
        text="hello",
...
def test_output_path_rejects_mcp_token_directory(tmp_path, monkeypatch):
    ...
    atlas_home = tmp_path / "atlas-home"
    token_dir = atlas_home / "mcp-tokens"
