...
class GetSpillConfigTests(unittest.TestCase):
    ...
    def test_load_config_exception_is_swallowed(self):
        with patch("atlas_cli.config.load_config", side_effect=RuntimeError("bad")):
            cfg = hos.get_spill_config()
        self.assertEqual(cfg["max_chars"], hos.DEFAULT_MAX_CHARS)
        self.assertTrue(cfg["enabled"])
...
class SpillIfOversizedTests(unittest.TestCase):
    def setUp(self):
        self.tmpdir = tempfile.mkdtemp(prefix="atlas-spill-test-")
    ...
    def test_default_directory_uses_atlas_home(self):
        """When no directory override, spill under ATLAS_HOME/hook_outputs."""
        test_home = tempfile.mkdtemp(prefix="atlas-home-")
        try:
            with patch.dict(os.environ, {"ATLAS_HOME": test_home}):
                # Also patch get_atlas_home to the env var to mirror production.
            ...
            # ~/.atlas/hook_outputs depending on get_atlas_home behaviour.
            candidates = [
                Path(test_home) / "hook_outputs" / "sess",
                Path(os.path.expanduser("~/.atlas/hook_outputs/sess")),
            ]
