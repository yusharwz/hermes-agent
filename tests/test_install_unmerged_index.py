...
The ``atlas update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_removing_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m atlas_cli.main gateway run`` — image name
    ``pythonw``, not ``atlas.exe`` — so the ``taskkill /IM atlas.exe`` guard
    ...
    # The atlas.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM atlas.exe' in text
