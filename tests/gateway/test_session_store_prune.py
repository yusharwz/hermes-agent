...
def test_session_store_default_db_uses_runtime_atlas_home(tmp_path, monkeypatch):
    """SessionStore must honor runtime ATLAS_HOME when opening the default DB.

    Regression for the import-time DEFAULT_DB_PATH freeze: importing
    atlas_state before a fixture redirected ATLAS_HOME used to pin every
    default SessionDB() at the developer's real ~/.atlas/state.db.
    """
    config = GatewayConfig(default_reset_policy=SessionResetPolicy(mode="none"))
    fake_home = tmp_path / "alt_atlas_home"
    fake_home.mkdir()
