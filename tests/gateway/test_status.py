...
class TestGatewayPidState:
    ...
    def test_gateway_identity_files_use_process_home_not_context_override(
        ...
        """Regression: pid/lock/state files must use process-level ATLAS_HOME.
        ...
        the process-level ATLAS_HOME, not the profile's directory.  See #56986.
        """
...
class TestGatewayRuntimeStatus:
    ...
    def test_command_line_belongs_to_profile_normalizes_separators(self):
        """A Windows argv renders ATLAS_HOME with backslashes while the
        ...
        cmdline = r"atlas_home=c:\opt\data\profiles\coder atlas gateway run --replace"
...
class TestScopedLockTakeover:
    ...
    def test_verified_distinct_home_handoff_marks_target_before_sigterm(
        ...
        def terminate(pid, *, force=False):
            ...
            assert payload["target_atlas_home"] == str(target_home)
            assert payload["replacer_atlas_home"] == str(replacer_home)
...
class TestLaunchdPlistRespawnGovernance:
    def test_plist_has_throttle_interval(self, tmp_path, monkeypatch):
        monkeypatch.setenv("ATLAS_HOME", str(tmp_path))
        from atlas_cli.gateway import generate_launchd_plist
...
class TestResolveGatewayLiveness:
    ...
    def test_profile_dir_scopes_every_read_to_that_profile(self, tmp_path):
        ...
        ignore the ATLAS_HOME contextvar override (#56986), so the profile
