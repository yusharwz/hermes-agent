...
class TestSlackConnectCleanup:
    ...
    async def test_disconnect_closes_workspace_clients_and_clears_runtime_state(self):
        ...
        ``atlas gateway run --replace`` takes the old process through the
...
class TestSlackProxyBehavior:
    ...
    async def test_connect_uses_proxy_when_not_bypassed(self):
        ...
        assert "atlas_feedback" in created_apps[0].registered_actions
        assert "atlas_clarify_other" in created_apps[0].registered_actions
        ...
        assert any(
            pattern.fullmatch("atlas_clarify_choice_0")
            for pattern in clarify_choice_patterns
        )
        assert not any(
            pattern.fullmatch("atlas_clarify_choice")
            for pattern in clarify_choice_patterns
        )
...
class TestMissingCredentials:
    ...
    async def test_missing_app_token_sets_fatal_error(self):
        ...
        assert "atlas gateway setup" in fatal_errors[0]["message"].lower() or ".env" in fatal_errors[0]["message"]
...
class TestTrackingStructureBounds:
    ...
    async def test_slash_command_contexts_bounded(self, adapter):
        ...
        adapter.handle_atlas_command = AsyncMock(return_value=None)
        for i in range(10):
            command = {
                "command": "/atlas",
                "text": "/status",
...
class TestThreadImageContext:
    ...
    def _prep(self, adapter_with_session_store):
        ...
        a._download_slack_file = AsyncMock(return_value="/tmp/atlas-cached.png")
        return a
    ...
    async def test_cold_start_delivers_thread_root_image(
        ...
        assert msg_event.media_urls == ["/tmp/atlas-cached.png"]
...
class TestSlackUserAgent:
    ...
    ``user_agent_prefix=_ATLAS_SLACK_USER_AGENT_PREFIX`` on every
    ...
    def test_atlas_slack_user_agent_prefix_format(self):
        """Module constant matches the AtlasAgent/<version> convention used
        elsewhere in the codebase for platform-partner attribution."""
        assert _slack_mod._ATLAS_SLACK_USER_AGENT_PREFIX.startswith("AtlasAgent/")
