...
def test_media_delivery_denies_encrypted_bitwarden_cache(tmp_path, monkeypatch):
    ...
    atlas_home.mkdir()
    monkeypatch.setattr(base, "_ATLAS_HOME", atlas_home)
    monkeypatch.setattr(base, "_ATLAS_ROOT", atlas_home)
    path = atlas_home / "cache" / "bws_cache.enc.json"
    path.parent.mkdir()
...
class TestMediaDeliveryPathValidation:
    ...
    def test_allows_stale_kanban_attachment_but_not_neighboring_workspace(
        ...
        monkeypatch.setenv("ATLAS_KANBAN_HOME", str(tmp_path / "atlas"))
        monkeypatch.setenv("ATLAS_MEDIA_TRUST_RECENT_FILES", "0")
        board_root = tmp_path / "atlas" / "kanban" / "boards" / "research"
