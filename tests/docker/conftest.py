...
Override the image with ``ATLAS_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("ATLAS_TEST_IMAGE", "atlas-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``ATLAS_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("ATLAS_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged atlas user
# ---------------------------------------------------------------------------
#
# Background: every Atlas runtime path inside the container drops to UID
# 10000 (the ``atlas`` user) via ``s6-setuidgid atlas``. ``docker exec``
...
# fails (PermissionError swallowed) for atlas, so a test that ran as root
...
# helpers below run every probe under ``-u atlas`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: atlas).
