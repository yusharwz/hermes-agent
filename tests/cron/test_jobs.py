...
class TestGetDueJobs:
    ...
    def test_one_shot_not_redispatched_while_running(self, tmp_cron_dir, monkeypatch):
        ...
        for gap in (28, 61, 130):
            monkeypatch.setattr("cron.jobs._atlas_now",
    ...
    def test_run_claim_heartbeat_keeps_long_run_claimed_past_ttl(
        ...
        monkeypatch.delenv("ATLAS_CRON_TIMEOUT", raising=False)
        from cron.jobs import _atlas_now, _oneshot_run_claim_ttl_seconds
...
class TestLateEnvRepointScopesStore:
    """A ATLAS_HOME set AFTER cron.jobs import must scope the store even
    ...
    def test_use_cron_store_override_still_wins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ATLAS_HOME", str(tmp_path / "env-home"))
    ...
    def test_public_io_after_late_env_repoint_leaves_old_file_untouched(
        ...
        called after a post-import ATLAS_HOME repoint must operate on the NEW
        ...
        monkeypatch.setattr(jobs, "ATLAS_DIR", sim_old_home)
        ...
        monkeypatch.setenv("ATLAS_HOME", str(new_home))
