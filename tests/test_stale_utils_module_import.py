...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.atlas/atlas-agent/utils.py)
...
2. ``atlas update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``atlas_cli/main.py`` (the ``atlas update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``atlas_constants`` after the
