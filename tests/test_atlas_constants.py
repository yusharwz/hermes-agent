...
class TestGetProcessAtlasHome:
    """Tests for get_process_atlas_home() — process launch scope.
    ...
    def test_env_set_returns_that_path(self, tmp_path, monkeypatch):
        ...
        assert get_process_atlas_home() == home
...
class TestResolvePerModelReasoningEffort:
    ...
    def test_exact_match(self):
        ...
        from atlas_constants import resolve_per_model_reasoning_effort
...
class TestWslPathTranslation:
    ...
    def test_windows_drive_to_wsl_mount(self):
        assert atlas_constants.windows_path_to_wsl(r"C:\Users\alex") == "/mnt/c/Users/alex"
        assert atlas_constants.windows_path_to_wsl("C:/Users/alex") == "/mnt/c/Users/alex"
        assert atlas_constants.windows_path_to_wsl("D:\\") == "/mnt/d/"

    def test_windows_drive_ignores_non_drive_paths(self):
        assert atlas_constants.windows_path_to_wsl("/home/alex") is None
        assert atlas_constants.windows_path_to_wsl("relative\\dir") is None




    def test_translate_maps_windows_and_unc_on_wsl(self, monkeypatch):
        monkeypatch.setattr(atlas_constants, "is_wsl", lambda: True)
        assert atlas_constants.translate_cwd_for_wsl_backend(r"C:\Users\alex") == "/mnt/c/Users/alex"
        assert atlas_constants.translate_cwd_for_wsl_backend(r"\\wsl.localhost\Ubuntu\home\alex") == "/home/alex"
        # Already-POSIX paths pass through untouched.
        assert atlas_constants.translate_cwd_for_wsl_backend("/home/alex") == "/home/alex"
...
class TestWhatsAppSessionDir:
    ...
    the desktop pairing endpoint, and ``atlas whatsapp``. The last hardcoded
    ...
    def test_new_installs_get_the_consolidated_path(self, tmp_path, monkeypatch):
        monkeypatch.setenv("ATLAS_HOME", str(tmp_path))
        assert atlas_constants.get_whatsapp_session_dir() == (
    ...
    def test_an_existing_legacy_pairing_is_kept(self, tmp_path, monkeypatch):
        ...
        assert atlas_constants.get_whatsapp_session_dir() == legacy

    def test_an_empty_legacy_directory_does_not_shadow_a_real_pairing(
        self, tmp_path, monkeypatch
    ):
        """``atlas whatsapp`` used to mkdir the legacy path before reading it."""
        monkeypatch.setenv("ATLAS_HOME", str(tmp_path))
        ...
        assert atlas_constants.get_whatsapp_session_dir() == current
    ...
    def test_asking_does_not_create_the_directory(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ATLAS_HOME", str(tmp_path))
        resolved = atlas_constants.get_whatsapp_session_dir()
    ...
    def test_every_caller_agrees(self, tmp_path, monkeypatch):
        ...
        assert web_server._whatsapp_session_path() == (
            atlas_constants.get_whatsapp_session_dir()
        )
...
class TestWhatsAppLinkedState:
    ...
    def _session(self, tmp_path, monkeypatch):
        monkeypatch.setenv("ATLAS_HOME", str(tmp_path))
        session = atlas_constants.get_whatsapp_session_dir()
        session.mkdir(parents=True)
        return session

    def test_no_session_is_not_linked(self, tmp_path, monkeypatch):
        monkeypatch.setenv("ATLAS_HOME", str(tmp_path))
        assert atlas_constants.whatsapp_session_is_linked() is False
    ...
    def test_credentials_alone_are_linked(self, tmp_path, monkeypatch):
        ...
        assert atlas_constants.whatsapp_session_is_linked() is True
    ...
    def test_a_revoked_session_is_not_linked(self, tmp_path, monkeypatch):
        ...
        session = self._session(tmp_path, monkeypatch)
        (session / "creds.json").write_text("{}")
        (session / atlas_constants.WHATSAPP_LOGGED_OUT_MARKER).write_text(
    ...
    def test_re_pairing_clears_the_verdict(self, tmp_path, monkeypatch):
        ...
        session = self._session(tmp_path, monkeypatch)
        (session / "creds.json").write_text("{}")
        marker = session / atlas_constants.WHATSAPP_LOGGED_OUT_MARKER
        marker.write_text("{}")
        assert atlas_constants.whatsapp_session_is_linked() is False
