...
class TestCredentialPoolEndpoints:
    ...
    def test_env_seeded_delete_stays_deleted(self):
        ...
        load_pool() re-seeds from ~/.atlas/.env on every call, so removing
        just the pool row silently reverts on the next dashboard refresh.
        The endpoint must mirror `atlas auth remove`: clean up the backing
    ...
    def test_post_readd_lifts_suppression(self):
        ...
        Mirrors `atlas auth add`, which clears every suppression for the
...
class TestPairingEndpoints:
    ...
    def test_pairing_is_isolated_per_profile(self):
        ...
        (get_atlas_home() / "profiles" / "work").mkdir(parents=True, exist_ok=True)
...
# ---------------------------------------------------------------------------
# _spawn_atlas_action env scrubbing (#52470)
# ---------------------------------------------------------------------------

def test_spawn_atlas_action_scrubs_gateway_loop_guard_env(monkeypatch, tmp_path):
    """The dashboard runs inside the gateway, so os.environ has
    _ATLAS_GATEWAY=1. Spawned actions (e.g. `gateway restart`) must NOT inherit
    ...
    ws._spawn_atlas_action(["gateway", "restart"], "gateway-restart")

    assert "_ATLAS_GATEWAY" not in captured["env"]
    assert captured["env"]["ATLAS_NONINTERACTIVE"] == "1"
