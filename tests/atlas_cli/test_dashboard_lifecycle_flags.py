...
class TestDashboardStatus:
    def test_status_no_processes(self, capsys):
        with patch("atlas_cli.main._scan_dashboard_processes", return_value=[]), \
    ...
    def test_status_with_processes(self, capsys):
        processes = [
            (12345, "atlas dashboard --port 9119"),
            (12346, "python -m atlas_cli.main dashboard --host 0.0.0.0 --port 9120"),
        ]
        with patch("atlas_cli.main._scan_dashboard_processes", return_value=processes), \
             patch("gateway.status._pid_exists", return_value=True), \
             patch("atlas_cli.main._dashboard_listening", return_value=True), \
             pytest.raises(SystemExit) as exc:
            cmd_dashboard(_ns(status=True))
