...
On hosts with many profile-backed ``atlas-gateway*.service`` units,
``atlas update`` used to wrap the entire per-scope unit loop in a single
...
class TestFleetRestartTimeoutIsolation:
    def test_timeout_on_middle_unit_continues_remaining_units(self):
        units = [
            "atlas-gateway-xiaomo1",
            "atlas-gateway-xiaomo2",
            "atlas-gateway-xiaomo3",
            "atlas-gateway-xiaomo4",
            "atlas-gateway-xiaomo5",
            "atlas-gateway-xiaomo6",
            "atlas-gateway-xiaomo7",
            "atlas-gateway",
        ]
        restarted: list[str] = []
        failed: list[str] = []
        ...
        def process_unit(svc_name: str) -> None:
            if svc_name == "atlas-gateway-xiaomo5":
        ...
        assert failed == ["atlas-gateway-xiaomo5"]
        assert restarted == [
            "atlas-gateway-xiaomo1",
            "atlas-gateway-xiaomo2",
            "atlas-gateway-xiaomo3",
        ...
        assert timeout_cmds == [
            ["systemctl", "--user", "--no-ask-password", "restart", "atlas-gateway-xiaomo5"]
        ]
    ...
    def test_non_gateway_units_in_list_output_are_ignored(self):
        ...
        _for_each_systemd_gateway_unit(
            "\n".join(
                [
                    ...
                    "atlas-gateway-coder.service loaded active running",
        ...
        assert seen == ["atlas-gateway-coder"]
    ...
    def test_process_errors_other_than_timeout_still_propagate(self):
        ...
        with pytest.raises(RuntimeError, match="not a timeout"):
            _for_each_systemd_gateway_unit(
                _list_units_stdout(["atlas-gateway"]),
                process_unit=process_unit,
...
class TestIncompleteFleetRestartWarning:
    def test_warns_with_exact_unrestarted_units(self, capsys):
        _warn_incomplete_gateway_fleet_restart(
            ["atlas-gateway-xiaomo5", "atlas-gateway-xiaomo6", "atlas-gateway-xiaomo5"]
        )
        out = capsys.readouterr().out
        assert "Update incomplete" in out
        assert out.count("atlas-gateway-xiaomo5") == 1
        assert "atlas-gateway-xiaomo6" in out
