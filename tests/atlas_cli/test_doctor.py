...
class TestDoctorPlatformHints:
    ...
    def test_sqlite_upgrade_hint_recreates_docker_containers(self, monkeypatch):
        ...
        assert "docker pull nousresearch/atlas-agent:latest" in hint
        assert "recreate all Atlas containers" in hint
        assert "atlas update" not in hint

    def test_sqlite_upgrade_hint_keeps_git_runtime_repair(self):
        hint = doctor._sqlite_upgrade_hint("git")

        assert "run `atlas update`" in hint
...
def _run_doctor_with_managed_agent_browser(monkeypatch, tmp_path, runnable):
    ...
    Atlas-managed node bin (~/.atlas/node/bin), not on PATH or in
...
class TestGitHubTokenCheck:
    ...
    def _isolate_home(monkeypatch, home):
        """Point doctor at the temp ATLAS_HOME.

        ``run_doctor`` reads the module-level ``ATLAS_HOME`` constant (cached
        at import time), NOT the env var — so ``setenv("ATLAS_HOME")`` alone
        leaves doctor probing the REAL ~/.atlas. On a dev machine with a
...
class TestDoctorDeprecatedConfigAndEnv:
    ...
    def test_atlas_tool_progress_warning_says_unsupported_since_floor(self):
        """ATLAS_TOOL_PROGRESS lost its last consumer (the retired v3→4
        ...
        findings = dict(
            doctor_mod.collect_deprecated_env_vars({"ATLAS_TOOL_PROGRESS": "true"})
        )
        assert "ignored/unsupported since config floor v12" in findings["ATLAS_TOOL_PROGRESS"]
        # The MODE variant is still read by the gateway fallback → keeps the
        # plain deprecation wording.
        mode = dict(
            doctor_mod.collect_deprecated_env_vars({"ATLAS_TOOL_PROGRESS_MODE": "all"})
        )
        assert mode["ATLAS_TOOL_PROGRESS_MODE"] == "display.tool_progress in config.yaml"

    def _run_doctor_with_config(self, monkeypatch, tmp_path, *, config_yaml: str, env_text: str = ""):
        atlas_home = tmp_path / ".atlas"
        atlas_home.mkdir(parents=True)
        (atlas_home / "config.yaml").write_text(config_yaml, encoding="utf-8")
        env_body = env_text if env_text else "OPENAI_API_KEY=sk-test\n"
        (atlas_home / ".env").write_text(env_body, encoding="utf-8")
    ...
    def test_report_does_not_count_as_blocking_issue(self, monkeypatch, tmp_path, capsys):
        ...
        findings = doctor_mod.report_deprecated_config_and_env(
            ...
            {"ATLAS_TOOL_PROGRESS_MODE": "verbose"},
        )
        out = capsys.readouterr().out
        assert len(findings) == 2
        assert "Deprecated: delegation.max_async_children" in out
        assert "Deprecated: ATLAS_TOOL_PROGRESS_MODE" in out
