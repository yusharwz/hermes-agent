...
class TestSharedBoardPaths:
    ...
    def test_dispatcher_spawn_injects_kanban_paths_without_stale_session(
        ...
        # ATLAS_SESSION_* identity inherited from the long-lived gateway.
        # The one exception is ATLAS_SESSION_SOURCE, which the dispatcher
...
def test_connect_falls_back_to_delete_on_locking_protocol(tmp_path, monkeypatch, caplog):
    ...
    import atlas_state as _atlas_state
    monkeypatch.setattr(
        _atlas_state, "is_sqlite_wal_reset_vulnerable",
        lambda version_info=None: False,
    )
    _atlas_state._wal_fallback_warned_paths.clear()
    ...
    with _patch("atlas_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
        with caplog.at_level("ERROR", logger="atlas_state"):
            conn = kb.connect()
...
def test_connect_works_when_wal_is_silently_refused(tmp_path, monkeypatch, caplog):
    ...
    with _patch(
        "atlas_cli.kanban_db.sqlite3.connect",
