...
def test_has_agent_browser_resolves_via_atlas_managed_node_path(monkeypatch, tmp_path):
    """The managed-Node rung: a runnable agent-browser under the Atlas Node
    ...
    monkeypatch.setattr(
        "atlas_constants.with_atlas_node_path", lambda: {"PATH": str(managed_dir)}
    )
    monkeypatch.setattr(
        "atlas_constants.agent_browser_runnable",
