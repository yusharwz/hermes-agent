...
def test_s6_log_run_creates_leaf_as_atlas_without_chown(
    s6_scandir, fake_subprocess_run,
) -> None:
    """log/run must not root-chown/unlink volume paths; create leaf as atlas.

    #45258 parent ownership is stage2's job (``logs/gateways`` seeded as
    atlas). Restartable log/run must not pathname-chown or pathname-rm a
    atlas-writable tree from root — that is a symlink TOCTOU hole.
    """
    ...
    assert not any(line.lstrip().startswith("chown ") for line in log_text.splitlines()), (
        "restartable log/run must not invoke chown on atlas-writable paths; "
        f"saw: {log_text!r}"
    )
    assert 's6-setuidgid atlas mkdir -p "$log_dir"' in log_text
    assert 's6-setuidgid atlas rm -f "$log_dir/lock"' in log_text
    assert 'else\n  mkdir -p "$log_dir"\n  rm -f "$log_dir/lock"\nfi\n' in log_text
    ...
    mkdir_as_atlas_idx = log_text.index('s6-setuidgid atlas mkdir -p "$log_dir"')
    rm_as_atlas_idx = log_text.index('s6-setuidgid atlas rm -f "$log_dir/lock"')
    exec_idx = log_text.index("s6-log 1 ")
    assert mkdir_as_atlas_idx < rm_as_atlas_idx < exec_idx
...
def test_s6_log_run_never_invokes_chown_with_symlinked_log_dir(tmp_path) -> None:
    ...
    gateways = atlas_home / "logs" / "gateways"
    ...
    # Mark the drop so fake rm can refuse unlink outside ATLAS_HOME the way
    # a real atlas uid cannot delete a foreign root-owned lock.
    ...
    (bin_dir / "s6-setuidgid").write_text(
        "#!/bin/sh\n"
        "shift\n"
        'ATLAS_TEST_DROPPED=1 exec "$@"\n',
        encoding="utf-8",
    )
    real_rm = "/bin/rm"
    (bin_dir / "rm").write_text(
        "#!/bin/sh\n"
        # Privilege-dropped: no-op. Models that atlas cannot unlink a foreign
