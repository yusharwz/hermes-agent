...
Canonical owners:
  ...
  * ``atlas_cli/config.py`` — ``load_config()`` / ``load_config_readonly()``
...
ALLOWLIST = {
    ...
    # gateway.run._atlas_home (delegates to read_raw_config otherwise).
    "gateway/run.py",
    # Reads the MANAGED-scope config.yaml (/etc/atlas/...), not the user's —
    # it IS the overlay source; the canonical loaders call into it.
    "atlas_cli/managed_scope.py",
