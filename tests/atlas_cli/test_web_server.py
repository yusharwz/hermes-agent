...
class TestWebServerEndpoints:
    ...
    def test_messaging_platforms_profile_scopes_gateway_reads(self, monkeypatch):
        ...
        ATLAS_HOME contextvar override (#56986), so /api/messaging/platforms
    ...
    def _isolate_honcho_config(self):
        # Honcho tests write the suite-wide ATLAS_HOME honcho.json; snapshot and
        # restore it so provider status/config state never leaks across tests.
        from atlas_constants import get_atlas_home

        path = get_atlas_home() / "honcho.json"
    ...
    def test_put_honcho_writes_host_block_root_and_secret(self, monkeypatch, tmp_path):
        ...
        cfg = json.loads((get_atlas_home() / "honcho.json").read_text(encoding="utf-8"))
        # baseUrl is root-scoped; the rest live in the active host block.
        assert cfg["baseUrl"] == "https://honcho.example.dev"
        assert cfg["hosts"]["atlas"]["workspace"] == "myws"
        assert cfg["hosts"]["atlas"]["peerName"] == "eri"
        assert cfg["hosts"]["atlas"]["environment"] == "local"
        assert cfg["hosts"]["atlas"]["sessionStrategy"] == "per-repo"
        # The key lands where the client reads first; GET keeps it write-only.
        assert cfg["hosts"]["atlas"]["apiKey"] == "hch-test-key"
    ...
    def test_platform_scoped_messaging_env_vars_are_channel_managed(self):
        ...
        tests/atlas_cli/test_setup_hidden_env.py.
        """
    ...
    def test_deleting_the_active_custom_endpoint_clears_its_model_mirror(self):
        ...
        """
        from atlas_cli.config import custom_endpoint_key_env, get_env_value, load_config
    ...
    def test_custom_endpoint_save_leaves_a_hand_written_env_ref_alone(self, monkeypatch):
        ...
        from atlas_cli.config import custom_endpoint_key_env, get_config_path, get_env_value
    ...
    def test_two_endpoints_on_one_host_keep_separate_credentials(self):
        ...
        the first's key.
        """
        from atlas_cli.config import custom_endpoint_key_env, get_env_value
...
class TestBuildSchemaFromConfig:
    ...
    def test_timezone_field_is_searchable_select(self):
        ...
        """
        from atlas_cli.web_server import CONFIG_SCHEMA, _timezone_options
...
class TestNewEndpoints:
    ...
    def test_terminal_ssh_probe_ready_when_configured(self, monkeypatch):
        ...
        config["terminal"]["ssh_user"] = "atlas"
        save_config(config)
        ...
        assert "atlas@devbox.example.com" in ssh["detail"]
...
class TestDiscoverUserThemes:
    ...
    def test_ignores_transient_profile_override(self, tmp_path, monkeypatch):
        ...
        token = set_atlas_home_override(str(other))
        try:
            results = web_server._discover_user_themes()
        finally:
...
class TestThemeBootstrapCSS:
    ...
    @staticmethod
    def _write_theme(atlas_home, name="ocean"):
        themes_dir = atlas_home / "dashboard-themes"
    ...
    def test_user_theme_renders_bundle_vars(self, tmp_path, monkeypatch):
        ...
        assert css.startswith('<style id="atlas-theme-bootstrap">')
...
class TestDashboardPluginManifestExtensions:
    ...
    def test_user_plugins_ignore_profile_home_override(self, tmp_path, monkeypatch):
        ...
        ATLAS_HOME override scopes a request to another profile."""
        from atlas_constants import (
            reset_atlas_home_override,
            set_atlas_home_override,
        )
        ...
        monkeypatch.setenv("ATLAS_HOME", str(launch_home))
        from atlas_cli import web_server
        token = set_atlas_home_override(str(other))
        try:
...
class TestPtyWebSocket:
    ...
    def test_tui_python_command_uses_child_path(self, tmp_path):
        ...
        command = f"atlas-review-python{Path(sys.executable).suffix}"
        ...
        env = {
            "ATLAS_CWD": str(tmp_path),
            "ATLAS_PYTHON": command,
            "PATH": str(bin_dir),
        }

        main_mod._apply_tui_python_env(env)

        assert env["ATLAS_PYTHON"] == command
...
class TestServeIndexMissingIndex:
    ...
    def _client_with_dist(tmp_path, monkeypatch, *, write_index: bool):
        ...
        monkeypatch.setattr(ws, "WEB_DIST", dist)
        monkeypatch.delenv("ATLAS_SERVE_HEADLESS", raising=False)
        spa_app = FastAPI()
        ws.mount_spa(spa_app)
