...
class TestEmptyConfigSections:
    ...
    def test_null_override_of_non_dict_default_still_applies(self, tmp_path):
        ...
        from atlas_cli.config import _deep_merge
...
class TestSanitizeEnvLines:
    ...
    def test_migrate_reports_normalized_line_formatting(self, capsys):
        ...
        with (
            patch("atlas_cli.config.sanitize_env_file", return_value=2),
            patch(
                "atlas_cli.config.check_config_version",
                return_value=(latest_version, latest_version),
            ),
            patch("atlas_cli.config.read_raw_config", return_value={}),
            patch("atlas_cli.config.get_missing_env_vars", return_value=[]),
            patch("atlas_cli.config.get_missing_config_fields", return_value=[]),
            patch("atlas_cli.config.get_missing_skill_config_vars", return_value=[]),
        ):
...
class TestConfigSupportFloor:
    ...
    def test_v11_config_is_refused_and_untouched(self, tmp_path, capsys):
        ...
        assert "run `atlas setup` to regenerate" in captured.out
    ...
    def test_floor_message_uses_display_atlas_home(self):
        from atlas_cli.config_migrations import support_floor_message
        from atlas_constants import display_atlas_home

        msg = support_floor_message()
        assert f"{display_atlas_home()}/config.yaml" in msg
...
class TestCustomProviderCompatibility:
    ...
    def _run_ladder(current_ver: int):
        from atlas_cli.config_migrations import run_migrations
...
"""
    ...
    def test_persist_migration_writes_full_read_raw_config(self, tmp_path):
        from atlas_cli.config import _persist_migration, read_raw_config
    ...
    def test_preserves_existing_codex_app_server_auto_value(self, tmp_path):
        with patch.dict(os.environ, {"ATLAS_HOME": str(tmp_path)}):
            self._write(
                ...
                "_config_version: 31\n"
                "compression:\n"
                "  codex_app_server_auto: atlas\n",
            )

            migrate_config(interactive=False, quiet=True)

            raw = yaml.safe_load((tmp_path / "config.yaml").read_text())
            assert raw["compression"]["codex_app_server_auto"] == "atlas"
