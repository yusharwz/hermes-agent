...
def _setup_update_mocks(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(atlas_main, "_upgrade_pip_before_lazy_refresh", lambda *a, **kw: None)
    monkeypatch.setattr(atlas_main, "_refresh_active_lazy_features", lambda *a, **kw: True)
...
def test_refresh_active_memory_provider_dependencies_reinstalls_active_provider(monkeypatch):
    ...
    monkeypatch.setattr(
        "atlas_cli.memory_setup._install_dependencies",
        lambda provider_name, force=False: recorded.append((provider_name, force)),
    )

    atlas_main._refresh_active_memory_provider_dependencies()
...
def test_reload_updated_runtime_modules_restores_new_atlas_constants_symbol(monkeypatch):
    """A pre-pull module object missing a new helper is repaired by reload."""
    import atlas_constants

    monkeypatch.delattr(atlas_constants, "apply_subprocess_home_env", raising=False)
    assert not hasattr(atlas_constants, "apply_subprocess_home_env")

    atlas_main._reload_updated_runtime_modules()

    assert callable(atlas_constants.apply_subprocess_home_env)
...
def test_update_autostash_survives_undeletable_untracked_dir(tmp_path):
    ...
    (pkg / "atlas-agent.rb").write_text("formula\n")
