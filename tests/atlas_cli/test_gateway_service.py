...
class TestSystemUnitRefreshSyncsAtlasHome:
    """sudo system refresh must not flip TimeoutStopSec via /root/.atlas."""

    def test_refresh_adopts_unit_atlas_home_before_rewriting(self, tmp_path, monkeypatch):
        root_home = tmp_path / "root"
        alice_home = tmp_path / "alice"
        root_atlas = root_home / ".atlas"
        alice_atlas = alice_home / ".atlas"
        root_atlas.mkdir(parents=True)
        alice_atlas.mkdir(parents=True)
        (root_atlas / "config.yaml").write_text(
            "agent:\n  restart_drain_timeout: 60\n", encoding="utf-8"
        )
        (alice_atlas / "config.yaml").write_text(
        ...
        # Correct installed unit (operator's ATLAS_HOME + drain timeout).
        monkeypatch.setenv("ATLAS_HOME", str(alice_atlas))
        ...
        # Simulate sudo without inherited ATLAS_HOME (falls back to root).
        monkeypatch.setenv("ATLAS_HOME", str(root_atlas))
        ...
        assert os.environ["ATLAS_HOME"] == str(alice_atlas)
    ...
    def test_is_current_syncs_before_reading_unit(self, tmp_path, monkeypatch):
        ...
        unit's pinned ATLAS_HOME *before* it reads/compares the unit.
        ...
        monkeypatch.setattr(gateway_cli, "_sync_atlas_home_from_systemd_unit", tracking_sync)
    ...
    def test_start_and_restart_delegate_sync_to_chokepoint(self, monkeypatch):
        ...
        for entry in ("systemd_start", "systemd_restart"):
            ...
            monkeypatch.setattr(
                gateway_cli,
                "_sync_atlas_home_from_systemd_unit",
...
class TestLegacyAtlasUnitDetection:
    ...
    def test_accepts_alternate_execstart_formats(self, tmp_path, monkeypatch):
        ...
        for i, execstart in enumerate(variants):
            name = "atlas.service" if i == 0 else "atlas.service"  # same name
