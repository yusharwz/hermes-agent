...
* Be supported-platform-only — no-op silently elsewhere so ``atlas update``
...
class TestRequireConfirmedUpdate:
    """`atlas update` passes require_confirmed_update=True: the full
    ...
    Explicit `atlas computer-use install --upgrade` keeps the old
    ...
    def test_explicit_upgrade_still_falls_through_on_indeterminate(self):
        # `atlas computer-use install --upgrade` (default flag): the old
...
class TestUpdateCheckTimeoutDefaults:
    ...
    full reinstall fall-through during `atlas update`.
    """
