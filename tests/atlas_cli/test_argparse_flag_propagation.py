...
class TestAcceptHooksOnAgentSubparsers:
    ...
    # One driver subprocess parses ALL argvs: atlas_cli.main is a very heavy
    # import (previously 11 separate `python -m atlas_cli.main` spawns with a
...
"""
    ...
    def test_accepted_at_every_position(self):
        """Every `atlas <argv>` must exit 0 (help) rather than failing
        with `unrecognized arguments`."""
        import json
        import subprocess
...
class TestChatSubparserInheritedValueFlags:
    ...
    Regression test for #28780: `atlas -t web chat` silently dropped the
    ...
    Uses the real `atlas_cli._parser.build_top_level_parser()` rather than
    ...
    def test_chat_subparser_inherited_value_flags_use_suppress(self):
        ...
        assert not offenders, (
            ...
            "top-level value when used as `atlas <flag> <value> chat`:\n  "
