...
def atlas_home(tmp_path, monkeypatch):
    """Point ATLAS_HOME at a tmp dir so we never touch the real auth store.

    Required because ``_auth_file_path()`` has a seat belt that refuses to
    resolve to the real user's ~/.atlas/auth.json under pytest.
    """
...
class TestAuthStoreEncodingRoundTrip:
    def test_load_reads_utf8_with_non_ascii_label(self, atlas_home, windows_default_encoding):
    ...
    def test_load_does_not_corrupt_store_on_non_ascii(self, atlas_home, windows_default_encoding):
        ...
        assert not (atlas_home / "auth.json.corrupt").exists()
    ...
    def test_load_handles_utf8_with_bom(self, atlas_home):
...
class TestExplicitEncodingPassed:
    ...
    def test_load_auth_store_passes_utf8_encoding(self, atlas_home):
        auth_path = atlas_home / "auth.json"
...
# --- sibling readers of the same ~/.atlas/auth.json in other modules -------
#
# _load_auth_store lives in atlas_cli/auth.py, but several other modules read
# the same ~/.atlas/auth.json directly. They had the same UTF-8-vs-cp1252
# asymmetry on Windows — these tests pin the sibling reads too.

class TestAuthJsonSiblingReaders:
    def test_has_xai_credentials_reads_non_ascii_store(self, atlas_home, windows_default_encoding, monkeypatch):
        ...
        _write_utf8(atlas_home / "auth.json", store)

        from tools.xai_http import has_xai_credentials

        assert has_xai_credentials() is True

    def test_auxiliary_nous_provider_reads_non_ascii_store(self, atlas_home, windows_default_encoding, monkeypatch):
    ...
    def test_read_shared_nous_state_reads_non_ascii_store(
        ...
        """atlas_cli.auth._read_shared_nous_state must read a non-ASCII store.
    ...
    def test_has_any_provider_configured_reads_non_ascii_auth_store(
        self, atlas_home, monkeypatch, windows_default_encoding
    ):
        """atlas_cli.main._has_any_provider_configured reads auth.json.
        ...
        # atlas_cli.auth; patch it on the source module so the local import
        # sees the fake.
