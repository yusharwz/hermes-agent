...
class TestCmdUpdateNpmLockfileCache:
    @staticmethod
    def _cache_file(atlas_root, project_root):
        cache_key = hashlib.sha256(str(project_root).encode()).hexdigest()[:12]
        return atlas_root / f".npm_lock_hash_{cache_key}"
    ...
    def test_package_json_only_edit_defeats_skip(self, tmp_path, monkeypatch):
        ...
        npm — lockfile unchanged. `atlas update` must still install (the
    ...
    def test_update_uses_one_shared_npm_cache_across_profiles(
        ...
        shared_root = tmp_path / ".atlas"
        named_profile = shared_root / "profiles" / "work"
        named_profile.mkdir(parents=True)

        monkeypatch.setattr(hm, "PROJECT_ROOT", checkout)
        monkeypatch.setattr(atlas_constants.Path, "home", lambda: tmp_path)
        monkeypatch.setattr(
            atlas_constants, "find_node_executable", lambda _name: "/usr/bin/npm"
        )
        ...
        with patch.object(
            ...
            monkeypatch.setenv("ATLAS_HOME", str(shared_root))
            hm._update_node_dependencies()

            monkeypatch.setenv("ATLAS_HOME", str(named_profile))
            hm._update_node_dependencies()
