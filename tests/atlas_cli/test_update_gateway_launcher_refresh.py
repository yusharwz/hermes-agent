...
2. ``atlas_cli.main._refresh_windows_gateway_launchers`` — ``atlas
...
def test_restart_spec_normalizes_legacy_pythonw_argv(tmp_path):
    ...
    argv = [str(pythonw), "-m", "atlas_cli.main", "gateway", "run"]
    ...
    ), mock.patch("atlas_cli.config.get_atlas_home", return_value=str(tmp_path)):
...
# ---------------------------------------------------------------------------
# _refresh_windows_gateway_launchers: atlas update regenerates launchers
# ---------------------------------------------------------------------------
