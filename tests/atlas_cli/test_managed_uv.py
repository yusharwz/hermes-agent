...
def _runtime_info(
    ...
    from atlas_cli.sqlite_runtime import SQLiteRuntimeInfo
...
def _RRR(status):
    ...
    """
    from atlas_cli.managed_uv import RuntimeRepairResult
...
class TestEnsureUv:
    ...
    def test_installs_if_missing(self, tmp_path):
        with patch("atlas_cli.managed_uv.get_atlas_home", return_value=tmp_path), \
             patch("atlas_cli.managed_uv.repair_vulnerable_runtime", return_value=_RRR("not-applicable")), \
    ...
    def test_install_reports_runtime_repair_to_observer(self, tmp_path):
        from atlas_cli.managed_uv import (
            RuntimeRepairResult,
            ensure_uv,
        )
        ...
        observed = []
        with patch(
            "atlas_cli.managed_uv.get_atlas_home",
            return_value=tmp_path,
        ), patch(
            "atlas_cli.managed_uv._install_uv",
            side_effect=fake_install,
        ), patch(
            "atlas_cli.managed_uv.repair_vulnerable_runtime",
            return_value=repair,
        ):
...
class TestUpdateManagedUv:
    ...
    def test_fresh_stamp_skips_network_self_update_but_not_repair(self, tmp_path, monkeypatch):
        ...
        from atlas_cli.managed_uv import RuntimeRepairResult, update_managed_uv

        uv = tmp_path / "bin" / "uv"
        _make_executable(uv)
        # Fresh stamp under the isolated ATLAS_HOME.
        import atlas_constants
        stamp = atlas_constants.get_atlas_home() / "cache" / ".uv_self_update_stamp"
    ...
    def test_stale_stamp_runs_self_update_and_refreshes_stamp(self, tmp_path):
        ...
        from atlas_cli.managed_uv import UV_SELF_UPDATE_INTERVAL_SECONDS, update_managed_uv
...
class TestManagedPythonStore:
    def test_store_is_checkout_scoped_across_profiles(self, tmp_path, monkeypatch):
        from atlas_cli.managed_uv import managed_python_install_dir

        checkout = tmp_path / "checkout"
        monkeypatch.setenv("ATLAS_HOME", str(tmp_path / "profiles" / "alpha"))
        alpha = managed_python_install_dir(checkout)
        monkeypatch.setenv("ATLAS_HOME", str(tmp_path / "profiles" / "beta"))
        beta = managed_python_install_dir(checkout)

        expected = checkout / ".atlas-runtime" / "python"
        assert alpha == expected
        assert beta == expected

    def test_environment_is_private_and_sanitized(self, tmp_path):
        from atlas_cli.managed_uv import managed_python_env
...
class TestRuntimeRepair:
    def test_safe_runtime_is_a_noop(self, tmp_path):
        from atlas_cli.managed_uv import repair_vulnerable_runtime

        root, live, sentinel = _make_runtime_install(tmp_path)
        current = _runtime_info(live / "bin" / "python", (3, 53, 1))
        with patch("atlas_cli.managed_uv.platform.system", return_value="Linux"), \
             patch(
                 "atlas_cli.managed_uv.probe_sqlite_runtime",
                 return_value=current,
             ), \
             patch(
                 "atlas_cli.managed_uv._install_safe_python_generation"
             ) as mock_install:
        ...
        assert not (root / ".atlas-runtime").exists()
    ...
    def test_failed_candidate_preserves_live_venv(self, tmp_path):
        ...
        generation = root / ".atlas-runtime" / "python" / "generation-test"
        ...
        with patch("atlas_cli.managed_uv.platform.system", return_value="Linux"), \
             ...
             patch(
                 "atlas_cli.managed_uv._stage_candidate_venv",
                 return_value=None,
             ):
            result = repair_vulnerable_runtime("uv", project_root=root)
        ...
        reacquired = _acquire_repair_lock(root / ".atlas-runtime")
    ...
    def test_successful_repair_removes_parked_backup(self, tmp_path):
        ...
        candidate_python.write_text("candidate interpreter", encoding="utf-8")
        fixed = _runtime_info(candidate_python, (3, 53, 1))
        candidate_venv = root / ".atlas-runtime" / "venv-candidate"
        ...
        with patch("atlas_cli.managed_uv.platform.system", return_value="Linux"), \
             ...
             patch(
                 "atlas_cli.managed_uv._smoke_candidate_venv",
...
class TestRuntimeCutover:
    def test_os_lock_blocks_concurrent_repair_and_releases(self, tmp_path):
        from atlas_cli.managed_uv import _acquire_repair_lock, _release_repair_lock

        runtime_root = tmp_path / ".atlas-runtime"
    ...
    def test_post_swap_smoke_failure_rolls_back_live_venv(self, tmp_path):
        from atlas_cli.managed_uv import _cut_over_candidate

        root, live, sentinel = _make_runtime_install(tmp_path)
        runtime_root = root / ".atlas-runtime"
...
class TestRuntimeRequestMinorLine:
    ...
    def test_requests_minor_line(self):
        from atlas_cli.managed_uv import _runtime_request
    ...
    def _run_generation(tmp_path, monkeypatch, current_version, candidate_version):
        ...
        import atlas_cli.managed_uv as managed_uv
        from atlas_cli.sqlite_runtime import SQLiteRuntimeInfo
...
class TestRefreshManagedUvCatalog:
    ...
    def test_version_change_reports_true(self, tmp_path):
        ...
        with patch("atlas_cli.managed_uv.get_atlas_home", return_value=tmp_path), \
             patch("atlas_cli.managed_uv.platform.system", return_value="Linux"), \
             patch("atlas_cli.managed_uv._install_uv"), \
             patch(
                 "atlas_cli.managed_uv._uv_version_string",
...
class TestRepairRetriesAfterUvRefresh:
    def _run_repair(self, tmp_path, *, refresh_result, second_attempt):
        ...
        with patch("atlas_cli.managed_uv.platform.system", return_value="Linux"), \
             ...
             patch(
                 "atlas_cli.managed_uv._refresh_managed_uv_catalog",
    ...
    def test_retry_success_proceeds_to_staging(self, tmp_path):
        def second_attempt(root):
            generation = root / ".atlas-runtime" / "python" / "generation-retry"
            candidate_python = generation / "bin" / "python"
            candidate_python.parent.mkdir(parents=True)
...
class TestDefaultLiveVenv:
    ...
    'not-applicable' on every atlas update and stayed on journal_mode=DELETE
    ...
    def test_dot_venv_only_is_targeted(self, tmp_path):
        from atlas_cli.managed_uv import _default_live_venv
