"""Tests for the top-level `./atlas` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./atlas` should use `atlas_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "atlas"
    called = []

    fake_main_module = types.ModuleType("atlas_cli.main")

    def fake_main():
        called.append("atlas_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "atlas_cli.main", fake_main_module)
