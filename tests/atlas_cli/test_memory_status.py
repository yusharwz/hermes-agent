"""Tests for `atlas memory status` CLI command.
...
def _run_cmd_status(capfd, mem_config=None, memory_tools=None):
    ...
    """
    from atlas_cli.memory_setup import cmd_status
    ...
    with patch("atlas_cli.config.load_config", return_value=config):
        with patch("atlas_cli.memory_setup._get_available_providers", return_value=[]):
            with patch(
