"""Focused tests for the Atlas shared-metrics durable store."""
...
import pytest
from atlas_cli.observability import shared_metrics as shared_metrics_module
from atlas_cli.observability.shared_metrics import SharedMetricsStore
from atlas_cli.observability.shared_metrics_contract import (
    COUNT_BUCKETS,
    DURATION_BUCKETS,
    EXECUTION_SURFACES,
    MODEL_FAMILIES,
...
def test_model_call_counter_survives_restart_and_exports_only_new_deltas(tmp_path):
    ...
    assert first_package["schema_version"] == "atlas.shared_metrics.v1"
    assert first_package["resource"] == {"atlas_version": "test-version"}
...
def test_due_export_runs_once_per_utc_day_and_catches_up_pending_deltas(
    ...
    assert store.counter_snapshot()[0] == {
        ...
        "metric_name": "atlas.model_call.count",
...
def test_retention_prunes_only_expired_exported_history(tmp_path):
    ...
    with sqlite3.connect(database_path) as connection:
        connection.execute(
            ...
            WHERE atlas_version = 'expired-version'
            """
        )
        connection.execute(
            """
    ...
    with sqlite3.connect(database_path) as connection:
        ...
        aggregate_versions = {
            ...
            for row in connection.execute(
                "SELECT atlas_version FROM counter_aggregates"
            ).fetchall()
        }
