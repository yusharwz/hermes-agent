...
def _advance_backup_clock(seconds: float = 1.1) -> None:
    """Skew atlas_cli.backup's datetime forward instead of sleeping.
    ...
    import atlas_cli.backup as _backup

    shim = getattr(_backup.datetime, "_atlas_test_shim", None)
    if shim is None:
        class _ShimDatetime(_dt.datetime):
            _atlas_test_shim = True
...
def _make_atlas_tree(root: Path) -> None:
    ...
    for db_name in ("memory_store.db", "atlas_state.db"):
...
class TestQuickSnapshot:
    ...
    def test_failed_state_db_copy_is_loud(self, atlas_home, monkeypatch, capsys):
        """#68474: unreadable state.db must not look like a silent success."""
        from atlas_cli import backup as backup_mod

        def boom(src, dst):
            return False

        monkeypatch.setattr(backup_mod, "_safe_copy_db", boom)
        snap_id = backup_mod.create_quick_snapshot(atlas_home=atlas_home)
        err = capsys.readouterr().out
        assert "SQLite safe copy FAILED" in err or "CRITICAL" in err
        assert "state.db" in err
        # Other small files may still snapshot
        if snap_id:
            manifest = (atlas_home / "state-snapshots" / snap_id / "manifest.json")
...
# ---------------------------------------------------------------------------
    ...
    def test_oversized_db_suppresses_pruning(self, atlas_home, capsys):
        ...
        first_id = create_quick_snapshot(label="complete", atlas_home=atlas_home)
        assert first_id is not None
        first_dir = atlas_home / "state-snapshots" / first_id
        ...
        second_id = create_quick_snapshot(
            label="oversized", atlas_home=atlas_home, max_file_size=1024, keep=1
        )
        assert second_id is not None
        second_dir = atlas_home / "state-snapshots" / second_id
        ...
        all_snaps = list_quick_snapshots(limit=100, atlas_home=atlas_home)
...
class TestRunPreUpdateBackup:
    ...
    def atlas_home(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Config reads resolve ATLAS_HOME dynamically and their caches are
    ...
    @staticmethod
    def _set_mode(atlas_home, value):
        import yaml
    ...
    @staticmethod
    def _zips(atlas_home):
        d = atlas_home / "backups"
        return list(d.glob("pre-update-*.zip")) if d.exists() else []

    @staticmethod
    def _snaps(atlas_home):
        d = atlas_home / "state-snapshots"
        return [p for p in d.iterdir() if p.is_dir()] if d.exists() else []




    def test_config_off_disables_everything_silently(self, atlas_home, capsys):
        ...
        assert out == ""
        assert not self._snaps(atlas_home)
        assert not self._zips(atlas_home)



    def test_config_full_mode(self, atlas_home, capsys):
        self._set_mode(atlas_home, "full")
        from atlas_cli.main import _run_pre_update_backup
        snap_id = _run_pre_update_backup(Namespace(no_backup=False, backup=False))
        out = capsys.readouterr().out
        assert snap_id is not None
        ...
        assert len(self._zips(atlas_home)) == 1
