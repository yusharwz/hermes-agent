"""ensure_atlas_home is memoized per home path (perf: it runs on every
...
def test_distinct_home_paths_each_get_the_skeleton(tmp_path, monkeypatch):
    first = tmp_path / "a" / ".atlas"
    second = tmp_path / "b" / ".atlas"

    monkeypatch.setenv("ATLAS_HOME", str(first))
    cfg.ensure_atlas_home()

    # Profile switch: ATLAS_HOME moves → the new path is ensured too.
