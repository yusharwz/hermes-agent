"""Tests for the direct Atlas-to-Relay shared-metrics runtime."""
...
from atlas_cli import lifecycle, plugins
...
def test_direct_runtime_records_without_enabling_a_plugin(direct_runtime, tmp_path):
    ...
    assert scope_starts[1][1] == "atlas.task_run"
    ...
    root = tmp_path / "atlas-home" / "telemetry" / "shared_metrics"
    ...
    assert metrics["atlas.model_call.count"]["dimensions"]["model_family"] == "claude"
    assert metrics["atlas.model_call.count"]["value"] == 1
    assert metrics["atlas.task_run.started"] == {
    ...
    terminal = metrics["atlas.task_run.finished"]["dimensions"]
...
def test_real_binding_drives_lifecycle_aggregation_export_and_snapshot(
    ...
    from atlas_cli.observability.shared_metrics import SharedMetricsStore

    root = tmp_path / "atlas-home" / "telemetry" / "shared_metrics"
    store = SharedMetricsStore(root / "metrics.sqlite3", root / "outbox")
    tomorrow = datetime.now(timezone.utc) + timedelta(days=1)
    monkeypatch.setattr(
        "atlas_cli.observability.shared_metrics._utc_now",
        lambda: tomorrow,
    )
    ...
    assert len(by_metric["atlas.task_run.started"]) == 1
    assert by_metric["atlas.task_run.started"][0]["value"] == 3
    assert {
        counter["dimensions"]["outcome"]
        for counter in by_metric["atlas.model_call.count"]
    } == {"success", "failed", "cancelled"}
    terminal_by_outcome = {
        counter["dimensions"]["outcome"]: counter
        for counter in by_metric["atlas.task_run.finished"]
    }
    ...
    for package in package_payloads:
        assert package["schema_version"] == "atlas.shared_metrics.v1"
...
def test_core_runtime_is_fail_open_without_a_published_binding(monkeypatch, caplog):
    ...
    assert not relay_runtime.emit_mark("atlas.probe", session_id="s1")
    assert "Atlas Relay runtime initialization failed" in caplog.text
    relay_runtime._reset_for_tests()
...
def test_managed_config_cannot_override_shared_metrics_consent(
    ...
    from atlas_cli import config, managed_scope
...
def test_disabling_shared_metrics_stops_collection_and_shutdown_export(
    ...
    runtime.subscriber(SimpleNamespace(
        ...
        scope_category="start",
        metadata={
            "atlas.metrics.schema_version": "atlas.metrics.event.v1",
...
def test_failed_flush_keeps_daily_export_open_for_later_task(
    ...
    metrics = {metric["name"]: metric for metric in package["metrics"]}
    assert metrics["atlas.task_run.started"]["value"] == 2
    assert metrics["atlas.task_run.finished"]["value"] == 2
    assert flush_attempts == 2
    assert "Atlas shared-metrics task flush failed" in caplog.text
