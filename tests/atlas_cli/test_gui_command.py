...
def test_gui_install_env_prepends_managed_node_on_bare_path(tmp_path, monkeypatch):
    ...
    the parent PATH is stripped, so the install env MUST carry the Atlas-managed
    Node ahead of that bare PATH or the install dies with ``node: not found``.
    """
    import os

    from atlas_constants import iter_atlas_node_dirs

    root = _make_desktop_tree(tmp_path)
    monkeypatch.setattr(cli_main, "PROJECT_ROOT", root)
    _make_packaged_executable(root, monkeypatch, platform="win32")

    # A managed Node tree on disk so with_atlas_node_path() actually prepends it.
    ...
    install_ok = subprocess.CompletedProcess(["npm", "ci"], 0)
    launch_ok = subprocess.CompletedProcess(["atlas"], 0)

    with patch("atlas_cli.main._resolve_node_runtime_npm", return_value="/usr/bin/npm"), \
         patch("atlas_cli.main._run_npm_install_deterministic", return_value=install_ok) as mock_install, \
         patch("atlas_cli.main._desktop_build_needed", return_value=True), \
         patch("atlas_cli.main._write_desktop_build_stamp"), \
         patch("atlas_cli.main.subprocess.run", side_effect=[subprocess.CompletedProcess([], 0), launch_ok]), \
         pytest.raises(SystemExit):
        cli_main.cmd_gui(_ns(skip_build=False))

    managed_dirs = [str(p) for p in iter_atlas_node_dirs() if p.is_dir()]
...
def _make_signable_app(desktop_dir: Path) -> Path:
    """Build a fake packaged Atlas.app with the pieces the signer must find."""
    ...
    app = desktop_dir / "release" / "mac-arm64" / "Atlas.app"
    _write_info_plist(app, "id.co.dritech.atlas")
    (app / "Contents" / "MacOS").mkdir(parents=True)
    (app / "Contents" / "MacOS" / "Atlas").write_text("", encoding="utf-8")

    helper = app / "Contents" / "Frameworks" / "Atlas Helper.app"
    _write_info_plist(helper, "id.co.dritech.atlas.helper")
...
def test_desktop_macos_local_codesign_signs_native_binaries(tmp_path, monkeypatch):
    ...
    Atlas.app component, silently skipping every .node/.dylib/crashpad
