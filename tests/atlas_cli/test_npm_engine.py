"""Tests for npm ``EBADENGINE`` recovery (``atlas_cli/npm_engine.py``).

The behaviour under test is a contract about *reacting* to npm's own engine
check: npm states the range it wants in the failure, Atlas upgrades only an
...
from atlas_cli.npm_engine import (
    actual_npm_version,
    is_ebadengine,
    managed_npm_prefix,
...
npm error engine Not compatible with your version of node/npm: atlas-agent@1.0.0
npm error notsup Not compatible with your version of node/npm: atlas-agent@1.0.0
...
class TestManagedDetection:
    ...
    no other npm — this is the boundary between "Atlas fixes it" and "the
