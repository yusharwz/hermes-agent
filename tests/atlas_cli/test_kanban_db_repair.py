...
and the ``atlas kanban repair`` CLI verb."""
...
# ---------------------------------------------------------------------------
# repair_db() API + `atlas kanban repair` CLI verb
# ---------------------------------------------------------------------------

def _run_kanban_cli(argv: list[str]) -> int:
    """Drive the real argparse surface exactly like `atlas kanban …`."""
    import argparse
...
def cli_home(tmp_path, monkeypatch):
    """Isolated ATLAS_HOME so kanban_db_path() resolves inside tmp_path."""
    home = tmp_path / ".atlas"
    home.mkdir()
