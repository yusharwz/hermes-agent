...
def _run_prune(monkeypatch, capsys, argv_tail, candidates=None):
    """Run `atlas sessions prune <argv_tail>` against a FakeDB, capturing
    ...
    monkeypatch.setattr(atlas_state, "SessionDB", lambda: FakeDB())
    monkeypatch.setattr(
        sys, "argv", ["atlas", "sessions", "prune", *argv_tail]
    )
...
def test_sessions_prune_bare_keeps_90_day_default(monkeypatch, capsys):
    """A truly bare `atlas sessions prune` keeps the implicit 90-day cutoff."""
...
def test_sessions_prune_preview_shows_oldest_newest(monkeypatch, capsys):
    ...
    from atlas_cli.session_filters import format_epoch
