...
# ---------------------------------------------------------------------------
# CLI judge gate tests (atlas kanban complete bypass fix)
# ---------------------------------------------------------------------------

class TestCLIJudgeGate:
    """atlas kanban complete must apply the same goal_mode judge gate as the
    ...
    def _run(self, monkeypatch, *, goal_mode=True, judge_available=True,
        ...
        from atlas_cli.kanban import _cmd_complete
        ...
        monkeypatch.setattr("atlas_cli.kanban.kb.get_task", lambda conn, tid: fake_task)
        monkeypatch.setattr("atlas_cli.kanban.kb.complete_task", fake_complete_task)
        monkeypatch.setattr("atlas_cli.kanban.kb.connect_closing", fake_connect_closing)
        monkeypatch.setattr("atlas_cli.kanban._worker_run_id_for", lambda _: None)
