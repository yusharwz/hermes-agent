...
Atlas Agent (see website/docs/getting-started/platform-support.md). The
...
build sandbox. ``nix/python.nix`` sets ``ATLAS_NIX_BUILD=1`` on the
Atlas package derivation, so only that build may create an artifact.
...
_IN_NIX_BUILD = os.environ.get("ATLAS_NIX_BUILD") == "1"

_BLOCK_MESSAGE = (
    "Building wheels or sdists for atlas-agent is not supported.\n"
    "Atlas is distributed via the shell installer, Docker image, or Nix.\n"
    ...
    "the Atlas Nix derivation sets ATLAS_NIX_BUILD=1. If it does, file a bug."
)
