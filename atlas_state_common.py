...
Extracted verbatim from atlas_state.py so the SessionDB mixin modules
(atlas_state_search / atlas_state_schema / atlas_state_portability) can
reference them without importing atlas_state (which would be a cycle).
atlas_state re-imports every name here for backward compatibility.
"""
...
# optimized via ``atlas sessions optimize-storage``. A legacy DB sits at
...
# `atlas sessions optimize-storage` rebuilds it on a capable host.
...
# triggers repairable UNTIL the user opts into `atlas db optimize`. This is
