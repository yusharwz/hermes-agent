...
describe('createSlashHandler', () => {
  ...
  it('falls through to command.dispatch for skill commands, sending the body but showing the invocation', async () => {
    const skillMessage =
      '[IMPORTANT: The user has invoked the "atlas-agent-dev" skill, indicating they want you to follow its instructions.\n' +
    ...
    await vi.waitFor(() => {
      expect(ctx.transcript.send).toHaveBeenCalledWith(skillMessage, true, '/atlas-agent-dev')
    })
