...
describe('DEFAULT_THEME', () => {
  ...
  it('has color palette', async () => {
    ...
    expect(DEFAULT_THEME.color.primary).toBe('#2FBFDE')
...
describe('LIGHT_THEME', () => {
  it('avoids bright-yellow accents unreadable on white backgrounds (#11300)', async () => {
    ...
    expect(LIGHT_THEME.color.primary).not.toBe('#2FBFDE')
    expect(LIGHT_THEME.color.accent).not.toBe('#FFBF00')
    expect(LIGHT_THEME.color.muted).not.toBe('#14788F')
    expect(LIGHT_THEME.color.statusWarn).not.toBe('#2FBFDE')
  })
...
describe('fromSkin', () => {
  ...
  it('derives completion current background from resolved completion background (polarity-compatible)', async () => {
    ...
    const { fromSkin } = await importThemeWithEnv({ ATLAS_TUI_BACKGROUND: '#ffffff' })
  ...
  it('normalizes non-banner foregrounds on light Apple Terminal', async () => {
    ...
    expect(theme.color.primary).toBe('#2FBFDE')
    expect(theme.color.accent).toBe('#FFBF00')
    expect(theme.color.border).toBe('#1A94B8')
  ...
  it('skinIsLight: the authored background decides; host detection only when absent', async () => {
    const { skinIsLight } = await importThemeWithEnv({ ATLAS_TUI_BACKGROUND: '#ffffff' })
...
describe('derived tone ladder', () => {
  it('reproduces the original hand-tuned tones from seeds (reverse-engineered knobs)', async () => {
    ...
    const light = await importThemeWithEnv({ ATLAS_TUI_BACKGROUND: '#ffffff' })

    const cases: Array<[string, string, string]> = [
      [dark.DARK_THEME.color.muted, '#CC9B1F', 'dark muted'],
      [dark.DARK_THEME.color.label, '#29A6C2', 'dark label'],
...
describe('background-aware adaptation (OSC-11 light terminals)', () => {
  it('renders a dark-authored skin on light like minimumContrastRatio hosts do (the standardized look)', async () => {
    const { contrastRatio, fromSkin } = await importThemeWithEnv({ ATLAS_TUI_BACKGROUND: '#ffffff' })
  ...
  it('leaves the same skin untouched on a dark background', async () => {
    const { fromSkin } = await importThemeWithEnv({ ATLAS_TUI_BACKGROUND: '#1e1e2e' })
    const { color } = fromSkin(SLATE_COLORS, {})
  ...
  it('empty skin on a light background resolves to the light base palette', async () => {
    const { fromSkin, LIGHT_THEME } = await importThemeWithEnv({ ATLAS_TUI_BACKGROUND: '#ffffff' })
  ...
  it('defaultThemeForCurrentBackground follows a late ATLAS_TUI_BACKGROUND write', async () => {
    ...
    expect(defaultThemeForCurrentBackground({ ATLAS_TUI_BACKGROUND: '#ffffff' }).color).toEqual(LIGHT_THEME.color)
  })
