...
/**
 * User widget apps — Atlas authors its own TUI widgets, mirroring the
 * Python plugin contract: drop `<name>.mjs` into `$ATLAS_HOME/tui-widgets/`,
 ...
 * React.createElement.
 *
 * Trust model matches `~/.atlas/plugins/`: files under ATLAS_HOME execute
...
const widgetsDir = () => join(process.env.ATLAS_HOME?.trim() || join(homedir(), '.atlas'), 'tui-widgets')
 ...
 *  every change, so a widget Atlas writes appears within ~a second — no
...
export function watchUserWidgets(dir = widgetsDir()): void {
  ...
  if (!attach()) {
    ...
    // user (or Atlas) ever writes must hot-load too — a 10s poll here read
