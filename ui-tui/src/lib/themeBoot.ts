 ...
 * `atlas-boot-background` / `atlas-boot-color-scheme` localStorage keys.
 *
...
// Profile-aware: the Python launcher exports ATLAS_HOME (set by
// _apply_profile_override) before spawning the TUI. Falling back to
// ~/.atlas matches get_atlas_home()'s default.
const bootFilePath = () => join(process.env.ATLAS_HOME ?? join(homedir(), '.atlas'), 'tui-theme-boot.json')

// Never touch the user's real ~/.atlas from test runs (the TS suite has no
// ATLAS_HOME isolation fixture).
...
export interface BootSeedResult {
  ...
  /** True when the cached config pin was seeded into ATLAS_TUI_THEME. */
  seededPin: boolean
}
...
export function seedBootEnvironment(boot: BootTheme | null, env: NodeJS.ProcessEnv): BootSeedResult {
  ...
  if (!boot || env.ATLAS_TUI_THEME || env.ATLAS_TUI_LIGHT) {
    return result
  }
  ...
  if (
    ...
    !env.ATLAS_TUI_BACKGROUND
  ) {
    env.ATLAS_TUI_BACKGROUND = boot.background
...
export function invalidateBootBackground(env: NodeJS.ProcessEnv = process.env): boolean {
  if (!seeded.seededBackground || env.ATLAS_TUI_BACKGROUND !== seeded.seededBackground) {
    return false
  }

  delete env.ATLAS_TUI_BACKGROUND
...
/** True when this boot replayed a cached config pin into ATLAS_TUI_THEME.
