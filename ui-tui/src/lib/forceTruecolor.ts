...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.ATLAS_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Atlas on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via ATLAS_TUI_TRUECOLOR=1.
