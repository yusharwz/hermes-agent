...
const logDir = join(process.env.ATLAS_HOME?.trim() || join(homedir(), '.atlas'), 'logs')
...
// a real ~/.atlas (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('atlas-tui: parent lifecycle log unavailable\n')
    }
  }
}
