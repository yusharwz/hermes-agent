...
import { terminalBackgroundHex } from '@atlas/ink'
...
export const debugCommands: SlashCommand[] = [
  ...
  {
    help: 'rescan $ATLAS_HOME/tui-widgets and (re)register user widget apps',
    name: 'widgets-reload',
    run: (_arg, ctx) => {
      ...
      ctx.transcript.panel('Theme', [
        {
          rows: [
            ...
            ['ATLAS_TUI_BACKGROUND', process.env.ATLAS_TUI_BACKGROUND ?? '(unset)'],
            ['ATLAS_TUI_THEME', process.env.ATLAS_TUI_THEME ?? '(unset)'],
