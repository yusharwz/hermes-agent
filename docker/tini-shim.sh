...
# Atlas WebUI, NAS "update" flows that preserve an old entrypoint, etc.)
...
#
# Test hook: ATLAS_TINI_SHIM_TARGET overrides the `/init` path (and
# ATLAS_TINI_SHIM_WRAPPER the main-wrapper path) so unit tests can
# record argv without a real s6 tree.

set -e

INIT_TARGET="${ATLAS_TINI_SHIM_TARGET:-/init}"
WRAPPER="${ATLAS_TINI_SHIM_WRAPPER:-/opt/atlas/docker/main-wrapper.sh}"
