...
_SESSION_CHAT_TYPE: ContextVar = ContextVar("ATLAS_SESSION_CHAT_TYPE", default=_UNSET)
...
# intentionally separate from ATLAS_SESSION_ID: the latter is the durable
...
_SESSION_UI_SESSION_ID: ContextVar = ContextVar("ATLAS_UI_SESSION_ID", default=_UNSET)
...
_VAR_MAP = {
    ...
    "ATLAS_SESSION_CHAT_TYPE": _SESSION_CHAT_TYPE,
    ...
    "ATLAS_UI_SESSION_ID": _SESSION_UI_SESSION_ID,
...
def set_current_session_id(session_id: str) -> None:
    ...
    ``ATLAS_SESSION_ID`` for the rest of the process — leaking the child id
    ...
    # parent's session identity. See AtlasPRDelegationSessionContext task.
    try:
...
# value (``telegram``) to ATLAS_SESSION_PLATFORM, while the CLI, TUI, and
# desktop bind ATLAS_SESSION_SOURCE (``cli``, ``tui``, ``desktop``) and leave
...
# ATLAS_SESSION_PLATFORM but have no attachment channel behind them.
...
def session_is_messaging_surface() -> bool:
    ...
    Resolves ``ATLAS_PLATFORM``, then the session platform, then the session
    source, and reports messaging when any of them names a surface outside
    :data:`NON_MESSAGING_SESSION_SURFACES`.
    """
    import os

    platform = os.getenv("ATLAS_PLATFORM") or get_session_env("ATLAS_SESSION_PLATFORM", "")
...
def async_delivery_supported() -> bool:
    ...
    or a one-shot runner that exits after its final response (``atlas -z``,
    cron — see :func:`declare_stateless_channel`) — and dispatcher-spawned
    Kanban workers (identified by ``ATLAS_KANBAN_TASK``), which are one-shot
