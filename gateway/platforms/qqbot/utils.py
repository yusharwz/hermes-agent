...
def _get_atlas_version() -> str:
    """Return the atlas-agent package version, or 'dev' if unavailable."""
    try:
...
def build_user_agent() -> str:
    ...
    Format::
        ...
        QQBotAdapter/<qqbot_version> (Python/<py_version>; <os>; Atlas/<atlas_version>)

    Example::

        QQBotAdapter/1.0.0 (Python/3.11.15; darwin; Atlas/0.9.0)
    """
    py_version = f"{sys.version_info.major}.{sys.version_info.minor}.{sys.version_info.micro}"
    os_name = platform.system().lower()
    atlas_version = _get_atlas_version()
    return f"QQBotAdapter/{QQBOT_VERSION} (Python/{py_version}; {os_name}; Atlas/{atlas_version})"
