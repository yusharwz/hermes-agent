...
def _atlas_version() -> str:
    """Return the canonical Atlas Agent version string.

    ``atlas_cli.__version__`` is the runtime source of truth used by the CLI,
    ...
    ``atlas_agent-*.dist-info`` after a source update until the environment is
...
def _resolve_request_runtime_agent_kwargs(provider: str, target_model: Optional[str] = None) -> Dict[str, Any]:
    ...
    """
    from atlas_cli.runtime_provider import resolve_runtime_provider, format_runtime_provider_error, _get_model_config
    ...
    max_tokens = None
    env_max_tokens = os.environ.get("ATLAS_MAX_TOKENS")
    if env_max_tokens:
        try:
...
def _request_agent_overrides(
    ...
    *,
    ...
    ``/v1/models`` advertises a stable virtual model (usually ``atlas-agent``)
    ...
    ``direct_model_requests`` config value here, while Atlas-native
    endpoints (session chat, /v1/runs) always allow it.  A request that
    sends an explicit ``provider`` is unambiguously Atlas-aware and is
    always honored.
    """
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send an explicit ``provider`` — and the Atlas-native session-chat
    ...
    def _resolve_model_name(explicit: str) -> str:
        ...
        :func:`atlas_cli.model_switch.resolve_effective_model` (the shared
        override > mid-tier > default precedence owner).
        """
        from atlas_cli.model_switch import resolve_effective_model
        ...
        try:
            ...
            pass
        return resolve_effective_model(explicit, profile_name, "atlas-agent")
        ...
        if not profile:
            try:
                ...
                if is_multiplex_active():
                    ...
                    return _profile_runtime_scope(get_atlas_home())
            except Exception:
                pass
            return nullcontext()
        from gateway.run import _profile_runtime_scope
    ...
    def _ensure_session_db(self):
        ...
        redirects ``get_atlas_home()``, so each profile gets its own DB —
        ...
        try:
            ...
            return self._open_and_cache_session_db(get_atlas_home())
    ...
    def _normalize_session_source(value: Any) -> str:
        ...
        allowed = {"api_server", "atlas_browser", "browser", "cli", "telegram", "discord", "slack", "desktop", "dashboard"}
        if text in allowed:
            return "atlas_browser" if text == "browser" else text
        return "api_server"
    ...
    def _create_agent(
        ...
        # atlas_cli.model_switch.resolve_effective_model (session /model
    ...
    async def _handle_model_options(self, request: "web.Request") -> "web.Response":
        """GET /api/model/options — return Atlas provider/model inventory.
        ...
        Atlas provider catalog instead of scraping the single OpenAI-compatible
        `/v1/models` alias.
        """
        auth_err = self._check_auth(request)
        if auth_err:
    ...
    async def _handle_session_model_lock(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            "object": "atlas.session.model_lock",
            "session_id": session_id,
            "runtime": runtime,
        })
    @_admit_api_agent_request
    ...
    async def _handle_responses(self, request: "web.Request") -> "web.Response":
        ...
        # response and the X-Atlas-Session-Id header.  Without this,
        ...
        response_headers = {"X-Atlas-Session-Id": _effective_session_id}
        if gateway_session_key:
            response_headers["X-Atlas-Session-Key"] = gateway_session_key
        return web.json_response(response_data, headers=response_headers)
        ...
        def _run():
            ...
            with self._profile_scope(request_profile):
                ...
                try:
                    ...
                    if include_runtime:
                        runtime = dict(getattr(agent, "_atlas_api_runtime", {}) or {})
