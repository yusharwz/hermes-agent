...
# as MPEG-2 Layer II are audio to Atlas but unsupported by sendAudio/sendVoice.
_AUDIO_MIME_TYPES = {
    ".ogg": "audio/ogg",
    ".opus": "audio/opus",
    ".mp3": "audio/mpeg",
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    if (
        ...
        and raw_message.get("_atlas_no_thread_response")
    ):
...
def build_auto_tts_output_path(platform) -> str:
    ...
    TTS tool's ``ATLAS_SESSION_PLATFORM`` contextvar — that contextvar is
...
def get_inbound_media_max_bytes() -> int:
    ...
    """
    try:
        from atlas_cli.config import load_config_readonly as _load_config
...
def _kanban_attachment_roots() -> List[Path]:
    ...
    home_override = os.environ.get("ATLAS_KANBAN_HOME", "").strip()
    root = Path(home_override).expanduser() if home_override else _ATLAS_ROOT
...
class BasePlatformAdapter(ABC):
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        # Cross-ATLAS_HOME token takeover is armed by GatewayRunner only for
    ...
    def _acquire_platform_lock(self, scope: str, identity: str, resource_desc: str) -> bool:
        ...
        A live cross-ATLAS_HOME holder may be replaced only when the runner
            ...
            if response:
                ...
                if (self._should_auto_tts_for_chat(event.source.chat_id)
                        ...
                        and not self._streaming_tts_turn_completed(
                            session_key,
                            getattr(interrupt_event, "_atlas_run_generation", None),
                            event=event,
                        )):
                    try:
                        ...
                        if check_tts_requirements():
                            ...
                            # ATLAS_SESSION_PLATFORM contextvar the tool would
    ...
    def build_source(
        ...
        per-profile ATLAS_HOME isolation.
        """
