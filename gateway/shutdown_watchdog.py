...
2. An event-loop heartbeat file at ``<ATLAS_HOME>/state/gateway.heartbeat`` so
...
def _process_atlas_home() -> Path:
    """ATLAS_HOME for process-level identity files (ignore profile overrides)."""
...
def get_loop_heartbeat_path(home: Optional[Path] = None) -> Path:
    """Return ``<ATLAS_HOME>/state/gateway.heartbeat``."""
    base = home if home is not None else _process_atlas_home()
    ...
    *,
    ...
    def _watchdog() -> None:
        ...
        try:
            ...
            pass
        try:
            from atlas_logging import drain_log_queue
