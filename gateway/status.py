...
from atlas_constants import get_atlas_home, _get_platform_default_atlas_home
...
def _get_starts_log_path() -> Path:
    ...
    collision."""
    return get_atlas_home() / "gateway-starts.log"
...
def _get_process_atlas_home() -> Path:
    """Return the process-level ATLAS_HOME, skipping context-local overrides.
    ...
    ``get_atlas_home()`` honors ``_ATLAS_HOME_OVERRIDE`` contextvar used for
...
def _canonical_atlas_home(path: Path | str) -> Path:
    """Return a stable absolute ATLAS_HOME path for persisted identity data."""
    return Path(path).expanduser().resolve(strict=False)


def _same_atlas_home(left: Path | str, right: Path | str) -> bool:
    """Compare ATLAS_HOME paths with the host platform's case semantics."""
    return os.path.normcase(str(_canonical_atlas_home(left))) == os.path.normcase(
        str(_canonical_atlas_home(right))
    )


def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting ATLAS_HOME."""
    home = _get_process_atlas_home()
...
# Reject epoch values before 2000-01-01T00:00:00Z: nothing in Atlas' lifetime
...
def looks_like_update_runner_command_line(command: str | None) -> bool:
    ...
    atlas_cli.ninegate_update --apply`` — but it is a Atlas-owned process
    ...
    if not command:
        return False
    return "atlas_cli.ninegate_update" in command
...
def _command_line_belongs_to_profile(command: str, profile_home: Path) -> bool:
    ...
    # str(Path) renders backslashes while a ATLAS_HOME= value on the argv
...
def _build_pid_record() -> dict:
    return {
        ...
        # ATLAS_HOME-local.  Persist the owning gateway's process home so an
        # explicit cross-profile --replace can place its planned-takeover
        # marker where the target process will actually read it.
        "atlas_home": str(_canonical_atlas_home(_get_process_atlas_home())),
    }
...
def resolve_gateway_liveness(
    *,
    ...
    passes its ``atlas_cli.web_server`` bindings so the long-standing
...
def _get_takeover_marker_path(atlas_home: Optional[Path] = None) -> Path:
    """Return the path to the --replace takeover marker file.

    ``atlas_home`` is supplied only for a verified cross-home handoff.  The
    target process always consumes the marker from its own process-level home.
    """
    home = atlas_home or _get_process_atlas_home()
    return _canonical_atlas_home(home) / _TAKEOVER_MARKER_FILENAME
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # TARGET home.  That permits a deliberate cross-ATLAS_HOME --replace while
    ...
    our_home = _get_process_atlas_home()
    target_home = record.get("target_atlas_home")
    if target_home is not None:
        if not isinstance(target_home, str) or not _same_atlas_home(
            target_home, our_home
        ):
            return False
    else:
        replacer_home = record.get("replacer_atlas_home")
        if replacer_home is not None and not _same_atlas_home(
...
def write_takeover_marker(
    ...
    *,
    ...
    gateway's ATLAS_HOME rather than the replacer's.  Same-home callers omit
    ...
    """
    try:
        marker_home = _canonical_atlas_home(
            target_home or _get_process_atlas_home()
        )
        ...
        record = {
            ...
            "target_atlas_home": str(marker_home),
            "replacer_pid": os.getpid(),
            "replacer_atlas_home": str(
                _canonical_atlas_home(_get_process_atlas_home())
            ),
...
def _validated_scoped_lock_gateway_owner(
    ...
    the lock record, the target ATLAS_HOME's gateway PID record, and the live
    ...
    pid_record_home = pid_record.get("atlas_home")
    if not isinstance(pid_record_home, str) or not _same_atlas_home(
