...
class PlatformEntry:
    ...
    # Env vars this platform needs (for ``atlas setup`` display).
    ...
    # built-ins).  Used by ``atlas gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # plugin-discovery time added several seconds to *every* `atlas`
        # invocation -- including plain `atlas chat`, which never touches any
        ...
        # `atlas setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``atlas setup``/``gateway status``, channel
