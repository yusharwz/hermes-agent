...
def _merge_pairing_dir(active_dir: Path, alternate_dir: Path) -> None:
    ...
    Older installs use ``{ATLAS_HOME}/pairing`` while newer code/docs may
    write ``{ATLAS_HOME}/platforms/pairing``. If both directories exist, the
...
class PairingStore:
    ...
    profile's own ATLAS_HOME using the same legacy/consolidated layout rules
    as ``atlas -p <name> pairing ...``. This keeps multiplex gateways and
    profile-scoped CLI approvals on one whitelist. Without a profile, storage
    is the global pairing directory for the current ATLAS_HOME.
    """

    def __init__(self, profile: Optional[str] = None):
        # Resolve storage directory lazily — tests use a temp ATLAS_HOME
        ...
        if profile:
            ...
            # ``atlas -p <profile> pairing ...`` process does. Merge the
    ...
    def _load_json(self, path: Path) -> dict:
        if path.exists():
            try:
                ...
                # the file, then the gateway process — running as `atlas` after
                ...
                logger.warning(
                    ...
                    "If you ran `docker exec <container> atlas pairing approve ...` as root, "
                    "re-run with `docker exec -u atlas <container> ...` and "
                    "chown the existing file to the atlas user, or restart the "
    ...
    def approve_request(self, platform: str, request_id: str) -> Optional[dict]:
        ...
        This is the grant path for authenticated admin surfaces (``atlas
