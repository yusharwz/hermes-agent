...
def relay_display_name() -> Optional[str]:
    ...
    if not value:
        try:
            from atlas_cli.skin_engine import get_active_skin  # late import: boot-safe
        ...
        # multi-agent scope it would prefix every reply "**Atlas Agent:**" —
...
def _resolve_relay_identity_token() -> str:
    ...
    ``atlas gateway enroll`` CLI. Two modes, in precedence order:
