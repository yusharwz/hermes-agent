...
has to be thrown away or summarized. Atlas has always done this in one batch:
...
Atlas folds the single oldest un-absorbed exchange into a running summary. The
...
The in-memory splice alone isn't enough. Atlas's normal session flush is
...
With `micro_compact` unset or `false` Atlas behaves exactly as it always has:
...
Defaults to `$ATLAS_HOME/logs/agent.log`. It reports passes, outcome mix, net
