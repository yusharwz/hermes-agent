...
`atlas config set gateway.profile_routes ...` writes).
...
| `profile` | yes | Target profile name (must exist under `~/.atlas/profiles/<name>`). |
...
activates the per-profile runtime scope (per-profile `ATLAS_HOME`, secret scope, and
