...
When running Atlas inside Docker, the default `network_mode: host` gives the
...
The Atlas [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
#
# Usage:
#   ATLAS_UID=$(id -u) ATLAS_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=atlas,atlas-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://atlas-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Atlas trust model and vulnerability reporting
