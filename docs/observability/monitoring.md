...
Atlas gateway daemon, exported over OTLP/HTTP to an operator-configured
...
(`plugins/observability/nemo_relay/`) and its Atlas-owned subscribers.

## What gets exported

| Signal | OTLP route | Content |
| --- | --- | --- |
| Gateway gauges | `/v1/metrics` | `atlas.gateway.up/state/busy/drainable/active_agents/background_work/background_delegations/restart_requested`, `atlas.platform.up/degraded` with bounded `error_code` attributes |
...
`atlas.gateway.active_agents`, `atlas.gateway.background_work`, and
`atlas.gateway.background_delegations` are complementary. `active_agents`
...
```bash
atlas monitoring status
```

The OpenTelemetry SDK is an optional extra (`pip install 'atlas-agent[otlp]'`),
...
a plain `atlas gateway run`: the exporter lives in the gateway process, so
...
`atlas.gateway.up`, `atlas.platform.up`, and `atlas.platform.degraded`.
...
absent_over_time(atlas_gateway_up[5m])

# Locally owned bridge is explicitly down.
atlas_platform_up == 0

# Scheduler thread is stale even though the gateway may still be alive.
atlas_cron_scheduler_heartbeat_age_seconds > 180

# Ticker loops but has not completed a successful tick recently.
atlas_cron_scheduler_last_success_age_seconds > 300

# One or more jobs are beyond their existing scheduler grace window.
atlas_cron_jobs_overdue > 0

# Catch-up counter increased, proving at least one stale occurrence was
# collapsed and run once after a delay.
increase(atlas_cron_scheduler_catch_up_occurrences[15m]) > 0
```

Cron execution lifecycle records arrive as `atlas.cron_execution` spans.
Alert or derive events from bounded attributes such as:

```text
atlas.status = failed|unknown
atlas.delivery_outcome = failed|not_configured
atlas.error_class = auth_failed|rate_limited|timeout|network_error|
...
3. a cron lifecycle feed keyed only by opaque `atlas.job_key`;
...
Atlas Agent-owned Relay transport health remains in scope. A separate gateway
...
python scripts/observability/otel_capture_collector.py \
  --host 127.0.0.1 --port 4318 --log /tmp/atlas_otel_capture.jsonl
...
python scripts/observability/gateway_health_export_probe.py \
  ...
  --log /tmp/atlas_otel_capture.jsonl --wait 8
...
`atlas.error_class = ...` list in this file's alert section and the enum's unit
...
```bash
atlas monitoring status                 # posture
python scripts/observability/gateway_health_export_probe.py \
  --endpoint http://127.0.0.1:4318/v1/traces \
...
The `atlas monitoring` CLI intentionally exposes `status` only. This first
release covers only Atlas Agent-owned service-health and operational-diagnostic
signals, including Atlas Agent-owned Relay transport health. Team Gateway's
