...
Atlas includes NeMo Relay as a normal runtime dependency on platforms for
which Relay publishes a native wheel. The shared-metrics integration is built
into Atlas and does not require `atlas plugins enable
observability/nemo_relay`. Atlas remains importable without Relay on other
native targets. Those targets use an explicit reduced-capability no-op host:
Atlas execution remains available, while Relay scopes, middleware, plugins,
and subscribers are unavailable. The `atlas-agent[nemo-relay]` extra remains
as a no-op compatibility alias for existing installation commands.

Atlas requires NeMo Relay 0.6.0 or later within the 0.6 release line. That
...
Atlas installs the platform-specific `nemo-relay` native wheel from the
...
through that native module in the Atlas process so configured interceptors can
...
Collection remains off unless Atlas policy enables it:
...
Atlas core owns one Relay host and one isolated Relay session scope per Atlas
session. Core lifecycle producers use
`atlas_cli.observability.relay_runtime` to obtain the shared session handle or
run Relay scope, LLM, tool, and mark APIs in that session context. New product
marks do not require Atlas plugin registration. Shared-metrics marks must
...
```text
Atlas turn, API, and tool hooks
  -> Relay session, task, and LLM lifecycle
  -> Atlas shared-metrics subscriber
  -> SQLite counters
  -> immutable JSON delta package
```

Atlas sends an empty `LLMRequest` into the metrics-owned lifecycle. This does
...
Each task run is a Relay `Function` scope named `atlas.task_run`, parented to
the owning Atlas session. The start counter contains only bounded execution
...
provider attempts for the same Atlas API request ID; they do not inflate the
logical model-call count. Tool calls are deduplicated by their Atlas tool-call
...
cancellations. Active task ownership follows the task ID if Atlas rotates its
conversation session during context compression.

Local state is written under:

```text
$ATLAS_HOME/telemetry/shared_metrics/metrics.sqlite3
$ATLAS_HOME/telemetry/shared_metrics/outbox/*.json
```
...
schema field name, its current scope is one `ATLAS_HOME`, so it is more
...
Deleting `$ATLAS_HOME/telemetry/shared_metrics` resets the identifier together
...
Run a real Atlas CLI turn against the deterministic local model server:
