...
``atlas_state.SessionDB``. It defines no ``__init__`` and no state of its
...
``SessionDB.__init__``. It must never import atlas_state (cycle) — shared
module-level constants live in atlas_state_common.
"""
...
from atlas_state_common import (
    DEFERRED_INDEX_SQL,
    FTS_SQL,
    FTS_STORAGE_VERSION,
    FTS_TRIGRAM_SQL,
...
# Moved methods logged under the "atlas_state" logger before the split;
# keep that logger identity so log filtering/capture behavior is unchanged.
logger = logging.getLogger("atlas_state")
    ...
    def _init_schema(self):
        ...
        if row is None:
            ...
            if current_version < 23:
                ...
                # optimization is available. `atlas sessions optimize-storage`
        ...
        if fts5_available:
            ...
            # not yet opted into `atlas db optimize`) must keep its EXISTING
