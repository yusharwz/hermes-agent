...
Write plugins for the Atlas desktop app: statusbar items, layout panes,
...
system (`~/.atlas/plugins/`) is otherwise documented separately.
...
- The Atlas desktop app (it loads plugins; the CLI/gateway alone does not).
- Write access to `$ATLAS_HOME/desktop-plugins/` (usually
  `~/.atlas/desktop-plugins/`).

## How to Run

1. Create `$ATLAS_HOME/desktop-plugins/<name>/plugin.js` from
   `templates/plugin.js` (in this skill directory) — that's
   `~/.atlas/...` by default, or `~/.atlas/profiles/<profile>/...` under a
...
The ONLY import surface is `@atlas/plugin-sdk` (plus `react` /
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.atlas/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
...
- Do not import anything except `@atlas/plugin-sdk`, `react`, and
