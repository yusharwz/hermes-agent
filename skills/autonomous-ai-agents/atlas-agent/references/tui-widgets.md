...
Author widget apps for the Atlas TUI (`atlas --tui`): glanceable ambient
...
- The TUI must be in use (`atlas --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.atlas/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.atlas/logs/tui_gateway_crash.log` if a widget never appears.
