...
Set via `atlas model` (picker) or `atlas setup`. 35+ provider profiles ship as
...
| nous | OAuth device code | `atlas auth add nous` (or `NOUS_API_KEY`) |
| openai-codex | OAuth | `atlas auth add openai-codex` |
| qwen-oauth | OAuth | `atlas auth add qwen-oauth` |
| minimax-oauth | OAuth | `atlas auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`atlas auth`).
Fallback chain when the primary fails: `atlas fallback add|remove|list`.

### User-defined model aliases

Work with `/model <name>` in CLI and every gateway platform. Resolved by
`atlas_cli/model_switch.py::resolve_alias()`; user aliases are checked BEFORE
...
#   atlas config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:
