...
## Layer 1 — Is this thing a Atlas plugin, or a separate app?
...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the Atlas process**. Its LLM calls go through Atlas's already-configured provider. | Already uses Portal if user's Atlas is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
plugin that already runs inside Atlas. That LLM call is already authenticated
via Atlas's provider config. The plugin's own server auth (e.g.
...
What Atlas Agent has that *feels* like OAuth — `atlas login --provider nous`
opening a browser, user signs in, token lands in `~/.atlas/auth.json` — is a
**Atlas-specific browser flow**. Under the hood it produces a credential
Atlas uses as a bearer. It is not a public OAuth provider that Karakeep et al.
...
1. Read Atlas's existing Portal credential out of `~/.atlas/auth.json`.
...
rides on the credential Atlas already holds.

Where this could live in Atlas:
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for Atlas itself.
...
1. First decide: Atlas plugin (runs inside Atlas) or separate app? If plugin,
   it already uses Portal via Atlas's provider config — done.
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   Atlas agent itself") is fully supported. Atlas treats agent
...
pitch in the Tool Gateway announcement is specifically about Atlas Agent's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when Atlas is configured to use Portal as its provider. It is
