...
- **Route scripts** (`--script` on subscribe, or `script:` on a config route): a script under `~/.atlas/scripts/` receives the payload as JSON on stdin. JSON stdout replaces the payload before prompt templating; empty stdout, `[SILENT]`, or a nonzero exit ignores the webhook. `.sh`/`.bash` run with bash, everything else with Python. Scripts cannot live outside `~/.atlas/scripts/` (path traversal is blocked).

```bash
atlas webhook subscribe todoist-atlas \
  --prompt "Task changed: {payload.content}" \
  --script "todoist-atlas-label.py" \
