# Themes / Skins — Author a Atlas Color Theme

Author a Atlas **skin** — one YAML file that themes the CLI, the TUI, and the
desktop GUI at once. The skin engine (`atlas_cli/skin_engine.py`) resolves the
active skin and the gateway pushes it to every surface, so a file dropped in
`~/.atlas/skins/` is the theme analogue of a plugin: no code, all surfaces. This
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for Atlas itself.
...
- Write access to the Atlas home dir — `~/.atlas` by default, or `$ATLAS_HOME`
  / the active profile's dir. Skins live in `<atlas-home>/skins/`.
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `atlas config set`).
...
3. `write_file` it to `<atlas-home>/skins/<name>.yaml`.
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `atlas config set display.skin default` (or they can `/skin default`).
...
```
atlas skin set <key> <hex>      # e.g. atlas skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `atlas skin set background "#08201f"` changes
only the background; `atlas skin use <name>` / `atlas skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode `~/.atlas`** when a profile is active — resolve the real home
  from `$ATLAS_HOME` first, falling back to `~/.atlas`.
...
- **Never hand-edit `config.yaml` to activate.** Use `atlas config set
...
- **You apply it, not the user.** `atlas config set display.skin <name>` is
...
- `read_file` the written `<atlas-home>/skins/<name>.yaml` and confirm valid
  YAML with the intended `name` and `colors`.
- Run `atlas config get display.skin` and confirm it reports `<name>`.
