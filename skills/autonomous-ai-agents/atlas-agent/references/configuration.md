...
`atlas config check` reports sections missing from an older config.

### Toolsets

Enable/disable via `atlas tools` (interactive) or `atlas tools enable/disable NAME`.
Full enumeration: `TOOLSETS` dict in `toolsets.py` (`_ATLAS_CORE_TOOLS` is the default bundle most platforms inherit).
