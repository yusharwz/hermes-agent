...
ATLAS_CFG="${ATLAS_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Atlas config has twozero_td MCP entry ──
if [[ ! -f "$ATLAS_CFG" ]]; then
    echo -e " ${FAIL} Atlas config not found at ${ATLAS_CFG}"
    manual_steps+=("Create ${ATLAS_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$ATLAS_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Atlas config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Atlas config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${ATLAS_CFG} manually"); }
    manual_steps+=("Restart Atlas session to pick up config change")
fi
