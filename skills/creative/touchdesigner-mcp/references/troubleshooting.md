...
`$ATLAS_HOME/config.yaml` (defaults to `~/.atlas/config.yaml` when `ATLAS_HOME` is unset)
...
Restart the Atlas session for changes to take effect. The MCP connection is
