"""ACP auth helpers — detect and advertise Atlas authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "atlas-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Atlas runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Atlas can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Atlas.
    ...
    not have Atlas provider credentials configured yet, so Atlas always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Atlas using the currently configured "
