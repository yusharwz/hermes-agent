...
def _current_cron_store() -> _CronStorePaths:
    ...
    3. the ACTIVE profile home, resolved fresh via get_atlas_home()
       (context-local override, then the ATLAS_HOME env var) — so a test
       or embedder that re-points ATLAS_HOME after this module was
...
# the cron inactivity timeout is disabled (ATLAS_CRON_TIMEOUT=0 → unlimited),
...
# TTL only recovers a claim left by a tick that DIED mid-run. ATLAS_CRON_TIMEOUT
...
def _oneshot_run_claim_ttl_seconds() -> float:
    ...
    Derived from ``ATLAS_CRON_TIMEOUT`` (the cron inactivity timeout the
...
def _preserve_file_ownership(path: Path, before: Optional[os.stat_result]) -> None:
    ...
    command (``docker exec atlas atlas cron create ...`` — ``docker exec``
...
def _atomic_write_epoch(path: Path) -> None:
    ...
    in another process (``atlas cron status``) never sees a torn/truncated
...
def get_ticker_heartbeat_age() -> Optional[float]:
    ...
    ``atlas cron status`` must report per-profile liveness (#69377).
    """
    store = _current_cron_store()
...
def record_ticker_error(message: str) -> None:
    ...
    The ticker thread lives inside the gateway process; ``atlas cron
...
def _write_wedged_oneshot_diagnostic(job: Dict[str, Any]) -> None:
    ...
    try:
        ...
        text = (
            ...
            f"- removed at: {_atlas_now().isoformat()}\n\n"
...
def _get_due_jobs_locked() -> List[Dict[str, Any]]:
    ...
    # (derived from ATLAS_CRON_TIMEOUT). See _oneshot_run_claim_ttl_seconds.
    ...
    for job in jobs:
        ...
        try:
            ...
            if next_run_dt <= now:
                ...
                # desktop both run in-process 60s tickers on one ATLAS_HOME)
                ...
                # (_oneshot_run_claim_ttl_seconds, derived from ATLAS_CRON_TIMEOUT),
