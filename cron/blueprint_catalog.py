...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``atlas://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``atlas://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"atlas://blueprint/{quote(blueprint.key)}{qs}"
