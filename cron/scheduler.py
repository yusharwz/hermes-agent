...
def _interpreter_shutting_down(exc: Optional[BaseException] = None) -> bool:
    ...
    ``atlas update`` / ``atlas gateway stop`` / systemd restart, or an
...
def run_job(
    ...
    #
    # Bounded with its own timeout (separate from ATLAS_CRON_TIMEOUT, which
    ...
    try:
        ...
        _raw_env_timeout = os.getenv("ATLAS_CRON_SESSION_DB_TIMEOUT", "").strip()
        if _raw_env_timeout:
            try:
                _session_db_timeout = float(_raw_env_timeout)
            except (ValueError, TypeError):
                logger.warning(
                    "Invalid ATLAS_CRON_SESSION_DB_TIMEOUT=%r; using config/default",
                    _raw_env_timeout,
                )
    ...
    _ctx_tokens = set_session_vars(
        ...
        # ATLAS_SESSION_* routing keys just below, so an async delegation's
        ...
        # os.environ ATLAS_SESSION_KEY, which risks routing a cron subagent's output
    ...
    try:
        ...
        # cron-fleet default) > ATLAS_MODEL env > config.yaml ``model:``
