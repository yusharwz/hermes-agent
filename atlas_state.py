...
from atlas_cli.sqlite_runtime import (
    is_sqlite_wal_reset_vulnerable as _is_sqlite_wal_reset_vulnerable,
)
from typing import Any, Callable, Dict, List, Optional, Tuple, TypeVar

from atlas_state_common import (  # noqa: F401  (re-exported for back-compat)
    _BRANCH_CHILD_SQL,
...
from atlas_state_portability import SessionPortabilityMixin
from atlas_state_schema import SessionSchemaMixin
from atlas_state_search import SessionSearchMixin
...
def _workspace_key_clause(key: str) -> Tuple[str, List[str]]:
    ...
    Used by ``atlas -c``/``--resume`` to continue the most recent session in
...
def _default_db_path() -> Path:
    ...
    freezes the developer's real ``~/.atlas`` even when a test fixture later
    redirects ``ATLAS_HOME`` — importing this module during collection was
    ...
    2. Otherwise resolve ``get_atlas_home()`` fresh so a runtime
       ``ATLAS_HOME`` redirect takes effect regardless of import order.
    """
    if DEFAULT_DB_PATH != _IMPORT_DEFAULT_DB_PATH:
        return DEFAULT_DB_PATH
    return get_atlas_home() / "state.db"
...
def _log_wal_reset_bug_once(
    ...
    *,
    ...
    logger.warning(
        ...
        "Atlas-managed installs can repair the embedded runtime with "
        "`atlas update`. See `atlas doctor`. This warning fires once per "
...
def apply_database_pragmas(
    ...
    *,
    ...
    """
    try:
        # Local import avoids a circular import with atlas_cli.config.
        from atlas_cli.config import cfg_get, load_config_readonly
...
def _backup_db_file(db_path: Path) -> Optional[Path]:
    ...
    connection's POSIX advisory locks (see ``atlas_cli.sqlite_safe_read``).
    ...
    try:
        from atlas_cli.sqlite_safe_read import has_live_connection
    except ImportError:
...
def preflight_db_writability(
    ...
    *,
    ...
    - **Repairs** permissions with ``chmod u+rw`` when the file lives inside
      the Atlas home tree (``get_atlas_home()``) — the safe repair scope:
      Atlas owns those files, and the OS makes ``chmod`` fail on files the
    ...
    to check). Shared by :class:`SessionDB` and ``atlas_cli.kanban_db``.
    """
    raw = str(db_path)
    ...
    try:
        home: Optional[Path] = Path(get_atlas_home()).resolve()
    ...
    def _ensure_writable(p: Path, *, is_dir: bool = False) -> None:
        ...
        raise sqlite3.OperationalError(
            ...
            f"user. Atlas needs read-write access to open the database. "
    ...
    try:
        ...
        for fts_table in ("messages_fts", "messages_fts_trigram", "messages_fts_cjk"):
            try:
                ...
                # (atlas_state.py:645-723). The supported degraded-runtime
                # path (SessionDB._is_fts5_unavailable_error + the
                # regression suite in tests/test_atlas_state.py:600-632)
...
# (``~/.atlas/lib/libfts5_cjk.so``, built by ``native/fts5_cjk/build.sh``).
...
# next ``atlas sessions optimize-storage`` on a capable host).
#
...
def fts5_cjk_so_path() -> Path:
    ...
    env = os.getenv("ATLAS_FTS5_CJK_SO")
    if env:
        return Path(env).expanduser()
    return get_atlas_home() / "lib" / "libfts5_cjk.so"


def _cjk_fts_config_enabled() -> bool:
    """config.yaml ``sessions.cjk_fts`` (default on), via its env bridge."""
    return os.getenv("ATLAS_CJK_FTS", "1").strip().lower() not in (
        "0", "false", "off", "no",
    )
...
def _connect_tracked_db(path, tracking_path=None, **kwargs):
    ...
    (scaffold/embed installs that ship atlas_state without atlas_cli). A
    ...
    """
    try:
        from atlas_cli.sqlite_safe_read import connect_tracked
    except ImportError:
        logger.debug(
            "atlas_cli.sqlite_safe_read unavailable; opening %s untracked "
    ...
    # patch atlas_state.sqlite3.connect keep control of connection creation;
...
def is_zeroed_state_db(
    ...
    Prefer ``atlas_cli.backup.is_zeroed_sqlite_file`` when available; this
    local copy keeps SessionDB openable without importing the CLI package
    in constrained embed paths.
    """
    try:
        from atlas_cli.backup import is_zeroed_sqlite_file
    ...
    if size <= 0:
        return False
    from atlas_cli.sqlite_safe_read import read_header_bytes_preopen
    ...
    try:
        ...
        if not acquired:
            ...
            logger.error(
                ...
                "state-snapshots via `atlas snapshot list` / "
                "`atlas snapshot restore <id>`.",
                path,
            )
            return None
...
class SessionDB(SessionSearchMixin, SessionSchemaMixin, SessionPortabilityMixin):
    ...
    # legitimately held for multi-second stretches by sibling Atlas
    ...
    # `atlas update` leaves mixed-version processes sharing the DB until
        ...
        try:
            ...
            if (
                ...
                msg = (
                    ...
                    f"Restore from {snaps} via `atlas snapshot list` / "
                    f"`atlas snapshot restore <id>` if available. "
            ...
            # NOTE: the v23 FTS optimization is OPT-IN (`atlas db optimize`),
            ...
            if cjk_present:
                ...
                if live:
                    ...
                    logger.warning(
                        ...
                        "run `atlas sessions optimize-storage` on a host "
        ...
        while True:
            try:
                ...
                if "locked" in err_msg or "busy" in err_msg:
                    ...
                    raise sqlite3.OperationalError(
                        f"database is locked (another Atlas process held the "
    ...
    #
    # `optimize_fts_storage()` (the `atlas sessions optimize-storage`
    ...
    # ── Opt-in v23 FTS storage optimization (`atlas sessions optimize-storage`) ──
    #
    ...
    def _rows_to_conversation(
        ...
        *,
        ...
        if repair_alternation and messages:
            # Lazy import: atlas_state already depends on agent.* (see
    ...
    def search_sessions(
        ...
        ``atlas -c``/``--resume`` so the "last" session is the last one in
    ...
    def retag_kanban_worker_sessions(self, workspaces_root: str) -> int:
        ...
        Workers used to spawn without ``ATLAS_SESSION_SOURCE``, so their runs
